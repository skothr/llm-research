...
def compute_centroids(vocab: dict[str, Any], sink_dims: list[int]) -> dict[str, torch.Tensor]:
...
def project_to_anchors(h: torch.Tensor, centroids: dict[str, torch.Tensor],
                        sink_dims: list[int]) -> dict[str, float]:
...
def draw_anchor_glyph(ax: Any, cx: float, cy: float, projections: dict[str, float],
                       categories: list[str], radius: float, scale_max: float,
                       lw: float = 2.0, show_labels: bool = False) -> None:
    n = len(categories)
    ...
    for i, cat in enumerate(categories):
        ...
        if v < 0:
            ...
            x_end = cx - radius * mag * np.cos(angles[i])
            y_end = cy - radius * mag * np.sin(angles[i])
            ax.plot([cx, x_end], [cy, y_end], color=color, linewidth=lw, alpha=0.5,
        ...
        if show_labels:
            ...
            ax.text(x_lbl, y_lbl, cat, ha="center", va="center", fontsize=6, alpha=0.7,
                    color=CATEGORY_COLORS.get(cat, "#999999"))
...
def main() -> None:
    ...
    for j, s in enumerate(steps):
        ...
        ax_g.axis("off")
        draw_anchor_glyph(ax_g, 0.0, 0.0, projs, categories, radius=1.0, scale_max=scale_max,
    ...
    sample_projs = [project_to_anchors(h, centroids, sink_dims) for _, h, _ in samples_to_render]
    ...
    for ax, (title, _, _), projs in zip(axes, samples_to_render, sample_projs):
        ...
        ax.set_aspect("equal")
        ax.axis("off")
        draw_anchor_glyph(ax, 0.0, 0.0, projs, categories, radius=1.0, scale_max=sample_scale,
                          lw=2.5, show_labels=True)
        top3 = sorted(projs.items(), key=lambda x: -x[1])[:3]
