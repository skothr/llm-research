...
for f in "${LENS_C4EN}" "${LENS_WT_1P5B}" "${LENS_WT_7B}" "${HELDOUT_C4EN}" "${HELDOUT_WT}" \
    "${SCAN_WT_7B}"; do
...
# validation_max_vf_diff is 4.379e-01 by construction (a single-position
# outlier; layer means agree to 2.6e-3) — see the consistency report below.
...
# runs are already done by this point and aborting would discard them.
