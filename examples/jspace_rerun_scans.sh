...
# Purpose
# -------
# 13 runs. Runs 1-10 regenerate, in place, the 10 committed C4-dependent
# derived artifacts of research arc 04 (`research/arcs/04_jspace/data/`)
# against the three lenses refit on the *redacted* C4/wikitext corpora. The
# committed versions were computed on pre-redaction text and are stale.
# Runs 11-13 are the issue-#83 fixed-K paper-metric runs (see below).
#
# Plan reference
# --------------
...
#
# Runtime: ~1.4-1.6 h for runs 1-10, plus ~45-50 min for runs 11-13 (three
# runs of ~15-17 min, measured 2026-09-23). Each per-run figure below is either
# the plan's measured cost or the stale artifact's own recorded
# mean_seconds_per_prompt x its prompt count; add ~5.5 min model load per 7B
...
HELDOUT_WT="${DATA}/heldout_prompts_wikitext103_n30.json"
# Input to runs 12/13; committed, not regenerated by this script.
SCAN_WT_7B="${DATA}/structure_scan_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100.pt"
...
for f in "${LENS_C4EN}" "${LENS_WT_1P5B}" "${LENS_WT_7B}" "${HELDOUT_C4EN}" "${HELDOUT_WT}"; do
    [[ -f "${f}" ]] || fail "missing required input: ${f} (Step 0/1 incomplete?)"
done
# SCAN_WT_7B is committed, not produced by Steps 0/1; -f also passes on an
# un-pulled LFS pointer stub, so check its header too.
[[ -f "${SCAN_WT_7B}" ]] && [[ "$(head -c 42 "${SCAN_WT_7B}" | tr -d '\0')" != "version https://git-lfs.github.com/spec/v1" ]] \
    || fail "committed input missing or an LFS pointer: ${SCAN_WT_7B} (checkout without \`git lfs pull\`, or a renamed artifact?)"
...
START_EPOCH="$(date +%s)"
echo "=== jspace C4-redaction re-run, Step 2 (10 derived artifacts) + #83 K-fixed runs (3) ==="
...
# --- Issue #83: dimension-matched K (--k-fixed), 2026-09-23 ------------------
# The paper-metric excess scales with K/d_model, so the 1.5B-vs-7B comparison
# at K = median occupancy (K~25 at d=1536 vs K~23 at d=3584) is not
# like-for-like. These three runs hold K fixed at every layer: K=58 matches
# the 1.5B K/d (25/1536 = 0.0163 vs 58/3584 = 0.0162); K=25 matches the 1.5B
# K itself. top-K = selection-order prefix of the k_max=64 pursuit support.
# Not part of the C4-redaction re-run above; they reuse its 7B lens.
# The two grid runs validate against the July structure scan (fit on the
# pre-refit lens, NOT regenerated by this script), so their
# validation_max_vf_diff is 4.379e-01 by construction (max over positions;
# the per-layer mean varfrac@25 agrees with the July artifact to 2.6e-3 as a
# signed difference of means, which does not bound how many positions differ;
# per-position diffs were not persisted) — see the consistency report below.
# ~15-17 min each (logs: 15.1, 17.3, 15.6 min on 2026-09-23).
...
run paper_metric_heldoutc4en_7b_k58 \
    ...
    -- "${PY}" examples/jspace_paper_metric_varfrac.py \
        ...
        --k-max 64 \
        --k-fixed 58 \
        --out "${DATA}/paper_metric_varfrac_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100_heldoutc4en_k58.pt"

# 12/13 — 7B wikitext grid, refit lens, K=25 (lens-drift control vs the
...
run paper_metric_7b_refitlens_k25 \
    ...
    -- "${PY}" examples/jspace_paper_metric_varfrac.py \
        ...
        --k-max 64 \
        --k-fixed 25 \
        --out "${DATA}/paper_metric_varfrac_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100_refitlens_k25.pt"
...
run paper_metric_7b_refitlens_k58 \
    ...
    -- "${PY}" examples/jspace_paper_metric_varfrac.py \
        ...
        --k-max 64 \
        --k-fixed 58 \
        --out "${DATA}/paper_metric_varfrac_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100_refitlens_k58.pt"

# --- Post-run consistency report --------------------------------------------
# Each paper-metric artifact records `config.validation_max_vf_diff`: the max
# |replicated - stored| varfrac@k-snap against its --scan input. The four
# NAMES artifacts recorded 0.0 (MANIFEST provenance: "validated bit-exact").
# The two #83 refit-lens grid runs (KNOWN_NONZERO) validate against the July
# pre-refit structure scan, so they carry a known non-zero diff (4.379e-01)
# and are reported against that value instead. That check is a max over
# positions within 5e-4 of 0.4379, so it cannot tell the expected lens-drift
# outlier from a broad regression; the substantive replication check is the
# per-layer mean comparison against the July artifact recorded in the
# observation (max |delta vf_ours_mean| = 2.6e-3).
# For a NAMES artifact, a non-zero value means the paper-metric run and its
# structure scan disagree and the pair must not be used as a matched set.
# Reported, not enforced: the runs are already done by this point and aborting
# would discard them.
echo "=== paper-metric scan-validation check (expect 0.0; known 4.379e-01 for the refit-lens grid pair) ==="
"${PY}" - <<'PYEOF'
...
KNOWN_NONZERO = {
    "paper_metric_varfrac_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100_refitlens_k25.pt": 0.4379,
    "paper_metric_varfrac_qwen2.5-7b-instruct_jlens_qwen2.5-7b_nf4_n100_refitlens_k58.pt": 0.4379,
}
bad = []
for name in NAMES:
    ...
    if diff != 0.0:
        bad.append(name)
for name, want in KNOWN_NONZERO.items():
    cfg = torch.load(DATA / name, map_location="cpu", weights_only=False)["config"]
    diff = cfg["validation_max_vf_diff"]
    flag = "known" if abs(diff - want) < 5e-4 else "MISMATCH"
    print(f"  [{flag}] validation_max_vf_diff={diff!r} (known {want})  {name}")
    if flag == "MISMATCH":
        bad.append(name)
if bad:
    print(
