...
def per_source_cos(
    C: torch.Tensor, src_labels: list[str]
...
def main() -> None:
    ...
    print(
        f"  original    : PC1={float(var_orig[0] / total_orig):.3f}  PC2={float(var_orig[1] / total_orig):.3f}  "
        f"PC3={float(var_orig[2] / total_orig):.3f}  PC4={float(var_orig[3] / total_orig):.3f}"
    )
    print(
        f"  sink-removed: PC1={float(var_res[0] / total_res):.3f}  PC2={float(var_res[1] / total_res):.3f}  "
    ...
    print(
        f"  {'source':<18}  {'orig intra':>10}  {'orig cross':>10}  {'res intra':>10}  {'res cross':>10}  {'intra delta':>12}"
    )
    for s in sorted(cos_orig.keys()):
        oi, oc = cos_orig[s]
        ri, rc = cos_res[s]
        print(
            f"  {s:<18}  {oi:>+10.3f}  {oc:>+10.3f}  {ri:>+10.3f}  {rc:>+10.3f}  {ri - oi:>+12.3f}"
        )

    eye = torch.eye(H.shape[0], dtype=torch.bool)
    print(f"\noff-diag pairwise cosines:")
    print(
        f"  original    : mean={float(C_orig[~eye].mean()):.3f}  min={float(C_orig[~eye].min()):.3f}  max={float(C_orig[~eye].max()):.3f}"
    )
    print(
        f"  sink-removed: mean={float(C_res[~eye].mean()):.3f}  min={float(C_res[~eye].min()):.3f}  max={float(C_res[~eye].max()):.3f}"
    )
    ...
    for ax, C, title in [
        (axes[0], C_orig, "Original pairwise cosine"),
    ...
    scatter_pca(
        ...
        coords_res,
        items,
        "Sink-removed PCA — by source",
        (float(var_res[0] / total_res), float(var_res[1] / total_res)),
    )

    for ax, dim_idx, title in [
        (
            axes[0, 1],
            feat_dim_a,
            f"Sink-removed PCA — by h[{feat_dim_a}] value (feature dim)",
        ),
        (
            axes[1, 0],
            feat_dim_b,
            f"Sink-removed PCA — by h[{feat_dim_b}] value (feature dim)",
        ),
    ]:
    ...
    if len(has_ar) > 0:
        ...
        ax.scatter(
            x[~np.isin(np.arange(len(items)), has_ar_arr)],
    ...
    out = write_artifact("sink_removed_atlas.pt")
    torch.save(
        {
            "sink_dims": sink_dims,
