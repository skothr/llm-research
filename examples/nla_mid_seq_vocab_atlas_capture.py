"""Mid-sequence vocab atlas capture.
...
anchor position — inside a carrier that continues past the anchor — should
preserve token-specific identity before integration overwhelms it.
...
def main() -> None:
    ...
    for idx, (anchor, category) in enumerate(anchor_pairs, 1):
        ...
        try:
            anchor_last_pos, anchor_first_pos = find_anchor_end_token_pos(
                tok, chat_str, anchor
            )
        ...
        if anchor_last_pos >= n_tok:
            print(
                f"  [{idx:>3}/{total}] SKIP {category}/{anchor!r}: pos {anchor_last_pos} >= n_tok {n_tok}"
            )
        ...
        h = (
            out.hidden_states[LAYER][0, anchor_last_pos, :]
            .detach()
            .float()
            .cpu()
            .clone()
        ...
        print(
            f"  [{idx:>3}/{total}] {category:<15} {anchor!r:<22}  "
