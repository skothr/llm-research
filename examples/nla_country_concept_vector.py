...
TEST_PROMPTS: list[tuple[str, str]] = [
    ...
    (
        "weird_dream",
        "In a dream last night, I argued with Germany over breakfast cereal.",
    ),
...
def main() -> None:
    ...
    for i, p in enumerate(SOURCE_COUNTRY, 1):
        ...
        h_country.append(h)
        print(
            f"  [country {i:>2}] {p!r}  ||h||={h.norm().item():.2f}  ({time.time() - t0:.1f}s)"
        )
    for i, p in enumerate(SOURCE_NON_COUNTRY, 1):
        ...
        print(
            f"  [non-country {i:>2}] {p!r}  ||h||={h.norm().item():.2f}  ({time.time() - t0:.1f}s)"
        )
    for label, p in TEST_PROMPTS:
        ...
        h_test[label] = (p, h)
        print(
            f"  [test {label}] {p!r}  ||h||={h.norm().item():.2f}  ({time.time() - t0:.1f}s)"
        )
    ...
    print(BAR)
    print(
        "PROJECTION TABLE  (positive = aligned with 'country' direction, negative = away)"
    )
    print(BAR)
    ...
    out_path = write_artifact("country_concept_vector.pt")
    torch.save(
        {
            "h_country": h_country,
