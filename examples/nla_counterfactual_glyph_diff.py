...
def main() -> None:
    ...
    for pid in pair_ids:
        ...
        if nat and forc:
            print(
                f"  {pid:>20}  natural={nat['actual_token']!r:>14}  forced={forc['actual_token']!r:>14}"
            )
    ...
    for r, pid in enumerate(pair_ids):
        ...
        for c, (title_suffix, vec, normalizer, cap) in enumerate(
            [
                (
                    f"natural  '{nat['actual_token']}'",
                    nat["h"][feature_dims].numpy(),
                    abs_max,
                    nat,
                ),
                (
                    f"forced   '{forc['actual_token']}'",
                    forc["h"][feature_dims].numpy(),
                    abs_max,
                    forc,
                ),
                (
                    f"forced - natural diff",
                    (forc["h"] - nat["h"])[feature_dims].numpy(),
                    abs_max_diff,
                    None,
                ),
            ]
    ...
    fig.suptitle(
        f"Counterfactual glyph diff: forced - natural in the feature-dim subspace\n"
