...
def main() -> None:
    ...
    for label, h_orig, av_text in targets:
        ...
        h_pred = nla_reconstruct(
            av_text, backbone=backbone, value_head=value_head, tok=tok, meta=meta,
        )
    ...
    print(f"PLATEAU ATTRACTOR VERDICT:")
    ...
    if cos_rt > 0.85:
        print(f"  → STRONG ATTRACTOR (cos > 0.85): plateau is self-consistent")
    elif cos_rt > max(cos_to_A, cos_to_B) + 0.1:
        print(f"  → MODERATE ATTRACTOR: round-trip stays closer to plateau than either anchor")
    elif cos_to_A > cos_rt or cos_to_B > cos_rt:
        print(f"  → TRANSIT ZONE: AR re-encoding drifts toward {'A' if cos_to_A > cos_to_B else 'B'}")
    else:
