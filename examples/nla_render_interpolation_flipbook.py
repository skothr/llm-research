...
def main() -> None:
    ...
    print(
        f"cos(h_A, h_B) = {torch.nn.functional.cosine_similarity(h_A, h_B, dim=0).item():+.4f}"
    )
    ...
    for j, (lbl, t, h_t, av_text) in enumerate(row_h):
        ...
        draw_signature_glyph(
            ax_g1, 0.0, 0.0, vec_g, radius=1.0, scale_max=abs_max_general, lw=2.5
        )
        ...
        draw_signature_glyph(
            ax_g2, 0.0, 0.0, vec_i, radius=1.0, scale_max=abs_max_interp, lw=2.5
        )
        label_glyph_axes(ax_g2, interp_dims, fontsize=5)
        if j == 0:
            ax_g2.set_title(
                f"interpolation-specific glyph\n(top-8 by |h_A - h_B|)",
                fontsize=8,
                pad=2,
            )
    ...
    cos_A = torch.nn.functional.cosine_similarity(
        h_ts, h_A.unsqueeze(0).expand_as(h_ts), dim=1
    ).numpy()
    cos_B = torch.nn.functional.cosine_similarity(
        h_ts, h_B.unsqueeze(0).expand_as(h_ts), dim=1
    ).numpy()
