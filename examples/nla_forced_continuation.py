...
def find_token_position(tok: Any, prompt_chat_ids: torch.Tensor, completion_text: str,
                        target_str: str) -> tuple[torch.Tensor, int, str]:
    ...
    """
    completion_ids = tok(completion_text, add_special_tokens=False, return_tensors="pt")["input_ids"][0]
    ...
    if found_rel is None:
        ...
        for i, tid in enumerate(completion_list):
            if tid == first_id:
                ...
                break
    if found_rel is None:
        raise RuntimeError(f"target {target_str!r} not found in completion {completion_text!r}")
...
def capture_pair(model: Any, tok: Any, pair: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for version_name, completion_key, target_key in [
        ...
        for target in targets:
            full_ids, abs_pos, actual_token = find_token_position(tok, prompt_ids, completion, target)
            with torch.no_grad():
            ...
            print(f"     target={target!r:<14}  pos={abs_pos:>3}  actual_token={actual_token!r:<14}  "
...
def main() -> None:
    ...
    if captures_needing_av:
        ...
        for i, c in enumerate(captures_needing_av, 1):
            print(f"  [{i}/{len(captures_needing_av)}] {c['pair_id']}.{c['version']}  "
                  f"target={c['target']!r} ...", end="", flush=True)
            t0 = time.time()
    ...
    for pair_id, caps in by_pair.items():
        ...
        for c in caps:
            ...
            print(f"  -- {label:<28}  ||h||={c['norm']:>6.2f}  argmax-next={c['argmax_next']!r}")
