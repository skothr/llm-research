...
def project(h: torch.Tensor, discr: dict[str, torch.Tensor],
            categories: list[str], sink_dims: list[int]) -> dict[str, float]:
...
def draw_glyph(ax: Any, projs: dict[str, float], categories: list[str],
               scale_max: float, lw: float = 2.0) -> None:
    n = len(categories)
    angles = np.linspace(0, 2 * np.pi, n, endpoint=False)
    ring = plt.Circle((0.0, 0.0), 1.0, color="#cccccc", linewidth=0.4, fill=False, alpha=0.4)
    ax.add_patch(ring)
    for i, cat in enumerate(categories):
        ...
        if v < 0:
            ax.plot([0, x_end], [0, y_end], color=color, linewidth=lw, alpha=0.6,
                    linestyle=":", solid_capstyle="round")
        else:
            ax.plot([0, x_end], [0, y_end], color=color, linewidth=lw, alpha=0.95,
                    solid_capstyle="round")


def main() -> None:
    stab = torch.load(ARTIFACTS / "discriminant_stability.pt", weights_only=False)
    ...
    scale_max = max(
        ...
        for a in anchors for c in contexts if c in all_projs[a]
    )
    print(f"scale_max across stability captures: {scale_max:.3f}")

    n_anchors = len(anchors)
    n_cols = 1 + len(contexts)  # 1 for label + 4 for glyphs
    fig, axes = plt.subplots(n_anchors, n_cols,
                              figsize=(2.5 * n_cols, 2.5 * n_anchors))
    for r, anchor in enumerate(anchors):
        exp = expected_cat[anchor]
        # Label column
        ax = axes[r, 0]
        ax.axis("off")
        ax.text(0.5, 0.55, anchor, ha="center", va="center", fontsize=16,
        ...
        proj_vec_per_ctx = {ctx: torch.tensor([all_projs[anchor][ctx][c] for c in categories])
                              for ctx in contexts if ctx in all_projs[anchor]}
        ...
        for c_idx, ctx in enumerate(contexts):
            ...
            if ctx in all_projs[anchor]:
                draw_glyph(ax, all_projs[anchor][ctx], categories, scale_max=scale_max, lw=2.0)
                # Top-1 of this glyph
    ...
    for anchor in anchors:
        ...
        exp_mean = float(np.mean(exp_projs))
        exp_std = float(np.std(exp_projs))
        print(f"{anchor:<12} {exp:<10} {mean_ctx_cos:>+12.3f} {exp_mean:>+14.3f} {exp_std:>+14.3f}")
