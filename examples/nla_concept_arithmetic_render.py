...
def main() -> None:
    out = torch.load(ARTIFACTS / "concept_arithmetic_atlas.pt", weights_only=False)
    combos = out["combos"]
    n = len(combos)
    ...
    for j, c in enumerate(combos):
        ...
        ax_l.axis("off")
        ax_l.add_patch(plt.Rectangle((0.0, 0.0), 1.0, 1.0, transform=ax_l.transAxes,
                                       facecolor=color, alpha=0.10, edgecolor="none"))
        ax_l.text(0.02, 0.78, f"[{c['category']}]", ha="left", va="top",
                  fontsize=9, family="monospace", color=color, weight="bold",
                  transform=ax_l.transAxes)
        ax_l.text(0.02, 0.58, c["label"], ha="left", va="top",
                  fontsize=11, family="monospace", weight="bold",
                  transform=ax_l.transAxes)
        n_raw = float(c["h_raw"].norm().item())
        ax_l.text(0.02, 0.30,
                  f"||h_raw|| = {n_raw:.2f}\nrescaled to 150",
                  ha="left", va="top", fontsize=8, family="monospace", alpha=0.7,
                  transform=ax_l.transAxes)
        ax_l.text(0.02, 0.10,
                  f"predicted: {c['predict']}",
                  ha="left", va="top", fontsize=8, family="serif", style="italic",
                  alpha=0.8, transform=ax_l.transAxes)
        ...
        ax_r.text(0.0, 0.95, wrapped, ha="left", va="top",
                  fontsize=8, family="serif", transform=ax_r.transAxes)
