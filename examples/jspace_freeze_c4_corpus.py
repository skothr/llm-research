...
def freeze_c4_prompts(
    n_prompts: int, *, min_chars: int, seed: int, buffer_size: int
) -> list[str]:
    """Return the first ``n_prompts`` C4-en records of at least ``min_chars``
    characters, in seeded-shuffle order. Deterministic given ``seed`` and
    ``buffer_size`` over the immutable allenai/c4 `en` train split."""
    if n_prompts <= 0:
        return []
...
def main() -> int:
    ...
    doc = {
        ...
        "selection": (
            f"shuffle(seed={args.seed}, buffer_size={args.buffer_size}) then "
            f"first {args.n_prompts} records with "
