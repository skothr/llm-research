...
The 'TODO(human)' marker in score_hot_dim() is where we ask the human
to define what "hot dimension" means semantically.
"""
...
def load_all() -> list[dict[str, Any]]:
    ...
    if p.exists():
        ...
        for i, h in enumerate(data["h_country"]):
            items.append({
                "src": "country_src", "prompt_id": f"country_{i}",
                "token": "", "step": None,
        ...
        for i, h in enumerate(data["h_non_country"]):
            items.append({
                "src": "non_country_src", "prompt_id": f"non_country_{i}",
                "token": "", "step": None,
                "h": h.detach().float().cpu(),
                "av_text": "",
            })
        for label, (_, h) in data["h_test"].items():
            items.append({
                "src": "country_test", "prompt_id": label,
                "token": "", "step": None,
                "h": h.detach().float().cpu(),
                "av_text": "",
            })
...
def per_dim_stats(H: torch.Tensor, counts_top5: Counter, counts_top100: Counter,
    ...
    for idx in seen:
        out[idx] = {
            ...
            "median_over_max": float(median_abs[idx].item() / (max_abs[idx].item() + 1e-9)),
        }
    return out
...
def main() -> None:
    ...
    def show(title: str, positions: torch.Tensor, values: torch.Tensor, k: int = 20) -> None:
        print(f"\n--- {title} ---")
        for j in range(k):
            pos = int(positions[j].item())
            i_a, i_b = int(iu[0][pos].item()), int(iu[1][pos].item())
            a, b = items[i_a], items[i_b]
            print(f"  cos={values[j].item():+.3f}  "
    ...
    show("top-20 least-similar pairs", sorted_pos[-20:].flip(0), sorted_vals[-20:].flip(0))
    ...
    for s in unique_srcs:
        ...
        print(f"  {s:<18} (n={n_s:>3}):  intra={intra_mean:+.3f}   cross={cross:+.3f}   diff={intra_mean - cross:+.3f}")
    ...
    for idx, c in counts_top5.most_common(20):
        ...
        print(f"  idx {idx:>5}: top5_count={c:>4}/{n}   rank_weighted={rk:>6.1f}   mag_sum={mag:>9.2f}")
    ...
    for i in range(min(20, len(S))):
        ...
        cum += var_i
        print(f"  PC{i+1:>2}: sigma={S[i].item():>9.2f}  var={var_i:>10.2f}  frac={var_i/total_var:>5.3f}  cum={cum/total_var:>5.3f}")
    ...
    print(f"  {'idx':>5}  {'frq5':>5}  {'frq100':>6}  {'meanV':>8}  {'meanA':>7}  {'maxA':>7}  {'sign+':>6}  {'cv_abs':>7}  {'med/max':>7}")
    for idx in top_dims:
        s = stats_by_dim[idx]
        print(f"  {idx:>5}  {s['freq_top5']:>5.2f}  {s['freq_top100']:>6.2f}  "
