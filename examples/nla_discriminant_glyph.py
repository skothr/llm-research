...
def compute_discriminant_dirs(
    ...
    for cat in cats:
        ...
        other_hs = torch.stack(
            [caps[i]["h"] for i in range(len(caps)) if caps[i]["category"] != cat]
        )
...
def main() -> None:
    ...
    off = C_d[~eye]
    print(
        f"discriminant pairwise cosines: mean={float(off.mean().item()):+.3f}  "
    ...
    sample_projs = [
        project_to_discriminants(h, discr, sink_dims) for _, h in samples_to_render
    ]
