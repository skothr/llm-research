...
def main() -> None:
    ...
    print(
        f"vocab-atlas PCA variance: PC1={float(var[0] / total_var):.3f}  PC2={float(var[1] / total_var):.3f}  "
    ...
    ax.set_title(
        f"fig19 — Vocab atlas: 128 anchor tokens at end-of-user-message position\n"
    ...
    SRC_COLORS = {
        ...
        "forced": "#ff9896",
        "country_src": "#98df8a",
        "non_country_src": "#c5b0d5",
        "country_test": "#c49c94",
    }
    for i, it in enumerate(items_meta):
        c = SRC_COLORS.get(it["src"], "#cccccc")
        ax.scatter(
            coords_all[n_vocab + i, 0].item(),
            coords_all[n_vocab + i, 1].item(),
            c=c,
            s=15,
            alpha=0.4,
            edgecolors="none",
    ...
    for i, cap in enumerate(vocab_caps):
        ...
        ax.annotate(
            cap["word"],
            (coords_all[i, 0].item(), coords_all[i, 1].item()),
            textcoords="offset points",
            xytext=(5, 4),
            fontsize=7,
            alpha=0.95,
    ...
    ax.set_title(
        f"fig20 — Combined atlas: 128 vocab anchors (labeled) + 167 existing captures (faded dots)\n"
    ...
    flip_path = find_artifact("interpolation_flipbook.pt")
    if flip_path is not None:
        ...
        for j, s in enumerate(steps):
            ...
            top3_words = [
                (vocab_caps[idx]["word"], float(cos_to_anchors[j, idx].item()))
                for idx in top3_idx
            ]
        ...
        label_pos = [
            (cat_centers[i] + cat_centers[i + 1]) / 2 for i in range(len(cat_order))
        ]
        ...
        fig.suptitle(
            f"fig21 — Interpolation trajectory through anchor space\n"
        ...
        print("  (skipping fig21: interpolation_flipbook.pt not found)")
    ...
    ax.set_title(
        f"fig22 — Anchor pairwise cosine (sink-removed), grouped by category\n"
