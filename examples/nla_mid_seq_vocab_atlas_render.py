...
def main() -> None:
    ...
    ax1.bar(
        ...
        color="#ff7f0e",
        alpha=0.85,
        label="mid-sequence (this test)",
    )
    ax1.set_ylabel("within-class signal: mean cos(h_a, d_cat)")
    ax1.set_title(
        "fig31(top) — within-class discriminant signal per category, by capture protocol\n"
    ...
    ax2.set_title(
        "fig31(bottom) — max off-class projection per category. "
    ...
    ax.set_title(
        "fig32 — Anchor->category classification accuracy by capture protocol\n"
