...
def draw_signature_glyph(ax: Any, cx: float, cy: float, values: np.ndarray,
                          radius: float, scale_max: float) -> None:
    ...
    for i, v in enumerate(values):
        ...
        ax.plot([cx, x_end], [cy, y_end], color=color, linewidth=0.9, alpha=0.85,
                solid_capstyle="round")


def main() -> None:
    atlas = torch.load(ARTIFACTS / "sink_removed_atlas.pt", weights_only=False)
    ...
    print(f"feature dims (angle order, 0 deg to 315 deg in 45-deg steps): {feature_dims}")
    ...
    for src, color in SRC_COLORS.items():
        ...
        ax.scatter(x, y, c=color, label=f"{src} (n={len(idxs)})",
                   alpha=0.35, s=30, edgecolors="black", linewidths=0.3, zorder=1)

    # glyphs over every point
    for i in range(len(items)):
        draw_signature_glyph(ax, xs[i], ys[i], feat_vals[i], radius=radius, scale_max=abs_max)
    ...
    for i, (ang, d) in enumerate(zip(angles, feature_dims)):
        ...
        ax.annotate(f"dim {d}", (x_end, y_end),
                    textcoords="offset points",
                    xytext=(int(8 * np.cos(ang)), int(8 * np.sin(ang))),
                    fontsize=7, ha="center", va="center")
    ax.annotate("ray angle = feature dim\nray length = |h[dim]|\nred = +, blue = -",
                (legend_x, legend_y - 2.0 * legend_r), fontsize=8,
                ha="center", va="top",
                bbox={"boxstyle": "round,pad=0.4", "facecolor": "white", "edgecolor": "gray"})

    ax.set_xlabel(f"PC1 (sink-removed, {float(atlas['var_res'][0] / atlas['var_res'].sum()):.1%})")
    ax.set_ylabel(f"PC2 (sink-removed, {float(atlas['var_res'][1] / atlas['var_res'].sum()):.1%})")
    ax.set_title("Feature signature atlas: 8-ray glyphs over PC1/PC2 (sink-removed)\n"
    ...
    country_idxs = [i for i, it in enumerate(items)
    ...
    for src in ("country_src", "non_country_src", "country_test"):
        ...
        if idxs:
            axes[0].scatter(xs[idxs], ys[idxs], c=SRC_COLORS[src], label=src,
                            s=80, alpha=0.35, edgecolors="black", linewidths=0.5, zorder=1)
    for i in country_idxs:
        draw_signature_glyph(axes[0], xs[i], ys[i], feat_vals[i],
                             radius=radius_zoom, scale_max=abs_max)
        label = items[i]["prompt_id"][:18]
        axes[0].annotate(label, (xs[i], ys[i]),
    ...
    axes[0].set_title("Country cluster zoom (29 captures): nearly-identical glyphs\n"
    ...
    axes[1].plot(haiku_xs, haiku_ys, color="#ff7f0e", alpha=0.5, linewidth=1.5, zorder=1)
    radius_haiku = 0.06 * (haiku_xs.max() - haiku_xs.min())
    for i in haiku_idxs:
        draw_signature_glyph(axes[1], xs[i], ys[i], feat_vals[i],
                             radius=radius_haiku, scale_max=abs_max)
        tok = items[i]["token"]
        step = items[i]["step"]
        axes[1].annotate(f"{step}: {tok!r}", (xs[i], ys[i]),
    ...
    axes[1].set_title("Haiku generation trajectory (15 steps): glyphs morph step by step\n"
