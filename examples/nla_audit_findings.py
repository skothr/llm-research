            ...
            items.append(
                {
                    ...
                    "h_pred": cap.get("h_pred").detach().float().cpu()
...
def main() -> None:
    ...
    claim_near(
        "dim 2570 sign_consist", 0.000, stats_by_dim[2570]["sign_consist"], atol=0.001
    )
    ...
    claim_near(
        "dim 458 sign_consist", 0.000, stats_by_dim[458]["sign_consist"], atol=0.001
    )
    ...
    claim_near(
        "dim 32 sign_consist", 0.299, stats_by_dim[32]["sign_consist"], atol=0.005
    )
    claim_near("dim 32 cv_abs", 0.67, stats_by_dim[32]["cv_abs"], atol=0.01)
    claim_near(
        "dim 608 sign_consist", 0.629, stats_by_dim[608]["sign_consist"], atol=0.005
    )
    ...
    claim_near(
        "PC1 variance fraction (original)", 0.165, float(var[0] / total_var), atol=0.005
    )
    claim_near(
        "PC2 variance fraction (original)", 0.077, float(var[1] / total_var), atol=0.005
    )
    ...
    off = C[~eye]
    claim_near(
        "mean off-diag cosine (original)", 0.403, float(off.mean().item()), atol=0.005
    )
    ...
    off_res = C_res[~eye]
    claim_near(
        "mean off-diag cosine (sink-removed)",
        0.179,
        float(off_res.mean().item()),
        atol=0.005,
    )
    claim_near(
        "min cosine (sink-removed)", -0.069, float(off_res.min().item()), atol=0.01
    )
    ...
    claim_near(
        "aggregate intra cos (sink-removed)", 0.156, intra_res("aggregate"), atol=0.005
    )
    claim_near(
        "country_src intra cos (sink-removed)",
        0.777,
        intra_res("country_src"),
        atol=0.005,
    )
    claim_near(
        "non_country_src intra cos (sink-removed)",
        0.804,
        intra_res("non_country_src"),
        atol=0.005,
    )
    ...
    claim_near(
        "variance fraction kept after sink removal",
        0.950,
        total_var_res / total_var,
        atol=0.005,
    )
    claim_near(
    ...
    claim_near(
        "cos(CAV_unit, e_32) = direction_unit[32]",
        0.0510,
        float(direction_unit[32].item()),
        atol=0.0005,
    )
    # H3 prediction: >= 0.4. Confirm falsification.
    claim(
        "H3 (cos >= 0.4) falsified",
        abs(float(direction_unit[32].item())) < 0.4,
        "falsified",
        "falsified" if abs(float(direction_unit[32].item())) < 0.4 else "NOT falsified",
    )
    ...
    claim_near(
        "dim 1803 sq share",
        0.0154,
        float((direction_unit[1803] ** 2).item()),
        atol=0.001,
    )
    ...
    claim(
        "sinks in top-8 CAV contributors >= 2",
        len(sinks_in_top8) >= 2,
        ">=2 sink dims",
        f"{len(sinks_in_top8)} sink dims: {sinks_in_top8}",
    )
    ...
    haiku_aggregate = [
        it
        for it in ar_items
        if it["src"] == "aggregate" and it["prompt_id"] == "creative_haiku"
    ]
    ...
    for a, b in zip(h_agg_set, h_gen_set):
        if not torch.allclose(a["h"], b["h"], atol=1e-5):
            ...
            break
    claim(
        "haiku captures duplicated across aggregate+haiku_gen",
        all_match,
        "duplicates",
        "duplicates" if all_match else "NOT duplicates",
    )
    # Unique AR captures
    ...
    for pid, expected_token, expected_norm in [
        ...
        f_cap = next(
            c
            for c in forc_by_pair[pid]
            if c["actual_token"].strip() == expected_token.strip()
        )
    ...
    for tok, expected_norm in [
        (" sensing", 29.86),
        (" test", 28.06),
        (" refuse", 35.55),
    ]:
        f_cap = next(
            c for c in forc_by_pair["refusal_metaware"] if c["actual_token"] == tok
        )
        diff = f_cap["h"] - n_cap["h"]
        claim_near(
            f"refusal_metaware {tok!r} ||Δh||_feat",
            expected_norm,
            norm_feat(diff),
            atol=0.05,
        )
    ...
    if flip_path.exists():
        ...
        claim_near(
            "||h_A - h_B|| (inter-anchor distance)",
            51.94,
            float((h_A_flip - h_B_flip).norm().item()),
            atol=0.05,
        )
        ...
        claim_near(
            "per-step ||Δh|| (mean)", 2.734, float(step_dists.mean().item()), atol=0.01
        )
        claim_near(
            "per-step ||Δh|| (std, near-zero for linear interp)",
            0.0,
            float(step_dists.std().item()),
            atol=0.01,
        )
        # midpoint norm should dip below max(||h_A||, ||h_B||)
        midpoint_norm = float(h_ts_flip[len(steps_flip) // 2].norm().item())
        claim(
            "midpoint ||h_t|| < min(||h_A||, ||h_B||) (geometric bow)",
            midpoint_norm
            < min(float(h_A_flip.norm().item()), float(h_B_flip.norm().item())),
            "below both anchor norms",
            f"midpoint={midpoint_norm:.2f}",
        )
    else:
    ...
    if vocab_path.exists():
        ...
        claim_near(
            "capital intra-cos (sink-removed)", 0.983, intra_cat("capital"), atol=0.005
        )
        claim_near(
            "country intra-cos (sink-removed)", 0.981, intra_cat("country"), atol=0.005
        )
        claim_near(
            "emotion intra-cos (sink-removed, loosest)",
            0.849,
            intra_cat("emotion"),
            atol=0.01,
        )
        claim_near(
            "refusal intra-cos (sink-removed)", 0.847, intra_cat("refusal"), atol=0.01
        )
        ...
        claim_near(
            "mean cross-centroid cosine (the all-axes-active problem)",
            0.850,
            float(C_cen[~eye_cat].mean().item()),
            atol=0.005,
        )
        grand_mean = cen_stack.mean(dim=0)
        claim_near(
            "||grand_mean of centroids|| (would be 0.209 if spread)",
            0.926,
            float(grand_mean.norm().item()),
            atol=0.005,
        )
        ...
        d_off = C_d[~eye_cat]
        claim_near(
            "mean cross-discriminant cosine (after fix)",
            0.006,
            float(d_off.mean().item()),
            atol=0.01,
        )
        claim_near(
            "min cross-discriminant cosine (country↔demonstrative opposite)",
            -0.638,
            float(d_off.min().item()),
            atol=0.01,
        )
        ...
        claim_near(
            "country-capital discriminant cosine",
            0.938,
            float(C_d[ci, cap_i].item()),
            atol=0.005,
        )
        ...
        if stab_path.exists():
            ...
            def ctx_cos_for(anchor: str) -> float:
                ...
                for ctx in contexts:
                    ...
                    proj = torch.tensor(
                        [float(torch.dot(h_unit, discr[c]).item()) for c in categories]
                    )
                    projs.append(proj)
            ...
            claim_near(
                "ctx-cos for `the` (most stable)", 0.917, ctx_cos_for("the"), atol=0.005
            )
            claim_near(
                "ctx-cos for `happy` (least stable)",
                0.399,
                ctx_cos_for("happy"),
                atol=0.005,
            )
            ...
            for ctx in contexts:
                ...
                for d in sinks:
                    h[d] = 0.0
                h_unit = h / (h.norm() + 1e-9)
                happy_emo_projs.append(
                    float(torch.dot(h_unit, discr["emotion"]).item())
                )
            claim_near(
                "happy → emotion projection mean (weak; the topic-not-token finding)",
                0.083,
                float(sum(happy_emo_projs) / len(happy_emo_projs)),
                atol=0.005,
            )
        else:
        ...
        if mid_path.exists():
            ...
            for cap in mid_caps:
                ...
                h_unit = h / (h.norm() + 1e-9)
                projs = {
                    c: float(torch.dot(h_unit, discr[c]).item()) for c in categories
                }
                sig = projs[cap["category"]]
                mid_sigs.append(sig)
                by_cat_sigs.setdefault(cap["category"], []).append(sig)
                is_correct = (
                    max(projs.items(), key=lambda x: x[1])[0] == cap["category"]
                )
            ...
            claim_near(
                "mid_seq aggregate within-class signal (~8x weaker than eop)",
                0.0491,
                mid_sig_agg,
                atol=0.005,
            )
            claim_near(
                "mid_seq aggregate argmax accuracy (per-cat mean, vs 75.4% eop)",
                0.3204,
                mid_acc_agg,
                atol=0.005,
            )
            ...
            for cap, h_res in zip(mid_caps, H_mid_res):
                ...
                projs = {
                    c: float(torch.dot(h_unit, discr_mid[c]).item()) for c in categories
                }
                mid_native_sigs.setdefault(cap["category"], []).append(
                    projs[cap["category"]]
                )
                is_correct = (
                    max(projs.items(), key=lambda x: x[1])[0] == cap["category"]
            ...
            claim_near(
                "mid-h × mid-discr in-protocol signal (40% higher than eop in-protocol)",
                0.5632,
                mid_native_sig,
                atol=0.005,
            )
            claim_near(
                "mid-h × mid-discr argmax accuracy", 0.9710, mid_native_acc, atol=0.005
            )
            ...
            claim_near(
                "mean cross-protocol diagonal cosine (axis stability)",
                0.0784,
                float(diag.mean().item()),
                atol=0.005,
            )
            claim_near(
                "max diagonal (emotion most-stable axis)",
                0.1704,
                float(diag.max().item()),
                atol=0.005,
            )
            ...
            claim_near(
                "emotion-emotion cross-protocol cosine",
                0.1704,
                float(cross[emotion_idx, emotion_idx].item()),
                atol=0.005,
            )
        else:
        ...
        if arith_path.exists():
            ...
            for c_a in combos_a:
                claim(
                    "av_text non-empty for " + c_a["label"][:30],
                    bool(c_a.get("av_text", "").strip()),
                    True,
                    True,
                )
                n_rescaled = float(c_a["h_rescaled"].norm().item())
                claim_near(
                    f"||h_rescaled|| ≈ 150 for {c_a['label'][:30]}",
                    150.0,
                    n_rescaled,
                    atol=0.5,
                )
        ...
        if dense_path.exists():
            ...
            claim(
                "midpoint norm < anchor norm (anti-parallel anchors)",
                mid_norm < 65.0,
                "<65",
                mid_norm,
            )
            ...
            claim(
                "plateau decodes stable across t∈[0.395, 0.4400] (<=3 unique first-lines)",
                len(plateau_first_lines) <= 3,
                "<=3 unique",
                len(plateau_first_lines),
            )
        else:
        ...
        if plat_path.exists():
            ...
            claim_near(
                "plateau round-trip cosine (attractor threshold +0.85)",
                0.8995,
                plat_r["cosine_round_trip"],
                atol=0.005,
            )
            claim_near(
                "plateau h_pred → anchor A drift",
                0.8386,
                plat_r["cos_to_anchor_A"],
                atol=0.005,
            )
            claim_near(
                "plateau h_pred → anchor B drift",
                0.8154,
                plat_r["cos_to_anchor_B"],
                atol=0.005,
            )
            ...
            claim(
                "plateau h_pred closer to plateau than to A (margin > 0)",
                margin_to_A > 0,
                ">0",
                margin_to_A,
            )
            claim(
                "plateau h_pred closer to plateau than to B (margin > 0)",
                margin_to_B > 0,
                ">0",
                margin_to_B,
            )
