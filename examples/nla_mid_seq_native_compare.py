...
from nla_discriminant_glyph import (
    apply_sink_removal, compute_discriminant_dirs, project_to_discriminants,
)
...
def _aggregate_signal(captures: list[dict[str, Any]],
                       discr: dict[str, torch.Tensor],
                       sink_dims: list[int]) -> tuple[float, float, dict[str, float]]:
    ...
    acc_agg = _mean([_mean([1.0 if b else 0.0 for b in vs])
                     for vs in by_cat_correct.values()])
...
def main() -> None:
    ...
    for cap_name, cap_src in [("eop", eop["captures"]), ("mid", mid["captures"])]:
        for d_name, d_src in [("eop", discr_eop), ("mid", discr_mid)]:
            ...
            cells[(cap_name, d_name)] = {
                "agg_signal": sig, "agg_acc": acc, "per_cat_signal": per_cat,
            }
            print(f"  captures={cap_name:>3}  discr={d_name:>3}  "
    ...
    print(f"  min  diagonal: {diag_cos.min():+.4f}  ({categories[int(diag_cos.argmin())]})")
    print(f"  max  diagonal: {diag_cos.max():+.4f}  ({categories[int(diag_cos.argmax())]})")
    ...
    x = np.arange(n_cat)
    width = 0.4
    eop_native = np.array([cells[("eop", "eop")]["per_cat_signal"][c] for c in categories])
    mid_native = np.array([cells[("mid", "mid")]["per_cat_signal"][c] for c in categories])
    mid_cross = np.array([cells[("mid", "eop")]["per_cat_signal"][c] for c in categories])

    fig, ax = plt.subplots(figsize=(16, 6))
    ax.bar(x - width, eop_native, width, color="#1f77b4", alpha=0.85,
           label=f"eop-h × eop-discr (in-protocol, mean +{eop_native.mean():.3f})")
    ax.bar(x, mid_native, width, color="#2ca02c", alpha=0.85,
           label=f"mid-h × mid-discr (in-protocol, mean +{mid_native.mean():.3f})")
    ax.bar(x + width, mid_cross, width, color="#ff7f0e", alpha=0.85,
    ...
    for i in range(n_cat):
        ax.text(i, i, f"{cross_cos[i, i]:+.2f}", ha="center", va="center",
    ...
    torch.save({
        ...
        "sink_dims": sink_dims,
    }, ARTIFACTS / "mid_seq_native_compare.pt")
    print(f"\nWrote {ARTIFACTS / 'mid_seq_native_compare.pt'}")
