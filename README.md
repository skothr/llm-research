...
**C4-en** corpus slice carried 120 pieces of third-party personal data and was
redacted on 2026-07-29; because the redaction is not length-preserving, every
result computed on that corpus — the **corpus-invariance** and
**held-out-sample** checks and the **H3 @10 corpus-dependence qualifier** — was
recomputed on the redacted text (2026-08-15/16). Outcome: two audit pins moved,
each by less than 0.02, both in the held-out channel, and no headline conclusion
changed; the two failure modes pre-registered before the re-run did not
materialise. Arc 04's primary fitting corpus is wikitext-103, which was scanned
and left unmodified, and no other arc used C4. The full record — per-class
counts, root cause, per-claim blast radius, and the reproduction recipe — is in
...
- **`examples/`** — Per-arc pipeline scripts, prefixed by arc: `nla_*`
  ...
  `nla_*` pipeline specifically — the other three families follow
...
settle `xr-hyper` cross-refs), plus a conditional fifth pass for any paper still
showing unresolved cites. A LaTeX toolchain
...
from them. All four re-measured on 2026-08-17, each arc committing its run as
`data/audit_2026-08-17.log`:

```bash
python examples/nla_audit_findings.py         # arc 01 → SUMMARY: 196 PASS | 0 FAIL
python examples/subliminal_audit_findings.py  # arc 02 → SUMMARY: 102 PASS | 0 FAIL | 5 UNVERIFIABLE
...
the lens tensors are on disk. See the arc READMEs
...
figures (`920 | 10`; the cache-present `978` was never re-verified). The
pre-2026-08-17 totals — 178, 94, and arc 04's `921 | 7` / `956 | 4` — are
pre-sweep states: arc 04's is committed as
[`data/audit_2026-08-16.log`](research/arcs/04_jspace/data/audit_2026-08-16.log),
and arcs 01 and 03 recorded theirs in their READMEs before the sweep, so those
survive in git history rather than as committed logs.
...
carry their own terms, recorded in a `LICENSE-DATA.md` beside the data in each
arc that has any — arcs 01, 03, and 04; arc 04's is the reference pattern,
naming the source, the licence, and the attribution it obliges. Primary-source
PDFs committed under `theory/sources/papers/` likewise retain their own
licences and are redistributed under them, not under this repo's.
