...
| fig15_structural_block.png | The 21-dim entangled block: norm-fraction-vs-token-id deciles vs random 21-dim control + per-script means (head-loaded, script-independent) | emb_structural_block_render.py | emb_structural_block.pt |
...
| fig17_component_deltas.png | Attention vs FFN residual-delta block norm fraction by layer + static block/control weight ratios (RMSNorm gain, o_proj/down_proj writes, gate/up reads) | emb_trace_render.py | emb_trace_components.pt |
| fig18_sink_census.png | Massive-activation dims (peak vs first layer; arc-1 sink dims marked) + P2 block norm-fraction persistence profile, delimiter vs control vs first position | emb_trace_render.py | emb_trace_components.pt + emb_trace_analysis.pt |
| fig19_delim_head_census.png | P1a: per-head delim->delim (d2d) vs offset-matched control->control (c2c) attention, 26 delimiter-tracking heads highlighted (d2d/c2c>=3x AND excess>=0.03), T1 block-reader heads ringed to show reader/delimiter dissociation; companion excess-by-layer panel with top reader L0H15 (excess rank 21/784) marked | emb_trace_attention_render.py | emb_trace_attention.pt + emb_trace_analysis.pt |
| fig20_delim_layer_profile.png | P1c: layer profile of comma-side (d2d) vs period->comma (p2c) aggregation, max-head and mean-head panels, peak layers marked — both peak at layer 0 (the falsification: period does not finalize deeper) | emb_trace_attention_render.py | emb_trace_attention.pt |
| fig21_delim_band_decomp.png | P1d: per-band excess q.k logit vs rotary wavelength (symlog-x), summed over the top-10 delimiter heads, with the measured inter-delimiter offset distribution (median 8 tok, IQR 5-14) and the 5-30 tok mid-band target overlaid — 99.1% of +excess sits in near-DC bands (wl>1e4), 0.4% in the offset-matched mid bands (the falsification) | emb_trace_attention_render.py | emb_trace_attention.pt |
