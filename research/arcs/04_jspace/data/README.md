...
fits produced both committed layer subsets. They also produced every
derived artifact that names one of these three lenses and predates
2026-08-16. Those full July lenses were never committed. The committed
cache lenses are the C4-redaction re-run refits. The two 1.5B refits
completed 2026-07-29, and the 7B refit completed 2026-08-16. The refits
produced the ten artifacts regenerated by runs 1-10 of
`examples/jspace_rerun_scans.sh` and the three issue-#83 fixed-K
paper-metric artifacts. Each MANIFEST `inputs` entry names its lens
generation, through the `_L*_JULY` and `_L*_REFIT` strings in
`examples/jspace_data_manifest.py`. The refit strings carry each lens's
sha256.
