...
training compute. AdamW is the production default at every disclosed
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-rms}.} --- that the
2024--2026 alternatives all measure themselves against. Lion's
sign-momentum and Sophia's diagonal-Hessian preconditioning each
...
speedup, downstream-evaluation gap.} Muon \citep{muon-moonlight2025},
which orthogonalizes momentum on matrix parameters via a Newton--Schulz
polynomial iteration, is the only published optimizer with a
$\sim\!2\times$ compute-efficiency lift verified at LLM scale: the
Moonlight 3B-active / 16B-MoE total run on $5.7$T tokens
\citep[\S 3.3]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-3-moonlight}.} The
schedule axis runs in parallel: cosine decay from GPT-3 / Llama-1/2/3
through to the Warmup--Stable--Decay (WSD) family that originated with
MiniCPM \citep{phi4} and is the schedule used by the Moonlight Muon
...
which dominate parameter count in a Transformer --- the geometry of
the update is now a first-class design variable. AdamW updates each
coordinate independently under a per-coordinate variance estimate;
Muon updates each matrix as a matrix, projecting the momentum onto
...
excerpt: \texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-2-scaling}.}
...
$\mathbf{W}$ & $\R^{A \times B}$ & a matrix-shaped parameter (FFN, attention QKV/O) \\
...
$\mu$ & $\R$ & Muon momentum decay (typical $0.95$) \\
...
$\hat{v}_t = v_t / (1 - \beta_2^t)$. The ``W'' in AdamW
\citep[Loshchilov \&
...
weight decay inside Adam's per-coordinate rescaling), AdamW applies
...
\tag{2}
\end{equation}
\Cref{eq:adamw} is the universal AdamW step at LLM scale --- LLaMA
\citep{meta-llama3}, DeepSeek-V3 \citep[\S 4]{deepseek-v3}, and Phi-4
...
For each matrix parameter $\mathbf{W} \in \R^{A \times B}$, the AdamW
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-rms}.} This number
is the calibration target every alternative measures itself against:
matching it lets one reuse AdamW's tuned $(\eta, \lambda)$ across
optimizers, sparing a frontier-scale sweep. \Cref{sec:opt-muon}
reports how the Muon family lands exactly on this number by design.

\paragraph{Optimizer-state cost.}
AdamW carries $2P$ words of moment state $(m, v)$ on top of the
parameters and gradients --- $4P$ words total in mixed-precision
...
Two AdamW alternatives published in 2023 reported
...
cost is one elementwise sign rather than a square root. Lion's
...
Sophia \citep[Liu, Li et al.\ 2023, ICLR
...
$h_t$, computed via a Hutchinson-style probe on the network's curvature
operator, and clips the resulting step. The reported $\sim\!2\times$
speedup over AdamW on training loss held in published replications.
The downstream gap is the load-bearing complication: AdamW catches up
or surpasses Sophia on standard evaluation suites at matched
\emph{wall-clock} time, despite Sophia's lower training loss.

\begin{contradiction}
\label{contra:opt-loss-vs-downstream}
Both Lion and Sophia exhibit the same pattern. Lower training loss at
...
and the downstream metric (MMLU, GSM8K, HumanEval, BBH-class)
...
2024--2025 LLM optimizer literature
...
\citet{muon-moonlight2025} scale up the Muon optimizer
...
section primarily cites.} to LLM training. Muon's central
...
column space accumulate signal faster than others --- and the AdamW
update preserves this anisotropy because per-coordinate rescaling is
basis-dependent. Muon's update orthogonalizes the momentum before
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-1-update-rule}.}:
\begin{equation}
\begin{aligned}
...
excerpt: \texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-1-ns}.}
with coefficients
$(a, b, c) = (3.4445,\, -4.7750,\, 2.0315)$
\citep[K.\ Jordan et al.\ 2024, cited in \S
2.1]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-1-ns}.} Five
iterations suffice in production (the polynomial converges to the
polar factor on each side of the singular-value spectrum at a quintic
...
$\{1\}$ in $N$ iterations. Returning to math: the canonical Muon
...
\cref{eq:muon-vanilla} converges faster than AdamW at small scale but
the gain diminishes at LLM scale because of two failure modes that
...
\citep[\S 2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-wd}.} The fix is
to fold AdamW-style decoupled weight decay into the Muon step:
\begin{equation}
...
excerpt: \texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-wd}.}
With \cref{eq:muon-wd}, Muon outperforms both vanilla Muon and AdamW
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-rms}.} states that
for a full-rank matrix parameter of shape $A \times B$, the
theoretical Muon update RMS is $\sqrt{1/\max(A, B)}$. Tall-thin
matrices (large $\max(A, B)$, e.g.\ FFN $\mathbf{W}_{\text{up}} \in
\R^{4D \times D}$) get under-updated; small per-head matrices
(treating each KV head in GQA \citep{ainslie2023} or MLA
...
$\sqrt{\max(A, B)}$ and a constant factor $0.2$ that lands in AdamW's
...
excerpt: \texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-rms}.}
The $0.2$ factor is exactly what calibrates the update RMS to AdamW's
typical $0.2$--$0.4$ range, so AdamW's tuned $(\eta, \lambda)$ ``can
directly \emph{reuse}'' for Muon \citep[\S
2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-rms}.} This is the
...
Muon updates only matrix parameters \citep[\S
2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-2-hybrid}.}
Embeddings ($\R^{V \times D}$ are 2-D but treated as embeddings, not
matrices, in the Muon convention because the row dim $V$ is not a
hidden-dim), the LM head, RMSNorm gains, and any other 1-D parameter
stay on AdamW. A production training run with Muon is therefore a
\emph{hybrid optimizer}: Muon on the body, AdamW on the periphery,
...
parameters live in a coordinate system the optimizer can read off ---
the natural basis of $\R^P$. Muon's polar projection treats matrix
parameters as matrices: rotations of the row or column basis leave
...
excerpt: \texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-1-norm}.}
...
Muon's is a Schatten-$p$ norm for some large $p$, a function of
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-2-scaling}.} run a
...
\citep[Figure 1a]{muon-moonlight2025}: ``Muon only requires about
52\% training FLOPs to match the performance of AdamW under
compute optimal setting'' \citep[\S
3.2]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-2-scaling}.} The
...
The largest published Muon run is Moonlight, a $2.24$B-active /
$15.29$B-total parameter Mixture-of-Experts (or $3$B-active / $16$B-
total when including embedding) trained on $5.7$T tokens
\citep[\S 3.3]{muon-moonlight2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-3-moonlight}.} The
architecture is the DeepSeek-V3-small variant from \citet{deepseek-v3}
with the auxiliary-loss-free MoE load-balancing scheme; the precision
recipe is BF16 working with FP32 master weights. The schedule is the
...
Two schedules dominate frontier LLM training, with a third at the
research frontier.
...
\label{eq:cosine}
\end{equation}
preceded by linear warmup over $\sim$2k steps. Used by GPT-3
...
training-step count, must be committed at warmup --- continuing
...
The Moonlight Muon run uses an explicit WSD with a quirk: the
cooldown is preceded by an LR \emph{re-rise} that loads
high-quality data into a controlled mini-warmup-then-decay phase.
Verbatim from \citet[\S
3.3]{muon-moonlight2025}\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-3-moonlight}.}:
\begin{equation}
...
SGD / AdamW under convex assumptions; large-scale LLM validation is
...
The optimizer-state partitioning that ZeRO-1 / FSDP exploit for
AdamW \citep[\S 2.2.4]{torchtitan2024} does not directly apply to
Muon: the Newton--Schulz iteration in \cref{eq:newton-schulz}
operates on the full $A \times B$ momentum matrix, not on the
per-DP-rank shard. \citet[\S
2.3]{muon-moonlight2025}\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-3-distributed}.}
introduce Distributed Muon, which partitions optimizer state on DP as
ZeRO-1 does and adds two operations:
\begin{enumerate}
\item \textbf{DP gather.} For each matrix parameter, gather the
DP-partitioned gradients into the full $A \times B$ matrix on the
local rank.
...
DeviceMesh \citep[\S 4]{torchtitan2024}: how Distributed Muon
interacts with TP / PP / EP, why some axes are
...
The Muon-Moonlight scaling law \eqref{eq:muon-scaling-fit} fits a
...
approximately constant in $C$, so the prediction is that Muon's lift
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-1-challenges}.} The
...
distributed effects (gradient noise from large batches, EP routing
...
LLM-scale optimizer-efficiency evidence on record but not yet a
frontier-scale ground truth.
\end{contradiction}

\begin{contradiction}
\label{contra:training-loss-vs-downstream}
Lion \citep[Chen et al.\ 2023]{muon-moonlight2025} and Sophia
...
LLM-specific replications. Muon's scaling-law claim
...
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-3-3-moonlight}.} of
the Moonlight tech report is supportive (Moonlight's downstream
metrics are reported competitive with similarly-sized DeepSeek-V3
...
Distributed Muon's $32$k-GPU behavior and the $G = d \cdot t \cdot c
\cdot p \cdot e$ composition belong to \cref{sec:distributed-training}.
The Muon-update-RMS-vs-BF16-range coupling
...
This section fixed the optimizer (AdamW as the production default;
Muon as the only published 2024--2026 optimizer with a $\sim 2\times$
LLM-scale compute-efficiency lift) and the schedule (WSD displacing
...
composes with the full DP / TP / PP / CP / EP DeviceMesh ---
including the Distributed Muon DP-gather + Newton--Schulz partial
...
\Cref{sec:mixed-precision} explores how the AdamW
update-RMS calibration (\cref{tab:opt-shapes}, $0.2$--$0.4$) and
Muon's matched $0.2\sqrt{\max(A, B)}$ scaling
...
stable, and how the format-zoo (FP32 master, BF16 working, FP8
GEMMs, NVFP4 frontier) lands on the optimizer-precision boundary
