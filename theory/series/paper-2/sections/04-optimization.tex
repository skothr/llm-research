...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms}.} --- that the
...
speedup, downstream-evaluation gap.} \glsterm{muon}{Muon} \citep{muon-moonlight2025},
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-3-moonlight}.} The
...
MiniCPM \citep{phi4} and is the schedule used by the Moonlight \glsterm{muon}{Muon}
...
\glsterm{muon}{Muon} updates each matrix as a matrix, projecting the momentum onto
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-2-scaling}.}
...
$\mu$ & $\R$ & \glsterm{muon}{Muon} momentum decay (typical $0.95$) \\
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-hybrid}.}}
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms}.} This number
...
reports how the \glsterm{muon}{Muon} family lands exactly on this number by design.
...
$h_t$, computed via a Hutchinson-style \glsterm{probe}{probe} on the network's curvature
...
\citet{muon-moonlight2025} scale up the \glsterm{muon}{Muon} optimizer
...
section primarily cites.} to \glsterm{llm}{LLM} training. \glsterm{muon}{Muon}'s central
...
basis-dependent. \glsterm{muon}{Muon}'s update orthogonalizes the momentum before
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-1-update-rule}.}:
\begin{equation}
\begin{aligned}
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-1-ns}.}
with coefficients
$(a, b, c) = (3.4445,\, -4.7750,\, 2.0315)$
\citep[K.\ Jordan et al.\ 2024, cited in \S
2.1]{muon-moonlight2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-1-ns}.} Five
...
$\{1\}$ in $N$ iterations. Returning to math: the canonical \glsterm{muon}{Muon}
...
\citep[\S 2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-wd}.} The fix is
to fold \glsterm{adamw}{AdamW}-style decoupled weight decay into the \glsterm{muon}{Muon} step:
\begin{equation}
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-wd}.}
With \cref{eq:muon-wd}, \glsterm{muon}{Muon} outperforms both vanilla \glsterm{muon}{Muon} and \glsterm{adamw}{AdamW}
in the over-train regime that \cref{tab:frontier-NDC} of
\cref{sec:scaling-laws} characterizes.

\textbf{\glsterm{per-shape-rms-scaling}{Per-shape RMS scaling}.} Lemma 1 of
\citet[\S 2.2]{muon-moonlight2025}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms}.} states that
for a full-rank matrix parameter of shape $A \times B$, the
theoretical \glsterm{muon}{Muon} update RMS is $\sqrt{1/\max(A, B)}$. Tall-thin
matrices (large $\max(A, B)$, e.g.\ \glsterm{ffn}{FFN} $\mathbf{W}_{\text{up}} \in
\R^{4D \times D}$) get under-updated; small per-head matrices
(treating each KV head in \glsterm{gqa}{GQA} \citep{ainslie2023} or \glsterm{mla}{MLA}
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms}.}
...
directly \emph{reuse}'' for \glsterm{muon}{Muon} \citep[\S
2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms}.} This is the
...
\glsterm{muon}{Muon} updates only matrix \glsterm{parameters}{parameters} \citep[\S
2.2]{muon-moonlight2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-hybrid}.}
Embeddings ($\R^{V \times D}$ are 2-D but treated as embeddings, not
matrices, in the \glsterm{muon}{Muon} convention because the row dim $V$ is not a
hidden-dim), the LM head, \glsterm{rmsnorm}{RMSNorm} gains, and any other 1-D parameter
stay on \glsterm{adamw}{AdamW}. A production training run with \glsterm{muon}{Muon} is therefore a
\emph{hybrid optimizer}: \glsterm{muon}{Muon} on the body, \glsterm{adamw}{AdamW} on the periphery,
...
the natural basis of $\R^P$. \glsterm{muon}{Muon}'s polar projection treats matrix
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-1-norm}.}
...
\glsterm{muon}{Muon}'s is a \glsterm{schatten-p-norm}{Schatten-$p$ norm} for some large $p$, a function of
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-2-scaling}.} run a
...
\citep[Figure 1a]{muon-moonlight2025}: ``\glsterm{muon}{Muon} only requires about
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-2-scaling}.} The
...
The largest published \glsterm{muon}{Muon} run is Moonlight, a $2.24$B-active /
...
The Moonlight \glsterm{muon}{Muon} run uses an explicit WSD with a quirk: the
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-3-moonlight}.}:
\begin{equation}
...
\glsterm{muon}{Muon}: the Newton--Schulz iteration in \cref{eq:newton-schulz}
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-3-distributed}.}
introduce Distributed \glsterm{muon}{Muon}, which partitions optimizer state on \glsterm{data-parallel}{DP} as
...
DeviceMesh \citep[\S 4]{torchtitan2024}: how Distributed \glsterm{muon}{Muon}
...
The \glsterm{muon}{Muon}-Moonlight \glsterm{scaling-law}{scaling law} \eqref{eq:muon-scaling-fit} fits a
...
approximately constant in $C$, so the prediction is that \glsterm{muon}{Muon}'s lift
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-1-challenges}.} The
...
\glsterm{llm}{LLM}-specific replications. \glsterm{muon}{Muon}'s scaling-law claim
...
\nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-3-3-moonlight}.} of
...
Distributed \glsterm{muon}{Muon}'s $32$k-GPU behavior and the $G = d \cdot t \cdot c
\cdot p \cdot e$ composition belong to \cref{sec:distributed-training}.
The \glsterm{muon}{Muon}-update-RMS-vs-BF16-range coupling
...
\glsterm{muon}{Muon} as the only published 2024--2026 optimizer with a $\sim 2\times$
...
including the Distributed \glsterm{muon}{Muon} \glsterm{data-parallel}{DP}-gather + Newton--Schulz partial
...
\glsterm{muon}{Muon}'s matched $0.2\sqrt{\max(A, B)}$ scaling
