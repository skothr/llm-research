...
sparse $\{0,1\}$ verifier signal that drives a $30$K-sample \glsterm{grpo-2}{GRPO} run on
...
\glsterm{dpo}{DPO}-vs-PPO-vs-\glsterm{grpo-2}{GRPO} \glsterm{debate}{debate}, and the recurrent claim that ``\glsterm{rlvr-3}{RLVR} creates
...
\item \textbf{Supervised fine-tuning.} The first post-training stage:
  ...
  (\cref{sec:sft}).
\item \textbf{Preference-RL.} \glsterm{rlhf}{RLHF}, \glsterm{rlaif-3}{RLAIF} / \glsterm{constitutional-ai}{Constitutional AI}, \glsterm{dpo}{DPO} and
...
\item \textbf{Reasoning-RL and adaptation.} RL on verifiable rewards
  (\glsterm{rlvr-3}{RLVR} / \glsterm{grpo-2}{GRPO} / \glsterm{dapo-2}{DAPO}) for math, code, and proof, plus the weight-space
  operations (\glsterm{lora}{LoRA} / \glsterm{qlora}{QLoRA} / model soups / TIES) that compose
...
power laws on small-to-medium \glsterm{decoder-only}{decoder-only} LMs and recommended scaling
...
matched their actual token horizons (Kaplan's \glsterm{cosine-schedule}{cosine schedule} decayed
...
\glsterm{rlhf}{RLHF} closed-form RL optimum
...
without an explicit \glsterm{reward-model}{reward model}, without on-policy sampling, without
a \glsterm{value}{value} head. Same fixed point, two-orders-cheaper per step. The 2023
narrative ``\glsterm{dpo}{DPO} replaces PPO'' was overstated: at frontier scale the
two coexist (Llama-3 and T\"ulu-3 ship \glsterm{dpo}{DPO}; DeepMind / Gemma-3 keep
PPO with weight-averaged reward models), and \citet{meng2024-simpo}
reframes the \glsterm{dpo}{DPO} family as a small organized lattice with three
...
$15.6\%$ to $77.9\%$ over a single \glsterm{grpo-2}{GRPO} run, with the model emitting a
...
narrowed that claim: \glsterm{rlvr-3}{RLVR} sharpens pass@1 on problems already solvable
at some pass@$K$, but does not expand the set of problems solvable at
any $K$; the \glsterm{capability-ceiling}{capability ceiling} is set by pre-training. \glsterm{rlvr-3}{RLVR} is
...
onto a demonstration distribution. \glsterm{rlhf}{RLHF} / \glsterm{dpo}{DPO} / \glsterm{rlaif-3}{RLAIF} perform a reverse-KL
reweighting of $\pi^{\mathrm{SFT}}$ toward higher reward, with the anchor
$\beta\,\KL(\pi_\theta \|\, \pi^{\mathrm{SFT}})$ controlling how far the
policy may drift. \glsterm{rlvr-3}{RLVR} performs the same reverse-KL reweighting with the
...
\glsterm{rlhf}{RLHF}, \glsterm{rlaif-3}{RLAIF}, \glsterm{dpo}{DPO}, and \glsterm{rlvr-3}{RLVR}. The pipeline is a \emph{chain} of such
...
disclosed stack: \glsterm{precision-pinning}{DeepSeek-V3} / R1, Llama 3 / 4, Qwen 2.5 / 3, Phi-4,
T\"ulu 3, OLMo 2, \glsterm{mistral}{Mistral} Large, Gemma 3. Every disclosed pipeline
...
GPT-3 \citep{brown2020} shipped without stages 4--6; InstructGPT
\citep{ouyang2022-instructgpt} added stages 4--5 (SFT and PPO-\glsterm{rlhf}{RLHF});
the \glsterm{rlvr-3}{RLVR} / \glsterm{grpo-2}{GRPO} / \glsterm{dapo-2}{DAPO} line \citep{shao2024,deepseek-r1,dapo2025} added
...
Paper 2 assumes Paper 1's residual-stream / \glsterm{ffn}{FFN} / attention notation.
The \glsterm{transformer}{Transformer} block, with its alternating attention and feed-forward
...
$d_h$, \glsterm{vocabulary}{vocabulary} size $V$, and layer count $L$ from Paper 1's
...
\cref{sec:rlvr-grpo}'s \glsterm{grpo-2}{GRPO} and \glsterm{dapo-2}{DAPO} machinery is the \emph{training
leg} of Paper 3's compute / search / verification triangle for
reasoning. The \emph{inference} side --- \glsterm{chain-of-thought}{chain-of-thought} sampling,
\glsterm{self-consistency-2}{self-consistency}, process reward models, MCTS-style tree search at
...
surface that \glsterm{rlhf}{RLHF} / \glsterm{constitutional-ai}{CAI} shapes.
...
training (\cref{sec:distributed-training} on \glsterm{expert-parallel}{EP}, \cref{sec:mixed-precision}
on \glsterm{fp8}{FP8} expert-collapse) and is cited from there. (ii) Inference-time
...
\item[\cref{sec:pre-training-data} --- Pre-training data.] Curate
  $\to$ ablate $\to$ mix. The \glsterm{custom-heuristic-filters}{FineWeb} pipeline end-to-end (96 \glsterm{common-crawl}{CC}
  snapshots, MinHash 14$\times$8, custom quantile-derived filters), the
  Data Mixing Laws functional form, and the \glsterm{fineweb-edu}{FineWeb-Edu}
...
\item[\cref{sec:scaling-laws} --- Scaling laws.] Kaplan's three power
  ...
  at frontier scale.
\item[\cref{sec:optimization} --- Optimization.] \glsterm{adamw}{AdamW} as the
  production default; \glsterm{lion}{Lion} and \glsterm{sophia}{Sophia} as training-loss-only speedups;
  \glsterm{per-shape-rms-scaling}{Muon}'s Newton--Schulz orthogonalization with a verified
  $\sim\!2\times$ compute-efficiency lift at $16$B-MoE / $5.7$T tokens;
  the schedule axis (cosine $\to$ \glsterm{warmup}{Warmup}--Stable--Decay $\to$
  Schedule-Free).
\item[\cref{sec:distributed-training} --- Distributed training.] Five
  composable axes on a single \texttt{DeviceMesh}:
  $G = d \cdot t \cdot c \cdot p \cdot e$ for \glsterm{data-parallel}{DP} $\times$ \glsterm{tensor-parallel}{TP} $\times$
  \glsterm{context-parallel}{CP} $\times$ \glsterm{pipeline-parallel}{PP} $\times$ \glsterm{expert-parallel}{EP}. Per-axis cost analysis, \glsterm{precision-pinning}{DeepSeek-V3}'s
  \glsterm{tensor-parallel}{TP}-free plan as case study, TorchTitan's 4D composability,
  Distributed \glsterm{per-shape-rms-scaling}{Muon}.
\item[\cref{sec:mixed-precision} --- Mixed precision and stability.]
  ...
  (FP32 / FP16 / BF16 / \glsterm{fp8}{FP8} / \glsterm{nvfp4}{NVFP4} / BitNet); \glsterm{precision-pinning}{DeepSeek-V3}'s
  fine-grained \glsterm{fp8}{FP8} recipe with zero irrecoverable loss spikes over
  $14.8$T tokens; loss-spike taxonomy and mitigations.
\item[\cref{sec:sft} --- Supervised fine-tuning.] Cross-entropy with
  prompt-masking; the data-not-loss thesis; lineage from \glsterm{flan}{FLAN} through
  InstructGPT to T\"ulu-3, \glsterm{magpie}{MAGPIE}, R1-Distill, and s1; multi-turn
...
\item[\cref{sec:rlhf} --- RLHF.] InstructGPT's three-stage pipeline;
  ...
  reward-model variants (ensembles, \glsterm{warm}{WARM}/\glsterm{warp}{WARP}).
\item[\cref{sec:dpo} --- DPO and offline preference.] The
  reparameterization that turns the \glsterm{rlhf}{RLHF} optimum into a classification
  loss on the same preference data; the variant lattice (\glsterm{ipo}{IPO}, \glsterm{kto}{KTO},
  \glsterm{orpo}{ORPO}, \glsterm{simpo}{SimPO}, CPO) along three knobs.
\item[\cref{sec:rlaif-cai} --- RLAIF and Constitutional AI.] The same
  \glsterm{rlhf}{RLHF} machinery with an AI labeler; \glsterm{constitutional-ai}{CAI}'s two-stage pipeline (\glsterm{sl-cai-2}{SL-CAI}
  $\to$ \glsterm{rl-cai-2}{RL-CAI}); the \glsterm{constitution}{constitution} as the \emph{only} human-authored
  alignment input; \glsterm{non-evasion}{non-evasion} behavior and its dual-use surface.
\item[\cref{sec:rlvr-grpo} --- RLVR and GRPO.] Programmatic verifiers
  ...
  co-sized \glsterm{value}{value} models; DeepSeek-R1's \glsterm{grpo-2}{GRPO} trajectory and the \glsterm{dapo-2}{DAPO}
  improvements (\glsterm{clip-higher}{Clip-Higher}, \glsterm{dynamic-sampling}{Dynamic Sampling}, Token-Level Loss,
  Overlong Reward Shaping). Cross-paper anchor for Paper~3.
\item[\cref{sec:adaptation-and-merging} --- Adaptation and merging.]
  \glsterm{lora}{LoRA} / \glsterm{qlora}{QLoRA} as low-rank parameter-efficient fine-tuning; Model
  Soups, Task Vectors, \glsterm{ties-merging}{TIES-Merging} as weight-space operations; the
  linear-mode-connectivity story as the speculative theoretical
  underpinning.
\item[\cref{sec:frontier-snapshot} --- Frontier-2026 snapshot.] The
  six-stage table across the disclosed frontier (\glsterm{precision-pinning}{DeepSeek-V3}\allowbreak{} /\allowbreak{} R1,
  Llama 3\allowbreak{} /\allowbreak{} 4, Qwen 2.5\allowbreak{} /\allowbreak{} 3, Phi-4, T\"ulu 3, OLMo 2, \glsterm{mistral}{Mistral} Large,
...
\item[\cref{sec:contradictions} --- Live contradictions.] Concentrated
  ...
  the body: Kaplan-vs-Chinchilla, optimizer speedups, \glsterm{tensor-parallel}{TP}-yes-vs-\glsterm{tensor-parallel}{TP}-no,
  \glsterm{fp8}{FP8} / \glsterm{nvfp4}{NVFP4} / BitNet production status, SFT data quantity-vs-quality,
  reward-hacking, \glsterm{dpo}{DPO}-vs-PPO, \glsterm{constitutional-ai}{CAI} internalization, \glsterm{rlvr-3}{RLVR} capability,
