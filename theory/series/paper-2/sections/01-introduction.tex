...
sparse $\{0,1\}$ verifier signal that drives a $30$K-sample \glsterm{grpo}{GRPO} run on
...
\glsterm{dpo}{DPO}-vs-PPO-vs-\glsterm{grpo}{GRPO} \glsterm{debate}{debate}, and the recurrent claim that ``\glsterm{rlvr}{RLVR} creates
...
\item \textbf{Supervised fine-tuning.} The first post-training stage:
  ...
  (\cref{sec:sft}).
\item \textbf{Preference-RL.} \glsterm{rlhf}{RLHF}, \glsterm{rlaif}{RLAIF} / \glsterm{constitutional-ai}{Constitutional AI}, \glsterm{dpo}{DPO} and
...
\item \textbf{Reasoning-RL and adaptation.} RL on verifiable rewards
  (\glsterm{rlvr}{RLVR} / \glsterm{grpo}{GRPO} / \glsterm{dapo}{DAPO}) for math, code, and proof, plus the weight-space
...
$15.6\%$ to $77.9\%$ over a single \glsterm{grpo}{GRPO} run, with the model emitting a
...
narrowed that claim: \glsterm{rlvr}{RLVR} sharpens pass@1 on problems already solvable
at some pass@$K$, but does not expand the set of problems solvable at
any $K$; the \glsterm{capability-ceiling}{capability ceiling} is set by pre-training. \glsterm{rlvr}{RLVR} is
...
onto a demonstration distribution. \glsterm{rlhf}{RLHF} / \glsterm{dpo}{DPO} / \glsterm{rlaif}{RLAIF} perform a reverse-KL
reweighting of $\pi^{\mathrm{SFT}}$ toward higher reward, with the anchor
$\beta\,\KL(\pi_\theta \|\, \pi^{\mathrm{SFT}})$ controlling how far the
policy may drift. \glsterm{rlvr}{RLVR} performs the same reverse-KL reweighting with the
...
\glsterm{rlhf}{RLHF}, \glsterm{rlaif}{RLAIF}, \glsterm{dpo}{DPO}, and \glsterm{rlvr}{RLVR}. The pipeline is a \emph{chain} of such
...
the \glsterm{rlvr}{RLVR} / \glsterm{grpo}{GRPO} / \glsterm{dapo}{DAPO} line \citep{shao2024,deepseek-r1,dapo2025} added
...
\cref{sec:rlvr-grpo}'s \glsterm{grpo}{GRPO} and \glsterm{dapo}{DAPO} machinery is the \emph{training
...
\glsterm{self-consistency}{self-consistency}, process reward models, MCTS-style tree search at
...
\item[\cref{sec:optimization} --- Optimization.] \glsterm{adamw}{AdamW} as the
  ...
  \glsterm{muon}{Muon}'s Newton--Schulz orthogonalization with a verified
...
\item[\cref{sec:rlaif-cai} --- RLAIF and Constitutional AI.] The same
  \glsterm{rlhf}{RLHF} machinery with an AI labeler; \glsterm{constitutional-ai}{CAI}'s two-stage pipeline (\glsterm{sl-cai}{SL-CAI}
  $\to$ \glsterm{rl-cai}{RL-CAI}); the \glsterm{constitution}{constitution} as the \emph{only} human-authored
...
\item[\cref{sec:rlvr-grpo} --- RLVR and GRPO.] Programmatic verifiers
  ...
  co-sized \glsterm{value}{value} models; DeepSeek-R1's \glsterm{grpo}{GRPO} trajectory and the \glsterm{dapo}{DAPO}
...
\item[\cref{sec:contradictions} --- Live contradictions.] Concentrated
  ...
  reward-hacking, \glsterm{dpo}{DPO}-vs-PPO, \glsterm{constitutional-ai}{CAI} internalization, \glsterm{rlvr}{RLVR} capability,
