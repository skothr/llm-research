...
on data: SFT cross-entropy, \glsterm{rlhf}{RLHF}/\glsterm{dpo}{DPO} preference signals, \glsterm{rlvr}{RLVR}'s
...
\emph{\glsterm{linear-mode-connectivity}{linear mode connectivity}} (\glsterm{linear-mode-connectivity}{LMC}), with \citet{frankle2020-lmc}\deepencite{frankle2020-lmc
§3} the canonical reference\footnote{\glsterm{linear-mode-connectivity}{LMC} was introduced for SGD-trained
...
The condition is empirically tight in a load-bearing way. \glsterm{linear-mode-connectivity}{LMC} holds for
...
basin-centering effect predicted by \glsterm{linear-mode-connectivity}{LMC}. The
...
\glsterm{rlhf}{RLHF} (\S8), \glsterm{dpo}{DPO} (\S9), \glsterm{rlaif}{RLAIF}/\glsterm{constitutional-ai}{CAI} (\S10), \glsterm{rlvr}{RLVR}/\glsterm{grpo}{GRPO} (\S11) --- moved the
...
fine-tunes that share an ancestor. Both rest on the \glsterm{linear-mode-connectivity}{LMC} substrate of
