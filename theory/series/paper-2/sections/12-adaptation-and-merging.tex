...
on data: SFT cross-entropy, \glsterm{rlhf}{RLHF}/\glsterm{dpo}{DPO} preference signals, \glsterm{rlvr-3}{RLVR}'s
...
Parameter-efficient fine-tuning (\glsterm{peft}{PEFT}) writes a low-rank patch onto a
frozen base; \glsterm{model-merging}{model merging} averages, signs, and trims weight vectors
...
open-weight \glsterm{llm}{LLM} since LLaMA-2 ships with a \glsterm{lora}{LoRA} adapter ecosystem, and
...
\nolinkurl{kb/excerpts/hu2021-lora.md#sec-3}.}:
\begin{equation}
...
trained. The forward pass at a \glsterm{hidden-state}{hidden state} $x$ is
\begin{equation}
...
$\sum_\ell r(d_\ell + k_\ell)$. With \glsterm{lora}{LoRA} applied to all attention
...
The deployment-time mechanic is what separates \glsterm{lora}{LoRA} from earlier
...
latency at every forward pass; \glsterm{lora}{LoRA} computes
\begin{equation}
...
inference-time mechanic that S-\glsterm{lora}{LoRA} and Punica multi-tenant systems
exploit (cross-ref Paper~1's serving discussion).

\subsection{QLoRA: quantize the base, train the adapter}
\label{sec:adapt-qlora}

\citet{dettmers2023-qlora} extend \glsterm{lora}{LoRA} by quantizing the frozen base
$W_0$ to four bits while keeping the \glsterm{lora}{LoRA} factors in BF16. The headline
\citep[abstract]{dettmers2023-qlora}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dettmers2023-qlora.md#abstract}.} is a 65B-parameter
...
\emph{\glsterm{nf4}{NormalFloat-4}} (\glsterm{nf4}{NF4}) format stores 16 bin centers chosen as the
...
\nolinkurl{kb/excerpts/dettmers2023-qlora.md#sec-3-nf4}.}, so \glsterm{nf4}{NF4}'s bin
...
\nolinkurl{kb/excerpts/dettmers2023-qlora.md#sec-3-doublequant}.}. At
65B \glsterm{parameters}{parameters} the saving is $\sim\!1$~GB.

\paragraph{Paged optimizers.} Optimizer state for the \glsterm{lora}{LoRA} factors is
...
\nolinkurl{kb/excerpts/dettmers2023-qlora.md#sec-3-paged}.}. Long-sequence
batches that would OOM the 48~GB GPU instead spill to host RAM at a
small bandwidth cost.

\glsterm{qlora}{QLoRA}-tuned models recover within $\sim\!1\%$ of full BF16 fine-tunes
...
$\theta_T$ on task $T$, the \emph{\glsterm{task-vector}{task vector}} is\footnote{KB excerpt:
\nolinkurl{kb/excerpts/ilharco2022-task-vectors.md#sec-2}.}
\begin{equation}
...
toxicity-\glsterm{task-vector}{task vector} reduces toxic generation while preserving general
...
\nolinkurl{kb/excerpts/wortsman2022-model-soups.md#sec-3}.} address the
...
\nolinkurl{kb/excerpts/yadav2023-ties-merging.md#sec-3}.}: when two
...
\emph{\glsterm{linear-mode-connectivity-2}{linear mode connectivity}} (\glsterm{linear-mode-connectivity-2}{LMC}), with \citet{frankle2020-lmc}\deepencite{frankle2020-lmc
§3} the canonical reference\footnote{\glsterm{linear-mode-connectivity-2}{LMC} was introduced for SGD-trained
...
The condition is empirically tight in a load-bearing way. \glsterm{linear-mode-connectivity-2}{LMC} holds for
SFT-from-the-same-base; it holds partly for \glsterm{rlhf}{RLHF}-from-the-same-base
...
\glsterm{lora}{LoRA}'s adapter is a delta on a fixed $W_0$, soup recipes assume the
...
\glsterm{llm}{LLM} merging community at frontier scale — merged models routinely
...
no third-party head-to-head at frontier-\glsterm{llm}{LLM} scale, and no canonical
theory predicts when merging produces interference vs.\ the
basin-centering effect predicted by \glsterm{linear-mode-connectivity-2}{LMC}. The
...
\glsterm{rlhf}{RLHF} (\S8), \glsterm{dpo}{DPO} (\S9), \glsterm{rlaif-3}{RLAIF}/\glsterm{constitutional-ai}{CAI} (\S10), \glsterm{rlvr-3}{RLVR}/\glsterm{grpo-2}{GRPO} (\S11) --- moved the
policy in data-driven directions; \cref{sec:adaptation-and-merging}
makes the dual move in weight space, with \glsterm{peft}{PEFT}
...
fine-tunes that share an ancestor. Both rest on the \glsterm{linear-mode-connectivity-2}{LMC} substrate of
...
(\glsterm{precision-pinning}{DeepSeek-V3}/R1, Llama-3/4, Qwen-2.5/3, Phi-4, Tülu-3, OLMo-2, Gemma-3,
\glsterm{mistral}{Mistral}) and the partial disclosures from the closed labs. The
