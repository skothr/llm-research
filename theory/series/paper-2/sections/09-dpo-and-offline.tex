...
loss on the same preference data the reward model of
...
no sampler, no value head): \emph{same fixed point on the same data,
dramatically cheaper per step.} The variant family that has grown
around DPO since 2023 (IPO, KTO, ORPO, SimPO, CPO) inherits this
...
$\mathcal{D} = \{(x_i, y_w^i, y_l^i)\}_{i=1}^N$ then yields the DPO
...
\cref{eq:rlhf-rm} of \cref{sec:rlhf}, evaluated on the implicit reward
...
\cref{eq:dpo-loss}, then by construction the implicit reward
...
$\pi^*$ of \cref{eq:dpo-rl-optimum} on that reward. \emph{DPO and
PPO-RLHF share the same fixed point on the same preference data}
...
rollouts scored by a learned $r_\phi$; DPO walks toward $\pi^*$ along
...
\end{equation}
the \emph{preference-margin gradient} that defines all DPO-class
...
implicit reward (the policy assigns lower implicit reward to the
...
\begin{intuition}
The DPO derivation is an \emph{inversion} trick. The RL optimum
...
model is secretly a reward model'' phrasing
...
\end{enumerate}
There is no sampling, no reward model, no value model, no reinforcement
...
``DPO-replaces-PPO'' move at frontier scale.
...
optional: although DPO does not have PPO's value-network instability,
...
\paragraph{On-policy preferences with off-policy optimization.} DPO's
...
to \emph{iterate}: run DPO to convergence, sample new responses from
the resulting policy, label them (human or LLM-judge) into new
preference triples, and run DPO again --- often with
$\pi^{\mathrm{ref}}$ kept at the original $\pi^{\mathrm{SFT}}$ across
rounds, so the implicit reward stays anchored to the SFT support. The
...
The DPO variant landscape since 2023 has not produced a clear winner;
...
\midrule
PPO-RLHF & $\pi^{\mathrm{SFT}}$ & learned $r_\phi$ & no & on-policy clipped surrogate \\
DPO & $\pi^{\mathrm{ref}}$ frozen & $\beta \log \pi_\theta / \pi^{\mathrm{ref}}$ & no & log-sigmoid \\
IPO & $\pi^{\mathrm{ref}}$ frozen & same as DPO & no & squared-gap \\
KTO & $\pi^{\mathrm{ref}}$ frozen & prospect-theory utility & no & asymmetric per-rating \\
ORPO & none & SFT loss + log-odds-ratio & no & combined SFT + OR \\
SimPO & none & $(\beta/|y|) \log \pi_\theta(y \mid x)$ & yes & log-sigmoid + margin $\gamma$ \\
...
\paragraph{IPO --- squared-gap loss.} The DPO log-sigmoid loss has a
known overconfidence pathology: when one preference is clearly
satisfied, the implicit reward gap $\hat r_\theta(y_w) - \hat
...
bulk. IPO replaces the log-sigmoid with a squared loss on the gap,
\begin{equation}
...
\citep[Azar et al.\ 2023; Eq.~17 in the paper's IPO derivation]{rafailov2023-dpo}.\footnote{IPO's
canonical form as reproduced in DPO survey arXiv:2503.11701 \S4.1.}
...
\paragraph{KTO --- single-rating data, prospect theory.} KTO operates
on \emph{single-rating} data $(x, y, \mathrm{label})$ with
$\mathrm{label} \in \{\textsf{good}, \textsf{bad}\}$, instead of the
pairwise $(x, y_w, y_l)$ DPO requires. The loss is asymmetric, weighted
...
KTO is the right method when the labeling pipeline can produce thumbs-
up / thumbs-down judgments but not pairwise rankings (e.g., binary
production-feedback streams).

\paragraph{ORPO --- combined SFT and preference, no reference.} ORPO
...
(SFT $\to$ DPO) to one (ORPO from base directly). The cost: the
reference-frozen guarantee that DPO inherits from
\cref{eq:dpo-rl-optimum} no longer applies, so ORPO is no longer the
same fixed point as PPO-RLHF on the same data --- it is a different
optimum that may or may not match.

\paragraph{SimPO --- reference-free, length-normalized.} SimPO drops
the reference and divides the implicit reward by sequence length
...
\label{eq:simpo-loss}
\end{equation}
Two changes vs.\ DPO. First, no reference model: the implicit reward
...
report SimPO outperforming DPO substantially on length-controlled
...
reference (like SimPO and ORPO) and adds an SFT regularizer on the
chosen response (like ORPO), but keeps the DPO log-sigmoid form
\citep[Xu et al.\ 2024]{rafailov2023-dpo}.
\deepencite{xu2024-cpo §3 — bibkey not yet in references.bib} CPO is
empirically positioned between SimPO and ORPO on the lattice and is
most-cited in machine-translation post-training.

\begin{analogy}
The variant family is best read as one organized axis: \emph{anchored
vs.\ reference-free}. Anchored methods (DPO, IPO, KTO) keep
$\pi^{\mathrm{ref}}$ in the loss; the implicit reward is a
\emph{ratio}, and the policy is constrained --- through the algebra of
$\log \pi_\theta / \pi^{\mathrm{ref}}$ --- to live on the support of
the reference. Reference-free methods (ORPO, SimPO, CPO) drop the
reference; the implicit reward becomes an \emph{absolute} quantity
...
substitute different mechanisms (SimPO's length-norm; ORPO's SFT loss;
...
PPO-RLHF, reference-free variants do not --- and what cost you pay ---
...
DPO's documented \emph{length bias} (longer outputs systematically beat
...
SimPO's length-normalized reward \cref{eq:simpo-reward} fixes the
mechanism by construction: dividing by $|y|$ converts the sum-of-tokens
implicit reward into a \emph{mean-of-tokens} implicit reward, which is
...
SimPO's gain over DPO on length-controlled
AlpacaEval~2.\footnote{KB excerpt:
\texttt{kb/excerpts/meng2024-simpo.md\#sec-4-headline}.}

\begin{intuition}
SimPO's length-normalization is to DPO's implicit reward what
perplexity is to log-likelihood. Raw log-likelihood favors short
sequences (each token contributes a negative term to the total);
perplexity normalizes by length to compare sequences fairly across
sequence-length regimes. Similarly, DPO's $\log \pi_\theta(y \mid x)$
favors sequences whose token-level log-probs are easy to inflate (the
loss reads a sum); SimPO's $\log \pi_\theta(y \mid x) / |y|$ normalizes
for length so the implicit reward is comparable across responses of
...
The empirical narrative around DPO at frontier scale has settled into
two camps. The Llama-3 / T\"ulu-3 camp \citep{meta-llama3, tulu3} runs
SFT $\to$ rejection-sampling SFT $\to$ multi-round iterated DPO and
reports that DPO required less compute than PPO and performed as well
or better at the post-training stage \citep[\S 4]{meta-llama3}. T\"ulu-3
\citep{tulu3} adopts the same SFT $\to$ DPO $\to$ RLVR
...
\citep{gemma3} retains a PPO-style RLHF stage with WARM/WARP-augmented
...
that the DPO-replaces-PPO move sacrifices on creative or open-ended
generation.
...
RLAIF / Constitutional AI \citep{bai2022-cai} family: AI judgments under
a constitution can replace human labels at the preference-data step,
and the resulting AI-preference triples can feed DPO directly with no
PPO loop in between. The composition --- AI-labeled preferences
$\to$ DPO --- is increasingly the default at open-source scale,
because both halves of the pipeline avoid human labeling and on-policy
RL. \cref{sec:rlvr-grpo}'s RLVR / GRPO stack is the verifier-domain
...
group-mean baseline. DPO and RLVR are orthogonal: DPO replaces the
\emph{algorithm} (PPO $\to$ classification on preferences); RLVR
replaces the \emph{reward source} (learned RM $\to$ verifier). Modern
post-training pipelines compose both --- T\"ulu-3 runs DPO for general
preferences and GRPO for verifier-domain reasoning --- treating them as
...
\begin{contradiction}
\textbf{DPO-vs-PPO at frontier scale.} The Llama-3 / T\"ulu-3 view
\citep{meta-llama3, tulu3} is that DPO is \emph{better} than PPO at
frontier scale, not just cheaper: with iterated on-policy preference
collection, the DPO-class pipeline matches or exceeds PPO-class
...
PPO + WARM/WARP-averaged reward ensembles is still the production
...
different KL schedules) and partly task-specific: DPO is more
...
\begin{contradiction}
\textbf{DPO does not avoid reward overoptimization.} The early DPO
narrative was structural: ``no explicit reward model means no reward
...
grows in PPO-RLHF. The mechanism is more subtle than ``DPO has no
reward model'': the policy log-ratio \emph{is} the reward model, and
the policy hacks itself. The empirical implication: the
$\KL$-anchor regularization is load-bearing for both PPO-RLHF and
DPO, and the practical mitigations --- on-policy iteration, KL
scheduling, early stopping --- are necessary for DPO too. Pick up
in \S 14.
\end{contradiction}

\begin{contradiction}
\textbf{Length bias is annotator-bias-or-loss-form.} SimPO frames the
length-normalization fix as a loss-form correction
\citep[\S 3]{meng2024-simpo}: DPO's implicit-reward sum-over-tokens
...
right: SimPO's $1/|y|$ fix demonstrably reduces length bias on
...
collection followed by both DPO and SimPO training, with held-out
...
\cref{sec:dpo-length} SimPO's length-bias fix; \cref{sec:dpo-frontier}
...
itself: AI-judge-under-a-constitution preferences feed the same DPO
loss directly, with \cref{eq:dpo-loss} unchanged but the dataset
$\mathcal{D}$ now generated by a preference-labeling LLM rather than
...
of \cref{sec:rlhf-rm} and the implicit reward of
...
(GRPO) that exploits the verifier's lack of a proxy-vs-gold gap. The
canonical 2026 post-training stack runs all three stages in sequence:
SFT (\cref{sec:sft}) $\to$ preference-anchored DPO or its variants
(\cref{sec:dpo}) $\to$ verifier-domain RLVR with GRPO
