...
excerpt: \nolinkurl{kb/excerpts/rafailov2023-dpo.md#sec-4}.} The thesis
...
\nolinkurl{kb/excerpts/rafailov2023-dpo.md#sec-4}.} \citet{rafailov2023-dpo}'s
...
\nolinkurl{kb/excerpts/rafailov2023-dpo.md#sec-4-loss}.}
\begin{equation}
...
\nolinkurl{kb/excerpts/rafailov2023-dpo.md#sec-4-gradient}.} gives
...
\toprule
Method & Reward form & Ref.\ policy & Len-norm & Loss shape \\
\midrule
PPO-\glsterm{rlhf}{RLHF} & learned $r_\phi$ & $\pi^{\mathrm{SFT}}$ & no & on-policy clipped surrogate \\
\glsterm{dpo}{DPO} & $\beta \log \pi_\theta / \pi^{\mathrm{ref}}$ & $\pi^{\mathrm{ref}}$ frozen & no & log-sigmoid \\
\glsterm{ipo}{IPO} & same as \glsterm{dpo}{DPO} & $\pi^{\mathrm{ref}}$ frozen & no & squared-gap \\
\glsterm{kto}{KTO} & prospect-theory utility & $\pi^{\mathrm{ref}}$ frozen & no & asymmetric per-rating \\
\glsterm{orpo}{ORPO} & SFT loss + log-odds-ratio & none & no & combined SFT + OR \\
\glsterm{simpo}{SimPO} & $(\beta/|y|) \log \pi_\theta(y \mid x)$ & none & yes & log-sigmoid + margin $\gamma$ \\
CPO & policy log-prob + SFT regularizer & none & no & log-sigmoid + SFT \\
\bottomrule
\end{tabularx}
...
\end{table}
\footnotetext{KB excerpts: \nolinkurl{kb/excerpts/rafailov2023-dpo.md#sec-4-loss},
\nolinkurl{kb/excerpts/meng2024-simpo.md#sec-2}.}
...
\citep[\S 2]{meng2024-simpo}:\footnote{KB excerpt:
\nolinkurl{kb/excerpts/meng2024-simpo.md#sec-2}.}
\begin{equation}
r_{\mathrm{SimPO}}(x, y)
...
\nolinkurl{kb/excerpts/meng2024-simpo.md#sec-4-headline-empirical}.}
...
\nolinkurl{kb/excerpts/meng2024-simpo.md#sec-2-length}.}
...
\citep{tulu3} adopts the same SFT $\to$ \glsterm{dpo}{DPO} $\to$ \glsterm{rlvr}{RLVR}
...
\glsterm{rlaif}{RLAIF} / \glsterm{constitutional-ai}{Constitutional AI} \citep{bai2022-cai} family: AI judgments under
...
RL. \cref{sec:rlvr-grpo}'s \glsterm{rlvr}{RLVR} / \glsterm{grpo}{GRPO} stack is the verifier-domain
...
group-mean baseline. \glsterm{dpo}{DPO} and \glsterm{rlvr}{RLVR} are orthogonal: \glsterm{dpo}{DPO} replaces the
\emph{algorithm} (PPO $\to$ classification on preferences); \glsterm{rlvr}{RLVR}
...
preferences and \glsterm{grpo}{GRPO} for verifier-domain reasoning --- treating them as
...
(\glsterm{grpo}{GRPO}) that exploits the verifier's lack of a proxy-vs-gold gap. The
...
(\cref{sec:dpo}) $\to$ verifier-domain \glsterm{rlvr}{RLVR} with \glsterm{grpo}{GRPO}
