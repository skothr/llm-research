...
degrees of \emph{Data Parallel} (DP), \emph{Tensor Parallel} (TP, often
fused with Sequence Parallel), \emph{Context Parallel} (CP),
\emph{Pipeline Parallel} (PP), and \emph{Expert Parallel} (EP)
...
costs; collect a memory table; walk DeepSeek-V3's plan as the
load-bearing case study; and close on three live contradictions
(TP-yes-vs-TP-no, optimal-plan-from-the-graph, the Blackwell NVL72
inversion) that §14 picks back up.

\subsection{The five axes}
\label{sec:dt-axes}

\paragraph{DP / FSDP / ZeRO.} Pure DP replicates parameters $\theta
...
FP32-master-plus-Adam is $4P$ words per worker (parameters + gradients
+ first moment + second moment) \citep[\S 2]{kalamkar2019-bfloat16}.
ZeRO-1 / ZeRO-2 / ZeRO-3 progressively shard optimizer state /
gradients / parameters across the $d$ workers, trading per-step
...
PyTorch's FSDP1 implemented full ZeRO-3 via a ``\texttt{FlatParameter}''
representation that bucketed all of a layer's parameters into one
contiguous tensor; FSDP2 replaces that with per-parameter
\texttt{DTensor} sharding, which composes cleanly with TP/CP/PP and
delivers ``often $7\%$ lower per-GPU memory requirement versus FSDP1''
plus ``an average of $1.5\%$'' throughput gain \citep[\S
2.1.2]{torchtitan2024}. Per-step cost: one all-gather of parameters
($\Theta(P)$) and one reduce-scatter of gradients ($\Theta(P)$),
amortized across the model's microbatches.

\paragraph{TP + SP.} Tensor Parallel \citep[\S 2.1.3]{torchtitan2024}
splits each \texttt{Linear} layer's weight matrix $W \in \R^{d_{\text{in}}
\times d_{\text{out}}}$ into $t$ row- or column-shards. PyTorch
...
Sequence Parallel (SP) is the always-paired companion: normalization
and dropout layers are sharded along the token dimension instead of
replicated, since otherwise their activations are TP-replicated and
become memory-dominant on long context \citep[\S 2.1.3]{torchtitan2024}.
Per-GPU activation under TP+SP shrinks by roughly $1/t$ along the
sharded dimension. Naive TP serializes the matmul against its
collective; \emph{Async TP} fractionalizes the sharded matmul into
...
\paragraph{CP (Context Parallel).} CP shards along the
\emph{token-position} axis: a $\mathsf{B}\!\times\!\mathsf{S}\!\times\!\mathsf{D}$
residual stream becomes $\mathsf{B}\!\times\!(\mathsf{S}/c)\!\times\!\mathsf{D}$
...
softmax denominators as it goes, so the full softmax is computed
...
8B with 8 H100 GPUs, using CP enabled training at context lengths up
to $262{,}144$ tokens, achieving minor MFU degradation as CP degree
increases.'' Without CP the same setup OOMs.

\paragraph{PP.} PP partitions the layer stack into $p$ stages, one
...
\texttt{kb/excerpts/deepseek-v3-training.md\#sec-3-2-1-bubble}.}, where
$F$ is the forward microbatch time, $B$ the (combined) backward time,
and $W$ the weight-gradient sub-step that ZeroBubble and DualPipe
extract from $B$. ZeroBubble reorders the freshly-isolated $W$ into
the bubble; DualPipe additionally feeds the pipeline from both ends
simultaneously so the first-microbatch ramp on one side overlaps with
the last-microbatch ramp on the other (we walk DualPipe in detail in
...
experts, total expert parameters $E \cdot P_E$ are too large for a
single GPU at frontier scale --- DeepSeek-V3 has $256$ routed experts
per layer \citep[\S 3.2]{deepseek-v3}. EP shards experts across $e$
...
\label{eq:ep-pattern}
\end{equation}
\citep[\S 3.2; \S 3.2.2]{deepseek-v3}. EP is bandwidth-heavy and
typically cross-node; in fact, per
\citet[\S 3.2.1]{deepseek-v3}\footnote{KB excerpt:
\texttt{kb/excerpts/deepseek-v3-training.md\#sec-3-2-1-dualpipe}.}, the
cross-node EP all-to-all gives DeepSeek-V3 ``an inefficient
computation-to-communication ratio of approximately 1:1,'' which is
exactly what DualPipe is designed to hide.

\subsection{Per-axis cost model}
\label{sec:dt-cost-model}

Let $P$ denote total parameters (FP32-master + Adam moments give $4P$
...
\midrule
Pure DP (ZeRO-0)            & $P$     & $\sim 4P$           & $A$              & all-reduce $\Theta(P)$ / step \\
...
ZeRO-3 / FSDP-full          & $P/d$   & $4P/d$              & $A$              & all-gather param shards / layer \\
TP only (size $t$)          & $P/t$   & $4P/t$              & $A/t$ (sharded)  & 2 all-reduces / linear ($\sim 2L$/step) \\
PP only (stages $p$)        & $P/p$   & $4P/p$              & $A \cdot p$ (in-flight) & P2P send/recv $\Theta(\text{act})$ / boundary \\
CP (size $c$)               & $P$     & $4P$                & $A/c$            & ring KV pass $\Theta(\mathsf{S}\!\cdot\!\mathsf{D})$ / attention \\
EP (size $e$)               & $P_{\text{shared}} + P_E E/e$ & matched & $\sim A$ + dispatch buffer & 2 all-to-alls / MoE layer \\
\bottomrule
\end{tabular}
...
\emph{PP saves parameters but multiplies in-flight activations by
$p$}: the bookkeeping for $p$ microbatches' worth of activation per
stage is exactly why DualPipe's reported ``$+1$ activation copy''
\citep[\S 3.2.1]{deepseek-v3} is a small price for halving the bubble.
The second is that \emph{FSDP and TP both shard parameters but at
different granularities}: FSDP collects per-step (one all-gather per
\texttt{DTensor} placement, amortized across the layer's compute), TP
collects per-layer (an all-reduce inside every linear). At small model
size FSDP's per-step collective dominates; at large model size TP's
per-layer collective dominates --- which is why pure-FSDP plans win on
sub-$10\text{B}$ models and TP+PP plans dominate at $400\text{B}{+}$
\citep[\S 1; \S 2.1.3]{torchtitan2024}. EP adds a third granularity:
the all-to-all is per-MoE-layer, so EP-heavy plans on $L$-layer models
...
\citet{torchtitan2024} package the four non-EP axes (DP / TP+SP / CP /
PP) on a single 4D \texttt{DeviceMesh} with FSDP2 as the default
...
\emph{torchao.float8} for selective per-tensor FP8 \texttt{Linear}
layers (\cref{sec:mixed-precision}), \emph{Async TP} for matmul-
collective overlap via \texttt{SymmetricMemory} intra-node buffers,
\emph{ZeroBubble / Flexible-Interleaved-1F1B} pipeline schedules
...
FSDP2's per-parameter \texttt{DTensor} sharding is the technology that
makes the rest compose: each parameter knows its own placement on the
mesh, so a TP-sharded \texttt{Linear} composes with FSDP2 by stacking
...
DeepSeek-V3 \citep{deepseek-v3} is the most documented frontier-scale
MoE training run as of 2026 --- $671\text{B}$ total / $37\text{B}$
active parameters, $14.8\text{T}$ tokens, $2{,}048$ H800 GPUs, and the
...
\texttt{kb/excerpts/deepseek-v3-training.md\#sec-1-cost}.}

The parallelism plan is \citep[\S 3.2]{deepseek-v3}\footnote{KB
excerpt:
\texttt{kb/excerpts/deepseek-v3-training.md\#sec-3-2-parallelism}.}:
\textbf{16-way PP with the DualPipe schedule, 64-way EP spanning 8
nodes, ZeRO-1 DP, and \emph{no TP}.} Concretely, with $G = 2{,}048$,
$p = 16$, $e = 64$, no TP and no CP, $d = G / (p \cdot e) = 2$ at the
DP axis.\footnote{The plan is best read as: PP $\times$ EP carves the
$2{,}048$ workers into $2$ DP replicas of a $16 \times 64$ parallelism
plane; ZeRO-1 splits optimizer state across the 2-way DP. Each MoE
layer's experts are partitioned across the 64-way EP plane; non-MoE
weights replicate within EP and shard across PP.} The deliberate
absence of TP is the load-bearing choice: ``we meticulously optimize
the memory footprint during training, thereby enabling us to train
DeepSeek-V3 \emph{without using costly Tensor Parallelism}''
\citep[\S 3.2]{deepseek-v3}. The activation relief that TP would
normally provide comes instead from (i) FP8 forward activations
(\cref{sec:mixed-precision}), (ii) the DualPipe overlap between
MoE all-to-alls and PP forwards/backwards, and (iii) DeepSeek-V3's
...
for input'' and ``backward for weights'' (the ZeroBubble decomposition
that isolates $W$). The key DualPipe move is to schedule these
...
\texttt{kb/excerpts/deepseek-v3-training.md\#sec-3-2-2-allreduce}.} A
naive EP all-to-all over $64$ workers spanning $8$ nodes would put
every token on the IB hop $7$ times. DeepSeek-V3's kernel rewrites the
...
DualPipe overlap of the dispatch and combine phases against compute,
...
\begin{intuition}
The DualPipe schedule plus the IB+NVLink kernel together turn
DeepSeek-V3's nominally $1{:}1$ compute-to-communication ratio
...
two optimizations work together: DualPipe gives the kernel a fixed
...
Muon} \citep[\S 2.3]{muon-moonlight2025}\footnote{KB excerpt:
\texttt{kb/excerpts/muon-moonlight2025.md\#sec-2-3-distributed}.}: a
sharded variant of the Muon optimizer (\cref{sec:optimization})
compatible with ZeRO-1 DP. The wrinkle is that Muon's Newton-Schulz
...
ZeRO-1-AdamW pattern:
\begin{enumerate}
\item \textbf{DP gather.} For each local DP-partitioned master weight
($1/d$ the size of the model weight), gather the corresponding
partitioned gradients across all DP ranks into the full gradient matrix
$\mathbf{M}_t$.
\item \textbf{Calculate full update.} Run the Newton-Schulz iteration
...
forced one axis: Megatron-LM was TP+PP (no FSDP), DeepSpeed was
...
\midrule
Megatron-LM (NVIDIA) & 2019-- & First TP+PP at scale & \citep{shoeybi2019-megatron} \\
ZeRO-1/2/3 (DeepSpeed) & 2020--21 & Optim/grad/param-shard ladder & \citep{rajbhandari2020-zero} \\
FSDP1 (PyTorch) & 2022 & Native ZeRO-3 with FlatParameter & \citep{zhao2023-fsdp} \\
Megatron-3D & 2021 & TP $\times$ PP $\times$ DP composition & \citep{narayanan2021-megatron-3d} \\
GSPMD (XLA) & 2021 & Sharding propagation in compiler & \deepencite{Xu et al.\ 2021 GSPMD --- not in papers.json} \\
TorchTitan (PyTorch) & 2024 & 4D + Float8 + AsyncTP composable & \citep{torchtitan2024} \\
FSDP2 (PyTorch) & 2024 & Per-parameter DTensor sharding & \citep[\S 2.1.2]{torchtitan2024} \\
DeepSpeed-MoE / Tutel & 2022--23 & EP + all-to-all dispatch kernels & \deepencite{Rajbhandari et al.\ 2022 DeepSpeed-MoE / Hwang et al.\ 2023 Tutel --- not in papers.json} \\
DeepSeek-V3 / DualPipe & 2024 & Bidirectional PP + ZB decomposition & \citep[\S 3.2.1]{deepseek-v3} \\
Distributed Muon (Moonlight) & 2025 & Per-rank Newton-Schulz on gathered grad & \citep[\S 2.3]{muon-moonlight2025} \\
\bottomrule
\end{tabular}
...
\begin{contradiction}
\textbf{TP-yes-vs-TP-no.} \citet{meta-llama3} train Llama 3 405B with
heavy TP+PP+DP --- the prototypical Megatron-3D shape. \citet{deepseek-v3}
train DeepSeek-V3 671B-A37B \emph{without TP at all}, claiming the
combination of ZeRO-1 DP + EP + DualPipe + FP8 supplies enough
activation relief to make TP's per-linear collective cost a net loss.
This is a real architectural-conditional disagreement, not a wash. TP
costs $2L$ inter-GPU all-reduces per step; EP costs $2L_{\text{MoE}}$
inter-node all-to-alls per step. DeepSeek's fine-grained MoE
architecture forces them to pay the EP all-to-all already
...
parameter level; without an EP all-to-all to fold TP work into, the
...
hand-tuned by ML systems engineers iterating on memory traces and MFU
profiles. A theoretically grounded plan-from-graph optimizer is open.
\end{contradiction}

\begin{contradiction}
\textbf{Blackwell NVL72 inversion.} The DeepSeek-V3 cross-node kernel
...
BF16 as a drop-in replacement for FP32 that abolishes loss scaling
\citep{kalamkar2019-bfloat16}; and DeepSeek-V3's fine-grained FP8
...
\texttt{DeviceMesh}, DeepSeek-V3's per-tile FP8 recipe composes
...
Muon's full-matrix $\mathbf{O}_t$ exceeds BF16 high-precision range
