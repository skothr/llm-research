...
stable through \glsterm{fp8}{FP8} at frontier scale \citep[\S 3.3]{deepseek-v3}.
The lineage runs FP32 baseline $\to$ FP16 with FP32 \glsterm{master-weights}{master weights} and
dynamic \glsterm{loss-scaling}{loss scaling} \citep{micikevicius2017-mixed-precision}
$\to$ BF16 as a drop-in replacement that abolishes \glsterm{loss-scaling}{loss scaling} on
hardware with native BF16 Tensor Cores \citep{kalamkar2019-bfloat16}
$\to$ \glsterm{fp8}{FP8} fine-grained quantization with promotion-to-CUDA-cores
accumulation \citep[\S 3.3]{deepseek-v3} $\to$ FP4-class formats
(Blackwell-era \glsterm{nvfp4}{NVFP4}, BitNet 1.58 ternary)\footnote{KB note:
\nolinkurl{kb/notes/training/mixed-precision-and-stability.md#3-1}.} that
...
precision — and the headline 2026 fact is that \glsterm{precision-pinning}{DeepSeek-V3}'s full
14.8T-token, 2-month, 2048-H800 pre-training reported zero
irrecoverable loss spikes in \glsterm{fp8}{FP8}
...
A pure-FP32 training step stores \glsterm{parameters}{parameters} $\theta \in \R^P$,
...
words: $P$ for \glsterm{parameters}{parameters}, $P$ for gradients, $2P$ for Adam's first and
...
activations, \glsterm{kv-cache}{KV cache}, or \glsterm{data-parallel}{DP} / \glsterm{tensor-parallel}{TP} / \glsterm{pipeline-parallel}{PP} duplication
...
deliver substantially higher throughput at FP16 / BF16 / \glsterm{fp8}{FP8} than at
FP32: a single \glsterm{fp8}{FP8} GEMM on H100 runs at $\sim 4\times$ the rate of an
...
gradient bandwidth is the dominant inter-node cost in \glsterm{data-parallel}{DP} / \glsterm{fsdp}{FSDP} runs
\citep[\S 2.2.4]{torchtitan2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/torchtitan2024.md#sec-2-2-4}.} The mixed-precision
question is therefore not whether to leave FP32 — every frontier-scale
\glsterm{llm}{LLM} does — but how far down the precision stack we can go before
...
\nolinkurl{kb/excerpts/micikevicius2017-mixed-precision.md#sec-2}.}:
...
\mathrm{Optim}(g^*)$ may be far below FP16's smallest normal \glsterm{value}{value}, but
...
\glsterm{loss-scaling}{Loss scaling} addresses the orthogonal underflow problem in the
gradients themselves. FP16's smallest normal \glsterm{value}{value} is
$\sim\!\!6\times 10^{-5}$, and gradient distributions during \glsterm{llm}{LLM}
...
\label{eq:loss-scale}
\end{equation}
Static \glsterm{loss-scaling}{loss scaling} picks a single $S \in [2^{10}, 2^{15}]$. Dynamic
\glsterm{loss-scaling}{loss scaling} probes the largest $S$ that does not overflow:
...
default for $\sim$3 years until BF16 made \glsterm{loss-scaling}{loss scaling} unnecessary.
...
\nolinkurl{kb/excerpts/kalamkar2019-bfloat16.md#sec-2}.}:
...
terms of accuracy without any change in hyper-\glsterm{parameters}{parameters}''
\citep[\S 4]{kalamkar2019-bfloat16}. BF16 is therefore a
\emph{drop-in} replacement for FP32 — no recipe changes, no loss
scaling, optionally no FP32 \glsterm{master-weights}{master weights}. The 7-mantissa precision
...
necessitating \glsterm{loss-scaling}{loss scaling}. BF16 captures the FP32-range tail directly,
...
2 and 3, \glsterm{mistral}{Mistral}, Qwen, Gemma, and OLMo, on the strength of the
...
The H100 and successor architectures provide native \glsterm{fp8}{FP8} Tensor Cores
...
dynamic range. \glsterm{precision-pinning}{DeepSeek-V3}'s recipe \citep[\S 3.3.1, \S
3.3.2]{deepseek-v3}\footnote{KB excerpts:
\nolinkurl{kb/excerpts/deepseek-v3-training.md#sec-3-3-fp8} and
\texttt{\#sec-3-3-2-finegrained}.} is the most aggressive published
production \glsterm{fp8}{FP8} training to date, and its decisions disagree with
...
object. The \glsterm{fp8}{FP8} GEMM $Y = X W$ is implemented as
\begin{equation}
Y = (Z_X \cdot X_{\text{FP8}}) (Z_W \cdot W_{\text{FP8}})
  = Z_X Z_W \, (X_{\text{FP8}} \, W_{\text{FP8}})_{\text{FP8 accum}},
\label{eq:fp8-gemm}
\end{equation}
with the inner GEMM on \glsterm{fp8}{FP8} Tensor Cores and the scalar product
...
\item \emph{Per-block / per-tile}: \glsterm{precision-pinning}{DeepSeek-V3}'s choice, with $1
...
\paragraph{All-E4M3 instead of E4M3/E5M2 hybrid.} \glsterm{precision-pinning}{DeepSeek-V3} uses
...
\paragraph{Promotion to CUDA cores at $N_C = 128$.} H100's \glsterm{fp8}{FP8} Tensor
...
The remaining \glsterm{precision-pinning}{DeepSeek-V3} choices follow from the same logic: master
...
\glsterm{softmax}{softmax}, rare-token underflow risk) do not match what \glsterm{fp8}{FP8}'s per-tile
...
speedup on Llama-3.1 with \glsterm{fp8}{FP8} + \glsterm{fsdp2}{FSDP2} + \glsterm{tensor-parallel}{TP} is reported in the
30--65\% range over BF16 \citep[\S 2.2.4]{torchtitan2024}, depending
on scale and which Linear layers are covered.

\begin{intuition}
Fine-grained scaling is what makes \glsterm{fp8}{FP8} viable for full pre-training.
...
Below \glsterm{fp8}{FP8} the field splits into two distinct programs.
...
\nolinkurl{kb/notes/training/mixed-precision-and-stability.md#1-3} and
\texttt{\#3-1}; format-zoo entry derived there from Blackwell white-
paper material.} The compute-throughput gain over \glsterm{fp8}{FP8} is roughly
$2\times$ by spec, mirroring the FP16-to-\glsterm{fp8}{FP8} step. The fine-grained-
...
\texttt{\glsterm{bitlinear}{BitLinear}}, trained from scratch, using an \emph{absmean}
...
excerpt: \nolinkurl{kb/excerpts/ma2024-bitnet.md#sec-2}.} Activations
...
The headline empirical claim is matched-\glsterm{perplexity}{perplexity} at 3B
\glsterm{parameters}{parameters}: BitNet b1.58 at 3B reaches $9.91$ PPL versus LLaMA-\glsterm{llm}{LLM}
...
The architecture imports the LLaMA-alike stack — \glsterm{rmsnorm}{RMSNorm}, \glsterm{swiglu}{SwiGLU},
\glsterm{rope}{RoPE}, no biases — to enable drop-in inference under llama.cpp / vLLM
...
$\mathcal{L}$ — are observed across all frontier-scale runs (\glsterm{palm}{PaLM}
...
for \glsterm{fp8}{FP8} (Eq.~\ref{eq:fp8-gemm}).
...
\glsterm{vocabulary}{vocabulary} entries' embedding columns receive small gradient
magnitudes that underflow in \glsterm{fp8}{FP8}. \glsterm{precision-pinning}{DeepSeek-V3}'s mitigation is direct:
keep embeddings in BF16/FP32 even when surrounding GEMMs are \glsterm{fp8}{FP8}
\citep[\S 3.3.1]{deepseek-v3}.

\paragraph{Attention-softmax kept higher-precision.} \glsterm{softmax}{Softmax} inputs
$QK^\top$ grow in magnitude as training proceeds, creating numeric
overflow when $|QK^\top|$ exceeds E4M3's $\pm 448$ bound. \glsterm{precision-pinning}{DeepSeek-V3}
...
$QK^\top$ pre-\glsterm{softmax}{softmax}) and \emph{QK-\glsterm{layer-normalization}{LayerNorm}} (normalizing $Q$ and
$K$ before the dot product, as adopted in LLaMA-2 and successors).

\paragraph{Optimizer-precision coupling.} \glsterm{per-shape-rms-scaling}{Muon}'s update RMS,
...
excerpt: \nolinkurl{kb/excerpts/muon-moonlight2025.md#sec-2-2-rms} and
\texttt{\#sec-2-2-wd}.} The \glsterm{per-shape-rms-scaling}{Muon}-Moonlight fix is mandatory weight
...
fail under \glsterm{fp8}{FP8}, especially for the expert-routing \glsterm{softmax}{softmax}. DeepSeek-
V3's \glsterm{auxiliary-loss-free-load-balancing}{auxiliary-loss-free load balancing} addresses this in tandem with
the \glsterm{fp8}{FP8} recipe \citep[abstract]{deepseek-v3}.
...
runs. \glsterm{precision-pinning}{DeepSeek-V3}'s cross-node all-to-all kernels use \glsterm{fp8}{FP8} dispatch and
BF16 combine \citep[\S 3.2.2]{deepseek-v3}, with the bandwidth budget
split between InfiniBand and NVLink.

\begin{contradiction}
\glsterm{nvfp4}{NVFP4} / FP4 production status is not settled. Blackwell-class
...
$\sim$7B \glsterm{parameters}{parameters}, and the published BitNet b1.58 2B4T tech report
\citep{bitnet-2b4t-2025} does not yet have a frontier-scale companion.
\glsterm{precision-pinning}{DeepSeek-V3}'s \glsterm{fp8}{FP8} result is the strongest production data point we
have at this precision level, and it is single-source: no other
publicly disclosed lab has matched the 14.8T-token zero-spike \glsterm{fp8}{FP8}
envelope \citep[abstract]{deepseek-v3}. Treat the entire sub-\glsterm{fp8}{FP8}
regime as open, and treat \glsterm{fp8}{FP8} production stability as resolved
\emph{contingent on} the \glsterm{precision-pinning}{DeepSeek-V3} recipe carrying over to other
architectures.
\end{contradiction}

\begin{intuition}
Precision as compression. Each precision step (FP32 $\to$ BF16 $\to$
\glsterm{fp8}{FP8} $\to$ FP4) halves storage and roughly doubles compute throughput
...
objective with prompt-masking, the data-curation lineage from \glsterm{flan}{FLAN}
through InstructGPT, Llama-3 \glsterm{rejection-sampling-sft}{rejection-sampling SFT}, T\"ulu-3, and
...
\glsterm{master-weights}{master weights}, and a learning rate roughly $10\times$ smaller than
