...
A scaling law is a fitted functional form $L(N, D, C)$ relating
cross-entropy loss to parameter count, training tokens, and training
...
\citep[\S 1.1]{kaplan2020} to ``scale parameters and tokens equally''
...
\citet{kaplan2020} fitted decoder-only Transformer LMs on WebText2
across $N \in [768,\, 1.5 \times 10^9]$ non-embedding parameters and
$D \in [22\text{M},\, 23\text{B}]$ tokens, GPT-2 BPE tokenizer,
context length $1024$, Adam (Adafactor at $N > 1\text{B}$), and a
cosine LR schedule with $3000$-step warmup decaying to zero
\citep[\S 2.2]{kaplan2020}. The headline finding is that
cross-entropy loss obeys a \emph{power law} in each of $N, D, C_{\min}$
...
\end{align}
Here $N$ counts \emph{non-embedding} parameters
...
parameters. This is the prescription the GPT-3 generation took
literally: $175\text{B}$ parameters trained on $\sim 300\text{B}$
...
every Pile subset, +0.6\,pp average across BIG-bench, and lifts MMLU
...
example we have in LLM scaling of an apparent recipe being driven by
the optimization protocol, not by the architecture. Notation note:
Kaplan's $N$ is non-embedding parameters
\citep[\S 1.3]{kaplan2020}; Hoffmann's $N$ is total parameters; the
...
DeepSeek-V3 trained $671\text{B}$ total / $37\text{B}$ active-MoE on
...
DeepSeek-V3 (2024) MoE       & $37$B    & $14.8$T  & $\sim 400$ & $\sim 20\times$ \\
\bottomrule
\end{tabular}
...
queries.'' Crucially, the Chinchilla parametric loss \cref{eq:cc-paraloss}
...
\citep{gemini2-5} demonstrated that test-time compute is now a
...
non-thinking cost per query for hard math, code, and agent loops. The
empirical surface for the joint $(C_{\text{train}}, C_{\text{test}})$
trade-off is documented but no closed-form scaling law captures it
...
FFN). \textmu P is exactly the per-category rescaling
...
\begin{definition}[\textmu P for Transformer; \citealp{yang2022-mup}]
The Maximal Update Parametrization (\textmu P) for a Transformer is
...
that preserves $\Theta(1)$-magnitude weight updates --- ``feature
learning'' --- in the infinite-width limit; SP either has trivial
NTK-regime dynamics or blows up. Returning to math: under SP, after
even one Adam step, attention pre-softmax scores $q^\top k$ acquire
correlation across $i$, so $\mathrm{Var}(q^\top k) = \Theta(d)$ by
Law-of-Large-Numbers (not CLT), and the Vaswani $1/\sqrt{d_k}$
rescaling makes the logits $\Theta(\sqrt{d})$ --- they blow up with
...
\citep{meta-llama4} cites a ``MetaP'' scheme for cross-scale HP
...
scarce; whether the loose variants retain the unique-feature-learning
guarantee is open.
\end{contradiction}
...
not above it. DeepSeek-V3 reports zero irrecoverable loss spikes over
$14.8\text{T}$ tokens of FP8 pre-training
\citep[abstract]{deepseek-v3}; Llama 4 likewise discloses FP8
...
enabling FP8 without dynamic rescaling. The full multi-axis
...
adopted (with disclosure asymmetry; see \cref{sec:scaling-mup}), FP8
...
C_{\text{test}}, b, q)$ scaling law --- documented empirically, not
...
(\cref{sec:optimization}) walks the optimizer axis: AdamW as the
production default, the training-loss-conditional speedup claims of
Lion and Sophia that did not hold on downstream evaluation, Muon's
Newton-Schulz-orthogonalized matrix update as the only published
optimizer with $\sim 2\times$ compute-efficiency lift at LLM scale,
and the WSD (Warmup-Stable-Decay) schedule that pairs with continual
...
orthogonal --- Muon's update RMS exceeds BF16's high-precision range
