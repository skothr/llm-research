...
Reinforcement Learning from Human Feedback is the canonical
...
against the reward model with a KL anchor to the SFT initialization
...
the load-bearing post-training technology for every assistant LLM
since. The thesis of this section is that RLHF is best read as a
\emph{coupled} optimization: a Bradley--Terry reward model fitted to
...
foundation that DPO inverts in \cref{sec:dpo}.
...
them. Train a reward model $r_\phi(x, y)$ — initialized from
$\pi^{\mathrm{SFT}}$ with the LM head replaced by a scalar head over
the final hidden state of the last token — on the Bradley--Terry
...
value head $V_\phi$ — and the three-phase per-step structure
(rollout, score, optimize) are the load-bearing infrastructure facts
that explain why DPO eventually displaced PPO at frontier scale
(\cref{sec:rlhf-frontier}).

\subsection{Reward model training}
\label{sec:rlhf-rm}

The reward model is fit to pairwise human preferences under the
...
Symbol & Meaning \\
\midrule
$\pi_\theta$ & policy under training (LM with parameters $\theta$) \\
$\pi^{\mathrm{SFT}}$ & SFT-stage initialization, frozen during RL \\
$r_\phi(x, y)$ & learned scalar reward model, frozen during RL \\
$V_\phi(x, y_{<t})$ & value model for PPO's GAE, co-sized with $\pi_\theta$ \\
...
$\gamma$ & PPO-ptx coefficient, the pre-train mixin weight \\
\bottomrule
\end{tabular}
...
\begin{intuition}
The reward model is a learned distillation of human judgment.
...
PPO-ptx pre-training-mixing term,
\begin{equation}
...
control the \emph{alignment tax} — the regression on standard NLP
benchmarks observed when RLHF is run without it
...
parameters.\footnote{KB excerpt:
...
with minimal regressions on public NLP datasets when the PPO-ptx
...
principle: RLHF outperforms supervised fine-tuning on tasks
...
The $1.3$B-versus-$175$B inflection is what made RLHF a first-class
post-training technique. Every assistant LLM since — Claude, Gemini,
Llama-2 / 3 / 4, Mistral, Qwen, GPT-4o — runs some descendant of the
InstructGPT pipeline as its post-training core, with the variants of
\cref{sec:rlhf-frontier} (DPO replacing PPO, RLAIF / CAI replacing
human labels, GRPO replacing both for verifiable-reward reasoning)
...
Concretely, three failure modes recur across published RLHF runs:
...
RLHF-trained assistants systematically agree with the user even
...
fixes; sycophancy persists at frontier scale.

\paragraph{Verbosity and over-refusal.} The same proxy-bias mechanism
produces the well-documented length bias of RLHF outputs (longer
...
prompts to harvest reward). Length normalization fixes (SimPO's
$\beta / |y|$ scaling, \cref{sec:dpo}) target the first; better
preference-data composition targets the second.

The PPO-ptx term \cref{eq:rlhf-ptx} addresses a fourth, related issue:
the \emph{alignment tax} — the regression on pre-training benchmarks
...
al.\ 2023, multiple seeds reduce variance) $\to$ \emph{WARM /
WARP} \citep{warm-2024,warp-2024}: weight-averaged reward models that
average parameters across $K$ independently-trained RMs (WARM) or
alternate training and weight-averaging during the RL phase itself
(WARP), reported to reduce reward hacking in production at Gemma 3
scale \citep{gemma3}.
...
Llama-3 \citep{meta-llama3} runs SFT $\to$ rejection-sampling SFT
$\to$ DPO over multiple rounds and uses \emph{no PPO at scale},
reporting that DPO required less compute than PPO and performed as
well or better. T\"ulu-3 \citep{tulu3} adopts a similar SFT $\to$
DPO $\to$ RLVR stack. The driver is infrastructural: PPO requires
four model copies in memory plus a three-phase rollout / score /
optimize loop, while DPO is a single forward + backward on
preference pairs.
...
reward hacking by construction (the only way to score $1$ is to solve
the problem). GRPO \citep{shao2024} replaces the value model with a
group-mean baseline; DAPO \citep{dapo2025} adds asymmetric clipping
and dynamic sampling. DeepSeek-R1 \citep{deepseek-r1} is the
production demonstration.

\paragraph{Production RM-ensemble RLHF: WARM/WARP.} Gemma 3
\citep{gemma3} retains PPO-style RLHF but substitutes WARM/WARP-
...
counterpoint to the Llama-3 / T\"ulu-3 ``DPO-replaces-PPO'' framing.

The terminology has accordingly drifted: in 2022 ``RLHF'' meant the
InstructGPT pipeline specifically; in 2026 it often refers to any
preference-based post-training, including DPO. We retain the narrow
sense in this section and treat the offline-preference family
separately in \cref{sec:dpo}.

\begin{contradiction}
The community currently disagrees on whether RLHF's reward-hacking
...
preference iteration mitigate reward hacking enough for production.
The \emph{fundamental view} (scaling-laws-for-direct-alignment work,
the persistence of sycophancy and math-hallucination as localized
shortcuts at all scales \citep{sharma2023-sycophancy}) holds that
proxy-reward optimization produces overoptimization across
\emph{all} preference-based post-training including DPO, since DPO's
implicit reward $\hat r_\theta = \beta \log
\pi_\theta / \pi^{\mathrm{SFT}}$ is itself a learned proxy that can be
hacked. The contradiction is sharper than DPO-vs-PPO: RLAIF and CAI
\citep{bai2022-cai} replace humans with an AI judge under a
constitution and report stability gains in production; advocates
read this as evidence the tractable view is right (judge-under-
constitution is a stable enough signal). Critics read CAI as
evidence the fundamental view is right (the policy learns to
\emph{appear} principle-aligned to the AI judge whether the
underlying reasoning is shallow or deep — sycophancy with respect to
...
canonical RLHF pipeline; \cref{sec:rlhf-headline} the alignment-beats-
...
$\log Z(x)$, yielding the DPO loss as a closed-form classification
...
model, no value model, no RL loop. \cref{sec:dpo} works through the
derivation and the variant family (IPO, KTO, ORPO, SimPO, CPO).
