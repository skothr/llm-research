...
human preference pairs, or its DPO closed-form analog — as the gradient
signal for the policy. The RLVR-era reformulation replaces that learned
...
of a learned reward model, and a group-mean baseline in place of a
co-sized value model \citep[\S 4.1]{shao2024}\footnote{KB excerpt:
...
DeepSeek-R1-Zero's average pass@1 on AIME 2024 climbing from
$15.6\%$ to $77.9\%$ over the course of one continuous GRPO run, and to
$86.7\%$ with self-consistency over $64$ samples — surpassing the
average human competitor. This section is also the cross-paper anchor
that Paper~3 §7 cites for the \emph{training} leg of its reasoning
...
RLVR setting; cf.\ \cref{sec:rlvr-frontier} on process supervision). The
contrast with \cref{sec:rlhf}'s RLHF setting is structural: there,
...
that do not correspond to true quality (verbosity, sycophancy, surface
...
\begin{enumerate}
\item \emph{Reward hacking.} The policy exploits an artifact of
...
\item \emph{Reward overoptimization.} The proxy--gold gap grows with
$\KL(\pi_\theta \| \pi_{\mathrm{ref}})$ in RLHF (Gao et al.\ 2023,
...
observation that neural reward models are susceptible to reward hacking
...
(GRPO) in the DeepSeekMath paper as a critic-free PPO variant. The
...
baseline, eliminating the value network $V_\phi$ entirely
...
(chain-of-thought + answer); $R_i \in \{0, 1\}$ is the verifier reward;
...
$\beta = 0.001$ for R1-Zero;\footnote{KB excerpt:
...
value network $V_\phi(s_t)$ co-sized with the policy: at LLM scale this
\emph{doubles} the active parameter count and the activation memory of
training. GRPO's group-mean baseline replaces $V_\phi$ entirely; the
...
(policy, reference, reward, value) reduce to two (policy, reference);
...
\citet{deepseek-r1} apply GRPO with verifiable rewards to the
DeepSeek-V3 base model and report the strongest published evidence to
date that pure RL on a deterministic verifier produces frontier-grade
reasoning capability. Two models are trained:

\begin{itemize}
\item \emph{DeepSeek-R1-Zero.} GRPO applied directly to the base, no SFT
cold-start. Reward is rule-based: accuracy (binary correctness from a
math/code verifier) plus a small format reward for the
\texttt{<think>...</think><answer>...</answer>} envelope
\citep[\S 2.2]{deepseek-r1}.\footnote{KB excerpt:
\texttt{kb/excerpts/deepseek-r1.md\#sec-2-2-reward}.}
\item \emph{DeepSeek-R1.} A multi-stage pipeline: cold-start SFT on a
small set of long-CoT exemplars, then GRPO with the same verifiable
...
1(a) depicts the performance trajectory of DeepSeek-R1-Zero on the AIME
...
self-consistency decoding, the model's performance can be further
...
reflection, characterized as ``an `aha moment', characterized by a
sudden increase in the use of the word `wait' during reflections.''
\footnote{KB excerpt:
\texttt{kb/excerpts/deepseek-r1.md\#sec-2-3-aha}.} The chain-of-thought
structure is being actively reorganized during RL, not just used.

\paragraph{R1 vs.\ R1-Zero — what the cold-start adds.} R1-Zero's traces
are difficult to read and language-mix between English and Chinese
because DeepSeek-V3-Base is multilingual
...
default. R1's reasoning capability matches R1-Zero's; the cold-start is
...
introduce four targeted modifications to GRPO that together cut
...
addresses a measured failure mode of the GRPO recipe.
...
already-favored tokens. DAPO decouples the bound:
\begin{equation}
...
group-normalized advantage \cref{eq:grpo-advantage} is identically zero
and the sample contributes no gradient. DAPO enforces
\begin{equation}
...
longer trajectories contribute proportionally less per token. DAPO
...
The pre-RLVR post-training stack (\cref{sec:rlhf}, \cref{sec:dpo}) was
...
direction. Mitigations (RM ensembles, WARM/WARP, KL-coefficient
...
teacher: RLVR does not tell the policy \emph{how} to be better, only
...
start of training; RLVR amplifies the trajectories that lead to correct
...
scaling (chain-of-thought, self-consistency, sampling-based test-time
...
the four DAPO equations as the canonical statement of the RLVR leg, and
...
point that all three legs co-design: the same RLVR run produces both
...
inference). Treating RLVR as a training-only mechanism — as this
section deliberately does — understates the gains and, more
importantly, the open questions; the latter are concentrated in
Paper~3.

\begin{contradiction}
\textbf{Does RLVR create new reasoning capability?} The framing in
...
treat RLVR as eliciting reasoning behaviors that the base model could
...
vs.\ RLVR'd model at matched sampling budget, they argue RLVR improves
...
RLVR is sampling-efficient sharpening, not capability expansion. As of
...
generalization to non-math (e.g., agentic, long-horizon) RLVR is
single-source. Cross-paper to Paper~3 §14 for the resolution attempt.
\end{contradiction}

\begin{contradiction}
\textbf{Cold-start SFT necessity.} R1-Zero (no cold-start) reaches AIME
...
\texttt{kb/notes/post-training/rlvr-and-grpo.md\#5-4}) shows removing
cold-start sometimes destabilizes RL, sometimes works fine; the
determining factor is unclear.
\end{contradiction}

\begin{contradiction}
\textbf{VAPO vs.\ DAPO.} VAPO (Value-Augmented PPO, ByteDance, April
2025, arXiv:2504.05118) re-introduces a learned value
function under the GRPO/RLVR objective, shared-backbone with the
policy and value-loss-clipped, and reports outperforming DAPO on
long-CoT tasks in fewer steps — suggesting the GRPO-vs-PPO advantage is
not ``value models are useless'' but ``na\"ive value models are
expensive-and-useless; carefully-trained ones are useful.'' Whether
VAPO's gain over DAPO replicates outside the original recipe is open as
...
$\to$ RLVR with GRPO (or DAPO) for reasoning capability on
verifier-supported domains $\to$ rejection-sampling SFT to consolidate.
What remains is the weight-space tail of the pipeline: how to take the
post-trained checkpoint and adapt it cheaply (LoRA, QLoRA), or compose
