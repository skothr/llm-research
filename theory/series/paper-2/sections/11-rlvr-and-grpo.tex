...
signal for the policy. The \glsterm{rlvr}{RLVR}-era reformulation replaces that learned
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} The thesis of this
...
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1}.} — together produce the
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} report
DeepSeek-\glsterm{r1-zero}{R1-Zero}'s average pass@1 on AIME 2024 climbing from
$15.6\%$ to $77.9\%$ over the course of one continuous \glsterm{grpo}{GRPO} run, and to
$86.7\%$ with \glsterm{self-consistency}{self-consistency} over $64$ samples — surpassing the
...
trajectory contains a well-formed \texttt{<think>}\allowbreak\texttt{...</think>} block before its answer) — a small constant
...
\glsterm{rlvr}{RLVR} setting; cf.\ \cref{sec:rlvr-frontier} on process supervision). The
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.}
...
(\glsterm{grpo}{GRPO}) in the DeepSeekMath paper as a critic-free PPO variant. The
...
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1-motivation}.} The objective,
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-1-hp}.} $\epsilon$ is the PPO
...
training. \glsterm{grpo}{GRPO}'s group-mean baseline replaces $V_\phi$ entirely; the
...
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1-motivation}.} The four
...
\citet{deepseek-r1} apply \glsterm{grpo}{GRPO} with verifiable rewards to the
...
\begin{itemize}
\item \emph{DeepSeek-\glsterm{r1-zero}{R1-Zero}.} \glsterm{grpo}{GRPO} applied directly to the base, no SFT
...
small set of long-\glsterm{chain-of-thought}{CoT} exemplars, then \glsterm{grpo}{GRPO} with the same verifiable
...
excerpt: \nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.}
\end{itemize}

The training-curve headline \citep[\S 2.3]{deepseek-r1}\footnote{KB
excerpt: \nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.}: ``Figure
...
\glsterm{self-consistency}{self-consistency} decoding, the model's performance can be further
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} The verifier reward
contains no length term — the policy discovers that long traces win on
math and code, on its own. Cross-paper to \hyperref[P3-sec:inference-compute-scaling]{Paper~3 §4} for the
...
\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} The \glsterm{chain-of-thought}{chain-of-thought}
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.} R1's cold-start
...
and \hyperref[P3-sec:r1-family]{Paper~3 §8}).} R1 generates $800$K reasoning traces (600K reasoning
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-4}.} The capability is
...
introduce four targeted modifications to \glsterm{grpo}{GRPO} that together cut
...
addresses a measured failure mode of the \glsterm{grpo}{GRPO} recipe.
...
already-favored tokens. \glsterm{dapo}{DAPO} decouples the bound:
\begin{equation}
...
and the sample contributes no gradient. \glsterm{dapo}{DAPO} enforces
\begin{equation}
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-dynamic-sampling}.} Compute is
...
longer trajectories contribute proportionally less per token. \glsterm{dapo}{DAPO}
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-token-loss}.} The change avoids
...
signal is uninformative — the model just ran out of budget. \glsterm{dapo}{DAPO}
...
The pre-\glsterm{rlvr}{RLVR} post-training stack (\cref{sec:rlhf}, \cref{sec:dpo}) was
...
\citep[\S 4]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-4}.} The post-training-stack
...
teacher: \glsterm{rlvr}{RLVR} does not tell the policy \emph{how} to be better, only
...
start of training; \glsterm{rlvr}{RLVR} amplifies the trajectories that lead to correct
...
The triangle (\hyperref[P3-sec:introduction]{Paper~3 §1}, \hyperref[P3-sec:rlvr-grpo-dapo]{§7}) has three legs: inference-time compute
scaling (\glsterm{chain-of-thought}{chain-of-thought}, \glsterm{self-consistency}{self-consistency}, sampling-based test-time
...
the leg developed here. \hyperref[P3-sec:rlvr-grpo-dapo]{Paper~3 §7} cites
\cref{eq:verifier-reward}, \cref{eq:grpo}, \cref{eq:grpo-advantage}, and
the four \glsterm{dapo}{DAPO} equations as the canonical statement of the \glsterm{rlvr}{RLVR} leg, and
...
point that all three legs co-design: the same \glsterm{rlvr}{RLVR} run produces both
...
inference). Treating \glsterm{rlvr}{RLVR} as a training-only mechanism — as this
section deliberately does — understates the gains and, more
importantly, the open questions; the latter are concentrated in
Paper~3.

\begin{contradiction}
\textbf{Does \glsterm{rlvr}{RLVR} create new reasoning capability?} The framing in
...
treat \glsterm{rlvr}{RLVR} as eliciting reasoning behaviors that the base model could
...
vs.\ \glsterm{rlvr}{RLVR}'d model at matched sampling budget, they argue \glsterm{rlvr}{RLVR} improves
...
\glsterm{rlvr}{RLVR} is sampling-efficient sharpening, not capability expansion. As of
...
generalization to non-math (e.g., agentic, long-horizon) \glsterm{rlvr}{RLVR} is
single-source. Cross-paper to \hyperref[P3-sec:contradictions]{Paper~3 §14} for the resolution attempt.
\end{contradiction}

\begin{contradiction}
\textbf{\glsterm{cold-start-sft}{Cold-start SFT} necessity.} \glsterm{r1-zero}{R1-Zero} (no cold-start) reaches AIME
...
\nolinkurl{kb/notes/post-training/rlvr-and-grpo.md#5-4}) shows removing
cold-start sometimes destabilizes RL, sometimes works fine; the
determining factor is unclear.
\end{contradiction}

\begin{contradiction}
\textbf{\glsterm{vapo}{VAPO} vs.\ \glsterm{dapo}{DAPO}.} \glsterm{vapo}{VAPO} (\glsterm{vapo}{Value-Augmented PPO}, ByteDance, April
2025, arXiv:2504.05118) re-introduces a learned \glsterm{value}{value}
function under the \glsterm{grpo}{GRPO}/\glsterm{rlvr}{RLVR} objective, shared-backbone with the
policy and \glsterm{value}{value}-loss-clipped, and reports outperforming \glsterm{dapo}{DAPO} on
long-\glsterm{chain-of-thought}{CoT} tasks in fewer steps — suggesting the \glsterm{grpo}{GRPO}-vs-PPO advantage is
...
\glsterm{vapo}{VAPO}'s gain over \glsterm{dapo}{DAPO} replicates outside the original recipe is open as
...
$\to$ \glsterm{rlvr}{RLVR} with \glsterm{grpo}{GRPO} (or \glsterm{dapo}{DAPO}) for reasoning capability on
