...
\textbf{chain-of-thought (CoT)} prompting, with the headline finding
that an 8-exemplar CoT prompt raises a 540B-parameter language model on
...
\texttt{kb/excerpts/wei2022-cot.md\#abstract}.} The same pattern was
...
empirical instance of \emph{trading inference compute for parameters}.
...
distinguished CoT from compute-monotonic prompting, and closes by
deferring CoT's faithfulness question to \cref{sec:cot-faithfulness}.
...
prompting decodes $y \sim p_\theta(\cdot \mid x)$ in a single step. CoT
...
\texttt{kb/excerpts/wei2022-cot.md\#sec-3-1}.}
\begin{equation}
  z \;\sim\; p_\theta(\cdot \mid x), \qquad y \;\sim\; p_\theta(\cdot \mid x, z).
  \label{eq:cot-decode}
\end{equation}
In practice the trace and answer are emitted jointly by autoregressive
...
factorisation as Eq.~1 of \texttt{chain-of-thought.md}.\footnote{KB
note: \texttt{kb/notes/reasoning/chain-of-thought.md\#1}.}
...
GSM8K few-shot CoT exemplars are on the order of $10^2$ tokens per
trace \citep[\S 3.1]{wei2022-cot}, while the long-CoT reasoning models
...
not by $y$. Decoding a single CoT response of shape
...
\paragraph{Few-shot exemplar count $K$.} The CoT prompt in
\citet{wei2022-cot} is a sequence of $K$ in-context demonstrations
$\{(x_i, z_i, y_i)\}_{i=1}^K$ followed by the query $x_*$, with each
$z_i$ a hand-written reasoning trace. The arithmetic-reasoning
benchmarks in \citet{wei2022-cot} use $K = 8$
\citep[\S 3.1]{wei2022-cot}.\footnote{KB excerpt:
\texttt{kb/excerpts/wei2022-cot.md\#sec-3-1}.} The exemplars do double
...
this protocol to three model families (LaMDA, GPT-3, PaLM) at sizes
spanning roughly $0.4\text{B}$ to $540\text{B}$ parameters, evaluated
...
\texttt{kb/excerpts/wei2022-cot.md\#sec-3-2}.} The headline
\emph{single-number} result is GSM8K at the 540B scale: an 8-exemplar
CoT prompt achieves state-of-the-art accuracy, surpassing a fine-tuned
GPT-3 supplemented with a verifier
\citep[abstract]{wei2022-cot}.\footnote{KB excerpt:
\texttt{kb/excerpts/wei2022-cot.md\#abstract}.}

\subsection{Zero-shot CoT (Kojima et al.\ 2022)}
\label{sec:cot-zeroshot}

\citet{kojima2022} report that the entire CoT-eliciting effect can be
...
CoT prompting over direct prompting is itself an
\emph{emergent} property of model scale. The headline figure
\citep[\S 3.3, fig.\ 4]{wei2022-cot}\footnote{KB excerpt:
\texttt{kb/excerpts/wei2022-cot.md\#sec-3-3}.} shows GSM8K accuracy
plotted against model size for three families (LaMDA, GPT-3, PaLM),
each with and without the 8-exemplar CoT prompt. Two qualitative
features survive every replication:
\begin{itemize}
\item \textbf{No gain — sometimes a loss — below ${\sim}100\text{B}$
parameters.} Below the inflection, CoT prompting often
...
\item \textbf{Substantial gain at PaLM-540B.} Above the inflection,
CoT prompting opens a sharp gap over direct prompting — the gap that
...
\texttt{kb/excerpts/wei2022-cot.md\#sec-6}.} The same paper hedges
that ``chain-of-thought'' need not reflect any cognitive sense of
...
Reading the 2022--2023 reception of CoT in retrospect, the field largely
...
unaccounted-for. Wei's 8-exemplar CoT prompt at 540B emits roughly
...
\citet{snell2024} promote this reading explicitly: test-time compute
becomes a \emph{scaling-law-shaped} axis with its own functional form,
on which CoT prompting was the first attested operating point. The
...
PaLM-540B can. The 2025--2026 reasoning-trained models of
...
becomes the dominant accuracy lever on hard math benchmarks. CoT was
not magic prompting; it was the first published instance of substituting
inference compute for parameters along an axis the field would later
formalise.

\begin{analogy}
CoT is to direct prompting what stochastic gradient descent with $K$
inner-loop steps is to a single gradient step: the same model now
spends additional compute per query before committing to an output,
...
trace ($p_\theta(y \mid x)$ alone), CoT prompting decodes a non-trivial
...
\texttt{kb/notes/reasoning/chain-of-thought.md\#4.1}.} Whether the gap
is caused by long-CoT SFT, by RLVR post-training, or by general scale
is not separated by the published 2026 ablations. The hedge that
\citet[\S 6]{wei2022-cot} themselves placed on the cognitive
interpretation of CoT --- ``chain-of-thought does not necessarily
...
prompts test-time compute substitutes for ${\sim}14\times$ more
...
itself concentrates on correct answers. The CoT phenomenon, in
...
treating it as a compute axis with its own scaling law.
