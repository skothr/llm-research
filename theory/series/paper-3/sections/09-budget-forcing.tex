...
paradigm.\footnote{KB note: \nolinkurl{kb/notes/reasoning/test-time-compute.md#2.3}.}
...
\nolinkurl{kb/excerpts/s1-2025.md#sec-3-1-budget}.} The exercise isolates
...
\nolinkurl{kb/notes/reasoning/test-time-compute.md#2.3}.} At inference
...
\begin{enumerate}
  \item \textbf{API-level thinking budgets.} Gemini~2.5 takes a
        ...
        \nolinkurl{kb/notes/reasoning/test-time-compute.md#2.3} §1.}
...
\citep[\S 3.1]{s1-2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/s1-2025.md#sec-3-1-budget}.} The paper's verbatim
...
\nolinkurl{kb/excerpts/s1-2025.md#abstract}.}
...
\begin{enumerate}
  \item Curate $1{,}000$ (problem, long-\glsterm{chain-of-thought}{CoT} trace, answer) examples
        ...
        \nolinkurl{kb/excerpts/s1-2025.md#sec-2-2-curation}.}
...
each new sample in \glsterm{self-consistency}{self-consistency} (\cref{sec:self-consistency})
...
\nolinkurl{kb/excerpts/s1-2025.md#sec-5-1-ablation}.} shows the
...
\citep[abstract, \S 4.2]{s1-2025}: pushing the minimum-\glsterm{thinking-budget}{thinking budget}
...
\nolinkurl{kb/excerpts/s1-2025.md#sec-4-2-table}.}
...
\begin{tabular}{lrrr}
\toprule
Model & AIME24 & MATH500 & \glsterm{gpqa-diamond}{GPQA Diamond} \\
\midrule
...
\nolinkurl{kb/excerpts/s1-2025.md#sec-5-1-ablation}.} Curation does most
...
note: \nolinkurl{kb/notes/reasoning/test-time-compute.md#3} flags this
...
\begin{itemize}
  ...
  \item \textbf{\glsterm{cold-start-sft}{Cold-start SFT} is training-time.} R1's Stage~1
        ...
        \nolinkurl{kb/notes/reasoning/reasoning-training.md#2} (Stage~1).}
        It changes $\theta$. Whatever distribution-shape the base
        model has after Stage~1 is what every later mechanism ---
        Stage~2 \glsterm{grpo}{GRPO}, Stage~3 \glsterm{rejection-sampling-sft}{rejection-sampling SFT}, and any
...
isolation through \glsterm{r1-zero}{R1-Zero}, where \glsterm{grpo}{GRPO} from the base model produces the
...
\begin{intuition}
The architecture taxonomy of Paper~1 (\hyperref[P1-sec:frontier-snapshot]{Paper~1 §reasoning-architectures})
...
\begin{enumerate}
  \item In the architecture taxonomy of Paper~1
        (\hyperref[P1-sec:frontier-snapshot]{Paper~1 §reasoning-architectures}), reasoning shows up as
  ...
  \item In the training taxonomy of Paper~2 (\hyperref[P2-sec:sft]{Paper~2 §SFT},
        ...
        SFT stage and the \glsterm{rlvr}{RLVR} stage that follows
...
\glsterm{rlvr}{RLVR} training, on parallel sampling (\cref{sec:self-consistency}), or
...
\glsterm{dapo}{DAPO}'s step-efficiency and R1's training-token costs for the
...
\cref{sec:self-consistency} and the \glsterm{rlvr}{RLVR} step count of
\cref{sec:rlvr}.
