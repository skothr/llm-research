...
inside one autoregressive decode rather than across many. This is the
...
curated long-CoT exemplars for SFT, plus a \textbf{budget forcing} rule
...
Transformer, but a training-and-inference protocol that designates
...
emitted before the answer phase. \emph{Sequential} test-time compute
...
$\Theta(L^2)$ in the trace-length-bound regime where the KV cache
...
This is the third TTC family of \cref{sec:test-time-compute}: not
...
\begin{enumerate}
  ...
  \item \textbf{Budget forcing.} A purely test-time edit to the
...
\citet[\S 3.1]{s1-2025} define budget forcing as two single-line
edits to autoregressive decoding, parameterised by a minimum budget
...
\begin{enumerate}
  \item Curate $1{,}000$ (problem, long-CoT trace, answer) examples
  ...
  \item Apply budget forcing at test time per
        \cref{eq:bf-min,eq:bf-max}.
\end{enumerate}
No reward model. No RL. No search tree. The budget-forcing rule itself
is $\sim 10$ lines of decoder code.

\begin{intuition}
Budget forcing lives on Snell's test-time-compute axis
...
each new sample in self-consistency (\cref{sec:self-consistency})
...
$50.0\%$ (SFT only, no budget forcing) to $56.7\%$ (SFT plus budget
forcing) on the same checkpoint. The lift is what budget forcing
\emph{adds} on top of SFT --- the sequential TTC dial, exposed as a
...
The s1 paper sweeps $L$ through budget forcing and reports
extrapolation \emph{beyond} the SFT trace lengths
\citep[abstract, \S 4.2]{s1-2025}: pushing the minimum-thinking budget
...
\citep[\S 4.2]{s1-2025}; no closed-form scaling law is reported. This
...
plus budget forcing reaches $56.7\%$ in
$\sim 7$ H100-GPU-hours.\footnote{KB excerpt:
\texttt{kb/excerpts/s1-2025.md\#sec-5-1-ablation}.} Curation does most
of the SFT-side work; budget forcing extracts the rest at inference.

\begin{contradiction}
The s1 thesis --- $1{,}000$ examples plus budget forcing
...
Budget forcing and R1's cold-start SFT (\cref{sec:r1-family}) are
...
\begin{itemize}
  \item \textbf{Budget forcing is inference-time.}
  ...
  \item \textbf{Cold-start SFT is training-time.} R1's Stage~1
        \citep[\S 2.3]{deepseek-r1} fine-tunes the base model on
        long-CoT exemplars before any RL, ensuring readable traces and
        ...
        Stage~2 GRPO, Stage~3 rejection-sampling SFT, and any
...
isolation through R1-Zero, where GRPO from the base model produces the
...
the Transformer when it must emit and consume long traces:
...
The cross-paper claim worth stating sharply: budget forcing is the
...
budget forcing \citep[\S 4.2, \S 5.1]{s1-2025}. The base architecture
...
\begin{enumerate}
  \item In the architecture taxonomy of Paper~1
        ...
        protocol layer.
  \item In the training taxonomy of Paper~2 (Paper~2 §SFT,
        §post-training), reasoning shows up as the curated long-CoT
        SFT stage and the RLVR stage that follows
...
layer --- is where budget forcing, \texttt{thinkingBudget} APIs,
...
Budget forcing exposes $L$ as a clean inference-time knob, but it
...
RLVR training, on parallel sampling (\cref{sec:self-consistency}), or
...
DAPO's step-efficiency and R1's training-token costs for the
...
\cref{sec:self-consistency} and the RLVR step count of
\cref{sec:rlvr}.
