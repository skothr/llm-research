...
\glsterm{self-consistency}{self-consistency} as a budget-friendly compromise
...
as a \emph{training} signal beyond verifier-only \glsterm{rlvr}{RLVR} is contested,
...
\Citet[\S 2.1, \S 2.2]{deepseek-r1} train \glsterm{r1-zero}{R1-Zero} with the \glsterm{grpo}{GRPO}
...
$15.6\%$ to $77.9\%$ during RL, and to $86.7\%$ with \glsterm{self-consistency}{self-consistency}
at inference time \citep[\S 2.3]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} A \glsterm{reasoning-model}{reasoning model}
...
top of verifier-only \glsterm{rlvr}{RLVR}?''

\begin{intuition}
\glsterm{rlvr}{RLVR} can be read as \glsterm{outcome-reward-model}{ORM}-with-a-perfect-verifier. The R1 reward
...
zero-error verifier and pushes the resulting binary signal through \glsterm{grpo}{GRPO}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/thinkprm2025.md#headline}.}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/thinkprm2025.md#sec-5}.} The
...
\nolinkurl{kb/notes/reasoning/process-supervision.md#4.2}.} The known
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} --- approaches the
...
of \glsterm{rlvr}{RLVR} is cheap because the verifier is exact (an answer-match check,
...
reasoning, planning --- neither \glsterm{rlvr}{RLVR} nor \glsterm{process-reward-model}{PRM}-shaped \glsterm{rlvr}{RLVR} has a clean
...
\begin{contradiction}
\textbf{PRMs as a training-time signal beyond verifier-only \glsterm{rlvr}{RLVR} ---
...
\glsterm{rlvr}{RLVR}-trained reasoning models, the \glsterm{outcome-reward-model}{ORM} in that comparison is replaced
...
at \emph{training time}, whether \glsterm{process-reward-model}{PRM}-shaped \glsterm{rlvr}{RLVR} beats verifier-only
\glsterm{rlvr}{RLVR} at matched compute is benchmark-specific and unsettled.
...
\end{equation}
versus the verifier-only \glsterm{rlvr}{RLVR} cost of $G \times C_{\text{verify-final}}$,
...
\paragraph{Signal density.} The benefit side. \glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo}{GRPO} replaces
...
quality). Returning to math: the \glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo}{GRPO} objective replaces
...
\glsterm{rlvr}{RLVR}; the \glsterm{process-reward-model}{PRM} is not a precondition.
...
\item \textbf{\glsterm{process-reward-model}{PRM}-shaped \glsterm{rlvr}{RLVR} has no clean empirical advantage over
verifier-only \glsterm{rlvr}{RLVR} at frontier scale as of 2026-Q2.} The R1 authors
...
training-time \glsterm{process-reward-model}{PRM}-shaped reward, or a verifier-only \glsterm{rlvr}{RLVR} signal
combined with \glsterm{self-consistency}{self-consistency}. The three options coexist in the 2026
...
half (open: \glsterm{process-reward-model}{PRM}-shaped \glsterm{rlvr}{RLVR} vs verifier-only \glsterm{rlvr}{RLVR} has no settled
...
\glsterm{rlvr}{RLVR}-generalisation-vs-memorisation question
