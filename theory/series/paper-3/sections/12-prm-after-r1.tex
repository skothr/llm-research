...
reasoning triangle on a 2023 generator: PRM-best-of-$N$ dominates
ORM-best-of-$N$ at matched $N$ on PRM800K-grade verifiers, with
min-aggregation as the strongest aggregator and PRM-weighted
self-consistency as a budget-friendly compromise
\citep[\S 4]{lightman2023-prm800k}. Two years later, the strongest
open-weight reasoning model --- DeepSeek-R1 --- reached its headline
AIME 2024 jump using \emph{terminal verifier rewards only} and
explicitly abstained from any neural reward model, whether outcome- or
...
The 2025--2026 picture is mixed. PRMs continue to add value at
\emph{inference} time in a number of measured settings (best-of-$N$
reranking, search guidance) on reasoning-trained policies; their value
as a \emph{training} signal beyond verifier-only RLVR is contested,
...
generative-PRM dominance claim and its 2026 caveats, and frames the
contradiction that \cref{sec:contradictions} catalogues.

\subsection{R1 is the data point}
\label{sec:prm-survival-r1}

\Citet[\S 2.1, \S 2.2]{deepseek-r1} train R1-Zero with the GRPO
objective of \cref{eq:grpo-objective} under a rule-based reward:
\begin{equation}
...
\verb|<think>...</think><answer>...</answer>| schema. No PRM appears
in \cref{eq:r1-rule-reward}; no neural reward model of any kind appears
...
$15.6\%$ to $77.9\%$ during RL, and to $86.7\%$ with self-consistency
at inference time \citep[\S 2.3]{deepseek-r1}.\footnote{KB excerpt:
\texttt{kb/excerpts/deepseek-r1.md\#sec-2-3-aime}.} A reasoning model
that does not place at the AIME human-competitor level cannot be
explained by ``the missing PRM''; the gap is
already closed without one. This is the load-bearing data point that
reframes the post-2023 PRM literature from ``does process supervision
...
top of verifier-only RLVR?''

\begin{intuition}
RLVR can be read as ORM-with-a-perfect-verifier. The R1 reward
...
where \citet{lightman2023-prm800k} compared learned-ORM-best-of-$N$
against learned-PRM-best-of-$N$, R1 substitutes the learned ORM with a
zero-error verifier and pushes the resulting binary signal through GRPO
rather than through reranking. The ORM half of the 2023 comparison has
been replaced by something strictly stronger; whether the PRM half
...
verifiable-step domains.} \Citet{thinkprm2025} report that ThinkPRM, a
generative verifier (\cref{eq:thinkprm-gen}) fine-tuned on $1\%$ of
PRM800K labels and self-generated synthetic verification traces,
beats discriminative PRMs trained on the full PRM800K on ProcessBench,
MATH-500, and AIME 2024 under best-of-$N$ selection and reward-guided
search; out-of-domain it beats the full-PRM800K discriminative baseline
by $8\%$ on a GPQA-Diamond subset and $4.5\%$ on
LiveCodeBench, and beats LLM-as-a-Judge by $7.2\%$ on ProcessBench at
...
verifier's CoT can apply algebraic-verification, type-checking, and
...
2025 PRM survey \citep{zheng2025-prm-survey} echoes the framing while
...
type compatibility) the verifier's CoT can actually
...
lower inference cost.} \Citet{she2025-r-prm} train a discriminative PRM
...
grows, R-PRM converges to ThinkPRM; as it shrinks to zero, R-PRM
...
the post-2023 PRM landscape is more multi-axial than the
PRM800K-grade-classifier baseline implied.
...
human labels than PRM800K (offset by self-generated synthetic
...
per-step or per-trace PRM score. Several R1-replication
...
failure mode is \emph{process hacking}: a misaligned PRM rewards
...
abstaining from neural reward models entirely. When the PRM aligns
...
PRM-best-of-$N$ gains over reasoning-trained policies on AIME 2024
...
reasoning model itself,'' at which point the marginal external-PRM
...
self-reflection and ``aha moment'' behaviours of
\citet[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
\texttt{kb/excerpts/deepseek-r1.md\#sec-2-3-aha}.} --- approaches the
PRM's signal, the verifier-fidelity gap that the PRM exploits closes.
This does not zero out, but it does erode: the same external PRM that
delivered $20$-point pass@$N$ gains over a 2023 GPT-4 generator on
PRM800K-MATH delivers smaller gains over an R1-distill at matched $N$.
...
of RLVR is cheap because the verifier is exact (an answer-match check,
...
reasoning, planning --- neither RLVR nor PRM-shaped RLVR has a clean
...
\begin{contradiction}
\textbf{PRMs as a training-time signal beyond verifier-only RLVR ---
unresolved.} The 2023 PRM800K-era result that PRM > ORM at matched
$N$ \citep[Fig.\ 4]{lightman2023-prm800k} was established on a
non-reasoning-trained generator. As the policy class moves to
RLVR-trained reasoning models, the ORM in that comparison is replaced
by a zero-error symbolic verifier (\cref{eq:r1-rule-reward}); the PRM
...
becomes a strong reasoning model itself.'' R1 itself ships with
\emph{no} PRM in the headline RL loop \citep[\S 2.2]{deepseek-r1}.
The PRM-after-R1 question splits into two: (i) at \emph{inference
time}, generative PRMs (ThinkPRM, R-PRM) continue to add value; (ii)
at \emph{training time}, whether PRM-shaped RLVR beats verifier-only
RLVR at matched compute is benchmark-specific and unsettled.
...
the per-step PRM signal pay for the labelling and verifier-inference
cost? Three terms are in tension.

\paragraph{Labelling cost.} Human PRM800K-style labelling is
...
inference tokens; ThinkPRM-style 1\%-PRM800K + synthetic-verification
...
PRM-shaped reward requires $G \times T$ verifier passes (one per step
...
\propto L_{\text{verify}}$ tokens; on a long-CoT trace with $T \in
[10, 100]$ and $L_{\text{verify}} \in [10^2, 10^3]$ tokens, the PRM-RL
verifier-inference cost can dwarf the policy-rollout cost itself.

\paragraph{Signal density.} The benefit side. PRM-shaped GRPO replaces
a single binary $r_i$ with a $T$-vector of step scores; under a
well-aligned PRM, the gradient identifies the worst step locally
...
off when the labelling signal is reliable. The 2023 PRM800K
...
moderately reliable); 2025 ThinkPRM is on a generative verifier's
self-graded signal (cheap, reliability tied to the verifier's own
quality). Returning to math: the PRM-shaped GRPO objective replaces
...
plus the 2025--2026 PRM survey support:

\begin{enumerate}
\item \textbf{R1 reached AIME 2024 pass@1 of $77.9\%$ with no PRM in
...
RLVR; the PRM is not a precondition.
...
training-data-volume axis (PRM800K) to a verifier-compute axis
($L_{\text{verify}}$).
\item \textbf{The marginal value of an external PRM at inference time
shrinks as the base model becomes a strong reasoning model itself}
\citep{zheng2025-prm-survey}. Mechanism: the policy's internal
verification approaches the PRM's signal, closing the
verifier-fidelity gap.\deepencite{zheng2025-prm-survey \S4 discussion}
\item \textbf{PRM-shaped RLVR has no clean empirical advantage over
verifier-only RLVR at frontier scale as of 2026-Q2.} The R1 authors
abstain from neural reward models on reward-hacking grounds; the
2025 survey reports mixed empirics.
\item \textbf{R-PRM and EDU-PRM} \citep{she2025-r-prm,cao2025-edu-prm}
demonstrate that the discriminative-vs-generative split is not the
only PRM design axis: reasoning preambles and entropy-driven step
boundaries each yield gains at lower verifier cost than full
generative ThinkPRM, suggesting the design space is more
...
the reasoning triangle is best instantiated as a search-time PRM, a
training-time PRM-shaped reward, or a verifier-only RLVR signal
combined with self-consistency. The three options coexist in the 2026
...
The PRM-after-R1 question splits cleanly into an inference-time half
(answered: generative PRMs continue to add value, with diminishing
returns as the policy's own verification improves) and a training-time
half (open: PRM-shaped RLVR vs verifier-only RLVR has no settled
...
collects this section's contradiction alongside the CoT-faithfulness
question (\cref{sec:cot-faithfulness}), the
RLVR-generalisation-vs-memorisation question
