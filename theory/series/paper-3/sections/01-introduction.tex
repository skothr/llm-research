...
DeepSeek-R1-Zero, started its reinforcement-learning phase from a
...
Gemini~2.5-thinking, Anthropic's Claude~3.7 extended-thinking mode,
...
\begin{enumerate}
\item \textbf{Inference-time compute} — chain-of-thought
...
\item \textbf{Reinforcement learning on verifiable rewards (RLVR)} ---
  group-relative policy optimisation over programmatically-checkable
  outcomes \citep{shao2024}, with DAPO's four targeted modifications
...
primary product surface (R1 ships the long-CoT trace; o1 ships a
...
other across the train/inference boundary. RLVR \emph{shapes} a policy
whose long-CoT output is worth amplifying; inference-time compute
...
explanatory power: long-CoT without an RL phase that taught the model
to use it is short-CoT in a costume; a strong verifier without a
proposer that produces diverse-and-mostly-right candidates does not
have anything to verify; an RL loop without a verifiable reward
collapses back to RLHF and its 2023-vintage pathologies. Returning to
canonical math: the GRPO objective transcribed in
...
which RLHF is succeeded by RL-on-verifiable-rewards as a distinct
stage, with different load-bearing variables (programmatic verifier
instead of learned reward, group baseline instead of value network).
That cross-reference is the inheritance route: \cref{sec:rlvr-grpo-dapo}
deepens Paper~2's RLVR treatment in the reasoning-specific direction
...
RL-trained reasoning model are tractable only because Paper~1's
long-context attention (sliding-window, FlashAttention-2/3,
KV-compression) makes their KV cache survivable.\footnote{The
...
whether a frontier reasoning model is feasible as a deployment
artefact, not only as a research curiosity.} Without those two
predecessors the triangle's three legs are a vocabulary; with them,
...
and \citet{kojima2022} on chain-of-thought as a few-shot and zero-shot
...
emergence-with-scale finding that distinguished CoT from
...
baseline well into the R1 era, where it lifts R1-Zero's AIME 2024 from
$77.9\%$ to $86.7\%$ \citep[\S 2.3]{deepseek-r1}.

\Cref{sec:inference-compute-scaling} is the first anchor section: the
\citet{snell2024} reframing of test-time compute as a scaling axis with
...
test-time compute substitutes for $\sim\!14\times$ pretraining FLOPs on
...
\citet{lightman2023-prm800k}'s process reward model is the canonical
...
the load-bearing mechanics. The post-2023 PRM lineage --- Math-Shepherd
on automated step labelling, ThinkPRM and R-PRM on generative
verifiers that themselves reason --- closes the section.
\Cref{sec:tree-search} is the second anchor section: the search-tree
datastructure over partial reasoning prefixes, the canonical reasoning-MCTS
...
\mathsf{true}\,]$, the GRPO objective \citep[\S 4.1]{shao2024} with
group-relative advantage replacing a value network, and DAPO's four
...
in detail --- DeepSeek-R1-Zero's continuous GRPO climb --- is the
...
$1{,}000$ long-CoT exemplars plus a budget-forcing rule that appends
...
when does the marginal FLOP go to inference search, to RLVR training,
...
2025--2026 PRM literature is split between diminishing-returns
...
R1 protocol (cold-start SFT $\to$ GRPO with verifiable rewards $\to$
rejection-sampling SFT on harvested traces $\to$ general-domain RL),
the R1-Zero ablation that strips the cold-start to isolate what the
...
The strong-co-design claim is that the legs interlock causally: RLVR
without long-CoT sampling cannot find the signal that lengthens
traces, long-CoT without RLVR is mechanistically possible but
...
reasons. The 2025 \citet{rlvr-limits-2025} result --- RLVR sharpens
...
RLVR is doing the right thing (it shapes a policy whose sampling and
verification are then meaningful); under the independent-gains
reading, RLVR is doing less than the headline numbers suggest because
...
the same axis (PRM utility post-R1, CoT faithfulness scaling,
...
reasoning model is, by contrast, well past the point where any single
leg is optional.
...
later leg of the triangle inherits: the chain-of-thought decoding
pattern.
