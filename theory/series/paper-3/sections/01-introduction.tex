...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} The same checkpoint,
DeepSeek-\glsterm{r1-zero}{R1-Zero}, started its reinforcement-learning phase from a
$15.6\%$ baseline on the same benchmark
\citep[\S 2.3]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} The size of the
...
\item \textbf{Reinforcement learning on verifiable rewards (\glsterm{rlvr}{RLVR})} ---
  group-relative policy optimisation over programmatically-checkable
  outcomes \citep{shao2024}, with \glsterm{dapo}{DAPO}'s four targeted modifications
...
other across the train/inference boundary. \glsterm{rlvr}{RLVR} \emph{shapes} a policy
...
canonical math: the \glsterm{grpo}{GRPO} objective transcribed in
...
deepens Paper~2's \glsterm{rlvr}{RLVR} treatment in the reasoning-specific direction
...
and \citet{kojima2022} on\linebreak[0] \glsterm{chain-of-thought}{chain-of-thought} as a few-shot and zero-shot
...
\mathsf{true}\,]$, the \glsterm{grpo}{GRPO} objective \citep[\S 4.1]{shao2024} with
group-relative advantage replacing a \glsterm{value}{value} network, and \glsterm{dapo}{DAPO}'s four
...
in detail --- DeepSeek-\glsterm{r1-zero}{R1-Zero}'s continuous \glsterm{grpo}{GRPO} climb --- is the
...
when does the marginal FLOP go to inference search, to \glsterm{rlvr}{RLVR} training,
...
R1 protocol (cold-start SFT $\to$ \glsterm{grpo}{GRPO} with verifiable rewards $\to$
...
The strong-co-design claim is that the legs interlock causally: \glsterm{rlvr}{RLVR}
without long-\glsterm{chain-of-thought}{CoT} sampling cannot find the signal that lengthens
traces, long-\glsterm{chain-of-thought}{CoT} without \glsterm{rlvr}{RLVR} is mechanistically possible but
...
reasons. The 2025 \citet{rlvr-limits-2025} result --- \glsterm{rlvr}{RLVR} sharpens
...
\glsterm{rlvr}{RLVR} is doing the right thing (it shapes a policy whose sampling and
verification are then meaningful); under the independent-gains
reading, \glsterm{rlvr}{RLVR} is doing less than the headline numbers suggest because
...
the same axis (\glsterm{process-reward-model}{PRM} utility post-R1, \glsterm{cot-faithfulness}{CoT faithfulness} scaling,
