...
\emph{currently-deployed} \glsterm{reasoning-model}{reasoning model} ships which legs of the
...
empirical: \emph{every} frontier-2026 reasoning entry ships \glsterm{rlvr-3}{RLVR} plus
long-\glsterm{chain-of-thought}{CoT}, \emph{none} of them ship a process \glsterm{reward-model}{reward model} in the headline
RL loop, and the productionised exposure of inference-time compute has
converged on a small number of API-shaped ``\glsterm{thinking-budget-2}{thinking budget}'' surfaces.
...
\toprule
\textbf{Model} & \textbf{Long-\glsterm{chain-of-thought}{CoT}} & \textbf{\glsterm{rlvr-3}{RLVR}} & \textbf{Verifier in RL loop} & \textbf{Inference search} & \textbf{Budget exposure} \\
\midrule
DeepSeek-R1 \citep{deepseek-r1}
  & yes; ${\sim}10^4$ tok traces \citep[\S 2.3]{deepseek-r1}
  & yes; \glsterm{grpo-2}{GRPO} \citep[\S 2.1]{deepseek-r1}, terminal binary reward
  & none; explicit no-\glsterm{process-reward-model}{PRM} commitment \citep[\S 2.2]{deepseek-r1}
  & none in headline;\allowbreak{} \glsterm{self-consistency-2}{self-consistency} optional \citep[\S 2.3]{deepseek-r1}
...
OpenAI o1 / o3
  ...
  & yes (RL-class on reasoning trajectories per the system card; whether the reward is verifier-derived in the \glsterm{rlvr-3}{RLVR} sense is not disclosed): ``the more reinforcement learning ($\ldots$) the more time it spends thinking'' \deepencite{openai-o1 — system card; cf.\ openai-o3 — system card; claimed in the system card, not externally reproduced}
  & not disclosed; headline reports do not name a \glsterm{process-reward-model}{PRM} \deepencite{openai-o1}
  & not disclosed
  & \nolinkurl{reasoning_effort} $\in\{$low,\allowbreak{} medium,\allowbreak{} high$\}$ \deepencite{openai-o-series — API reference}\\
Gemini~2.5-thinking \citep{gemini2-5}
  ...
  & not disclosed
  & \texttt{thinking\-Budget} (token cap) \deepencite{gemini2-5 — API reference} \\
\glsterm{claude}{Claude} 3.7 / 4 extended thinking
...
\end{table}
\footnotetext{KB excerpt: \nolinkurl{kb/excerpts/deepseek-r1.md#sec-2}.}
...
identifies a \glsterm{process-reward-model}{PRM} as a reward source in the headline RL loop. The R1
authors' explicit no-\glsterm{process-reward-model}{PRM} commitment
\citep[\S 2.2]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} is the strongest
...
headline'' or ``not disclosed.'' The MCTS-and-\glsterm{process-reward-model}{PRM}-guided lineage of
\cref{sec:tree-search} is a research lineage; deployed reasoning models
in 2026 ship long-\glsterm{chain-of-thought}{CoT} plus optional \glsterm{self-consistency-2}{self-consistency} on top. Whether
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} \Cref{sec:r1-family}
...
reward in its supported domains, and a learned \glsterm{reward-model}{reward model} is treated
...
\deepencite{openai-o1 — system card}; no \glsterm{process-reward-model}{PRM} is named in the headline
reward source; no \glsterm{inference-time-search}{inference-time search} algorithm is named in the
...
(long-\glsterm{chain-of-thought}{CoT} yes, \glsterm{rlvr-3}{RLVR} yes, terminal verifier in the publicly-disclosed
reward, no headline \glsterm{process-reward-model}{PRM}, no headline inference search) that the
...
\item \textbf{Anthropic \glsterm{claude}{Claude} 3.7 (and the \glsterm{claude}{Claude}~4 family)} exposes
...
\begin{enumerate}
\item \textbf{\glsterm{rlvr-3}{RLVR} is universal at frontier.} Every reasoning row of
...
reasoning trajectories. The \glsterm{dapo-2}{DAPO}-class RL improvements
of \cref{sec:rlvr-grpo-dapo} \citep{dapo2025}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dapo2025.md#sec-2}.} are part of the same family
deployed in some open-weight reproductions \citep{tulu3}.

\item \textbf{Long-\glsterm{chain-of-thought}{CoT} is universal.} Every row deploys a
...
\emph{training} signal beyond verifier-only \glsterm{rlvr-3}{RLVR} --- is not answered
...
\glsterm{precision-pinning}{DeepSeek-V3}'s ($\sim\!14.8\text{T}$ tokens, mix not fully disclosed
\citep[\S 2]{deepseek-r1}); the o-series and \glsterm{claude}{Claude} families do not
...
single dense \glsterm{transformer}{Transformer} trained with mode-tagged data
...
some rows in fact run undisclosed \glsterm{process-reward-model}{PRM}-guided beam or MCTS rollouts in
serving is not separable from the available evidence. The research
literature \cref{sec:tree-search} continues to push search-and-\glsterm{process-reward-model}{PRM}
...
either unrelated to algorithmic \glsterm{value}{value} or undisclosed.
...
face \glsterm{value}{value}: \emph{published} system cards say ``no \glsterm{process-reward-model}{PRM} in headline
reward,'' \emph{published} reports describe long-\glsterm{chain-of-thought}{CoT} and \glsterm{rlvr-3}{RLVR}, and
...
\citep[\S 2.2]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} Two readings of
...
long-\glsterm{chain-of-thought}{CoT} plus thinking-budget knobs as the dominant pattern, or
\emph{(ii)} the closed-frontier vendors are running undisclosed \glsterm{process-reward-model}{PRM}
...
KV-sharing variants (\glsterm{gqa}{GQA} / \glsterm{multi-head-latent-attention}{MLA}), \glsterm{rmsnorm}{RMSNorm} with \glsterm{pre-ln}{Pre-LN}, \glsterm{rope}{RoPE} with
position-extension methods, \glsterm{swiglu}{SwiGLU} FFNs, MoE at scale. The reasoning
...
(verifier-only \glsterm{rlvr-3}{RLVR}, long-\glsterm{chain-of-thought}{CoT} trace, thinking-budget API), a small
...
\glsterm{process-reward-model}{PRM} utility post-R1 (\cref{sec:prm-survival}), \glsterm{cot-faithfulness-2}{CoT faithfulness}
scaling (\cref{sec:cot-faithfulness}), \glsterm{rlvr-3}{RLVR} generalisation versus
memorisation
