...
empirical: \emph{every} frontier-2026 reasoning entry ships \glsterm{rlvr}{RLVR} plus
...
converged on a small number of API-shaped ``\glsterm{thinking-budget}{thinking budget}'' surfaces.
...
\toprule
\textbf{Model} & \textbf{Long-\glsterm{chain-of-thought}{CoT}} & \textbf{\glsterm{rlvr}{RLVR}} & \textbf{Verifier in RL loop} & \textbf{Inference search} & \textbf{Budget exposure} \\
\midrule
DeepSeek-R1 \citep{deepseek-r1}
  & yes; ${\sim}10^4$ tok traces \citep[\S 2.3]{deepseek-r1}
  & yes; \glsterm{grpo}{GRPO} \citep[\S 2.1]{deepseek-r1}, terminal binary reward
  & none; explicit no-\glsterm{process-reward-model}{PRM} commitment \citep[\S 2.2]{deepseek-r1}
  & none in headline;\allowbreak{} \glsterm{self-consistency}{self-consistency} optional \citep[\S 2.3]{deepseek-r1}
...
in 2026 ship long-\glsterm{chain-of-thought}{CoT} plus optional \glsterm{self-consistency}{self-consistency} on top. Whether
...
(long-\glsterm{chain-of-thought}{CoT} yes, \glsterm{rlvr}{RLVR} yes, terminal verifier in the publicly-disclosed
...
\begin{enumerate}
\item \textbf{\glsterm{rlvr}{RLVR} is universal at frontier.} Every reasoning row of
...
reasoning trajectories. The \glsterm{dapo}{DAPO}-class RL improvements
of \cref{sec:rlvr-grpo-dapo} \citep{dapo2025}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dapo2025.md#sec-3}.} are part of the same family
...
\emph{training} signal beyond verifier-only \glsterm{rlvr}{RLVR} --- is not answered
...
reward,'' \emph{published} reports describe long-\glsterm{chain-of-thought}{CoT} and \glsterm{rlvr}{RLVR}, and
...
KV-sharing variants (\glsterm{gqa}{GQA} / \glsterm{mla}{MLA}), \glsterm{rmsnorm}{RMSNorm} with \glsterm{pre-ln}{Pre-LN}, \glsterm{rope}{RoPE} with
...
(verifier-only \glsterm{rlvr}{RLVR}, long-\glsterm{chain-of-thought}{CoT} trace, thinking-budget API), a small
...
\glsterm{process-reward-model}{PRM} utility post-R1 (\cref{sec:prm-survival}), \glsterm{cot-faithfulness}{CoT faithfulness}
scaling (\cref{sec:cot-faithfulness}), \glsterm{rlvr}{RLVR} generalisation versus
memorisation
