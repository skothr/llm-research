...
\textbf{process reward model (PRM)} — a function that scores each step
of a chain-of-thought trace
...
motivation for the entire 2024--2026 PRM lineage. This section
formalises the per-step reward, walks the four canonical aggregation
rules, transcribes the PRM800K experiment, traces the lineage through
Math-Shepherd's automated labelling and ThinkPRM's generative-verifier
shift, and closes on the open question of whether external PRMs still
add value once the policy itself has been RL-trained for verifiable
reasoning.
...
\textbf{process reward model} is a function%
...
\begin{equation}
  ...
  \label{eq:prm-loss}
\end{equation}
The conditioning is the only structural change from the ORM loss
...
answer $y$. A PRM emits one score per prefix; an ORM emits one score
per trace.

To compare a PRM and an ORM at fixed $N$ in best-of-$N$ reranking, the
...
ablate all three on PRM800K-grade verifiers and find
...
\begin{intuition}
A PRM is a per-step graded checkmark; an ORM is one final
...
\citet{lightman2023-prm800k} construct \textbf{PRM800K}, a dataset of
...
resulting verifier is the foundational reference PRM against which all
subsequent open-source efforts (Math-Shepherd, ThinkPRM, R-PRM,
EDU-PRM) benchmark.
...
enough that it is the default aggregator in every downstream PRM
paper. \citet[\S 4.3]{lightman2023-prm800k} additionally show that
PRM-weighted self-consistency — majority vote with each ballot
weighted by the trace's min-PRM score — improves over plain
majority vote, recovering some of the PRM gain at lower verifier-cost
overhead.

\begin{intuition}
Why min wins: if a single algebraic slip turns ``$3x = 12$'' into
``$x = 5$'', every subsequent step inherits the error. The trace's
truth-value is bounded above by its weakest step. Mean and product
...
The PRM800K protocol is human-expensive --- $\approx 800$K step-labels
...
regex on the boxed final answer) can manufacture its own PRM training
...
that subsequent generative-PRM efforts (\cref{sec:prm-thinkprm}) and
reasoning-trained PRMs (\cref{sec:prm-r-edu}) build on.

\begin{analogy}
Math-Shepherd is the PRM equivalent of Q-learning's bootstrapped
value estimate: the value of a state is approximated by the
...
PRM trained on \cref{eq:prm-loss} with $\tilde c_t$ in place of $c_t$
...
scalar per prefix, the verifier is itself a small reasoning LLM that
reads the prefix, emits its own short verification chain-of-thought,
and outputs a verdict token \citep[\S 3]{thinkprm2025}.\footnote{KB
excerpt: \texttt{kb/excerpts/thinkprm2025.md\#sec-3}.} Formally,
ThinkPRM replaces the discriminative $R^P_\phi$ of \cref{eq:prm-signature}
...
The headline result: a long-CoT verifier fine-tuned on
\textbf{1\%} of PRM800K (roughly 8K step-labels rather than 800K)
plus self-generated synthetic verification traces beats discriminative
verifiers trained on the full PRM800K
\citep[abstract]{thinkprm2025}.\footnote{KB excerpt:
\texttt{kb/excerpts/thinkprm2025.md\#abstract}.} On out-of-domain
evaluation, ThinkPRM exceeds the full-PRM800K discriminative baseline
by \textbf{8\%} on a GPQA-Diamond subset and \textbf{4.5\%} on
LiveCodeBench, and beats LLM-as-a-Judge by \textbf{7.2\%} on
ProcessBench at matched verification-token budget
\citep[abstract]{thinkprm2025}.\footnote{KB excerpt:
...
The cost asymmetry is real: best-of-$N$ with a discriminative PRM
costs $O(N)$ classifier passes; with a generative PRM,
$O(N \times L_\text{verify})$ tokens, where $L_\text{verify}$ is the
verification trace length. The asymmetry is the same one that
distinguishes a chain-of-thought policy from a direct-answer policy
...
labelled example. The mechanism: a verifier's CoT can apply
...
The 2025 PRM literature splits along two axes orthogonal to the
discriminative-vs-generative split:

\paragraph{Reasoning-aware PRMs.} \citet{she2025-r-prm} train a PRM
...
\cref{eq:thinkprm-gen} is one of degree: ThinkPRM's verifier emits a
full long-CoT trace and a verdict token; R-PRM emits a short
...
linebreaks or paragraph markers. EDU-PRM defines step boundaries by a
...
EDU-PRM matches or beats Math-Shepherd on
ProcessBench.\deepencite{cao2025-edu-prm \S3 (excerpt skeleton-only as
...
The two strands jointly suggest that the 2023 PRM800K formulation —
...
verification programme. Each relaxation (Math-Shepherd, ThinkPRM,
R-PRM, EDU-PRM) trades human labels or syntactic priors for some
...
\citet{zheng2025-prm-survey} catalogue the 2023--2025 PRM landscape
...
verifier's CoT can apply; discriminative PRMs remain competitive on
...
strong reasoning model itself} — at which point the marginal gain
from an external PRM at test time decreases. This is the opening of
...
only} — no PRM in the RL loop \citep[\S 2.2]{deepseek-r1}.\footnote{KB
...
the strongest open-weight reasoning model.

The implication, surfaced jointly by \citet{thinkprm2025} and
\citet{zheng2025-prm-survey}: as the policy itself internalises
verification through long-CoT RL, the marginal value of an external
PRM at test time shrinks. The 2026 picture is mixed; the contradiction
deserves explicit treatment.

\begin{contradiction}
\textbf{PRM utility post-R1 — diminishing or persistent?}
The PRM800K-era headline ``PRM > ORM at matched $N$'' was established
on a non-reasoning-trained generator (a GPT-4 variant)
\citep[\S 4]{lightman2023-prm800k}. As the policy class moves to
RLVR-trained reasoning models, the picture shifts. Some 2026
measurements show PRM-best-of-$N$ over R1-distill outputs still
delivers gains; others find diminishing returns as the policy's own
internal verification approaches the PRM's signal
\citep[\S 5]{thinkprm2025}. The 2025 PRM survey
...
reasoning model itself. Whether external PRMs survive the
...
A second, related tension: \emph{process hacking}. PRM-shaped RLVR
\citep[\S 2.4]{lightman2023-prm800k}\footnote{KB note:
\texttt{kb/notes/reasoning/process-supervision.md\#4.2}.} replaces or
augments the binary outcome reward of RLVR (\cref{sec:rlvr}) with a
per-step PRM score. The middle-ground appeal is theoretical: dense
signal beats sparse signal for credit assignment. The empirical risk:
a misaligned PRM rewards steps that \emph{look} correct without
producing the right answer, the step-level analogue of outcome-hacking
under a learned reward. Whether PRM-shaped GRPO actually outperforms
verifier-only GRPO at matched compute is benchmark-specific and
unsettled in 2026.

\subsection{Forward link}
\label{sec:prm-forward}

The PRM scores per-step value $r_t \in [0, 1]$ on a partial prefix
...
beam, MCTS) that use a PRM as the value function $V_\phi(s_{1:t}) =
...
shape which branches survive expansion. The PRM is then re-encountered
