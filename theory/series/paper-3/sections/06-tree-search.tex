...
\Cref{sec:inference-compute-scaling} factored \glsterm{test-time-compute}{test-time compute} into a
...
\glsterm{process-reward-model}{PRM} as a \glsterm{value}{value} function to direct expansion. The taxonomy of the
...
\glsterm{reasoning-mcts}{reasoning-MCTS} recurrence from
\citet{rstar-math2025},\footnote{KB excerpt:
\nolinkurl{kb/excerpts/rstar-math2025.md#sec-3-mcts}.} walks the four
...
notes \glsterm{mcts-rag}{MCTS-RAG} \citep{hu2025-mcts-rag} as the retrieval-extended
cousin, and closes on the live contradiction between MCTS-headline
framings and \citet[\S 5--6]{snell2024}'s evidence that BFS-with-\glsterm{process-reward-model}{PRM} is
...
line, a bullet)%
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/inference-time-search.md#1-formal-definition}.}.
...
next steps under the underlying \glsterm{llm}{LLM};
\item the \emph{\glsterm{value}{value} function}
$V_\phi(x, z_{1:t}) \to \mathbb{R}$, which scores partial states. The
canonical instantiation is $V_\phi = R^P_\phi$, the per-step \glsterm{process-reward-model}{PRM} of
\cref{eq:prm-signature};
\item the \emph{terminal verifier}
$\mathcal{V}(x, y) \to \{0, 1\}$, which scores complete answers. For
math and code this is the same rule-based check that defines the \glsterm{rlvr}{RLVR}
...
$V_\phi$            & learned \glsterm{value}{value} function on partial prefixes (typically a \glsterm{process-reward-model}{PRM}) \\
...
partials with $V_\phi$; retain the top $B$ by \glsterm{value}{value}. Iterating to
...
steps according to the \glsterm{process-reward-model}{PRM}'s predicted step-wise reward-to-go estimate;
...
\nolinkurl{kb/excerpts/snell2024.md#sec-5-2-search}.} BFS-V differs from
classical beam only in the per-step scoring object: the \glsterm{process-reward-model}{PRM}'s
...
prompts, narrower on easier ones. Second, the per-step \glsterm{process-reward-model}{PRM} signal is
...
by node: a beam node's score is the worst \glsterm{process-reward-model}{PRM} score along its
...
\emph{rollout-derived \glsterm{value}{value} estimates} are accumulated at each node and
...
an SLM-based process \glsterm{reward-model}{reward model}''
\citep[abstract]{rstar-math2025}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/rstar-math2025.md#abstract}.} --- and report 7B
...
\nolinkurl{kb/excerpts/rstar-math2025.md#headline}.}

\paragraph{The PUCT selection rule.} At each iteration, descend the tree
from the root by repeatedly choosing the child $s$ that maximises%
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/inference-time-search.md#2-2-mcts-over-reasoning-steps-rstar-math}.}
\begin{equation}
  U(s) \;=\; Q(s) \;+\; c_\text{puct}\;
...
\cref{eq:puct} to descend until a leaf $\ell$ is reached. ``Leaf'' here
...
return, or a \glsterm{process-reward-model}{PRM}-weighted vote across rollouts; rStar-Math uses the
...
calibrated \glsterm{process-reward-model}{PRM}), the adaptive allocation buys MCTS a constant-factor
edge; on rough landscapes (under-trained \glsterm{process-reward-model}{PRM}, narrow correct path),
...
beat it. Returning to math: \cref{eq:puct}'s exploitation term $Q(s)$
...
root-to-node prefix $z_{1:t}$. The \glsterm{process-reward-model}{PRM} is the \glsterm{value}{value} function on the
...
Operation                          & Cost \\
\midrule
Selection (root $\to$ leaf $\ell$) & $O(D)$ tree-traversal ops, no \glsterm{llm}{LLM} calls \\
...
and traversal terms. Operationally, the \glsterm{process-reward-model}{PRM} call cost adds a constant
...
treatment%
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/inference-time-search.md#2-3-rest-mcts}.}:
\begin{enumerate}
\item Run MCTS with the current policy $\pi_\theta^{(r)}$ and \glsterm{process-reward-model}{PRM}
...
\item Update the \glsterm{process-reward-model}{PRM} $V_\phi^{(r+1)}$ using the new policy's
trajectory distribution and the trajectory-correctness labels.
\item Repeat from step 1.
\end{enumerate}
The structural identity with AlphaZero
\deepencite{silver2017-alphazero \S Methods (arXiv:1712.01815 numbers no
sections; the self-play-RL and MCTS description sits in the unnumbered
Methods section --- canonical citation still pending, the key is absent
from references.bib; KB note:
\nolinkurl{kb/notes/reasoning/inference-time-search.md#7-intuitions-and-analogies})}
is exact:
\begin{itemize}
...
$z_{1:t}$;
\item \glsterm{value}{value} head $\leftrightarrow$ \glsterm{process-reward-model}{PRM} $V_\phi$;
...
\glsterm{process-reward-model}{PRM} is replaced by a \emph{process preference model} (\glsterm{process-preference-model}{PPM}) trained
...
\nolinkurl{kb/excerpts/rstar-math2025.md#sec-3}.} The choice trades
...
The action space at each tree node is the \glsterm{llm}{LLM}'s next-step distribution
$\pi_\theta(s_{t+1} \mid x, z_{1:t})$ rather than the board's
legal-move distribution. The \glsterm{value}{value} head is the \glsterm{process-reward-model}{PRM}
...
math: \cref{eq:puct}'s PUCT formula is the same equation that drives
...
\glsterm{value}{value} head than the last, and the harvested trajectories are
...
a separate \glsterm{value}{value}-network bootstrap.
\end{analogy}
...
policy. The \glsterm{value}{value} function $V_\phi$ is generalised to score states
...
expression does not capture). \glsterm{mcts-rag}{MCTS-RAG} closes the gap between two
otherwise-disjoint inference-time-compute literatures --- search-based
\glsterm{test-time-compute}{TTC} for reasoning and RAG for knowledge access --- under one
tree-search framework.

\subsection{Search-vs-trained-CoT: the live contradiction}
\label{sec:tree-search-vs-cot}

The 2024 hypothesis articulated by the early \glsterm{reasoning-mcts}{reasoning-MCTS} literature
...
\nolinkurl{kb/excerpts/rstar-math2025.md#headline}.} \citet{deepseek-r1}
demonstrate the opposite extreme: a large policy with \glsterm{rlvr}{RLVR}-trained
long \glsterm{chain-of-thought}{CoT} and \emph{no} explicit \glsterm{inference-time-search}{inference-time search} reaches
77.9\% pass@1 on AIME 2024 and 86.7\% with \glsterm{self-consistency}{self-consistency}, on the
...
The clean reading%
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/inference-time-search.md#5-the-search-vs-trained-cot-debate}.}
...
standard distributions, internalised search via long \glsterm{chain-of-thought}{CoT} is
...
\begin{contradiction}
\textbf{BFS-with-\glsterm{process-reward-model}{PRM} vs.\ MCTS at fixed inference budget.}
\citet[\S 5--6]{snell2024} report BFS-V (the level-order
beam-with-\glsterm{process-reward-model}{PRM} of \cref{sec:tree-beam}) competitive with or outright
better than alternative search strategies at matched compute on the
MATH benchmark with \glsterm{palm}{PaLM}~2-S\* as the base model
\citep[\S 5.2]{snell2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-5-2-search}.} The implication
...
of \cref{eq:puct}. Reconciling the two: Snell's evaluation uses a
non-reasoning-trained base model and a single round of search; rStar-
Math's MCTS is co-trained with the policy and \glsterm{process-reward-model}{PRM} across four rounds
...
cost-per-trajectory dominates \glsterm{self-consistency}{self-consistency}
...
on the policy's strength, the \glsterm{value}{value} function's calibration, and
...
\cref{eq:cost-mcts} cost expression and the \cref{eq:puct} PUCT
recurrence both reappear there in a different role: ReST-MCTS\textsuperscript{*}'s
search-then-SFT loop and \glsterm{rlvr}{RLVR}'s group-of-rollouts both \emph{generate}
...
(\cref{sec:tree-mcts}), more \glsterm{rlvr}{RLVR} training (\cref{sec:rlvr-grpo-dapo}),
