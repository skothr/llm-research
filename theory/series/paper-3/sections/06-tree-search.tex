...
math and code this is the same rule-based check that defines the \glsterm{rlvr-3}{RLVR}
...
demonstrate the opposite extreme: a large policy with \glsterm{rlvr-3}{RLVR}-trained
long \glsterm{chain-of-thought}{CoT} and \emph{no} explicit \glsterm{inference-time-search}{inference-time search} reaches
77.9\% pass@1 on AIME 2024 and 86.7\% with \glsterm{self-consistency-2}{self-consistency}, on the
...
cost-per-trajectory dominates \glsterm{self-consistency-2}{self-consistency}
...
search-then-SFT loop and \glsterm{rlvr-3}{RLVR}'s group-of-rollouts both \emph{generate}
...
(\cref{sec:tree-mcts}), more \glsterm{rlvr-3}{RLVR} training (\cref{sec:rlvr-grpo-dapo}),
