...
marginal FLOP on \glsterm{inference-time-search}{inference-time search} beat spending it on
\emph{pretraining}? The empirical answer is conditional on difficulty
regime: on easy and intermediate prompts \glsterm{test-time-compute}{test-time compute} substitutes
for $\sim\!14\times$ more pretraining FLOPs at matched accuracy
\citep[\S 6]{snell2024};\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-1-flops-matched}.} on the hardest
...
RL on verifiable rewards (\glsterm{rlvr-3}{RLVR}) at \glsterm{grpo-2}{GRPO}/\glsterm{dapo-2}{DAPO} scale, which spends
...
(a) more inference search/sampling, (b) more \glsterm{rlvr-3}{RLVR} training, or (c)
...
appear because \glsterm{palm}{PaLM}~2-S\* (Codey) is not \glsterm{rlvr-3}{RLVR}-trained
\citep[\S 4]{snell2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-4-setup}.} What
...
\citep[\S 6]{snell2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-1-flops-matched}.} At matched
...
benchmark on \glsterm{palm}{PaLM}~2-S\*, and the cross-over has not been
...
empirical answers from the \glsterm{rlvr-3}{RLVR} literature, since no Snell-style
...
\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-1-hp}.} report
DeepSeek-\glsterm{r1-zero}{R1-Zero} training as $10{,}400$ \glsterm{grpo-2}{GRPO} steps on $G = 16$ rollouts
...
$15.6\%$ on \glsterm{precision-pinning}{DeepSeek-V3}-Base to $77.9\%$ on \glsterm{r1-zero}{R1-Zero}
\citep[\S 2.3]{deepseek-r1};\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} \glsterm{self-consistency-2}{self-consistency}
at inference adds a further $8.8\%$ to $86.7\%$ at $N = 64$ samples
\citep[\S 2.3]{deepseek-r1}. The $N = 64$ inference branch costs
$\sim 64$ forward-pass-equivalents per prompt; the \glsterm{grpo-2}{GRPO} training
branch costs $\sim 5\cdot 10^{9}$ tokens, amortised across every
deployed \glsterm{query}{query}. For a deployment that serves $\gtrsim 10^{8}$ queries
the per-\glsterm{query}{query} share of $C_\text{RL}$ is below the per-\glsterm{query}{query}
$C_\text{test}$ at $N = 64$; for a one-shot evaluation it is not.

\paragraph{DAPO's step-efficiency.}
\citet[\S 4]{dapo2025}%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/dapo2025.md#sec-4-headline}.}
report that \glsterm{dapo-2}{DAPO} reaches $50$ points on AIME~2024 with the
Qwen2.5-32B-base in roughly half the \glsterm{grpo-2}{GRPO} steps the DeepSeek-\glsterm{r1-zero}{R1-Zero}
...
of \cref{eq:three-channel}, \glsterm{dapo-2}{DAPO} halves $C_\text{RL}$ at matched
...
\glsterm{precision-pinning}{DeepSeek-V3}-Base model (the $C_\text{pre}$ channel, fixed), \glsterm{grpo-2}{GRPO} with
verifiable rewards lifts AIME~2024 from $15.6\%$ to $77.9\%$ pass@1
(the $C_\text{RL}$ channel, $\sim\!10^{4}$ \glsterm{grpo-2}{GRPO} steps); \glsterm{self-consistency-2}{self-consistency}
...
flops per \glsterm{query}{query}. The marginal-FLOP ranking is anecdotal and
...
\end{equation}
where the left side is a smaller base model post-trained with \glsterm{rlvr-3}{RLVR}
plus \glsterm{inference-time-search}{inference-time search}, and the right side is the
...
identity, not a \glsterm{scaling-law}{scaling law}: it commits to no functional form for
...
$C_\text{test}$ in search form (best-of-$N$-with-\glsterm{process-reward-model}{PRM}, beam, MCTS); the
hardest tail favours $C_\text{pre}$ \citep[\S 6]{snell2024}. \glsterm{rlvr-3}{RLVR}
...
\item \textbf{Verifier availability.} The whole \glsterm{rlvr-3}{RLVR} channel exists
...
The frontier extension --- \glsterm{llm-as-judge}{LLM-as-judge}, generative PRMs as soft
...
measured against a specific base model. \glsterm{palm}{PaLM}~2-S\* on MATH gives
...
\item \textbf{Deployment-volume amortisation.} The per-\glsterm{query}{query} share of
$C_\text{pre}$ and $C_\text{RL}$ scales as $1/(\text{queries served})$
while $C_\text{test}$ does not amortise. For a research evaluation,
$C_\text{test}$ dominates the per-\glsterm{query}{query} budget; for a frontier
...
\glsterm{palm}{PaLM}~2-S\*. \citet[\S 2.3]{deepseek-r1} establish that \glsterm{grpo-2}{GRPO} on
\glsterm{precision-pinning}{DeepSeek-V3}-Base converts $\sim\!10^{4}$ training steps into $+62.3$
points of AIME~2024 pass@1 with \glsterm{self-consistency-2}{self-consistency} adding a further
$+8.8$ at $N = 64$. \citet[\S 4]{dapo2025} establish that \glsterm{dapo-2}{DAPO} halves
...
first.} The R1 anecdote shows the first units of \glsterm{grpo-2}{GRPO} buy the largest
accuracy gain on math benchmarks
\citep[\S 2.3]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} The marginal
...
incremental sharpening.} \glsterm{self-consistency-2}{Self-consistency} on \glsterm{r1-zero}{R1-Zero} adds $8.8$
...
gap. \glsterm{rlvr-3}{RLVR} cannot bootstrap on a $G$-group of trajectories that all
score $R_i = 0$ either, because the group-normalised advantage of
\cref{eq:grpo-advantage} is zero by construction
\citep[\S 4.1]{shao2024}; this is the failure mode \glsterm{dapo-2}{DAPO}'s dynamic
sampling addresses by discarding all-zero groups
\citep[\S 3]{dapo2025}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-dynamic-sampling}.} but cannot
...
costs over per-\glsterm{query}{query} inference costs. None of them is a substitute for
...
triangle's central tradeoff. \Cref{sec:cot-faithfulness} (\glsterm{chain-of-thought}{CoT}
...
\glsterm{claude}{Claude}~3.7-extended-thinking, Qwen3-thinking, and QwQ; the snapshot
...
$(C_\text{train}, C_\text{RL}, C_\text{test})$ \glsterm{scaling-law}{scaling law} exists ---
