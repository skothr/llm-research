...
RL on verifiable rewards (\glsterm{rlvr}{RLVR}) at \glsterm{grpo}{GRPO}/\glsterm{dapo}{DAPO} scale, which spends
...
(a) more inference search/sampling, (b) more \glsterm{rlvr}{RLVR} training, or (c)
...
appear because \glsterm{palm}{PaLM}~2-S\* (Codey) is not \glsterm{rlvr}{RLVR}-trained
...
empirical answers from the \glsterm{rlvr}{RLVR} literature, since no Snell-style
...
DeepSeek-\glsterm{r1-zero}{R1-Zero} training as $10{,}400$ \glsterm{grpo}{GRPO} steps on $G = 16$ rollouts
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} \glsterm{self-consistency}{self-consistency}
at inference adds a further $8.8\%$ to $86.7\%$ at $N = 64$ samples
\citep[\S 2.3]{deepseek-r1}. The $N = 64$ inference branch costs
$\sim 64$ forward-pass-equivalents per prompt; the \glsterm{grpo}{GRPO} training
...
report that \glsterm{dapo}{DAPO} reaches $50$ points on AIME~2024 with the
Qwen2.5-32B-base in roughly half the \glsterm{grpo}{GRPO} steps the DeepSeek-\glsterm{r1-zero}{R1-Zero}
...
of \cref{eq:three-channel}, \glsterm{dapo}{DAPO} halves $C_\text{RL}$ at matched
...
\glsterm{precision-pinning}{DeepSeek-V3}-Base model (the $C_\text{pre}$ channel, fixed), \glsterm{grpo}{GRPO} with
verifiable rewards lifts AIME~2024 from $15.6\%$ to $77.9\%$ pass@1
(the $C_\text{RL}$ channel, $\sim\!10^{4}$ \glsterm{grpo}{GRPO} steps); \glsterm{self-consistency}{self-consistency}
...
\end{equation}
where the left side is a smaller base model post-trained with \glsterm{rlvr}{RLVR}
...
hardest tail favours $C_\text{pre}$ \citep[\S 6]{snell2024}. \glsterm{rlvr}{RLVR}
...
\item \textbf{Verifier availability.} The whole \glsterm{rlvr}{RLVR} channel exists
...
\glsterm{palm}{PaLM}~2-S\*. \citet[\S 2.3]{deepseek-r1} establish that \glsterm{grpo}{GRPO} on
\glsterm{precision-pinning}{DeepSeek-V3}-Base converts $\sim\!10^{4}$ training steps into $+62.3$
points of AIME~2024 pass@1 with \glsterm{self-consistency}{self-consistency} adding a further
$+8.8$ at $N = 64$. \citet[\S 4]{dapo2025} establish that \glsterm{dapo}{DAPO} halves
...
first.} The R1 anecdote shows the first units of \glsterm{grpo}{GRPO} buy the largest
...
incremental sharpening.} \glsterm{self-consistency}{Self-consistency} on \glsterm{r1-zero}{R1-Zero} adds $8.8$
...
gap. \glsterm{rlvr}{RLVR} cannot bootstrap on a $G$-group of trajectories that all
score $R_i = 0$ either, because the group-normalised advantage of
\cref{eq:grpo-advantage} is zero by construction
\citep[\S 4.1]{shao2024}; this is the failure mode \glsterm{dapo}{DAPO}'s dynamic
