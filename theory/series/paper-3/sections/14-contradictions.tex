...
\nolinkurl{kb/excerpts/lightman2023-prm800k.md#sec-4-2-aggregation}.} was
established on a non-reasoning-trained generator. Once the generator is
itself an \glsterm{rlvr-3}{RLVR}-trained reasoner, the comparison shifts: the \glsterm{outcome-reward-model}{ORM} in the
...
verifier-only \glsterm{rlvr-3}{RLVR} is contested
(\cref{sec:prm-survival-training}). Closure requires a controlled
\glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo-2}{GRPO} vs verifier-only \glsterm{grpo-2}{GRPO} comparison at matched
...
model accuracy drops while faithfulness rises under \glsterm{self-consistency-2}{self-consistency}
...
\glsterm{rlvr-3}{RLVR} post-training, and general scale at fixed others
...
$15.6\%\!\to\!77.9\%$ AIME 2024 pass@1 climb as \glsterm{rlvr-3}{RLVR}-elicited
reasoning capability. \Citet{rlvr-limits-2025}%
\deepencite{rlvr-limits-2025 §3-§5, KB-excerpt-pending}
read it differently: the
group-mean baseline of \glsterm{grpo-2}{GRPO} (\cref{sec:rlvr-grpo-objective}) bounds
\glsterm{rlvr-3}{RLVR}'s reach by the base policy's pass@$K$ at \emph{any} $K$, not by
its pass@$1$, and so the climb is ``the base model could already solve
$77.9\%$ of these problems at some sample budget; \glsterm{grpo-2}{GRPO} transferred
that to pass@1.'' Under this reading \glsterm{rlvr-3}{RLVR} \emph{sharpens} the policy
...
camp's synthesis is that ``\glsterm{rlvr-3}{RLVR} teaches \emph{trace-length use} (when
...
significant gap between \glsterm{rlvr-3}{RLVR}-trained and base models on
...
maintains an advantage over both the base model and \glsterm{grpo-2}{GRPO} as $K$ grows;
the field broadly agrees the boundary issue Yue diagnoses is real, but
disagrees about whether it is intrinsic to \glsterm{rlvr-3}{RLVR} or tractable under
...
\Citet[\S 2.3]{deepseek-r1} establish that \glsterm{grpo-2}{GRPO} on \glsterm{precision-pinning}{DeepSeek-V3}-Base
converts ${\sim}10^{4}$ training steps into $+62.3$ AIME 2024 pass@1
points with \glsterm{self-consistency-2}{self-consistency} adding a further $+8.8$ at $N\!=\!64$.
\Citet[\S 4]{dapo2025} establish that \glsterm{dapo-2}{DAPO} halves $C_\text{RL}$ at
...
$15.6\%\!\to\!77.9\%$ AIME 2024 climb to \glsterm{rlvr-3}{RLVR}-elicited reasoning. The
...
(ii) a control experiment that re-runs the \glsterm{grpo-2}{GRPO} recipe on Qwen2-7B ---
...
\emph{(C1)} a \glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo-2}{GRPO} vs verifier-only \glsterm{grpo-2}{GRPO} comparison at
...
\glsterm{rlvr-3}{RLVR} post-training, and general scale at fixed others, run at frontier
...
entry ships \glsterm{rlvr-3}{RLVR} plus long-\glsterm{chain-of-thought}{CoT}, \emph{none} ship a process reward
...
``\glsterm{thinking-budget-2}{thinking budget}'' surfaces. Contested: the contamination-vs-elicitation
...
this section's (C2) and (C5) directly; Paper~2's \glsterm{rlhf}{RLHF}-to-\glsterm{rlvr-3}{RLVR}
