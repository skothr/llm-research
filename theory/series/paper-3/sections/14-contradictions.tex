...
The \glsterm{prm800k}{PRM800K}-era headline ``\glsterm{process-reward-model}{PRM} > \glsterm{outcome-reward-model}{ORM} at matched $N$''
\citep[\S 4]{lightman2023-prm800k}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/lightman2023-prm800k.md#sec-4}.} was
established on a non-reasoning-trained generator. Once the generator is
itself an \glsterm{rlvr}{RLVR}-trained reasoner, the comparison shifts: the \glsterm{outcome-reward-model}{ORM} in the
original setup is replaced by a zero-error symbolic verifier
(\cref{sec:r1-protocol}), and the \glsterm{process-reward-model}{PRM} must now beat the policy's own
...
strong \glsterm{reasoning-model}{reasoning model} itself.'' R1 ships with no \glsterm{process-reward-model}{PRM} in the headline
...
verifier-only \glsterm{rlvr}{RLVR} is contested
(\cref{sec:prm-survival-training}). Closure requires a controlled
\glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo}{GRPO} vs verifier-only \glsterm{grpo}{GRPO} comparison at matched
...
model accuracy drops while faithfulness rises under \glsterm{self-consistency}{self-consistency}
\citep[\S 4]{mehta2026-faithfulness-scaling}\footnote{KB note:
\nolinkurl{kb/notes/reasoning/chain-of-thought.md#4.1}.} --- a pattern
...
intensity? The published 2026 ablations cannot separate long-\glsterm{chain-of-thought}{CoT} SFT,
\glsterm{rlvr}{RLVR} post-training, and general scale at fixed others
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} reads the
$15.6\%\!\to\!77.9\%$ AIME 2024 pass@1 climb as \glsterm{rlvr}{RLVR}-elicited
reasoning capability. \Citet{rlvr-limits-2025}%
\deepencite{rlvr-limits-2025 §3-§5, KB-excerpt-pending}
read it differently: the
group-mean baseline of \glsterm{grpo}{GRPO} (\cref{sec:rlvr-grpo-objective}) bounds
\glsterm{rlvr}{RLVR}'s reach by the base policy's pass@$K$ at \emph{any} $K$, not by
its pass@$1$, and so the climb is ``the base model could already solve
$77.9\%$ of these problems at some sample budget; \glsterm{grpo}{GRPO} transferred
that to pass@1.'' Under this reading \glsterm{rlvr}{RLVR} \emph{sharpens} the policy
...
with the claim of a \glsterm{capability-ceiling}{capability ceiling}, and one cleanly-stated form of the
camp's synthesis is that ``\glsterm{rlvr}{RLVR} teaches \emph{trace-length use} (when
...
\glsterm{chain-of-thought}{CoT}-Pass@K --- a metric that scores both the final answer and the
\glsterm{chain-of-thought}{chain-of-thought} derivation --- and report a consistent and
significant gap between \glsterm{rlvr}{RLVR}-trained and base models on
...
maintains an advantage over both the base model and \glsterm{grpo}{GRPO} as $K$ grows;
the field broadly agrees the boundary issue Yue diagnoses is real, but
disagrees about whether it is intrinsic to \glsterm{rlvr}{RLVR} or tractable under
...
substitution on the easy/intermediate MATH bin under \glsterm{palm}{PaLM}~2-S\*.
\Citet[\S 2.3]{deepseek-r1} establish that \glsterm{grpo}{GRPO} on \glsterm{precision-pinning}{DeepSeek-V3}-Base
converts ${\sim}10^{4}$ training steps into $+62.3$ AIME 2024 pass@1
points with \glsterm{self-consistency}{self-consistency} adding a further $+8.8$ at $N\!=\!64$.
\Citet[\S 4]{dapo2025} establish that \glsterm{dapo}{DAPO} halves $C_\text{RL}$ at
...
the universal \glsterm{scaling-law}{scaling law} that would generalise Chinchilla to the
...
$15.6\%\!\to\!77.9\%$ AIME 2024 climb to \glsterm{rlvr}{RLVR}-elicited reasoning. The
...
recall of memorised solutions surfaced through long-\glsterm{chain-of-thought}{CoT} self-prompting,
...
methodologies (\glsterm{n-gram-overlap}{n-gram overlap} on solutions, near-duplicate matching,
\glsterm{decontamination}{decontamination}-by-substring) all have known false-negative modes
...
load-bearing treatment of the methodology \glsterm{debate}{debate} itself --- which
...
\glsterm{decontamination}{decontamination} audit on the pre- and post-training data of R1, and
(ii) a control experiment that re-runs the \glsterm{grpo}{GRPO} recipe on Qwen2-7B ---
...
\Citet[\S 5--6]{snell2024} report BFS-V (the level-order beam-with-\glsterm{process-reward-model}{PRM}
...
\glsterm{palm}{PaLM}~2-S\* as the base model
\citep[\S 5.2]{snell2024}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-5-2-search}.}: the extra MCTS
...
search; rStar-Math co-trains MCTS with the policy and \glsterm{process-reward-model}{PRM} across four
...
in \cref{tab:frontier-2026} where the absence of a \glsterm{process-reward-model}{PRM} in the
...
verifier-only RL plus long-\glsterm{chain-of-thought}{CoT} plus thinking-budget knobs as the
dominant pattern, or \emph{(ii)} the closed-frontier vendors are
running undisclosed \glsterm{process-reward-model}{PRM} or search machinery and the disclosure ceiling
...
\emph{(C1)} a \glsterm{process-reward-model}{PRM}-shaped \glsterm{grpo}{GRPO} vs verifier-only \glsterm{grpo}{GRPO} comparison at
matched post-training compute on a reasoning-trained base;
\emph{(C2)} a one-axis-at-a-time factorial separating long-\glsterm{chain-of-thought}{CoT} SFT,
\glsterm{rlvr}{RLVR} post-training, and general scale at fixed others, run at frontier
...
entry ships \glsterm{rlvr}{RLVR} plus long-\glsterm{chain-of-thought}{CoT}, \emph{none} ship a process reward
...
``\glsterm{thinking-budget}{thinking budget}'' surfaces. Contested: the contamination-vs-elicitation
attribution of the AIME-class headlines (C3, C5), the form of the
inference-time \glsterm{scaling-law}{scaling law} (the $L(\theta, N)$ functional form open
question of \cref{sec:snell-open}, partially overlapping with C4), the
faithfulness behaviour of long-\glsterm{chain-of-thought}{CoT} under scale (C2), and the
\glsterm{process-reward-model}{PRM}-vs-verifier and beam-vs-MCTS implementation choices (C1, C6).
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} restricts what
...
--- inference compute, verifier signal, RL on \glsterm{verifiable-reward}{verifiable reward} --- and
the localisation of each open question on it. Paper~4
\S faithfulness and Paper~5 \S AIME-contamination both backreference
this section's (C2) and (C5) directly; Paper~2's \glsterm{rlhf}{RLHF}-to-\glsterm{rlvr}{RLVR}
