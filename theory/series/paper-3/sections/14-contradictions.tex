...
reasoning capability. \Citet{rlvr-limits-2025} read it differently: the
...
set. The 2026 consensus, as catalogued in \texttt{kb/index/contradictions.md}~\S~reasoning,
leans toward the \citeauthor{rlvr-limits-2025} reading with caveats:
the trace-growth result of \citet[\S 2.2]{deepseek-r1} (response
length $\sim 200 \to \sim 10{,}000$ tokens) is not yet reconciled with
the capability-ceiling claim, and the cleanest synthesis in the
literature is that ``RLVR teaches \emph{trace-length use} (when to
spend more tokens) without teaching new primitives''
\citep[\S 5]{rlvr-limits-2025}. Whether that synthesis exhausts the
picture or merely captures the dominant term is open. Closure requires
...
not novel reasoning composed under RL. \Citet{deepseek-r1} do not
report a contamination audit on the AIME 2024 train/test boundary,
standard contamination-detection methodologies (n-gram overlap on
solutions, near-duplicate matching, decontamination-by-substring) all
have known false-negative modes against post-hoc-paraphrased problems
and arithmetic isomorphisms, and as of 2026-Q2 no third-party study
has reproduced an audit on R1's pretraining mix
(\cref{sec:r1-contamination}). The contradiction interacts non-trivially
with (C3): if the base model's pass@$K$ at moderate $K$ is already
high because of contamination, then the
