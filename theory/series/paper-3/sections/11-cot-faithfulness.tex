...
finds that the gap between the two can be substantial. \textbf{\glsterm{chain-of-thought}{CoT}
...
2025--2026 measurements (Arcuschin et al.\ ``\glsterm{chain-of-thought}{CoT} in the wild'', Shen et
...
cross-paper threads: Paper~4's interpretability programme uses \glsterm{chain-of-thought}{CoT} as a
\glsterm{probe}{probe} surface and Paper~5's alignment programme treats \glsterm{chain-of-thought}{CoT} as a
...
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/chain-of-thought.md#4}.} the trace $z$ is
...
\footnote{Excerpt status: \texttt{shen2025-faithcot-bench} is indexed in
\nolinkurl{kb/index/papers.json} with \texttt{excerpts\_file: null} --- no
excerpt file exists yet, so this claim rests on the primary source alone
(checked 2026-08-19); \deepencite{shen2025-faithcot-bench \S 3, citation-pending}}
\begin{enumerate}
...
measurement of unfaithful \glsterm{chain-of-thought}{CoT} to date.%
\footnote{Excerpt status: \texttt{arcuschin2025-cot-wild} is indexed in
\nolinkurl{kb/index/papers.json} with \texttt{excerpts\_file: null} --- no
excerpt file exists yet, so this claim rests on the primary source alone
(checked 2026-08-19); \deepencite{arcuschin2025-cot-wild abstract,
...
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/chain-of-thought.md#4.1}.} The
...
\nolinkurl{kb/index/papers.json} with \texttt{excerpts\_file: null} --- no
excerpt file exists yet, so this claim rests on the primary source alone
(checked 2026-08-19); \deepencite{shen2025-faithcot-bench abstract,
citation-pending}} Their FINE-\glsterm{chain-of-thought}{CoT} dataset contributes
...
\citep[\S 3]{shen2025-faithcot-bench}.
\item \textbf{\glsterm{llm-as-judge}{LLM-as-judge} detectors} present the trajectory to a
...
\glsterm{llm-as-judge}{LLM-as-judge} fails and their internal logit signal does not visibly
...
\glsterm{self-consistency}{self-consistency} at $N > 1$ — \emph{also} improve faithfulness, or are
accuracy gains and faithfulness gains decoupled?%
\footnote{Excerpt status: \texttt{mehta2026-faithfulness-scaling} is indexed
in \nolinkurl{kb/index/papers.json} with \texttt{excerpts\_file: null} --- no
excerpt file exists yet, so this claim rests on the primary source alone
(checked 2026-08-19); \deepencite{mehta2026-faithfulness-scaling abstract,
citation-pending}} The study runs four frontier models
\deepencite{model names must be verified against the primary PDF of Mehta et al.\ 2026 (arXiv 2601.06423) before being asserted; the previous draft listed GPT-5.2, Claude Opus~4.5, Gemini-3-flash-preview, and DeepSeek-v3.2, none of which appear in any 2026-Q2 public model lineup and are presumed hallucinated; replace with the four names as they appear in the paper's Table~1 or equivalent} on $100$ GSM8K problems at varying \glsterm{self-consistency}{self-consistency}
...
\citep[abstract]{mehta2026-faithfulness-scaling}. One model
...
\deepencite{mehta2026-faithfulness-scaling: the specific model name for this configuration (previously listed as ``Claude Opus~4.5'') is unverified against the primary PDF; confirm the correct model name and per-model table location (section anchor pending population), citation-pending}}
For \cref{eq:cot-decode}'s reading of \glsterm{self-consistency}{self-consistency} this is a
...
The operational implication: \textbf{\glsterm{self-consistency}{self-consistency} must be tested
...
The clean read of \citet{deepseek-r1}'s \glsterm{grpo}{GRPO} outcome — long traces
emerge from \glsterm{rlvr}{RLVR}, accuracy rises in lockstep with trace length,
...
evidence.} \citet[\S 2.2]{deepseek-r1} report that \glsterm{rlvr}{RLVR} training grows
average response length from $\sim 200$ to $\sim 10{,}000$ tokens%
\deepencite{deepseek-r1 §2.2 trace-length growth curve, KB excerpt-anchor pending; current anchor covers the rule-based-reward passage}
with AIME 2024 pass\@1 climbing $15.6\% \to 77.9\%$%
\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} a correlation
...
faithfulness rises under \glsterm{self-consistency}{self-consistency}, a pattern incompatible with
...
cleanly attributed to long-\glsterm{chain-of-thought}{CoT} SFT, \glsterm{rlvr}{RLVR} post-training, or general
...
\footnote{KB note:
\nolinkurl{kb/notes/reasoning/chain-of-thought.md#4.1}.} The reason
matters: if the gap is \glsterm{rlvr}{RLVR}-driven, it is benchmarkable and tunable
...
autoencoders, \glsterm{activation-patching}{activation patching}, and \glsterm{circuit-tracing}{circuit tracing}. \glsterm{chain-of-thought}{CoT} traces are
the \emph{output-side} \glsterm{probe}{probe} surface for that programme — they are
human-readable, token-aligned with the \glsterm{residual-stream}{residual stream}, and stable
under repeated decoding. The faithfulness question gates this use:
\textbf{if traces are unfaithful, the \glsterm{probe}{probe} surface is decoupled from
the underlying computation}. \hyperref[P4-sec:introduction]{Paper~4's} interpretability programme returns to
this section as a precondition for treating \glsterm{chain-of-thought}{CoT}-as-\glsterm{probe}{probe} as a primary
methodology, and uses Mehta et al.'s per-model variation to argue that
\glsterm{probe}{probe}-surface validity must be re-established model-by-model rather
than assumed from the family.

\paragraph{Paper~5 — CoT as an alignment monitor.}
The alignment programme of Paper~5 treats the \glsterm{chain-of-thought}{CoT} channel as a
\emph{monitorable surface}: detectors that read traces look for
warning signs of misalignment, \glsterm{scheming}{scheming}, \glsterm{sycophancy}{sycophancy}, and
...
spot exactly where it most needs to see. \hyperref[P5-sec:alignment-faking]{Paper~5's §alignment-faking} cites
forward to this section as the empirical bound on \glsterm{chain-of-thought}{CoT}-as-monitor: a
...
\glsterm{rlvr}{RLVR}-trained policies internalise step-level verification. For traces,
the question is whether the \glsterm{chain-of-thought}{CoT} channel still earns its interpretability
...
\glsterm{value}{value} at inference time (\cref{sec:prm-thinkprm,sec:prm-survival}) while
their training-time \glsterm{value}{value} shrinks, and traces continue to drive answers
