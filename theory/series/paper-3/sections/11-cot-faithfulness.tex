...
\glsterm{probe-2}{probe} surface and Paper~5's alignment programme treats \glsterm{chain-of-thought}{CoT} as a
...
\nolinkurl{kb/excerpts/shen2025-faithcot-bench.md} is abstract-only as of
...
\footnote{Excerpt status:
\nolinkurl{kb/excerpts/arcuschin2025-cot-wild.md} is abstract-only as of
...
For \cref{eq:cot-decode}'s reading of \glsterm{self-consistency-2}{self-consistency} this is a
...
The operational implication: \textbf{\glsterm{self-consistency-2}{self-consistency} must be tested
...
The clean read of \citet{deepseek-r1}'s \glsterm{grpo-2}{GRPO} outcome — long traces
emerge from \glsterm{rlvr-3}{RLVR}, accuracy rises in lockstep with trace length,
...
evidence.} \citet[\S 2.2]{deepseek-r1} report that \glsterm{rlvr-3}{RLVR} training grows
...
faithfulness rises under \glsterm{self-consistency-2}{self-consistency}, a pattern incompatible with
...
cleanly attributed to long-\glsterm{chain-of-thought}{CoT} SFT, \glsterm{rlvr-3}{RLVR} post-training, or general
...
matters: if the gap is \glsterm{rlvr-3}{RLVR}-driven, it is benchmarkable and tunable
...
the \emph{output-side} \glsterm{probe-2}{probe} surface for that programme — they are
...
\textbf{if traces are unfaithful, the \glsterm{probe-2}{probe} surface is decoupled from
the underlying computation}. \hyperref[P4-sec:introduction]{Paper~4's} interpretability programme returns to
this section as a precondition for treating \glsterm{chain-of-thought}{CoT}-as-\glsterm{probe-2}{probe} as a primary
methodology, and uses Mehta et al.'s per-model variation to argue that
\glsterm{probe-2}{probe}-surface validity must be re-established model-by-model rather
...
\glsterm{rlvr-3}{RLVR}-trained policies internalise step-level verification. For traces,
