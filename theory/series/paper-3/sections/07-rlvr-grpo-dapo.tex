...
% Cross-paper: complement to \hyperref[P2-sec:rlvr-grpo]{Paper~2 §11} (RL on verifiable rewards: GRPO and
...
\emph{reinforcement learning on verifiable rewards} (\glsterm{rlvr}{RLVR}). The previous
...
fixed policy. \glsterm{rlvr}{RLVR} is the training-time leg that \emph{produces} the
policy whose \glsterm{test-time-compute}{test-time compute} is worth amplifying. Two coupled
substitutions define the paradigm: a programmatic verifier replaces a
learned \glsterm{reward-model}{reward model}, and a group-mean baseline replaces a co-sized
\glsterm{value}{value} network. The first removes reward-hacking as a structural failure
mode; the second halves the active-parameter footprint of training and
makes long-\glsterm{chain-of-thought}{CoT} trajectories practical to backpropagate through. The
empirical proof point treated in detail in \cref{sec:r1-protocol} ---
DeepSeek-\glsterm{r1-zero}{R1-Zero}'s AIME 2024 climb from $15.6\%$ to $77.9\%$ pass@1
under one continuous \glsterm{grpo}{GRPO} run \citep[\S
2.3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} --- is the cleanest
single number in the reasoning-\glsterm{llm}{LLM} literature, and the central evidence
that the triangle composes.

This section is deliberately complementary to Paper~2's RL-stage
treatment (\hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}, which covers \glsterm{grpo}{GRPO}/\glsterm{dapo}{DAPO} as one stage of the
six-stage post-training pipeline). The angle here is different: how
\glsterm{rlvr}{RLVR} \emph{shapes} the \glsterm{test-time-compute}{test-time compute} axis. The training reward is
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} --- the verifier
contains no length term, but the policy discovers that long traces win
on math and code. \glsterm{rlvr}{RLVR} is the training-time mechanism that ratchets
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}; KB note:
\nolinkurl{kb/notes/post-training/rlvr-and-grpo.md#1-1}.} The instances
...
(the final answer is graded, intermediate steps are not, in the \glsterm{rlvr}{RLVR}
...
of \glsterm{rlhf}{RLHF} closes by construction. \citet[\S 2.2]{deepseek-r1} state the
...
susceptible to \glsterm{reward-hacking}{reward hacking} during large-scale reinforcement
learning.''\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} The
post-training-pipeline framing of why this matters is
\hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}; the reasoning-triangle framing is that
\cref{eq:verifier-reward}'s sparseness is what \emph{forces} the policy
to discover long-\glsterm{chain-of-thought}{CoT} solutions in the first place --- the only signal
\glsterm{rlvr}{RLVR} provides is whether the final answer is correct, so the policy
...
(\glsterm{grpo}{GRPO}) in DeepSeekMath as a critic-free PPO variant: per-prompt
...
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1-motivation}.} The token-level
objective \citep[\S 4.1, Eq.\ 3]{shao2024}, which DeepSeek-R1 later
restates in a sequence-level shorthand at \citet[\S 2.1, Eq.\
1]{deepseek-r1}\footnote{KB excerpts:
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1} and
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-1-grpo}.}:
\begin{equation}
\begin{aligned}
...
\end{equation}
constant in $t$ across the trajectory \citep[\S 4.1, Eq.\ 3]{shao2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1}. DeepSeek-R1 restates the
same advantage as a sequence-level shorthand in \S 2.1 Eq.\ 1
\citep[\S 2.1, Eq.\ 1]{deepseek-r1}.}

\paragraph{Symbol table.} $q$ is the prompt; $G$ the group size,
typically $8$--$64$ ($G = 16$ in \glsterm{r1-zero}{R1-Zero} \citep[\S
2.1]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-1-hp}.}); $o_i$ the $i$-th
sampled trajectory (\glsterm{chain-of-thought}{chain-of-thought} + answer); $R_i \in \{0, 1\}$ the
verifier reward of \cref{eq:verifier-reward}; $\pi_\mathrm{ref}$ a
frozen reference (typically the SFT cold-start, see \hyperref[P2-sec:sft]{Paper~2 §SFT});
$\beta$ the KL coefficient ($0.001$ for \glsterm{r1-zero}{R1-Zero}
...
load-bearing fact about \glsterm{grpo}{GRPO} at \glsterm{llm}{LLM} scale is that
...
\nolinkurl{kb/excerpts/ouyang2022-instructgpt.md#sec-3}.} requires a \glsterm{value}{value}
network co-sized with the policy, doubling the active parameter count
and the activation memory of training. \glsterm{grpo}{GRPO} removes the second network
entirely. With \glsterm{value}{value} model: $\sim\!2P$ active \glsterm{parameters}{parameters} and $\sim\!2A$
activation memory per training step (where $P$ is policy \glsterm{parameters}{parameters} and
$A$ its activation footprint), plus the \glsterm{value}{value}-network's own optimizer
state. Without (\glsterm{grpo}{GRPO}): $P + A$ for the policy, $P$ frozen for the
reference, but $G \times$ rollouts per prompt at sequence length up to
$L_\mathrm{max}$. The four co-resident model copies of \glsterm{rlhf}{RLHF}'s PPO
infrastructure (policy, reference, reward, \glsterm{value}{value}) reduce to two
...
against the \glsterm{value}{value}-network's parameter and activation footprint, and at
sparse reward (where $V_\phi$ struggles to learn anyway) it is
favorable \citep[\S 4.1]{shao2024}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/shao2024.md#sec-4-1-motivation}.} For
...
\begin{intuition}
\glsterm{grpo}{GRPO} is the median-of-rollouts as a policy gradient. Sample $G$
...
--- and \glsterm{grpo}{GRPO}'s gradient is a trajectory-level importance-weighted
update on the better-than-$p_0(q)$ rollouts. Under the
\citet{rlvr-limits-2025} reading discussed in
\cref{sec:contradictions}, this implies \glsterm{rlvr}{RLVR} cannot make the policy
...
discussed in \hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}): if the group-mean baseline is the binding
mechanism, then \glsterm{rlvr}{RLVR}'s reach is bounded by the base policy's pass@$K$
...
\emph{at some sample budget}; \glsterm{grpo}{GRPO} transferred that to pass@1.''
...
introduce four modifications to \glsterm{grpo}{GRPO} that together cut training-step
count to AIME-2024 score 50 by roughly half on Qwen2.5-32B-base
relative to the \glsterm{grpo}{GRPO} baseline \citep[\S
4]{dapo2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dapo2025.md#sec-4-headline}.} Each modification
addresses a measured failure mode of the \glsterm{grpo}{GRPO} recipe under the
specific demands of long-\glsterm{chain-of-thought}{CoT} training; together they are the canonical
2025 statement of where the \glsterm{grpo}{GRPO} objective leaks gradient signal.

\paragraph{(i) Clip-Higher (Eq.\ 10).} \glsterm{grpo}{GRPO}'s symmetric clip
$[1 - \epsilon, 1 + \epsilon]$ caps probability \emph{increases} on
under-probable exploration tokens at the same rate as it caps
\emph{decreases} on already-favored tokens. \glsterm{dapo}{DAPO} decouples the bound:
\begin{equation}
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-clip-higher}.} The asymmetry
...
the failure mode in which the entropy of the policy at long-\glsterm{chain-of-thought}{CoT}
...
all-incorrect), the \glsterm{group-normalized-advantage}{group-normalized advantage} \cref{eq:grpo-advantage}
is identically zero by construction and the sample contributes no
gradient. \glsterm{dapo}{DAPO} enforces
\begin{equation}
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-dynamic-sampling}.} Compute
...
\paragraph{(iii) Token-Level Loss (Eq.\ 12).} \glsterm{grpo}{GRPO} normalizes by
$1/|o_i|$ inside the group sum and then by $1/G$ outside, so longer
trajectories contribute proportionally less per token. \glsterm{dapo}{DAPO} replaces
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-token-loss}.} This is the
modification most directly aligned with the reasoning-triangle thesis
of this paper: \glsterm{grpo}{GRPO}'s per-trajectory normalization systematically
...
\glsterm{dapo}{DAPO}'s token-level loss removes that bias.
...
\glsterm{dapo}{DAPO} replaces the hard zero with a length-based soft penalty:
\begin{equation}
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-3-overlong}.} Variance from
...
\begin{analogy}
The four \glsterm{dapo}{DAPO} modifications can be read as the field's
$\mathcal{O}(\epsilon^2)$ correction to \glsterm{grpo}{GRPO} once the long-\glsterm{chain-of-thought}{CoT} regime
...
\glsterm{dapo}{DAPO} equation localises one of those biases.
...
\glsterm{grpo}{GRPO}'s per-trajectory normalisation, and \cref{eq:dapo-overlong} the
...
$L_\mathrm{max}$ tokens (\glsterm{r1-zero}{R1-Zero} uses $L_\mathrm{max} = 32{,}768$
...
apply \glsterm{dapo}{DAPO}'s dynamic-sampling and token-level normalization); (4) take
...
Two evidence points define the 2025 \glsterm{rlvr}{RLVR}-for-reasoning literature.
First, \citet[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} report that
DeepSeek-\glsterm{r1-zero}{R1-Zero}'s average pass@1 on AIME 2024 climbs from $15.6\%$ to
$77.9\%$ over one continuous \glsterm{grpo}{GRPO} run on the \glsterm{precision-pinning}{DeepSeek-V3} base, with
...
\cref{eq:verifier-reward} plus a small \glsterm{format-reward}{format reward}. The detailed
protocol --- cold-start SFT, the \glsterm{rejection-sampling-sft}{rejection-sampling SFT}, the final
...
\nolinkurl{kb/excerpts/dapo2025.md#sec-4-headline}.} reach AIME 2024
score $50$ on Qwen2.5-32B-base in roughly half the training steps
relative to the \glsterm{grpo}{GRPO} baseline using the four modifications of
\cref{sec:rlvr-dapo}. The training code, curated dataset, and verl
framework integration are open-sourced. As of 2026-Q2 \glsterm{dapo}{DAPO} and its
descendants are the practical default for open-weight \glsterm{rlvr}{RLVR} on long-\glsterm{chain-of-thought}{CoT}
tasks; \glsterm{grpo}{GRPO} remains the canonical reference and the simpler starting
point.

The \emph{cross-leg} reading of these numbers is the section's thesis:
the response-length growth observed during the \glsterm{r1-zero}{R1-Zero} run (a few
hundred tokens early in training to roughly $10{,}000$ tokens at
convergence \citep[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.}) is, by definition,
movement along the inference-time-compute axis of
\cref{sec:inference-compute-axis}; and the ``\glsterm{aha-moment}{aha moment}'' --- a sudden
...
inference (\cref{sec:tree-search}). The same \glsterm{rlvr}{RLVR} run thus moves the
...
to the operational tradeoff between spending marginal compute on \glsterm{rlvr}{RLVR}
...
\begin{contradiction}
\textbf{Did \glsterm{grpo}{GRPO} show that \glsterm{value}{value} models are useless, or that
\emph{na\"ive} \glsterm{value}{value} models are useless?} \glsterm{vapo}{VAPO} (\glsterm{vapo}{Value-Augmented PPO},
ByteDance Seed, April 2025, arXiv:2504.05118) re-introduces a learned
\glsterm{value}{value} function under the \glsterm{grpo}{GRPO}/\glsterm{rlvr}{RLVR} objective, but trains it more
carefully than GAE-PPO: shared backbone with the policy,
\glsterm{value}{value}-loss-clipped, advantage normalized per-batch. The paper reports
\glsterm{vapo}{VAPO} outperforming \glsterm{dapo}{DAPO} on long-\glsterm{chain-of-thought}{CoT} tasks at fewer steps, suggesting
the \glsterm{grpo}{GRPO}-vs-PPO gap is driven not by ``$V_\phi$ is the wrong tool'' but
by ``$V_\phi$ trained the way GAE-PPO trains it is the wrong tool.''
The reasoning-triangle reading would split: if \glsterm{vapo}{VAPO} replicates,
group-baselines are an implementation choice, not a structural \glsterm{feature}{feature}
of the leg; if it does not, \glsterm{grpo}{GRPO}'s critic-free design is part of why
\glsterm{rlvr}{RLVR} scales to $10^4$-token traces in the first place. Whether \glsterm{vapo}{VAPO}'s
gain over \glsterm{dapo}{DAPO} replicates outside the original recipe is open as of
...
Cross-paper to \hyperref[P2-sec:rlvr-grpo]{Paper~2 §11} (the same contradiction in
...
The \glsterm{vapo}{VAPO} contradiction has a near-neighbor on the credit-assignment
side: \glsterm{vineppo}{VinePPO} \deepencite{kazemnejad2025-vineppo --- bibkey not yet in
references.bib; arXiv:2410.01679 \S 3} keeps a \glsterm{value}{value} baseline but
...
\glsterm{grpo}{GRPO}-no-clip) drop the PPO clip entirely, arguing the KL anchor and
...
long-horizon \glsterm{rlvr}{RLVR} is single-source. The unifying observation is that
...
which family member best amortises the cost of long-\glsterm{chain-of-thought}{CoT} rollouts.

\subsection{Forward link}
\label{sec:rlvr-forward}

This section established the third leg of the reasoning triangle in
isolation: the \glsterm{verifiable-reward}{verifiable reward} of \cref{eq:verifier-reward}, the
\glsterm{grpo}{GRPO} objective of \cref{eq:grpo} with its critic-free group-mean
advantage \cref{eq:grpo-advantage}, the four \glsterm{dapo}{DAPO} modifications
\cref{eq:dapo-clip}--\cref{eq:dapo-overlong}, and the empirical proof
points: \glsterm{r1-zero}{R1-Zero}'s AIME climb on \glsterm{grpo}{GRPO} and \glsterm{dapo}{DAPO}'s halved step-count to
matched score on Qwen2.5-32B-base. The post-training-pipeline framing
of the same machinery (\hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}) treats \glsterm{rlvr}{RLVR} as the successor to
\glsterm{rlhf}{RLHF} in the six-stage training pipeline; this section's framing instead
...
\emph{composes}. R1's four-stage pipeline --- cold-start SFT, \glsterm{grpo}{GRPO} with
verifiable rewards, \glsterm{rejection-sampling-sft}{rejection-sampling SFT}, general-domain RL ---
...
fixed total compute, when does the marginal FLOP go to \glsterm{rlvr}{RLVR} training
versus inference search versus pretraining? The open answer turns on
the same empirical anecdotes (\glsterm{dapo}{DAPO}'s step efficiency \citep[\S
4]{dapo2025}, R1's training-token cost \citep[\S 2.3]{deepseek-r1},
Snell's FLOPs-matched curves \citep[\S 6]{snell2024}\footnote{KB
excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-1-flops-matched}.}) that
