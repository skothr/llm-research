...
\emph{reinforcement learning on verifiable rewards} (\glsterm{rlvr-3}{RLVR}). The previous
...
fixed policy. \glsterm{rlvr-3}{RLVR} is the training-time leg that \emph{produces} the
...
under one continuous \glsterm{grpo-2}{GRPO} run \citep[\S
...
treatment (\hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}, which covers \glsterm{grpo-2}{GRPO}/\glsterm{dapo-2}{DAPO} as one stage of the
six-stage post-training pipeline). The angle here is different: how
\glsterm{rlvr-3}{RLVR} \emph{shapes} the \glsterm{test-time-compute}{test-time compute} axis. The training reward is
...
on math and code. \glsterm{rlvr-3}{RLVR} is the training-time mechanism that ratchets
...
(the final answer is graded, intermediate steps are not, in the \glsterm{rlvr-3}{RLVR}
...
\glsterm{rlvr-3}{RLVR} provides is whether the final answer is correct, so the policy
...
(\glsterm{grpo-2}{GRPO}) in DeepSeekMath as a critic-free PPO variant: per-prompt
...
load-bearing fact about \glsterm{grpo-2}{GRPO} at \glsterm{llm}{LLM} scale is that
...
and the activation memory of training. \glsterm{grpo-2}{GRPO} removes the second network
...
state. Without (\glsterm{grpo-2}{GRPO}): $P + A$ for the policy, $P$ frozen for the
...
--- and \glsterm{grpo-2}{GRPO}'s gradient is a trajectory-level importance-weighted
update on the better-than-$p_0(q)$ rollouts. Under the
\citet{rlvr-limits-2025} reading discussed in
\cref{sec:contradictions}, this implies \glsterm{rlvr-3}{RLVR} cannot make the policy
...
mechanism, then \glsterm{rlvr-3}{RLVR}'s reach is bounded by the base policy's pass@$K$
...
\emph{at some sample budget}; \glsterm{grpo-2}{GRPO} transferred that to pass@1.''
...
introduce four modifications to \glsterm{grpo-2}{GRPO} that together cut training-step
count to AIME-2024 score 50 by roughly half on Qwen2.5-32B-base
relative to the \glsterm{grpo-2}{GRPO} baseline \citep[\S
4]{dapo2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dapo2025.md#sec-4-headline}.} Each modification
addresses a measured failure mode of the \glsterm{grpo-2}{GRPO} recipe under the
specific demands of long-\glsterm{chain-of-thought}{CoT} training; together they are the canonical
2025 statement of where the \glsterm{grpo-2}{GRPO} objective leaks gradient signal.
...
\emph{decreases} on already-favored tokens. \glsterm{dapo-2}{DAPO} decouples the bound:
\begin{equation}
...
gradient. \glsterm{dapo-2}{DAPO} enforces
\begin{equation}
...
trajectories contribute proportionally less per token. \glsterm{dapo-2}{DAPO} replaces
...
of this paper: \glsterm{grpo-2}{GRPO}'s per-trajectory normalization systematically
...
\glsterm{dapo-2}{DAPO}'s token-level loss removes that bias.
...
\glsterm{dapo-2}{DAPO} replaces the hard zero with a length-based soft penalty:
\begin{equation}
...
\begin{analogy}
The four \glsterm{dapo-2}{DAPO} modifications can be read as the field's
$\mathcal{O}(\epsilon^2)$ correction to \glsterm{grpo-2}{GRPO} once the long-\glsterm{chain-of-thought}{CoT} regime
...
\glsterm{dapo-2}{DAPO} equation localises one of those biases.
...
\glsterm{grpo-2}{GRPO}'s per-trajectory normalisation, and \cref{eq:dapo-overlong} the
...
apply \glsterm{dapo-2}{DAPO}'s dynamic-sampling and token-level normalization); (4) take
...
Two evidence points define the 2025 \glsterm{rlvr-3}{RLVR}-for-reasoning literature.
...
$77.9\%$ over one continuous \glsterm{grpo-2}{GRPO} run on the \glsterm{precision-pinning}{DeepSeek-V3} base, with
...
relative to the \glsterm{grpo-2}{GRPO} baseline using the four modifications of
\cref{sec:rlvr-dapo}. The training code, curated dataset, and verl
framework integration are open-sourced. As of 2026-Q2 \glsterm{dapo-2}{DAPO} and its
descendants are the practical default for open-weight \glsterm{rlvr-3}{RLVR} on long-\glsterm{chain-of-thought}{CoT}
tasks; \glsterm{grpo-2}{GRPO} remains the canonical reference and the simpler starting
point.
...
inference (\cref{sec:tree-search}). The same \glsterm{rlvr-3}{RLVR} run thus moves the
...
to the operational tradeoff between spending marginal compute on \glsterm{rlvr-3}{RLVR}
...
\begin{contradiction}
\textbf{Did \glsterm{grpo-2}{GRPO} show that \glsterm{value}{value} models are useless, or that
...
\glsterm{value}{value} function under the \glsterm{grpo-2}{GRPO}/\glsterm{rlvr-3}{RLVR} objective, but trains it more
...
\glsterm{vapo}{VAPO} outperforming \glsterm{dapo-2}{DAPO} on long-\glsterm{chain-of-thought}{CoT} tasks at fewer steps, suggesting
the \glsterm{grpo-2}{GRPO}-vs-PPO gap is driven not by ``$V_\phi$ is the wrong tool'' but
...
of the leg; if it does not, \glsterm{grpo-2}{GRPO}'s critic-free design is part of why
\glsterm{rlvr-3}{RLVR} scales to $10^4$-token traces in the first place. Whether \glsterm{vapo}{VAPO}'s
gain over \glsterm{dapo-2}{DAPO} replicates outside the original recipe is open as of
...
\glsterm{grpo-2}{GRPO}-no-clip) drop the PPO clip entirely, arguing the KL anchor and
...
long-horizon \glsterm{rlvr-3}{RLVR} is single-source. The unifying observation is that
...
\glsterm{grpo-2}{GRPO} objective of \cref{eq:grpo} with its critic-free group-mean
advantage \cref{eq:grpo-advantage}, the four \glsterm{dapo-2}{DAPO} modifications
\cref{eq:dapo-clip}--\cref{eq:dapo-overlong}, and the empirical proof
points: \glsterm{r1-zero}{R1-Zero}'s AIME climb on \glsterm{grpo-2}{GRPO} and \glsterm{dapo-2}{DAPO}'s halved step-count to
matched score on Qwen2.5-32B-base. The post-training-pipeline framing
of the same machinery (\hyperref[P2-sec:rlvr-grpo]{Paper~2 §11}) treats \glsterm{rlvr-3}{RLVR} as the successor to
...
\emph{composes}. R1's four-stage pipeline --- cold-start SFT, \glsterm{grpo-2}{GRPO} with
...
fixed total compute, when does the marginal FLOP go to \glsterm{rlvr-3}{RLVR} training
versus inference search versus pretraining? The open answer turns on
the same empirical anecdotes (\glsterm{dapo-2}{DAPO}'s step efficiency \citep[\S
