...
\emph{compose}. \citet{deepseek-r1} train DeepSeek-R1 and DeepSeek-\glsterm{r1-zero}{R1-Zero}
on the \glsterm{precision-pinning}{DeepSeek-V3} base \citep[\S 2]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2}.} using a four-stage
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.}) using only the
verifiable terminal reward of \cref{eq:verifier-reward}, with no \glsterm{process-reward-model}{PRM}
in the headline RL loop \citep[\S
2.2]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} The headline
number --- AIME 2024 average pass@1 climbs from $15.6\%$ to $77.9\%$,
reaching $86.7\%$ with \glsterm{self-consistency-2}{self-consistency} over $64$ samples
\citep[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} --- is the
...
\glsterm{r1-zero}{R1-Zero} (no cold start) to isolate what the SFT initialisation does
and does not contribute, transcribes the long-\glsterm{chain-of-thought}{CoT} compounding curve
...
\nolinkurl{kb/notes/reasoning/reasoning-training.md#2}.} The pipeline is
...
($\sim\!10^4$ examples) of curated long-\glsterm{chain-of-thought}{CoT} exemplars is used for SFT
on the \glsterm{precision-pinning}{DeepSeek-V3} base \citep[\S
3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.} The cold-start
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2}.} that the \glsterm{format-reward}{format reward} of
\cref{sec:rlvr-verifier-reward} subsequently maintains, and seeds the
language-consistency property that \glsterm{r1-zero}{R1-Zero}'s purely-RL-trained traces
lack (\cref{sec:r1-zero-vs-r1}).

\paragraph{Stage 2 --- reasoning-oriented RL (GRPO with verifiable
rewards).} The cold-started policy is RL-trained with the \glsterm{grpo-2}{GRPO}
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-2-reward}.} The verifier is
a sum of an \emph{accuracy reward} (binary, from sympy-style
exact-match for math, unit-test execution for code, regex for
multiple-choice) and a small constant \emph{\glsterm{format-reward}{format reward}} for the
\texttt{<think>...</think>} envelope
\citep[\S 2.2]{deepseek-r1}.\footnote{KB note:
\nolinkurl{kb/notes/post-training/rlvr-and-grpo.md#2-3}.} R1's protocol
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-1-hp}.}: learning rate
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} \citet[\S
2.3]{deepseek-r1} additionally describe an
``\glsterm{aha-moment}{aha moment}'' --- a sudden spike in the policy's use of the
\texttt{wait} token during reflection passages.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aha}.} The verifier is
...
\paragraph{Stage 3 --- rejection-sampling SFT.} The Stage-2 \glsterm{rlvr-3}{RLVR}
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.} The Stage-2
...
3]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.} The released
DeepSeek-R1 weights are the output of this stage. Only Stage 2 of the
four is purely \glsterm{rlvr-3}{RLVR}; Stage 4 is an \glsterm{rlvr-3}{RLVR}/\glsterm{rlhf}{RLHF} hybrid whose preference
component is the same machinery as the ``RL stage'' that bookends
post-training in \hyperref[P2-sec:rlhf]{Paper~2 §RLHF} / \hyperref[P2-sec:rlvr-grpo]{§RLVR}. The two SFT stages (Stage 1 and
Stage 3) bookend the reasoning RL phase: cold-start SFT seeds it,
\glsterm{rejection-sampling-sft}{rejection-sampling SFT} consolidates its outputs.

\subsection{R1-Zero versus R1: what does cold-start add?}
\label{sec:r1-zero-vs-r1}

DeepSeek-\glsterm{r1-zero}{R1-Zero} is the same Stage-2 \glsterm{rlvr-3}{RLVR} run applied to the
\glsterm{precision-pinning}{DeepSeek-V3} base directly, without Stage 1's cold-start SFT
\citep[\S 2]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2}.} The structural
comparison cleanly isolates what the cold-start contributes. \glsterm{r1-zero}{R1-Zero}
reaches \emph{the same reasoning capability headline} ---
$15.6\%\!\to\!77.9\%$ AIME 2024 pass@1, $86.7\%$ with
\glsterm{self-consistency-2}{self-consistency} \citep[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime}.} --- but its
traces are unreadable: ``DeepSeek-\glsterm{r1-zero}{R1-Zero} struggles with challenges
like poor readability, and language mixing, as \glsterm{precision-pinning}{DeepSeek-V3}-Base is
trained on multiple languages, especially English and Chinese''
\citep[\S 3]{deepseek-r1}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-3-pipeline}.} The traces
...
\nolinkurl{kb/notes/reasoning/reasoning-training.md#2}.}: \emph{format}
(the \texttt{<think>...</think><answer>...</answer>} envelope is
seeded so the \glsterm{format-reward}{format reward} of \cref{sec:rlvr-verifier-reward} acts
...
\begin{intuition}
\glsterm{r1-zero}{R1-Zero} is the pure-RL reasoning experiment; R1 is the same
...
signal arrived. Returning to math: \glsterm{grpo-2}{GRPO}'s group-baseline
...
The \glsterm{r1-zero}{R1-Zero} / R1 split mirrors a long-standing pattern in RL: the
...
\nolinkurl{kb/notes/reasoning/reasoning-training.md#5} draws the
...
\emph{long-\glsterm{chain-of-thought}{CoT} compounding effect}: response length and verifier
accuracy grow together over the RL run, in a relationship that is
qualitatively monotone-concave in
$\log\!L$.\footnote{KB note:
\nolinkurl{kb/notes/reasoning/reasoning-training.md#2-stage-2}; KB
excerpt: \nolinkurl{kb/excerpts/deepseek-r1.md#sec-2-3-aime} and
...
reasoning-trained model under explicit \glsterm{budget-forcing}{budget forcing}
(\cref{sec:budget-forcing}), and by the Snell-anchor empirics
\citep[\S 5--6]{snell2024}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/snell2024.md#sec-5-2-search}.} on
test-time-strategy curves at fixed base models. As of 2026-Q2 no
parametric \glsterm{scaling-law}{scaling law} for $p(L)$ with the structural status of
...
training objective with its own loss term --- the \glsterm{grpo-2}{GRPO} objective
\cref{eq:grpo} contains no length term --- but a fixed-point of the
RL dynamics under sparse \glsterm{verifiable-reward}{verifiable reward}
\citep[\S 2.2.2]{deepseek-r1}.\footnote{KB note:
\nolinkurl{kb/notes/reasoning/reasoning-training.md#5}.} The shape of
the curve at inference time, holding the policy fixed and varying
$L$ via \glsterm{budget-forcing}{budget forcing}, is the subject of \cref{sec:budget-forcing};
...
\glsterm{grpo-2}{GRPO} gradient prefers the longer-trace mode of the rollout
...
\glsterm{rlvr-3}{RLVR}'s reach being bounded by the base policy's pass@$K$ at any $K$
...
costs sub-quadratically per token --- some combination of \glsterm{multi-head-latent-attention}{MLA}, \glsterm{gqa}{GQA},
or \glsterm{sliding-window-attention}{sliding-window attention} --- to make the per-step decoder
...
\nolinkurl{kb/excerpts/deepseek-r1.md#sec-4}.} include a separate track
...
SFT dataset}. The small-model trainer does not need to run \glsterm{rlvr-3}{RLVR}; the
trainer at the teacher scale does, but only once.

\begin{intuition}
At small scales the bottleneck on long-\glsterm{chain-of-thought}{CoT} capability is data ---
trace exemplars at the right depth and difficulty --- not algorithm.
Once a teacher emits a sufficient corpus of correct long-\glsterm{chain-of-thought}{CoT} traces
verified against ground truth, vanilla SFT on those traces transfers
the capability efficiently. \glsterm{rlvr-3}{RLVR}'s role in the lineage is to
...
\textbf{Tülu~3} \citep{tulu3} --- a reproducible \glsterm{rlvr-3}{RLVR}-on-Llama
...
project that reproduced \glsterm{r1-zero}{R1-Zero}-style runs on smaller bases with
...
SFT, then \glsterm{rlvr-3}{RLVR} on verifiable rewards, then either rejection-sampling
...
\glsterm{precision-pinning}{DeepSeek-V3}'s pretraining corpus?} The $15.6\%\!\to\!77.9\%$ AIME 2024
pass@1 jump \citep[\S 2.3]{deepseek-r1} attributes the gain to
\glsterm{rlvr-3}{RLVR}-elicited reasoning. The competing reading is that AIME-style
...
recall of memorised solutions surfaced through long-\glsterm{chain-of-thought}{CoT}
self-prompting, not novel reasoning composed under RL. Reported
contamination-detection methodologies (\glsterm{n-gram-overlap}{n-gram overlap} on solutions,
near-duplicate matching, \glsterm{decontamination}{decontamination}-by-substring) all have
...
contamination methodology is \hyperref[P5-sec:contamination]{Paper~5 §contamination}, which catalogues
the methodologies that disagree on what counts as contamination, and
\cref{sec:contradictions} returns to this specific
...
\citeauthor{rlvr-limits-2025} reading --- \glsterm{rlvr-3}{RLVR} sharpens pass@1 but
...
\glsterm{precision-pinning}{DeepSeek-V3} base from non-reasoning capability to AIME-2024 pass@1
of $77.9\%$ (and $86.7\%$ with \glsterm{self-consistency-2}{self-consistency}) using \glsterm{grpo-2}{GRPO}
\cref{eq:grpo} on the verifier reward \cref{eq:verifier-reward}, with
no \glsterm{process-reward-model}{PRM} in the headline RL loop and no neural \glsterm{reward-model}{reward model} anywhere
in the pipeline \citep[\S 2.2]{deepseek-r1}. The \glsterm{r1-zero}{R1-Zero} comparison
isolates cold-start SFT's role to legibility and format rather than
capability; the long-\glsterm{chain-of-thought}{CoT} compounding curve \cref{eq:r1-log-returns}
...
inference-time complement to R1's training-time long-\glsterm{chain-of-thought}{CoT} discovery,
...
for any downstream use of \glsterm{chain-of-thought}{CoT} as a \glsterm{probe-2}{probe} surface (\hyperref[P4-sec:introduction]{Paper~4}
in toto) or a monitorable channel (\hyperref[P5-sec:alignment-faking]{Paper~5 §alignment-faking}).
...
reading; the distinguishing empirics live in the post-R1 \glsterm{process-reward-model}{PRM}
