...
reaching $86.7\%$ with \glsterm{self-consistency}{self-consistency} over $64$ samples
\citep[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
...
rewards).} The cold-started policy is RL-trained with the \glsterm{grpo}{GRPO}
...
\paragraph{Stage 3 --- rejection-sampling SFT.} The Stage-2 \glsterm{rlvr}{RLVR}
...
four is purely \glsterm{rlvr}{RLVR}; Stage 4 is an \glsterm{rlvr}{RLVR}/\glsterm{rlhf}{RLHF} hybrid whose preference
...
DeepSeek-\glsterm{r1-zero}{R1-Zero} is the same Stage-2 \glsterm{rlvr}{RLVR} run applied to the
...
\glsterm{self-consistency}{self-consistency} \citep[\S 2.3]{deepseek-r1}\footnote{KB excerpt:
...
signal arrived. Returning to math: \glsterm{grpo}{GRPO}'s group-baseline
...
training objective with its own loss term --- the \glsterm{grpo}{GRPO} objective
...
\glsterm{grpo}{GRPO} gradient prefers the longer-trace mode of the rollout
...
\glsterm{rlvr}{RLVR}'s reach being bounded by the base policy's pass@$K$ at any $K$
...
costs sub-quadratically per token --- some combination of \glsterm{mla}{MLA}, \glsterm{gqa}{GQA},
...
SFT dataset}. The small-model trainer does not need to run \glsterm{rlvr}{RLVR}; the
...
the capability efficiently. \glsterm{rlvr}{RLVR}'s role in the lineage is to
...
\textbf{Tülu~3} \citep{tulu3} --- a reproducible \glsterm{rlvr}{RLVR}-on-Llama
...
SFT, then \glsterm{rlvr}{RLVR} on verifiable rewards, then either rejection-sampling
...
\glsterm{rlvr}{RLVR}-elicited reasoning. The competing reading is that AIME-style
...
\citeauthor{rlvr-limits-2025} reading --- \glsterm{rlvr}{RLVR} sharpens pass@1 but
...
of $77.9\%$ (and $86.7\%$ with \glsterm{self-consistency}{self-consistency}) using \glsterm{grpo}{GRPO}
...
for any downstream use of \glsterm{chain-of-thought}{CoT} as a \glsterm{probe}{probe} surface (\hyperref[P4-sec:introduction]{Paper~4}
