...
\label{sec:inference-compute-axis}
\label{sec:test-time-compute}

\Cref{sec:cot-phenomenon,sec:self-consistency} treat \glsterm{chain-of-thought}{chain-of-thought} and
\glsterm{self-consistency-2}{self-consistency} as decoder-side tactics: the model is fixed, the prompt
...
\citet{snell2024} reframe the entire family. \glsterm{test-time-compute}{Test-time compute}, in
...
\nolinkurl{kb/excerpts/snell2024.md#sec-1-flops-matched}.} --- promotes
...
Fix a base \glsterm{llm}{LLM} and a prompt $q$. Let $y^\ast(q)$ denote the
...
the \glsterm{llm}{LLM} on $q$ under test-time hyperparameters $\theta$ with a compute
budget of $N$ inference FLOPs (or, equivalently, generated tokens).
\citet[\S 3.1, Eq.~1]{snell2024} define the
\textbf{\glsterm{test-time-compute-optimal-scaling-strategy}{test-time compute-optimal scaling strategy}}%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-3-1}. Notation note: \citet{snell2024} write the subscript with mixed labels $a^\ast$ in the index and $y^\ast$ in the indicator; we unify to $y^\ast(q)$ throughout for readability without altering the equation's content.} as
\begin{equation}
...
$N$                                     & \glsterm{test-time-compute}{test-time compute} budget (FLOPs or token count) \\
...
\linebreak[0]\glsterm{chain-of-thought}{chain-of-thought} (\cref{sec:cot-phenomenon}), \glsterm{self-consistency-2}{self-consistency}
(\cref{sec:self-consistency}), best-of-$N$ with verifier
(\cref{sec:process-reward-models}), beam search and MCTS
(\cref{sec:tree-search}), revision-finetuned models, \glsterm{rlvr-3}{RLVR}-trained
long-\glsterm{chain-of-thought}{CoT} --- \citet[\S 2]{snell2024}%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-2-proposer-verifier}.}
...
with additional tokens the \glsterm{llm}{LLM} conditions on to obtain a different
output distribution. Concretely: prepend a \glsterm{chain-of-thought}{chain-of-thought} instruction
...
scorer. Concretely: best-of-$N$ with an outcome \glsterm{reward-model}{reward model}, best-of-$N$
with a process \glsterm{reward-model}{reward model}
(\cref{sec:process-reward-models}), beam search or MCTS guided by a
\glsterm{process-reward-model}{PRM} \citep[\S 5.2]{snell2024}.%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-5-2-search}.}
This reshapes $\mathrm{Target}(\theta, N, q)$ from the outside, by
post-hoc reweighting.
\end{enumerate}

The two levers compose. \glsterm{self-consistency-2}{Self-consistency} is the proposer-only special
case at unit verifier weight (\cref{sec:self-consistency}); best-of-$N$
with \glsterm{outcome-reward-model}{ORM} is the verifier-only special case on a fixed base distribution;
\glsterm{rlvr-3}{RLVR}-trained long-\glsterm{chain-of-thought}{CoT} plus \glsterm{process-reward-model}{PRM}-weighted \glsterm{self-consistency-2}{self-consistency}
...
component (the accept-rule-analog --- \glsterm{outcome-reward-model}{ORM}, \glsterm{process-reward-model}{PRM}, search-against-\glsterm{process-reward-model}{PRM}).
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-3-2-difficulty}.}
...
The operationalisation: take the base \glsterm{llm}{LLM}, sample $2048$ completions on
...
(best-of-$N$-weighted, beam-search, or MCTS against a \glsterm{process-reward-model}{PRM}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#abstract}.}
...
$4\times$.'' The number is specific to the MATH benchmark, the \glsterm{palm}{PaLM}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/snell2024.md#sec-1-flops-matched}.}
report:
\begin{quote}
...
workload), additional \glsterm{test-time-compute}{test-time compute} is often preferable to scaling
...
demonstrating that current approaches to scaling \glsterm{test-time-compute}{test-time compute} may
...
prompts in deployment), \glsterm{test-time-compute}{test-time compute} substitutes one-for-one with
...
section transcribes \citet[\S 3, Eq.~2]{hoffmann2022-chinchilla}'s
parametric loss
$L(N, D) = E + A/N^\alpha + B/D^\beta$ and derives the compute-optimal
allocation between \glsterm{parameters}{parameters} $N$ and tokens $D$ at fixed training
...
\emph{joint} closed-form $L(N, D, \theta, C_\text{test})$ \glsterm{scaling-law}{scaling law}
...
\midrule
Best-of-$N$ with \glsterm{outcome-reward-model}{ORM}            & $(K, T)$                 & $K \cdot L_\text{ans}$ \\
Best-of-$N$-weighted with \glsterm{process-reward-model}{PRM}   & $(K, T, \text{agg})$     & $K \cdot L_\text{ans} \cdot (1 + c_\text{verify})$ \\
Beam search against \glsterm{process-reward-model}{PRM}         & $(K, M, T)$              & $K \cdot D \cdot c_\text{step}$ \\
MCTS against \glsterm{process-reward-model}{PRM}                & $(N_\text{rollout}, c_\text{puct}, D)$ & $N_\text{rollout} \cdot D \cdot L_\text{step}$ \\
Revision chain                  & $(K_\text{revisions}, T)$ & $K_\text{revisions} \cdot L_\text{ans}$ \\
\glsterm{self-consistency-2}{Self-consistency}                & $(N, T)$                 & $N \cdot L_\text{ans}$ \\
\bottomrule
\end{tabular}
\end{center}
...
extend the menu to MCTS and to RL-trained long-\glsterm{chain-of-thought}{CoT} proposers.
...
\citep[\S 3, Eq.~2]{hoffmann2022-chinchilla}. The shape of the
...
short of fitting a parametric \glsterm{scaling-law}{scaling law}. \citet{s1-2025} report
...
$\sim 10\,000$ tokens during \glsterm{grpo-2}{GRPO} training (\cref{sec:r1-family}), but
...
analysis on the MATH benchmark using \glsterm{palm}{PaLM}~2-S\* (Codey) as the base
...
\cref{sec:snell-proposer-verifier}: \citet{lightman2023-prm800k}'s \glsterm{process-reward-model}{PRM}
loss, the three aggregation rules, and the post-2023 \glsterm{process-reward-model}{PRM} lineage. The
...
\citet[\S 5.2]{snell2024}'s ``search against a \glsterm{process-reward-model}{PRM}'' under the
$\theta = (K, M, c_\text{puct}, D)$ rows of
\cref{sec:snell-tensor-shapes}.
\item \Cref{sec:rlvr,sec:r1-family} treat the proposer leg's
training-time form: RL-trained long-\glsterm{chain-of-thought}{CoT} models (DeepSeek-R1, o1, the
...
compute, when do the marginal FLOPs go to inference search, \glsterm{rlvr-3}{RLVR}
training, or pretraining? Snell's curves give one slice
(inference vs.\ pretraining); the \glsterm{rlvr-3}{RLVR} literature gives partial
...
numbers (Chinchilla: \glsterm{parameters}{parameters}-vs-tokens at fixed
