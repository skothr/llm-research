...
2026 lineup --- \glsterm{mla}{MLA}-compressed-KV (\glsterm{precision-pinning}{DeepSeek-V3}), fine-grained MoE,
...
mid-layers flips TRUE/FALSE behavior, validating the \glsterm{truth-direction}{truth direction}'s
...
\textbf{Resolving experiment.} Pair every alignment-claim \glsterm{probe}{probe} with
...
distribution shift. A \glsterm{probe}{probe} whose ablation effect halves on a
...
``the \glsterm{truth-direction}{truth direction}'' is a useful one-dimensional projection of a
...
each layer. \glsterm{probe}{Probe} correlation-vs-causation is, by 2026 consensus, a
...
\glsterm{truth-direction}{truth direction} in LLaMA-2-13B late layers, abstract features on
...
intervention'' --- depend on a \glsterm{probe}{probe}-and-\glsterm{feature}{feature} substrate whose
