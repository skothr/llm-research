...
A high-accuracy \glsterm{probe}{probe} at layer $\ell$ tells us that property $y$ is
...
ablation} along the \glsterm{probe}{probe} direction $\hat{\mathbf{d}}_y$, which tests
...
a \glsterm{probe}{probe} direction into an inference-time control surface. This section
...
worked example, and shows how the resulting ``patch-then-\glsterm{probe}{probe}''
...
problem: a sufficiently expressive \glsterm{probe}{probe} can achieve high accuracy
even when the underlying model does not represent the property in any
useful way, because the \glsterm{probe}{probe} itself learns the property from
...
held fixed across the dataset; the \glsterm{probe}{probe} is then asked to learn this
...
\end{equation}
\citep{hewitt2019-control-tasks}. A \glsterm{probe}{probe} that scores high on both
real and control tasks has low selectivity: its accuracy is a property
of the \glsterm{probe}{probe}'s expressivity, not of the model's representation. A
\glsterm{probe}{probe} that scores high on the real task but low on the control task
...
property of interest. Conversely, a high-selectivity \glsterm{probe}{probe} is not yet
proof of \emph{causal} use, only of \emph{informational} encoding;
selectivity rules out \emph{\glsterm{probe}{probe}-expressivity} confounds, but two
...
field\footnote{KB excerpt: \nolinkurl{kb/excerpts/belinkov2022-probing-survey.md#abstract}.}.
The first is \citet{hewitt2019-control-tasks}'s
\textbf{\glsterm{probe}{probe}-expressivity} confound, addressed by Eq.~\eqref{eq:selectivity}.
The second is the \textbf{spurious-correlate} confound: even a
selective \glsterm{probe}{probe} may be reading a \glsterm{feature}{feature} $z$ that correlates with
$y$ in the probing dataset, not $y$ itself. The \glsterm{probe}{probe} direction
...
\emph{cross-distribution generalization}: train the \glsterm{probe}{probe} on dataset
...
the resulting direction; the \glsterm{probe}{probe} reads the trace, not the driver.
This is the most insidious confound because it is invisible to both
selectivity and cross-distribution generalization --- a \glsterm{probe}{probe} of a
...
\begin{contradiction}
\textbf{Decodable does not entail used.} A \glsterm{probe}{probe} with selectivity
...
representation; the \glsterm{probe}{probe} direction may correlate near-perfectly with
...
\glsterm{probe}{probe}-only validation is precisely what motivated the 2023+ pivot to
...
\glsterm{residual-stream}{residual stream} along a unit-norm \glsterm{probe}{probe} direction
...
activations at the period-token of layer 12. \glsterm{probe}{Probe} accuracy on the
...
\end{quote}
The TRUE/FALSE flip is the load-bearing observation. The \glsterm{probe}{probe} alone
...
test\footnote{KB excerpt: \nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-1-contributions}.}.

The strongest form of the conclusion --- ``$\mathbf{d}_{\mathrm{MM}}$
is the \glsterm{truth-direction}{truth direction}'' --- is, however, qualified by the misalignment
...
The Marks--Tegmark causal pipeline is more than a \glsterm{probe}{probe} with an
...
\glsterm{probe}{probe}. The first stage is \glsterm{activation-patching}{activation patching}: sweep the
...
the \glsterm{probe}{probe}'s domain is restricted to states that patching has already
...
the one being read aloud. A \glsterm{probe}{probe} alone tells us a book about $y$ is
...
\glsterm{probe}{probe}'s $\hat{\mathbf{d}}_y$ is the shelf (a one-dimensional
...
The patch-then-\glsterm{probe}{probe} pipeline is now the de facto causal-probing
...
the \glsterm{probe}{probe} domain, since the localized hidden states are the
...
the framing of this section, a probing question: a behavioral \glsterm{probe}{probe}
...
\S\ref{sec:ablate-add}. The \glsterm{probe}{probe}-on-activations and the
\glsterm{probe}{probe}-on-\glsterm{chain-of-thought}{CoT}-text are different observation modalities, but the
methodological pattern is the same one this section formalizes, and
the same set of confounders (\glsterm{probe}{probe} expressivity, spurious correlate,
...
$y$-deceptive'' depends on a \glsterm{probe}{probe} that has passed both the
...
\S\ref{sec:cross-method} returns to the patch-then-\glsterm{probe}{probe} pipeline as
