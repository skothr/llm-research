...
A high-accuracy probe at layer $\ell$ tells us that property $y$ is
...
ablation} along the probe direction $\hat{\mathbf{d}}_y$, which tests
whether removing $\mathbf{d}_y$ from the residual stream changes
...
a probe direction into an inference-time control surface. This section
...
worked example, and shows how the resulting ``patch-then-probe''
...
problem: a sufficiently expressive probe can achieve high accuracy
even when the underlying model does not represent the property in any
useful way, because the probe itself learns the property from
...
held fixed across the dataset; the probe is then asked to learn this
...
\end{equation}
\citep{hewitt2019-control-tasks}. A probe that scores high on both
real and control tasks has low selectivity: its accuracy is a property
of the probe's expressivity, not of the model's representation. A
probe that scores high on the real task but low on the control task
...
probes.'' Even a linear probe can fail selectivity on a poorly
...
property of interest. Conversely, a high-selectivity probe is not yet
proof of \emph{causal} use, only of \emph{informational} encoding;
selectivity rules out \emph{probe-expressivity} confounds, but two
...
methodological vocabulary of the
...
\textbf{probe-expressivity} confound, addressed by Eq.~\eqref{eq:selectivity}.
The second is the \textbf{spurious-correlate} confound: even a
selective probe may be reading a feature $z$ that correlates with
$y$ in the probing dataset, not $y$ itself. The probe direction
...
\emph{cross-distribution generalization}: train the probe on dataset
...
the resulting direction; the probe reads the trace, not the driver.
This is the most insidious confound because it is invisible to both
selectivity and cross-distribution generalization --- a probe of a
truly vestigial feature can be high-selectivity \emph{and} robust
across distributions, because the feature really is encoded by the
...
\begin{contradiction}
\textbf{Decodable does not entail used.} A probe with selectivity
...
representation; the probe direction may correlate near-perfectly with
...
probe-only validation is precisely what motivated the 2023+ pivot to
...
residual stream along a unit-norm probe direction
...
into the residual stream at one or more layers and re-running the rest
...
is $\mathbf{h} \in \mathsf{B}\!\times\!\mathsf{S}\!\times\!D$, the
...
copy of $\hat{\mathbf{d}}_y$ to the residual stream:
\begin{equation}
  \boxed{\;
  \mathbf{h} \;\mapsto\;
...
intervention on the residual stream's representational subspace) and
...
citation-pending}, who report that sycophancy in current chat models
factorizes into three independently steerable directions (agreement,
praise, genuine-agreement), each detected by mass-mean probing and
...
rank-1 subspace is zeroed out at the residual stream.
\end{intuition}
...
activations at the period-token of layer 12. Probe accuracy on the
...
\end{quote}
The TRUE/FALSE flip is the load-bearing observation. The probe alone
...
is the truth direction'' --- is, however, qualified by the misalignment
...
The Marks--Tegmark causal pipeline is more than a probe with an
...
probe. The first stage is activation patching: sweep the
...
the probe's domain is restricted to states that patching has already
...
the one being read aloud. A probe alone tells us a book about $y$ is
...
probe's $\hat{\mathbf{d}}_y$ is the shelf (a one-dimensional
...
The patch-then-probe pipeline is now the de facto causal-probing
...
the probe domain, since the localized hidden states are the
...
The chain-of-thought-faithfulness question of Paper~3 \S 11 is, in
the framing of this section, a probing question: a behavioral probe
...
\S\ref{sec:ablate-add}. The probe-on-activations and the
probe-on-CoT-text are different observation modalities, but the
methodological pattern is the same one this section formalizes, and
the same set of confounders (probe expressivity, spurious correlate,
...
methodology for deception, refusal-circumvention, sycophancy, and
scheming; every claim there of the form ``the model is being
$y$-deceptive'' depends on a probe that has passed both the
...
\S\ref{sec:cross-method} returns to the patch-then-probe pipeline as
