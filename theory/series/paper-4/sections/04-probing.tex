...
A \emph{\glsterm{probe}{probe}} is the supervised correlational readout: a (typically
...
model predict at layer $\ell$}, a \glsterm{probe}{probe} trains a fresh classifier on
...
sharper than the question: if a high-accuracy \glsterm{probe}{probe} exists for property
...
methodology, the canonical four \glsterm{probe}{probe} families, and the layer-position
story that lets a \glsterm{probe}{probe}-accuracy curve diagnose \emph{where} a property
is encoded.
...
multiclass property $y \in \{1, \dots, K\}$ the \glsterm{probe}{probe} is a \glsterm{softmax}{softmax}
classifier with $\mathbf{W} \in \mathbb{R}^{K \times D}$. The \glsterm{probe}{probe} is
...
$\lambda$. \glsterm{probe}{Probe} accuracy on a held-out test split is the headline
...
that the \glsterm{probe}{probe}'s expressivity does not masquerade as the model's
representation. Tensor-shape note: a batched cache of activations at
layer $\ell$ is $\BSH$; the \glsterm{probe}{probe}
...
(verbatim).}: a class-label \glsterm{probe}{probe} at layer $\ell$ does at least as well
as the same \glsterm{probe}{probe} at any earlier layer. The result anchored the
...
\glsterm{probe}{probe} is a single $k \times D$ matrix per (model, layer). Hewitt and
...
The 2023+ truth-direction literature found that a \glsterm{probe}{probe} with no learned
...
\emph{mass-mean} (or \emph{difference-in-means}) \glsterm{probe}{probe} direction is
...
A fourth \glsterm{probe}{probe} family drops the label requirement entirely. Contrast-Consistent
Search (CCS) \deepencite{burns2022-ccs §3, citation-pending} fits a
\glsterm{linear-probe}{linear probe} by minimizing a \glsterm{self-consistency}{self-consistency} loss across paired
contrast prompts (e.g., ``X is true.'' / ``X is false.''), enforcing
that the \glsterm{probe}{probe}'s outputs on the two completions of a contrast pair
...
\begin{intuition}
A \glsterm{probe}{probe} is a \emph{point-spread function} on the model's representation
...
must be \emph{in focus along that filter axis} for the \glsterm{probe}{probe} to detect
...
canonical form: in every case the \glsterm{probe}{probe} is a linear map
$\mathbb{R}^D \to \mathbb{R}^k$ (with $k = 1$ for binary, $k > 1$ for
structural) followed by a fixed decision rule, and \glsterm{probe}{probe} accuracy
...
\glsterm{probe}{Probe} accuracy is a function of $(\ell, t)$, and the shape of that
...
role, and other surface-syntactic properties \glsterm{probe}{probe} highest in
\emph{early} layers; the \glsterm{probe}{probe} accuracy curve rises steeply through the
...
worked example: the \glsterm{truth-direction}{truth direction} localizes around layers 12--15 and
...
text, the most-informative \glsterm{probe}{probe} position is often the
...
\toprule
\textbf{Family} & \textbf{Year} & \textbf{\glsterm{probe}{Probe} form} & \textbf{\glsterm{key}{Key} limitation} \\
\midrule
...
MLP \glsterm{probe}{probe}
  & 2010s
  & small feedforward net on $\mathbf{h}$
  & \glsterm{probe}{probe} leakage: a powerful \glsterm{probe}{probe} learns $y$ from low-information features \citep{hewitt2019-control-tasks} \\
...
Control-task baseline \citep{hewitt2019-control-tasks}
  & 2019
  & same \glsterm{probe}{probe} on randomly-permuted labels
...
Contrastive / CCS unsupervised
  & 2022
  & \glsterm{linear-probe}{linear probe}, \glsterm{self-consistency}{self-consistency} loss, no labels
...
that \glsterm{probe}{probe} accuracy alone is insufficient, since a sufficiently
expressive \glsterm{probe}{probe} can learn $y$ from features the model itself does not
...
consolidates the critique into three confounders — \glsterm{probe}{probe} expressivity,
...
confound (CCS), and the activation-steering family that turns a \glsterm{probe}{probe}
...
Synthesizing across the four \glsterm{probe}{probe} families, the practical recipe for
...
\begin{enumerate}
  ...
  \item \textbf{Cache} activations $\{\mathbf{h}^{(\ell, t)}_i\}_i$ via
    ...
    $(\ell, t)$ slice.
  \item \textbf{Fit} a \glsterm{probe}{probe} — Eq.~\eqref{eq:linear-probe} via logistic
  ...
  \item \textbf{Generalization tests} (optional but increasingly
    ...
    the \glsterm{probe}{probe} was reading a surface confounder rather than the abstract
  ...
  \item \textbf{Causal validation} (optional, but the \emph{closing}
    step for any claim that the model \emph{uses} the \glsterm{probe}{probe} direction).
...
from), and the \glsterm{probe}{probe}-accuracy curve is a snapshot of the model's
...
this section: \glsterm{probe}{probe}-accuracy results read off a checkpoint at
post-pre-training time inherit whatever pre-training data, mixture, and
optimizer choices produced that checkpoint, and a \glsterm{probe}{probe}-accuracy
...
high-accuracy \glsterm{probe}{probe} is correlational, not causal — is the
...
activation-steering family that turns a \glsterm{probe}{probe} direction into an
