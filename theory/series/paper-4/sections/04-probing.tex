...
A \emph{\glsterm{probe-2}{probe}} is the supervised correlational readout: a (typically
linear) classifier trained on \emph{frozen} activations of a pre-trained
\glsterm{transformer}{transformer} to predict an external property — truth, sentiment, syntactic
role, factuality, \glsterm{sycophancy}{sycophancy}. Where the lens of \S\ref{sec:logit-lens-family}
reuses the model's own unembedding $W_U$ to ask \emph{what would the
model predict at layer $\ell$}, a \glsterm{probe-2}{probe} trains a fresh classifier on
...
sharper than the question: if a high-accuracy \glsterm{probe-2}{probe} exists for property
...
methodology, the canonical four \glsterm{probe-2}{probe} families, and the layer-position
story that lets a \glsterm{probe-2}{probe}-accuracy curve diagnose \emph{where} a property
is encoded.

\subsection{Setup: the linear probe}
\label{sec:probe-setup}

Let $\mathcal{M}$ be a frozen pre-trained \glsterm{transformer}{transformer} with $L$ layers and
...
\emph{\glsterm{linear-probe}{linear probe}} for a binary property $y$ is a classifier
\begin{equation}
...
\nolinkurl{kb/excerpts/alain-bengio-2017.md#abstract}.}. For a
multiclass property $y \in \{1, \dots, K\}$ the \glsterm{probe-2}{probe} is a \glsterm{softmax}{softmax}
classifier with $\mathbf{W} \in \mathbb{R}^{K \times D}$. The \glsterm{probe-2}{probe} is
fit on a labeled dataset
$\mathcal{D} = \{(\mathbf{x}_i, y_i)\}$ by minimizing a regularized
\glsterm{cross-entropy-loss}{cross-entropy loss}
\begin{equation}
  ...
  \label{eq:probe-loss}
\end{equation}
with $\mathcal{M}$'s \glsterm{parameters}{parameters} held fixed throughout. The layer $\ell$
and the token position $t$ are hyperparameters on equal footing with
$\lambda$. \glsterm{probe-2}{Probe} accuracy on a held-out test split is the headline
...
that the \glsterm{probe-2}{probe}'s expressivity does not masquerade as the model's
representation. Tensor-shape note: a batched cache of activations at
layer $\ell$ is $\BSH$; the \glsterm{probe-2}{probe}
...
model''\footnote{KB excerpt: \nolinkurl{kb/excerpts/alain-bengio-2017.md#abstract}
(verbatim).}: a class-label \glsterm{probe-2}{probe} at layer $\ell$ does at least as well
as the same \glsterm{probe-2}{probe} at any earlier layer. The result anchored the
...
the \glsterm{llm}{LLM} picture is non-monotonic and property-dependent in ways the
...
\emph{\glsterm{structural-probe}{structural probe}}: a learned linear transform
...
\nolinkurl{kb/excerpts/hewitt2019-structural-probe.md#abstract}.}:
\begin{equation}
  \boxed{\;
...
\glsterm{probe-2}{probe} is a single $k \times D$ matrix per (model, layer). Hewitt and
...
point: a \glsterm{structural-probe}{structural probe} shifts the question from ``does the model
...
The 2023+ truth-direction literature found that a \glsterm{probe-2}{probe} with no learned
\glsterm{parameters}{parameters} at all often suffices, and is in fact \emph{more} causally
implicated than its trained counterparts. Given a balanced labeled set
$\{(\mathbf{h}_i, y_i \in \{+, -\})\}$ at a chosen $(\ell, t)$, the
\emph{mass-mean} (or \emph{difference-in-means}) \glsterm{probe-2}{probe} direction is
\citep{marks-tegmark-2023-truth}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#abstract},
...
A fourth \glsterm{probe-2}{probe} family drops the label requirement entirely. Contrast-Consistent
Search (CCS) \deepencite{burns2022-ccs §3, citation-pending} fits a
\glsterm{linear-probe}{linear probe} by minimizing a \glsterm{self-consistency-2}{self-consistency} loss across paired
contrast prompts (e.g., ``X is true.'' / ``X is false.''), enforcing
that the \glsterm{probe-2}{probe}'s outputs on the two completions of a contrast pair
should sum to one and be confident. The headline claim is that CCS
recovers a \emph{truth-like} direction in \glsterm{llm}{LLM} activations without
...
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-1-related}.} and
...
form of the spurious-correlate failure: any consistent latent \glsterm{feature}{feature}
...
\begin{intuition}
A \glsterm{probe-2}{probe} is a \emph{point-spread function} on the model's representation
...
must be \emph{in focus along that filter axis} for the \glsterm{probe-2}{probe} to detect
...
\glsterm{structural-probe}{structural probe} in Eq.~\eqref{eq:structural-probe} replaces the scalar
projection with a $k$-dimensional one, so the PSF becomes a
$k$-dimensional subspace rather than a single axis. Returning to
canonical form: in every case the \glsterm{probe-2}{probe} is a linear map
$\mathbb{R}^D \to \mathbb{R}^k$ (with $k = 1$ for binary, $k > 1$ for
structural) followed by a fixed decision rule, and \glsterm{probe-2}{probe} accuracy
...
\glsterm{probe-2}{Probe} accuracy is a function of $(\ell, t)$, and the shape of that
function tells a structural story about \emph{where} a property is
represented. Three regimes recur across the \glsterm{llm}{LLM} probing literature.

\paragraph{Surface features early.} Token identity, basic part-of-speech
role, and other surface-syntactic properties \glsterm{probe-2}{probe} highest in
\emph{early} layers; the \glsterm{probe-2}{probe} accuracy curve rises steeply through the
embedding and first few \glsterm{transformer}{transformer} blocks and then plateaus or decays
\citep{hewitt2019-structural-probe}. The \glsterm{structural-probe}{structural probe} of
...
linear separability with depth on vision class labels, but the \glsterm{llm}{LLM}
...
worked example: the \glsterm{truth-direction-2}{truth direction} localizes around layers 12--15 and
...
excerpt: \nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-3-layers},
\texttt{\#sec-4-misalignment}.}.

\paragraph{Position selection: the period-token trick.} For free-form
text, the most-informative \glsterm{probe-2}{probe} position is often the
...
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-3-layers}.}. The
...
the prior empirical observation of the same \glsterm{summarization-behavior}{summarization behavior} on
sentiment.
...
Compiled from \nolinkurl{kb/notes/interpretability/probing.md} \S 4.1.}
\label{tab:probe-families}
\small
\begin{tabularx}{\linewidth}{l l X X}
\toprule
\textbf{Family} & \textbf{Year} & \textbf{\glsterm{probe-2}{Probe} form} & \textbf{\glsterm{key}{Key} limitation} \\
\midrule
...
MLP \glsterm{probe-2}{probe}
  & 2010s
  & small feedforward net on $\mathbf{h}$
  & \glsterm{probe-2}{probe} leakage: a powerful \glsterm{probe-2}{probe} learns $y$ from low-information features \citep{hewitt2019-control-tasks} \\
\glsterm{structural-probe}{Structural probe} \citep{hewitt2019-structural-probe}
  & 2019
...
Control-task baseline \citep{hewitt2019-control-tasks}
  & 2019
  & same \glsterm{probe-2}{probe} on randomly-permuted labels
...
Contrastive / CCS unsupervised
  & 2022
  & \glsterm{linear-probe}{linear probe}, \glsterm{self-consistency-2}{self-consistency} loss, no labels
...
\glsterm{llm}{LLM} probing literature, in the form lifted from
\nolinkurl{kb/notes/interpretability/probing.md} \S 4.1. The lineage tells
...
that \glsterm{probe-2}{probe} accuracy alone is insufficient, since a sufficiently
expressive \glsterm{probe-2}{probe} can learn $y$ from features the model itself does not
use. The 2022 review of \citet{belinkov2022-probing-survey}\footnote{KB
excerpt: \nolinkurl{kb/excerpts/belinkov2022-probing-survey.md#abstract}.}
consolidates the critique into three confounders — \glsterm{probe-2}{probe} expressivity,
...
confound (CCS), and the activation-steering family that turns a \glsterm{probe-2}{probe}
...
Synthesizing across the four \glsterm{probe-2}{probe} families, the practical recipe for
...
\begin{enumerate}
  \item \textbf{Curate} a labeled dataset
    ...
    \nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-2-datasets}.}.
  \item \textbf{Cache} activations $\{\mathbf{h}^{(\ell, t)}_i\}_i$ via
    ...
    $(\ell, t)$ slice.
  \item \textbf{Fit} a \glsterm{probe-2}{probe} — Eq.~\eqref{eq:linear-probe} via logistic
  ...
  \item \textbf{Generalization tests} (optional but increasingly
    ...
    the \glsterm{probe-2}{probe} was reading a surface confounder rather than the abstract
  ...
  \item \textbf{Causal validation} (optional, but the \emph{closing}
    step for any claim that the model \emph{uses} the \glsterm{probe-2}{probe} direction).
    Ablate $\mathbf{d}_y$ from the \glsterm{residual-stream}{residual stream} during a forward pass
...
from), and the \glsterm{probe-2}{probe}-accuracy curve is a snapshot of the model's
...
this section: \glsterm{probe-2}{probe}-accuracy results read off a checkpoint at
post-pre-training time inherit whatever pre-training data, mixture, and
optimizer choices produced that checkpoint, and a \glsterm{probe-2}{probe}-accuracy
...
high-accuracy \glsterm{probe-2}{probe} is correlational, not causal — is the
load-bearing critique that
\S\ref{sec:correlation-causation} closes by pairing probes with
activation interventions: \glsterm{direction-ablation}{direction ablation}
...
activation-steering family that turns a \glsterm{probe-2}{probe} direction into an
