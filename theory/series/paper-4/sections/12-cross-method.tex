...
\glsterm{residual-stream}{residual stream} through the model's own unembedding. Probes are
...
$M\!\gg\!D$-dimensional sparse basis. \glsterm{activation-patching}{Activation patching} is causal at
the component level. \glsterm{circuit-tracing}{Circuit tracing} is causal at the \glsterm{feature}{feature} level.
...
divergence cases (\glsterm{truth-direction}{truth direction} across datasets; \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature}
...
$\sim 124$M \glsterm{parameters}{parameters}; the task is the canonical Wang--prompt
...
\nolinkurl{kb/excerpts/wang2022-ioi.md#sec-1-task}.}. Three
methodologically independent readouts converge on the same \glsterm{circuit}{circuit}
picture.

\citet{wang2022-ioi} identify a \glsterm{circuit}{circuit} of $26$ attention heads in
...
computational-graph edges, KL-thresholds the \glsterm{indirect-effect}{indirect effect} of
each, and prunes below threshold; the surviving subgraph on GPT-2
small recovers the Wang \glsterm{circuit}{circuit}'s load-bearing edges without
...
commit identified by Wang et al.\ at the head level. \glsterm{sparse-autoencoder}{SAE}-\glsterm{feature}{feature}
...
and the lens's prediction trajectory is the \glsterm{circuit}{circuit}
...
is $1.5$B \glsterm{parameters}{parameters}; the task is the
$(s, r, o)$-triple cloze of \S\ref{sec:rome-tracing} (e.g., ``The
Eiffel Tower is in \_\_''); the target is the object completion.

\citet{meng2022-rome} run \glsterm{causal-tracing}{causal tracing} across (component, token,
layer) cells and report two effects: the \glsterm{average-indirect-effect}{average indirect effect}
peaks at MLP layers around layer 15--20 at the last subject token,
and the \glsterm{average-indirect-effect}{AIE} for MLP perturbations ($6.6\%$) substantially exceeds
the \glsterm{average-indirect-effect}{AIE} for attention perturbations ($1.6\%$) at that locus
\citep[\S 2.3]{meng2022-rome}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-2-effects}.}. The first
method --- \glsterm{activation-patching}{activation patching} with Gaussian noise as the corruption
...
\glsterm{key}{key}--\glsterm{value}{value} constraint
...
the \emph{precise} layer-of-edit is not always the \glsterm{average-indirect-effect}{AIE} peak, but
the locus is robust.)
...
The fourth method enters from the \glsterm{sparse-autoencoder}{SAE} side. \glsterm{sparse-autoencoder}{SAE} features trained on
the \glsterm{residual-stream}{residual stream} and MLP outputs of GPT-2 XL recover
...
decomposed at the \glsterm{feature}{feature} level (\glsterm{sparse-autoencoder}{SAE} features). The joint claim is
...
\citep{zhang2023-apatching}) would not align with \glsterm{sparse-autoencoder}{SAE}-discovered
...
\boldsymbol{\mu}_-$, is the canonical truth \glsterm{probe}{probe} of
...
The \glsterm{truth-direction}{truth direction} recovered from the \texttt{cities} dataset
(``The capital of France is Paris.'') and the \glsterm{truth-direction}{truth direction}
...
excerpt: \nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-1-related}.}
...
mass-mean does not share. A third method on the same target enters from the \glsterm{sparse-autoencoder}{SAE} side.
\glsterm{sparse-autoencoder}{SAE}-discovered ``truthfulness''-related features
\citep[\S Safety-Relevant Features]{templeton2024-scaling-monosemanticity}\footnote{No
KB excerpt file: \texttt{templeton2024-scaling-\glsterm{monosemanticity}{monosemanticity}} is indexed in
\nolinkurl{kb/index/papers.json} with \texttt{excerpts\_file: null} (HTML-only
source, transcription pending; checked 2026-08-19). The \glsterm{transformer}{Transformer} Circuits
article numbers no sections, so it is cited by section title.}
\deepencite{templeton2024-scaling-monosemanticity \S Safety-Relevant Features,
...
cosine between the \glsterm{probe}{probe} direction and the \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature} decoder
...
\begin{contradiction}
\textbf{There may not be a single \glsterm{truth-direction}{truth direction}.} Mass-mean
...
finds a different direction that fails under negation; \glsterm{sparse-autoencoder}{SAE}-discovered
...
result on \glsterm{sycophancy}{sycophancy} generalizes the pattern: behaviorally-singular
properties may be representationally plural, and the field's
single-axis truth-\glsterm{probe}{probe} assumption may be the wrong shape of model
...
cross-model-family study reporting \glsterm{probe}{probe} direction, \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature}
direction, and patching-validated causal direction in matched
coordinates.
\end{contradiction}

This is principled divergence in part. A \glsterm{probe}{probe} asks ``what
direction linearly decodes property $y$ from this dataset's
activations?''; an \glsterm{sparse-autoencoder}{SAE} asks ``what sparse basis decomposes
...
load-bearing once the field makes single-axis truth-\glsterm{probe}{probe} claims
...
A second negative case is the \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature} inventory itself.
\citet{leask2025-sae-not-canonical} show that two independent \glsterm{sparse-autoencoder}{SAE}
training runs at different widths $M$ may produce different
\glsterm{feature}{feature} inventories on the same activations
\citep[\S 1]{leask2025-sae-not-canonical}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/leask2025-sae-not-canonical.md#sec-1-canonical}.}:
meta-SAEs decompose larger-\glsterm{sparse-autoencoder}{SAE} latents into combinations of
smaller-\glsterm{sparse-autoencoder}{SAE} latents (non-atomicity), and \glsterm{sae-stitching}{SAE stitching} reveals
...
``true features'' framing assumes a sufficiently large \glsterm{sparse-autoencoder}{SAE}
...
the cross-method consequence is that any claim grounding in \glsterm{sparse-autoencoder}{SAE}
features (``the \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature} for truth has cosine $\rho$ with the
\glsterm{probe}{probe} direction'') is a claim about \emph{this} \glsterm{sparse-autoencoder}{SAE}'s inventory,
...
\nolinkurl{kb/excerpts/belrose2023-tuned-lens.md#sec-2-failures}.}.
...
$L\!\cdot\!(D^2 + D)$ trained \glsterm{parameters}{parameters}.
\citet{chuang2023-dola}'s \glsterm{dola}{DoLa} contrastive decoding works on the
LLaMA-7B/13B/33B family; whether it generalizes to Llama 3 / 4,
Qwen 2.5 / 3, \glsterm{precision-pinning}{DeepSeek-V3}, and Gemma 3 has not been systematically
...
until a \glsterm{tuned-lens}{tuned lens} for the frontier-2026 architecture exists.
...
probes and \glsterm{sparse-autoencoder}{SAE} features ask different questions, but the
...
representation is multi-axis). The \glsterm{sparse-autoencoder}{SAE}-canonicality and lens
...
while on the open behaviors where they do not (truth, \glsterm{sycophancy}{sycophancy},
the \glsterm{sparse-autoencoder}{SAE} \glsterm{feature}{feature} inventory) the field has identified the
