...
A \emph{lens} takes a transformer's intermediate residual-stream state
$\mathbf{h}_\ell$ at some layer $\ell$ and returns a distribution over
the vocabulary $\mathcal{V}$ — the prediction the model would commit to
...
lens techniques that anchor the literature: the \emph{logit lens}
...
affine translator and fixes the logit lens's documented cross-model
failures; and \emph{DoLa} \citep{chuang2023-dola}, an inference-time
...
\citet{belrose2023-tuned-lens} frame the transformer as iterative
inference\footnote{KB excerpt: \texttt{kb/excerpts/belrose2023-tuned-lens.md\#sec-1-iterative}.}:
each block makes a small additive update to the residual stream, and
the per-layer prediction tends to converge smoothly toward the
final-layer output. Concretely, for a pre-LayerNorm transformer the
...
to $\ell$ and the layers after, the function from a hidden state
$\mathbf{h}_\ell$ to the final-layer logits is
...
rest of this section: residual stream $\mathbf{h}_\ell \in \mathbb{R}^D$
at one $(B, S)$ position, with $D$ the model dim, $V = |\mathcal{V}|$
the vocabulary, and $L$ the layer count from Paper~1's notation. A
batched residual stream is $\mathsf{B}\!\times\!\mathsf{S}\!\times\!D$;
the lens output is $\mathsf{B}\!\times\!\mathsf{S}\!\times\!V$.
...
The logit lens drops the post-$\ell$ residual updates from
...
\begin{equation}
  ...
  \label{eq:logit-lens}
\end{equation}
The result is a logit vector in $\mathbb{R}^V$; softmax gives the
layer-$\ell$ ``as-if-final'' distribution and $\arg\max$ gives the
top-$1$ token. Per-(layer, token) cost is one LayerNorm plus one
...
parameters and no hyperparameters beyond the choice of normalization;
it is the cheapest interpretability readout of any kind.

\begin{forumsignal}
The logit lens was introduced by nostalgebraist on LessWrong in August
...
the raw logit lens\footnote{KB excerpt:
\texttt{kb/excerpts/belrose2023-tuned-lens.md\#sec-2-failures}.}.
\textbf{Bias.} The logit lens systematically puts more mass on certain
vocabulary items than the final layer does. On Pythia 12B, the bias
...
\textbf{Cross-model unreliability.} The logit lens works ``reasonably
...
These three failure modes set up the tuned lens as the targeted
correction.
...
identity, so a freshly initialized tuned lens is identical to the logit
...
underlying model — the classic ``probe-as-feature-extractor'' confound
flagged by \citet{hewitt2019-control-tasks} and engaged in
\S\ref{sec:probing}.

\paragraph{Cost.} Tuned-lens parameters total $L \cdot (D^2 + D)$ — a
...
parameters \citep[\S 3.2]{belrose2023-tuned-lens}\footnote{KB excerpt:
\texttt{kb/excerpts/belrose2023-tuned-lens.md\#sec-3-tuned-lens}.} ---
...
\paragraph{Why a single affine map is enough.} The tuned lens fixes
...
logit lens. \citet[Fig.~1]{belrose2023-tuned-lens} report that the
tuned lens recovers coherent intermediate predictions throughout the
GPT-Neo-2.7B stack where the logit lens produced nonsense, and
...
approximately linear, not learned-feature-dependent.

\begin{intuition}
The lens framework exploits one architectural fact: the residual stream
is an additive bus. The final-layer hidden state is
...
already approximates the unembedding of $\mathbf{h}_L$ — the logit lens
...
lens is the logit lens with a per-layer linear basis correction; the
...
contrasting logits across layers. \emph{Decoding by Contrasting Layers}
(DoLa) defines, at each generation step,
\begin{equation}
...
(plausibly hallucination-shaped). DoLa requires no fine-tuning, adds
...
\begin{analogy}
DoLa is to the logit lens what a numerical \emph{difference} is to a
function evaluation. The logit lens reads
...
reducing perplexity. On \emph{prompt-injected} or otherwise anomalous
...
Llama 3 / 4, Qwen 2.5 / 3, DeepSeek-V3, Gemma 3 --- is largely
...
to architectures with substantially modified attention or FFN substrate
--- DeepSeek-V3's MLA-compressed-KV plus fine-grained-MoE, NSA-sparse
attention, sliding-window layers --- is partially empirical and not
yet documented at frontier scale. The residual stream is still
...
\emph{Causal Basis Extraction}, which finds a tuned-lens-derived
...
methodologies that converge on the GPT-2-small IOI-circuit picture, and
