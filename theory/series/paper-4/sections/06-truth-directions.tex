...
\glsterm{value}{value} with high \glsterm{probe}{probe} accuracy and is causally implicated in the
...
\emph{causal intervention} (ablating the \glsterm{probe}{probe}-derived direction in
...
methodological recommendation is that, among the four \glsterm{probe}{probe} families
...
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-3-layers}.}. On
...
saying ``the \glsterm{llm}{LLM} has a \glsterm{truth-direction}{truth direction}.''
...
minimizing a \glsterm{self-consistency}{self-consistency} loss
\begin{equation}
  ...
  \label{eq:ccs-loss}
\end{equation}
which jointly enforces (i) that the \glsterm{probe}{probe} assigns complementary
...
have opposite truth values --- and (ii) that the \glsterm{probe}{probe} is confident
on at least one side. The discovered $\mathbf{w}$ is reported to be
``truth-like'': \glsterm{probe}{probe} accuracy on held-out labeled true/false data is
...
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-1-related}.}: a
...
The implication for the ``does the model have a \glsterm{truth-direction}{truth direction}''
question is direct. The \glsterm{probe}{probe} direction recovered from
\texttt{cities} alone is \emph{not} the \glsterm{probe}{probe} direction recovered from
...
\begin{contradiction}
\textbf{Is there a canonical \glsterm{truth-direction}{truth direction}?} Three observations
...
sign, scale). Until then, ``the \glsterm{truth-direction}{truth direction}'' is a useful
...
\begin{itemize}
  ...
  \item \textbf{Period-token summarization.} The most-informative \glsterm{probe}{probe}
  ...
  \item \textbf{Mass-mean as the direction with strongest causal
    mediation.} Among the \glsterm{probe}{probe} families of \S\ref{sec:probing},
...
\begin{itemize}
  \item \textbf{Cross-prompt-format alignment.} Whether the
    ...
    \emph{\glsterm{probe}{probe}} (single-direction summary of a multi-axis subspace)
  ...
  \item \textbf{Cross-model-family transfer.} Truth directions in
    ...
    canonical \glsterm{truth-direction}{truth direction} up to model-specific basis or whether
...
the same \glsterm{probe}{probe}-then-causally-validate pipeline of
...
both: \glsterm{probe}{probe}-derived truth directions and \glsterm{sparse-autoencoder}{SAE}-derived truth-related
