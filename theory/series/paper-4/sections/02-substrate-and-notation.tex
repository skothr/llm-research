...
quantities the lens reads, the directions the \glsterm{probe}{probe} projects onto, the
...
stream. \glsterm{mha}{Multi-head attention} is a sum over heads of
...
\begin{itemize}
  \item \S\ref{sec:logit-lens-family} (lens). The \emph{token-aligned}
    ...
    reads the $\arg\max$.
  \item \S\ref{sec:probing} (\glsterm{probe}{probe}). A \textbf{property direction}
...
$\mathbf{d}_y$ & $\mathbb{R}^D$ (unit-norm) & property direction associated with label $y$ (\glsterm{probe}{probe} / steering target) \\
...
Eq.~\eqref{eq:residual-decomp}. A \glsterm{probe}{probe} (\S\ref{sec:probing}) reads
