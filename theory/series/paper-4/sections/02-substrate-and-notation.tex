...
decomposes a \glsterm{transformer}{transformer}'s internal computation. Before the methods can
disagree productively, the substrate they share must be pinned down.
This section codifies four pieces of architectural \glsterm{vocabulary}{vocabulary} --- the
\glsterm{residual-stream}{residual stream} as a linear additive bus, attention as a
QK\,$+$\,OV decomposition, the feedforward sublayer as a candidate
associative memory, and the \glsterm{layer-normalization}{LayerNorm}\,$+$\,unembedding as the canonical
...
quantities the lens reads, the directions the \glsterm{probe-2}{probe} projects onto, the
sites the patcher swaps, the subgraphs the \glsterm{circuit}{circuit} picks out.

\subsection{The residual stream as an additive bus}
\label{sec:substrate-residual}

Fix a \glsterm{decoder-only}{decoder-only} \glsterm{transformer}{transformer} $\mathcal{M}$ with $L$ layers, model
dimension $D$, \glsterm{vocabulary}{vocabulary} $\mathcal{V}$ of size $V = |\mathcal{V}|$,
...
state. Batched, the \glsterm{residual-stream}{residual stream} is a tensor of shape $\BSH$, with
$\mathsf{B}$ the batch axis. A head-split per-layer attention tensor is
$\BHSdh$. The output \glsterm{logits}{logits} are shape $\bsv$.

The pre-\glsterm{layer-normalization}{LayerNorm} \glsterm{transformer}{transformer} block updates the \glsterm{residual-stream}{residual stream}
additively\footnote{KB excerpt:
\nolinkurl{kb/excerpts/belrose2023-tuned-lens.md#sec-2-decomposition}.}
...
\glsterm{layer-normalization}{LayerNorm} or \glsterm{rmsnorm}{RMSNorm}. Eq.~\eqref{eq:residual-update} is the
load-bearing equation of this paper: every component reads
$\mathbf{h}^{(\ell, t)}$, computes some function of it, and writes the
result back into the same \glsterm{residual-stream}{residual stream} by addition. There is no
...
(\glsterm{layer-normalization}{LayerNorm} or \glsterm{rmsnorm}{RMSNorm}), and $W_U \in \mathbb{R}^{D \times V}$ is the
...
zero in Eq.~\eqref{eq:residual-decomp} gives the \emph{\glsterm{logit-lens}{logit lens}}
...
\begin{intuition}
The \glsterm{residual-stream}{residual stream} is a linear additive bus. Attention heads, MLPs,
...
linear combination of \glsterm{feature}{feature} directions in $\mathbb{R}^D$). Returning
...
term that the \emph{Mathematical Framework for \glsterm{transformer}{Transformer}
Circuits}~\deepencite{elhage2021-mathematical-framework HTML at
transformer-circuits.pub, citation-pending} factors into two
read--write circuits. The \textbf{QK \glsterm{circuit}{circuit}} computes the
attention-weight pattern from the \glsterm{residual-stream}{residual stream}:
\begin{equation}
  \alpha^{(\ell, h)}_{t, s}
  \;=\;
...
The \textbf{OV \glsterm{circuit}{circuit}} determines what is read and written along
...
stream. \glsterm{multi-head-attention}{Multi-head attention} is a sum over heads of
Eq.~\eqref{eq:ov}. The decomposition is load-bearing for \glsterm{circuit}{circuit}
analysis: when \citet{wang2022-ioi} report (\S\ref{sec:circuits})
that a Name Mover Head ``copies the attended name'', the
human-interpretable claim is exactly that its OV \glsterm{circuit}{circuit} acts as a
near-identity on the residual-stream subspace encoding ``name'', and
its QK \glsterm{circuit}{circuit} attends to the IO position; the
...
elementwise nonlinearity $\sigma$ (\glsterm{gelu}{GELU} / SiLU / \glsterm{swiglu}{SwiGLU} per the model;
...
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-2-2}.}. We adopt the
\glsterm{key}{key}--\glsterm{value}{value} framing as the \emph{anchor for §10}, not as a settled
fact: alternative framings (FFNs as \glsterm{feature}{feature}-direction \emph{detectors}
in \glsterm{superposition}{superposition}, the \citet{leask2025-sae-not-canonical} non-atomic
...
covered here. \glsterm{layer-normalization}{LayerNorm}~\citep{ba2016-layernorm} centers and rescales
$\mathbf{h}^{(\ell, t)}$ along its $D$-dim axis with a learned affine;
\glsterm{rmsnorm}{RMSNorm} drops the centering. Either is denoted $\mathrm{LN}[\,\cdot\,]$
...
$\mathrm{LN}$ followed by $W_U$; the layer-$L$ \glsterm{logits}{logits} are
\begin{equation}
...
Throughout this paper a \textbf{\glsterm{feature}{feature}} is a direction
...
from the \glsterm{residual-stream}{residual stream}, so the natural object of interpretability
...
\begin{itemize}
  \item \S\ref{sec:logit-lens-family} (lens). The \emph{token-aligned}
    ...
    reads the $\arg\max$.
  \item \S\ref{sec:probing} (\glsterm{probe-2}{probe}). A \textbf{property direction}
    $\mathbf{d}_y \in \mathbb{R}^D$ is a \glsterm{feature}{feature} associated with an
    external label $y$ --- truth, sentiment, syntactic role,
    \glsterm{sycophancy}{sycophancy} --- discovered either by supervised fitting
    (Eq.~\eqref{eq:linear-probe}-equivalent) or in closed form by the
    mass-mean rule
    $\mathbf{d}_{\mathrm{MM}} = \boldsymbol{\mu}_+ - \boldsymbol{\mu}_-$.
  \item \S\ref{sec:sparse-autoencoders} (\glsterm{sparse-autoencoder}{SAE}). A \textbf{dictionary
    \glsterm{feature}{feature}} $i$ at layer $\ell$, position $t$ is the activation
    $f_i^{(\ell, t)} \in \mathbb{R}_{\geq 0}$ of the $i$-th latent in
    a \glsterm{sparse-autoencoder}{sparse autoencoder} trained on $\mathbf{h}^{(\ell, t)}$, with
...
\end{itemize}
A \glsterm{feature}{feature} is therefore an \emph{axis}, not a \emph{coordinate}: the
neuron basis is one possible \glsterm{feature}{feature} basis, and the production
...
basis --- $W_U$'s columns, a learned $\mathbf{d}_y$, an \glsterm{sparse-autoencoder}{SAE}'s
...
A \textbf{\glsterm{circuit}{circuit}} $C \subseteq \mathcal{M}$ is a subgraph of
$\mathcal{M}$'s computational graph --- nodes are forward-pass
components (residual-stream sites, attention heads, MLP sublayers,
\glsterm{sparse-autoencoder}{SAE}/\glsterm{transcoder}{transcoder} features) and edges are the residual / attention /
projection links between them --- whose function $C(\mathbf{x})$
agrees with $M(\mathbf{x})$ on the task distribution after \emph{\glsterm{knockout}{knockout}}
of $\mathcal{M} \setminus C$~\citep[\S 2]{wang2022-ioi}\footnote{KB
excerpt: \nolinkurl{kb/excerpts/wang2022-ioi.md#sec-2-definition}. Wang
et al.\ define \glsterm{knockout}{knockout} via \glsterm{mean-ablation}{mean ablation} on a reference distribution
$p_{\mathrm{ABC}}$ rather than zero ablation, on the empirical
grounds that ``zero ablation [\ldots] lead[s] to noisy results''.}.
A \glsterm{circuit}{circuit} \emph{claim} commits to three quantitative criteria from
...
\end{description}
The substantive content of a \glsterm{circuit}{circuit} claim is therefore threefold:
the relevant components are \emph{localized} (a small fraction of the
graph), the explanatory edges are \emph{causal} (\glsterm{knockout}{knockout} matters),
...
features-in-circuits dichotomy is fundamental: a \glsterm{circuit}{circuit} can route a
\glsterm{feature}{feature} from one position to another without itself \emph{computing}
that \glsterm{feature}{feature}, so the wiring diagram (the \glsterm{circuit}{circuit}) and the encoding
(the \glsterm{feature}{feature}) are independent specifications of an
...
$\mathrm{LN}[\,\cdot\,]$ & $\mathbb{R}^D \to \mathbb{R}^D$ & \glsterm{layer-normalization}{LayerNorm} or \glsterm{rmsnorm}{RMSNorm}, model-specific \\
$W_U$ & $\mathbb{R}^{D \times V}$ & unembedding matrix; canonical readout is $\mathrm{LN}[\mathbf{h}]\,W_U$ \\
$\alpha^{(\ell, h)}_{t, s}$ & scalar in $[0,1]$ & attention weight from \glsterm{query}{query} $t$ to \glsterm{key}{key} $s$, head $h$, layer $\ell$ (Eq.~\eqref{eq:qk}) \\
...
$\mathbf{d}_y$ & $\mathbb{R}^D$ (unit-norm) & property direction associated with label $y$ (\glsterm{probe-2}{probe} / steering target) \\
$f_i^{(\ell, t)}$ & $\mathbb{R}_{\geq 0}$ & \glsterm{sparse-autoencoder}{SAE} / \glsterm{transcoder}{transcoder} \glsterm{feature}{feature} $i$'s activation at $(\ell, t)$ \\
$\mathbf{d}_i$ & $\mathbb{R}^D$ & decoder column for \glsterm{feature}{feature} $i$ (the \glsterm{feature}{feature}'s direction in residual space) \\
$C$ & subgraph of $\mathcal{M}$ & \glsterm{circuit}{circuit}, in the Wang-2022 sense \\
...
Eq.~\eqref{eq:residual-decomp}. A \glsterm{probe-2}{probe} (\S\ref{sec:probing}) reads
$\mathbf{h}^{(\ell, t)} \mapsto \sigma(\mathbf{d}_y^\top
\mathbf{h}^{(\ell, t)})$ for a learned property direction
$\mathbf{d}_y$. An \glsterm{sparse-autoencoder}{SAE} (\S\ref{sec:sparse-autoencoders}) decomposes
$\mathbf{h}^{(\ell, t)}$ into a sparse $\mathbf{f}^{(\ell, t)} \in
\mathbb{R}^M$ with components $f_i^{(\ell, t)}$ along directions
$\mathbf{d}_i$. \glsterm{activation-patching}{Activation patching} (\S\ref{sec:patching}) intervenes
on $\mathbf{h}^{(\ell, t)}$ (or on a finer-grained component) and
measures the \glsterm{indirect-effect}{indirect effect} on a downstream metric. \glsterm{circuit}{Circuit}
...
The unifying picture of this section is that the \glsterm{residual-stream}{residual stream}
...
property direction $\mathbf{d}_y$, the \glsterm{sparse-autoencoder}{SAE} direction $\mathbf{d}_i$,
the component $C$ at a \glsterm{knockout}{knockout} site, the subgraph $C$ as a wiring
