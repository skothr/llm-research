...
\label{sec:rome}
\label{sec:rome-corruption-sensitivity}

The activation-patching protocol of \S\ref{sec:patching} is a diagnostic:
its output is a heatmap of which (component, token, layer) cells
are causal mediators for a behavior. ROME
...
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-2-effects}.}. On 1000 factual
statements, two peaks dominate the
heatmap\footnote{KB excerpt:
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-2-2}.}: a \emph{late site}
...
versus $\mathrm{AIE}_{\mathrm{attn}} = 1.6\%$, with the \glsterm{average-indirect-effect}{AIE} peak
...
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-2-3}.}:
...
\nolinkurl{kb/excerpts/meng2022-rome.md#sec-3-1}.}: the second linear
layer of an MLP, $W^{(\ell)}_{\mathrm{proj}}$, is a \emph{linear
associative memory} in the sense of \citet{kohonen1972-associative}
and Anderson 1972. Any linear map $W$ can act as a \glsterm{key}{key}--\glsterm{value}{value} store for
...
layer (so $k$ is the MLP's encoded \glsterm{query}{query} for ``what subject is this''),
and $v = W^{(\ell)}_{\mathrm{proj}} k$ is the MLP output written into
the \glsterm{residual-stream}{residual stream} (so $v$ is the recalled information about the
subject).

Inserting a new association $(k_*, v_*)$ then has a closed-form
solution. Constrain the new map $\tilde{W}$ to satisfy
$\tilde{W} k_* = v_*$ exactly while leaving other \glsterm{key}{key}--\glsterm{value}{value} pairs
...
Inserting any new fact reduces to (i) computing the \glsterm{key}{key} $k_*$ for the
target subject by running the model up to the chosen MLP layer,
(ii) computing the \glsterm{value}{value} $v_*$ that would produce the target object
...
\glsterm{key}{key} $k$ being the model's internal representation of the subject and
the \glsterm{value}{value} $v$ being the residual-stream contribution that pushes the
...
$C^{-1}$ term reweights the update by which directions in \glsterm{key}{key}-space
...
excerpt: \nolinkurl{kb/excerpts/meng2022-rome.md#sec-3-2}.}.
...
as a \glsterm{key}{key}--\glsterm{value}{value} pair at MLP layer $15$, you should be able to rewrite
it by rewriting that pair, and you can.

\subsection{Hase 2023: locating is not editing-target}
\label{sec:rome-hase-critique}

The headline that ``\glsterm{causal-tracing}{causal tracing} localizes the layer to edit''
...
\emph{not monotonic} in the \glsterm{average-indirect-effect}{AIE} peak: layers with substantially lower
...
predict. The methodological consequence is that \emph{\glsterm{causal-tracing}{causal tracing}
...
survive is the prescriptive coupling: ``compute the \glsterm{average-indirect-effect}{AIE} heatmap, edit
...
\nolinkurl{kb/excerpts/zhang2023-apatching.md#sec-findings}.} report that
...
claim --- ``edit at exactly the \glsterm{average-indirect-effect}{AIE} peak, layer~15, for GPT-2 XL''
...
document that editing performance does not track \glsterm{average-indirect-effect}{AIE} peaks across
layers. The strongest defensible form of ROME's prescription, after
both critiques, is: \emph{\glsterm{causal-tracing}{causal tracing} identifies a layer band where
factual recall is mediated; within that band, layer-of-edit should be
selected on a held-out editing benchmark, not by \glsterm{average-indirect-effect}{AIE} peak alone}. The
locus is robust; the prescription is methodology-sensitive. Returning
to canonical math: the \glsterm{average-indirect-effect}{AIE} statistic of Eq.~\eqref{eq:aie} is the
...
Eq.~\eqref{eq:rome-edit} were developed on dense \glsterm{transformer}{transformer} MLPs in
...
architectural shifts complicate the picture. First, \glsterm{multi-head-latent-attention}{MLA}-compressed-KV
attention (the \glsterm{multi-head-latent-attention}{MLA} mechanism of \hyperref[P1-sec:attention-kv-sharing]{Paper~1 §5}; DeepSeek-V2/V3) and grouped-\glsterm{query}{query} attention (Llama 3 family)
...
(\glsterm{precision-pinning}{DeepSeek-V3} with 256 routed experts; Llama 4 MoE variants; Qwen 2.5
...
on Llama 3 / \glsterm{precision-pinning}{DeepSeek-V3} / Llama 4 with comparable benchmarks to
\citet{meng2022-rome} is, to our knowledge, not yet published; whether
the hypothesis survives the MoE / \glsterm{multi-head-latent-attention}{MLA} architectural shifts is open
empirical.
...
\glsterm{rlhf}{RLHF}, \glsterm{dpo}{DPO}) as the standard recipe for installing new behaviors and
knowledge; both update many or all \glsterm{parameters}{parameters} under a loss.
...
\S\ref{sec:rome-hase-critique}. \glsterm{lora}{LoRA}/\glsterm{qlora}{QLoRA} adapter merging
...
ROME is the bridge between \glsterm{activation-patching}{activation patching}'s diagnostic output
...
the last subject token) to a full \glsterm{circuit}{circuit} subgraph: where ROME's
factual-recall picture is essentially component-level (which sublayer,
at which token, at which layer), the \glsterm{circuit}{circuit} picture is
...
patching localizes the MLP; the \glsterm{tuned-lens}{tuned lens} shows the predicted-object
probability rising across the localized layer band; \glsterm{sparse-autoencoder}{SAE} features on
...
\emph{positive} cross-method evidence after the IOI \glsterm{circuit}{circuit}.
