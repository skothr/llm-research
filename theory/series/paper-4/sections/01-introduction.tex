...
\begin{enumerate}
  \item \textbf{Correlational vs causal.} Lenses and probes read off
    ...
    methodological seam: a high-accuracy \glsterm{probe}{probe} is consistent with the
  ...
  \item \textbf{Supervised vs unsupervised.} Probes train against
    ...
    properties the method can find: a \glsterm{probe}{probe} finds what it is asked to
...
the \glsterm{probe}{probe} reads what the model has computed; the \glsterm{sparse-autoencoder}{SAE} reads how the
...
questions --- e.g., a \glsterm{probe}{probe} (decodable?) and patching (used?) --- they
...
coverage of \glsterm{mla}{MLA} / MoE / \glsterm{nsa}{NSA} architectures), with a 2026 frontier
snapshot table of which method covers which model;
\S\ref{sec:contradictions} concentrates the live disagreements ---
\glsterm{sparse-autoencoder}{SAE} canonicality, lens cross-model unreliability, \glsterm{probe}{probe} correlation
