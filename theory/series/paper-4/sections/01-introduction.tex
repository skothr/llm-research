...
Interpretability is method-plural. A modern frontier \glsterm{transformer}{transformer} is, at
the level of weights and activations, a stack of attention and MLP
sublayers writing into a \glsterm{residual-stream}{residual stream} of dimension $D \in \{2048,
4096, 8192, 16384\}$ over sequences of length $S$ up to $\sim10^6$
tokens, with parameter counts from $10^9$ to $10^{12}$ and a \glsterm{vocabulary}{vocabulary}
...
$t$ is simultaneously a partial \glsterm{vocabulary}{vocabulary} distribution
...
load-bearing claim is sharper: the underlying \glsterm{transformer}{transformer}'s internal
...
\begin{description}
  \item[Lens techniques (\S\ref{sec:logit-lens-family}).] Project an
    ...
    $\mathcal{V}$. The \emph{\glsterm{logit-lens}{logit lens}}
    \citep{nostalgebraist2020-logit-lens} is the no-training baseline;
    the \emph{\glsterm{tuned-lens}{tuned lens}} \citep{belrose2023-tuned-lens} fixes
  ...
  \item[Activation patching (\S\ref{sec:patching}).] The canonical
    ...
    corrupted run; the \glsterm{indirect-effect}{indirect effect} $\mathrm{IE}_C = m_{\text{pt}} -
  ...
  \item[Circuit tracing (\S\ref{sec:circuits}).] Compose the previous
    ...
    program to production \glsterm{claude}{Claude} 3.5 Haiku via cross-layer transcoders.
\end{description}
...
\begin{enumerate}
  \item \textbf{Correlational vs causal.} Lenses and probes read off
    ...
    patching, ROME, and \glsterm{circuit-tracing}{circuit tracing} intervene on activations or
    weights and report what is \emph{used}. The correlation--causation
    gap (\S\ref{sec:correlation-causation}) is the
    methodological seam: a high-accuracy \glsterm{probe-2}{probe} is consistent with the
  ...
  \item \textbf{Component-level vs \glsterm{feature}{feature}-level.} \glsterm{activation-patching}{Activation patching}
    and \glsterm{circuit}{circuit}-tracing-via-attention-heads identify
    \emph{components} (heads, MLPs, hidden states) that mediate a
    behavior; SAEs and \glsterm{feature}{feature}-level \glsterm{circuit-tracing}{circuit tracing} identify
  ...
  \item \textbf{Supervised vs unsupervised.} Probes train against
    ...
    properties the method can find: a \glsterm{probe-2}{probe} finds what it is asked to
    find; an \glsterm{sparse-autoencoder}{SAE} finds what its sparsity prior considers a \glsterm{feature}{feature}.
  \item \textbf{Static readout vs intervention.} Lenses, probes, and
    SAEs are static --- they compute on cached activations without
    perturbing the forward pass. \glsterm{activation-patching}{Activation patching}, ROME, and
    \glsterm{circuit}{circuit}-validation knockouts are interventional. The static
...
($L \cdot (D^2 + D)$ tuned-lens \glsterm{parameters}{parameters} --- under $10^9$ for
...
GPT-3 pretraining \citep{gemma-scope-2024}); \glsterm{activation-patching}{activation patching} costs
$O(N_{\text{components}})$ forward passes per input pair; \glsterm{circuit}{circuit}
tracing on production models inherits the \glsterm{sparse-autoencoder}{SAE}/\glsterm{transcoder}{transcoder} training cost
...
run does not enter the field's working \glsterm{vocabulary}{vocabulary}.

\begin{intuition}
The five methods are five different instruments aimed at the same
\glsterm{residual-stream}{residual stream}. The lens reads what the model is about to predict;
the \glsterm{probe-2}{probe} reads what the model has computed; the \glsterm{sparse-autoencoder}{SAE} reads how the
model has decomposed what it computed; \glsterm{activation-patching}{activation patching} tests
which of those computations the model uses; \glsterm{circuit-tracing}{circuit tracing}
...
GPT-2-small IOI \glsterm{circuit}{circuit} \citep{wang2022-ioi}, GPT-2-XL factual recall
\citep{meng2022-rome}, and selected \glsterm{claude}{Claude} 3.5 Haiku capabilities
...
have not all been published for Llama 3/4, Qwen 2.5/3, \glsterm{precision-pinning}{DeepSeek-V3}, or
Gemma 3 (\S\ref{sec:scaling-snapshot}). \emph{Some claims}: the
mid-layer-MLP localization of factual recall, the IOI \glsterm{circuit}{circuit}'s
...
canonicality of \glsterm{sparse-autoencoder}{SAE} features
...
mechanism --- e.g., \glsterm{activation-patching}{activation patching} and ACDC on IOI --- they
...
questions --- e.g., a \glsterm{probe-2}{probe} (decodable?) and patching (used?) --- they
...
\glsterm{key}{key}--\glsterm{value}{value} associative memories, the unembedding $W_U$ as the
canonical readout, and the formal definitions of \emph{\glsterm{feature}{feature}} (a
direction $\mathbf{d} \in \mathbb{R}^D$) and \emph{\glsterm{circuit}{circuit}} (a
...
case study; \S\ref{sec:sparse-autoencoders} covers \glsterm{sparse-autoencoder}{SAE} architectures;
...
\glsterm{activation-patching}{activation patching}; \S\ref{sec:rome} covers ROME and the
interpretability-to-surgery bridge; \S\ref{sec:circuits} covers
\glsterm{circuit-tracing}{circuit tracing} from IOI to Haiku.
...
\glsterm{circuit}{circuit} hunts, full \glsterm{activation-patching}{activation patching} at frontier scale, lens
coverage of \glsterm{multi-head-latent-attention}{MLA} / MoE / \glsterm{nsa}{NSA} architectures), with a 2026 frontier
snapshot table of which method covers which model;
\S\ref{sec:contradictions} concentrates the live disagreements ---
\glsterm{sparse-autoencoder}{SAE} canonicality, lens cross-model unreliability, \glsterm{probe-2}{probe} correlation
vs causation, truth-direction multiplicity, \glsterm{circuit}{circuit}-tracing
