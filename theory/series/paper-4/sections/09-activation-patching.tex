...
The lens and probe of \S\ref{sec:logit-lens-family}--\S\ref{sec:probing}
read the residual stream; they tell us what intermediate states
\emph{contain}. They do not tell us what the model \emph{uses}. A
high-accuracy probe at layer $\ell$ is consistent with the property
...
else fixed, observe the output. Activation patching is the canonical
...
path patching \citep{wang2022-ioi}, attribution patching, and ACDC
...
(a single hidden state, an MLP output, an attention-head output,
...
\begin{enumerate}
  ...
  \item \textbf{Corrupted run.} Run $\mathcal{M}$ on $X_{*}$ and record
    ...
    metric $m_{*}$ (probability, logit difference, or KL --- see
    \S\ref{sec:patching-metric}).
  \item \textbf{Patched run.} Run $\mathcal{M}$ on $X_{*}$, but at
    component $C$ overwrite the activation with the cached clean value
    from step~(1). Record the patched output distribution
    $\mathbb{P}_{\text{pt}}$ and the metric $m_{\text{pt}}$.
\end{enumerate}

The \emph{indirect effect} of component $C$ on this prompt pair is the
...
of paired counterfactuals gives the \emph{average indirect effect}
...
batched tensor is $\mathsf{B}\!\times\!\mathsf{S}\!\times\!D$. A
...
$W^O$ projection; ``head $h$ contribution to the residual stream'' is
...
\hat{\mathbf{d}}$ along a probe direction $\hat{\mathbf{d}}$, leaving
the orthogonal complement unchanged --- the natural granularity for
``patch the truth direction'' or ``patch the Paris feature.''
...
the metric of \citet{meng2022-rome}. Logit difference, with $r'$ the
...
The methodological case for logit difference as the default is made by
\citet[\S 4.1]{heimersheim2024-apatching-howto}\footnote{KB excerpt:
\texttt{kb/excerpts/heimersheim2024.md\#sec-4-1}.}: it is a
``mostly linear function of the residual stream (unlike
probability-based metrics) which makes it easy to directly attribute
logit difference to individual components,'' and it is ``a `softer'
...
``non-linear, in the sense that [it] track[s] the logits exponentially.
...
\textbf{Gaussian Noising} (GN), used by \citet{meng2022-rome}, ``adds
...
(STR), used by \citet{wang2022-ioi}, ``replaces the key tokens by
...
inconsistent localization and circuit discovery outcomes (Section
...
\citet{meng2022-rome}'s GN-derived AIE peak at GPT-2-XL layer $15$
...
circuit ($C = A \wedge B$) has both $A$ and $B$ as necessary, so
noising flags both; denoising of either alone leaves the other at the
corrupted baseline and finds nothing. An OR circuit ($C = A \vee B$)
...
circuit topology, or to run both.
...
\midrule
Hidden state & $\mathbf{h}_i^{(\ell)} \in \mathbb{R}^{D}$ & \citet{meng2022-rome} \\
...
specific attention heads and is the granularity needed for circuit
...
finer-grained patching at the subspace level or a downstream probe-on-%
...
Standard activation patching gives the \emph{total} effect of a
...
$\mathcal{P}$ emanating from a node $h$, path patching runs a forward
...
decomposition (\S\ref{sec:circuits}) legible: full activation patching
attributes ``Name Mover Heads matter,'' but only path patching can
attribute ``S-Inhibition Heads write into the queries of the Name
Mover Heads,'' i.e., a directed effect $h \to R$ via a specific
projection. Without path patching the IOI circuit collapses into a
flat ranked list of important heads; with it, it is a wiring diagram.

\begin{intuition}
Activation patching is counterfactual surgery on the residual-stream
...
behaviorally meaningful metric (logit difference, by default). Path
...
The cost of full activation patching is one forward pass per component:
...
and path patching at depth-$k$ costs $O(N_{\text{components}}^{k})$.
On GPT-2 small this is feasible; on production-scale models it is not.
\emph{Attribution patching} replaces the substitution by its first-order
...
component $C$, and approximate the indirect effect of patching $C$ as
\begin{equation}
  \mathrm{IE}_C
  \;\approx\;
...
attention-softmax saturation and ReLU/GeLU dead zones being the
canonical failure cases.
\end{forumsignal}

The mediation-analysis lineage of activation patching, predating its
...
analysis'' as one of the synonyms under which activation patching
appears in the literature. The genealogy matters because it locates
patching in a pre-existing statistical-causal vocabulary
(total/indirect effect, mediation, intervention) that the
...
Metric & Logit difference (normalized) & \citet[\S 4.1]{heimersheim2024-apatching-howto} \\
...
false-positives}: because logit difference is a difference, ``it may
...
metrics, not abandoning logit difference. \emph{Probability
saturation}: when $\mathbb{P}(r) \in \{0, 1\}$ at either endpoint, the
...
hand}: a heatmap showing AIE concentrated at ``MLP layer 15, last
...
MLP, with the model storing the fact in a feature subspace within
that MLP, and with the model storing the fact partly there and partly
in a redundant location whose AIE is masked by backup
...
Activation patching is the method that turns the lens-and-probe
correlational reading of the residual stream
...
methods that produce the IOI circuit and the ACDC subgraph: Wang et
...
three-run protocol, and the cross-method evidence with SAE features
and the tuned lens (\S\ref{sec:cross-method}) is what gives the IOI
picture its load-bearing status. Activation patching is, in this
