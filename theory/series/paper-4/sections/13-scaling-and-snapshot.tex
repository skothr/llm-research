...
\nolinkurl{kb/excerpts/gemma-scope-2024.md#sec-1-compute}.}. The \glsterm{common-crawl}{CC}-BY-4.0 release
...
called once to produce activations, then the \glsterm{probe}{probe} runs on a CPU);
...
requires no training at all~\citep{marks-tegmark-2023-truth}. \glsterm{probe}{Probe}
\emph{cost} is dominated by the activation cache, not the \glsterm{probe}{probe} fit;
...
indifferent to model size) but the substrate: \glsterm{precision-pinning}{DeepSeek-V3} combines \glsterm{mla}{MLA}
...
The frontier-2026 architecture snapshot (Paper~1, §13) catalogs \glsterm{mla}{MLA},
...
$L_0$, reconstruction MSE, downstream-loss recovery, contrastive \glsterm{probe}{probe}
...
Mass-mean \glsterm{probe}{probe} & \citet{marks-tegmark-2023-truth} &
...
architecture snapshot of \emph{Paper~1, §13}: the \glsterm{mla}{MLA} / fine-grained
