...
A \emph{\glsterm{circuit}{circuit}} is the strongest unit of mechanistic explanation the
field has committed to: a sparse subgraph of the model's computational
graph whose nodes (attention heads, MLPs, \glsterm{transcoder}{transcoder} features) and edges
...
\nolinkurl{kb/excerpts/wang2022-ioi.md#sec-3-circuit}.};
...
\nolinkurl{kb/excerpts/conmy2023-acdc.md#sec-3-kl}.};
\citet{lindsey2025-circuit-tracing} extend the program to production
\glsterm{claude}{Claude} 3.5 Haiku via cross-layer transcoders and Jacobian attribution.
...
\glsterm{circuit}{circuit}. The function $C(x)$ is defined by \emph{\glsterm{knockout}{knockout}}: every node
...
\nolinkurl{kb/excerpts/wang2022-ioi.md#sec-2-definition}. Wang et al.\
explicitly prefer \glsterm{mean-ablation}{mean ablation} over zero ablation: ``zero ablation
[\ldots] lead[s] to noisy results in practice'' because forced-zero
activations are off-distribution.}. A \glsterm{circuit}{circuit} claim then commits to
three quantitative criteria, also from
\citet[\S 4]{wang2022-ioi}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/wang2022-ioi.md#sec-4-criteria}.}:
\begin{itemize}
  ...
  \item \textbf{Minimality:} $C$ contains no task-irrelevant nodes.
    ...
    behavior degrades.
\end{itemize}
The substantive content of a \glsterm{circuit}{circuit} claim, then, is threefold: that
the relevant features are \emph{localized} (a small fraction of the
graph), that the explanatory edges are \emph{causal} (\glsterm{knockout}{knockout} matters),
...
\begin{intuition}
A \glsterm{circuit}{circuit} is a wiring diagram of which components matter, not a content
...
of \emph{which direction in the \glsterm{residual-stream}{residual stream}} encodes ``name'' to
...
previous names; remove duplicates; output the remainder. The \glsterm{circuit}{circuit}
...
\begin{itemize}
  ...
  \item \textbf{S-Inhibition Heads.} Active at the \texttt{END} token,
    attend to \texttt{S2}, and write into the \emph{\glsterm{query}{query}} of the Name
...
\citet{olsson2022-induction-heads} \texttt{[A][B]\ldots[A]}$\,\to\,$\texttt{[B]}
...
The discovery method is \emph{\glsterm{path-patching}{path patching}}~\citep[\S 3.1]{wang2022-ioi}.
For inputs $x_{\text{orig}}$ and $x_{\text{new}}$ and a candidate head
$h$, \glsterm{path-patching}{path patching} ``runs a forward pass on $x_{\text{orig}}$, but for
...
attention heads. This isolates the direct effect of $h$ on the \glsterm{logits}{logits}
from indirect routing through other heads --- the partial-derivative
analogue of full \glsterm{activation-patching}{activation patching}, and the methodological refinement
that makes the seven-class decomposition legible.

The IOI \glsterm{circuit}{circuit} complicated three field expectations
...
\emph{against} the correct answer is a structural \glsterm{feature}{feature} of the
...
Hand-discovering the IOI \glsterm{circuit}{circuit} took multiple researcher-months.
\citet{conmy2023-acdc} systematize this work as an algorithm
(\textbf{ACDC}, Automatic \glsterm{circuit}{Circuit} DisCovery) over an explicitly
...
or, at finer granularity, between separate \glsterm{query}{query}/\glsterm{key}{key}/\glsterm{value}{value} activations,
individual neurons, or per-token-position
nodes\footnote{KB excerpt:
\nolinkurl{kb/excerpts/conmy2023-acdc.md#sec-2-2}.}. The
\emph{interchange-intervention} corruption convention --- preferred by
\citet{conmy2023-acdc} over zero or \glsterm{mean-ablation}{mean ablation} because both ``take
...
\nolinkurl{kb/excerpts/conmy2023-acdc.md#sec-3-overview}.}:
...
component types in [the manually-found] \glsterm{circuit}{circuit}'' and ``selected 68 of
...
\nolinkurl{kb/excerpts/conmy2023-acdc.md#abstract}.}. ACDC is evaluated
against two baselines: \emph{Subnetwork Probing} (a learned mask over
components, with the \glsterm{linear-probe}{linear probe} removed and KL substituted for the
...
\citet{conmy2023-acdc} to use corrupted-\glsterm{activation-patching}{activation patching} rather
...
ACDC is the algorithmic spine of automated \glsterm{circuit}{circuit} discovery, but it
inherits the cost of \glsterm{activation-patching}{activation patching}: each candidate edge requires
...
factorization whose decoder writes into the \glsterm{residual-stream}{residual stream} of every
\emph{subsequent} layer, not just the immediate next layer; this makes
skip-layer \glsterm{feature}{feature} flow explicit in the
graph~\citep{dunefsky2024-transcoders}. Second, a single forward pass
on the replaced model records the activations of all \glsterm{transcoder}{transcoder}
features $\{f_i^{(\ell, t)}\}$. Third, an \emph{\glsterm{attribution-graph}{attribution graph}} is
constructed by computing the linear contribution of every active
\glsterm{feature}{feature} to a target node (typically the unembedding direction for
the target token), via the Jacobian of the \glsterm{cross-layer-transcoder}{cross-layer transcoder}
system.

Concretely, the attribution edge from \glsterm{feature}{feature} $i$ at layer $\ell_1$,
position $t_1$ to \glsterm{feature}{feature} $j$ at layer $\ell_2$, position $t_2$ is
\begin{equation}
...
\end{equation}
where $\mathbf{d}_i^{(\ell_1)}$ is the decoder column of \glsterm{feature}{feature} $i$
in the layer-$\ell_1$ \glsterm{transcoder}{transcoder}, $\mathbf{e}_j^{(\ell_2)}$ is the
encoder row of \glsterm{feature}{feature} $j$ in the layer-$\ell_2$ \glsterm{transcoder}{transcoder}, and
...
\glsterm{residual-stream}{residual stream}; attention-weight-modulated through any intervening
...
input-dependent only through the \glsterm{attention-weights}{attention weights} and the active set
...
\begin{forumsignal}
\textbf{Single-source methodology (2026-Q2).} The cross-layer-\glsterm{transcoder}{transcoder}
...
\glsterm{transcoder}{transcoder}-seed-stability and external-reproduction concerns.
\end{forumsignal}
...
amortized cost is the \glsterm{cross-layer-transcoder}{cross-layer transcoder} training, which Anthropic
estimates at roughly comparable order to \glsterm{sparse-autoencoder}{SAE} training (Gemma Scope
reports \glsterm{sparse-autoencoder}{SAE} training at $\sim$20\% of the underlying model's
pretraining compute~\citep{gemma-scope-2024}). Reported findings on
\glsterm{claude}{Claude} 3.5 Haiku, transferred from
...
multi-step factual chains in which a ``Texas'' \glsterm{feature}{feature} in middle layers
licenses an ``Austin'' \glsterm{feature}{feature} in later layers for the prompt ``the
capital of the state Dallas is in''; refusal-\glsterm{circuit}{circuit} features on
harmful-request prompt classes; and partial cross-lingual \glsterm{feature}{feature}
...
returns components; attribution graphs amortize \glsterm{transcoder}{transcoder} training
...
The IOI \glsterm{circuit}{circuit} is the canonical convergence story.
...
functional classes Wang et al.\ describe. \glsterm{sparse-autoencoder}{SAE}-\glsterm{feature}{feature} analysis on the
...
\glsterm{circuit}{circuit}'s mid-late-layer commit to the IO token
...
\glsterm{causal-tracing}{causal tracing} on GPT-2 XL localizes factual recall to mid-layer MLPs
at the last subject token (\glsterm{average-indirect-effect}{AIE} peaks at layer 15;
...
zsRE~\citep[\S 3.2]{meng2022-rome}). \glsterm{sparse-autoencoder}{SAE} features on those layers
...
learning loss-curve discontinuity), by direct \glsterm{circuit}{circuit}
reverse-engineering, and is then \emph{reused} as a substructure of
the IOI \glsterm{circuit}{circuit}~\citep[\S 3]{wang2022-ioi}\deepencite{olsson2022-induction-heads
...
Convergence is not universal. The truth-direction \glsterm{circuit}{circuit}
...
\footnote{KB excerpt:
\nolinkurl{kb/excerpts/marks-tegmark-2023-truth.md#sec-4-misalignment}.}.
There is no single ``truth \glsterm{circuit}{circuit}'' in this picture: there is a
...
\glsterm{sycophancy}{Sycophancy} is a parallel non-convergence. Probing-derived steering
directions for \glsterm{sycophancy}{sycophancy} are reported across multiple labs, but no
two reports agree on a single ``\glsterm{sycophancy}{sycophancy} \glsterm{circuit}{circuit}'': the proposed
mechanisms span steering-vector additions to refusal-\glsterm{feature}{feature}
...
\glsterm{sycophancy}{sycophancy} decomposes into agreement, praise, and genuine-agreement
components individually suggests \emph{behaviorally singular,
representationally plural} --- if confirmed, the question ``what is
the \glsterm{sycophancy}{sycophancy} \glsterm{circuit}{circuit}?'' is malformed: there are several.

\begin{contradiction}
\textbf{\glsterm{circuit}{Circuit}-tracing scalability past Anthropic-2025.} The
...
($\sim\!20\%$ of pretraining for \glsterm{transcoder}{transcoder} training) is a real
...
critique: a different \glsterm{transcoder}{transcoder} training run may factor the same MLP
into a different sparse basis, and the stability of attribution-graph
conclusions across \glsterm{transcoder}{transcoder} seeds is open empirical. The strongest
form of the claim --- that automated \glsterm{circuit}{circuit} discovery scales to
\glsterm{claude}{Claude}-class models with results that converge across replications
...
component-level vs.\ \glsterm{feature}{feature}-level), apparent divergence may be
principled. The IOI \glsterm{circuit}{circuit} and factual-recall localization are the
positive cases; the truth-direction geometry and \glsterm{sparse-autoencoder}{SAE} canonicality are
...
test for whether \glsterm{circuit}{circuit}-tracing extends from a method to a paradigm.
