...
schemes; \glsterm{gqa}{GQA} or \glsterm{mla}{MLA} on the attention sublayer with the \glsterm{flashattention}{FlashAttention}-3
...
\begin{itemize}
  ...
  \item \S\ref{sec:attention-kv-sharing} is the anchor section on the
    KV-sharing axis: \glsterm{mha}{MHA} $\to$ \glsterm{mqa}{MQA} $\to$ \glsterm{gqa}{GQA} $\to$ \glsterm{mla}{MLA}, with the
    cache-cost-vs-quality trade curve and the
    \texttt{[CONTRADICTION]} that \glsterm{mla}{MLA}'s quality match against \glsterm{mha}{MHA} at
  ...
  \item \S\ref{sec:attention-hardware-axis} keeps the attention
    ...
    \glsterm{flashattention}{FlashAttention}-1 $\to$ FA2 $\to$ FA3 $\to$ \glsterm{nsa}{Native Sparse Attention}
...
\texttt{[CONTRADICTION]} markers from the body into one place: \glsterm{mla}{MLA}
