...
The \glsterm{transformer}{Transformer} is not one architecture; it is a family parameterized
by a small set of orthogonal choices. From the 2017 \glsterm{encoder-decoder}{encoder-decoder}
of \citet{vaswani2017} through the GPT-2 \glsterm{decoder-only}{decoder-only} collapse
...
sublayer over a \glsterm{residual-stream}{residual stream} of shape $\BSH$ -- while exploring
...
\begin{enumerate}
  \item \emph{\glsterm{tokenization}{Tokenization}, embedding, and unembedding} --- the
  ...
  \item \emph{Positional encoding} --- the mechanism that breaks
    \glsterm{self-attention}{self-attention}'s permutation symmetry, with a clean lineage from
    sinusoidal through rotary
    (\S\ref{sec:positional-encoding}).
  \item \emph{KV-sharing} --- how the $\mathsf{H}$ attention heads
    share their \glsterm{key}{key} and \glsterm{value}{value} projections; a single axis with four
  ...
  \item \emph{Feed-forward block} --- the activation/gating structure
    of a single \glsterm{ffn}{FFN} and the dense-vs-MoE multiplicity
    (\S\ref{sec:ffn}).
  \item \emph{Normalization} --- layer norm versus RMS norm, and where
    in the \glsterm{residual-stream}{residual stream} the norm sits
    (\S\ref{sec:normalization-axis}).
  \item \emph{State-space alternative} --- the \glsterm{state-space-model}{SSM} and \glsterm{state-space-model}{SSM}-attention
    hybrid path that does \emph{not} use \glsterm{scaled-dot-product-attention}{scaled dot-product attention}
  ...
  \item \emph{Multimodal extension} --- how non-text inputs reach the
    \glsterm{residual-stream}{residual stream} (\S\ref{sec:multimodal}).
...
workspace stays $\BHSdh$, and the LM head still maps to \glsterm{logits}{logits} of
...
\glsterm{sentencepiece}{SentencePiece} or tiktoken-family \glsterm{bpe}{BPE} with $\mathsf{V}$ in the
$128\text{k}$--$256\text{k}$ range and untied I/O embeddings; \glsterm{rope}{RoPE} at
every layer with one of a small family of long-\glsterm{context-extension}{context extension}
schemes; \glsterm{gqa}{GQA} or \glsterm{multi-head-latent-attention}{MLA} on the attention sublayer with the \glsterm{flashattention}{FlashAttention}-3
kernel as the default schedule; \glsterm{swiglu}{SwiGLU} as the per-expert \glsterm{ffn}{FFN}, dense or
sparse-MoE depending on scale; \glsterm{rmsnorm}{RMSNorm} in a \glsterm{pre-ln}{Pre-LN} (often Peri-LN)
...
\glsterm{precision-pinning}{DeepSeek-V3}-style multi-token-prediction heads. We document each axis
...
architectures is dominated by Meta, DeepSeek, \glsterm{mistral}{Mistral}, Qwen, and
...
pre-2017 lineage that the \glsterm{transformer}{Transformer} replaced (\glsterm{rnn}{RNN} seq2seq, additive
...
\begin{itemize}
  \item \S\ref{sec:tokenization-embedding} walks subword \glsterm{tokenization}{tokenization}
    (\glsterm{bpe}{BPE}, \glsterm{sentencepiece}{SentencePiece}, tiktoken-family byte-level), the tied-versus-
    untied I/O embedding decision and its scale-dependence, the
    \glsterm{vocabulary}{vocabulary}-size sweep from $\sim 32\text{k}$ to $256\text{k}$, and
    the still-open tokenizer-free contender (BLT).
  \item \S\ref{sec:positional-encoding} traces sinusoidal $\to$
    learned $\to$ \glsterm{t5}{T5} relative bias $\to$ \glsterm{alibi}{ALiBi} $\to$ \glsterm{rope}{RoPE} $\to$ \glsterm{nope}{NoPE},
    explains why \glsterm{rope}{RoPE} became dominant, and treats long-context
    extension (NTK-aware, \glsterm{yarn}{YaRN}, LongRoPE) as a positional question.
  \item \S\ref{sec:attention-kv-sharing} is the anchor section on the
    KV-sharing axis: \glsterm{mha}{MHA} $\to$ \glsterm{mqa}{MQA} $\to$ \glsterm{gqa}{GQA} $\to$ \glsterm{multi-head-latent-attention}{MLA}, with the
    cache-cost-vs-quality trade curve and the
    \texttt{[CONTRADICTION]} that \glsterm{multi-head-latent-attention}{MLA}'s quality match against \glsterm{mha}{MHA} at
  ...
  \item \S\ref{sec:attention-hardware-axis} keeps the attention
    ...
    \glsterm{flashattention}{FlashAttention}-1 $\to$ FA2 $\to$ FA3 $\to$ \glsterm{native-sparse-attention}{Native Sparse Attention}
    (\glsterm{nsa}{NSA}), foregrounding the distinction between exact tile-reordering
    and approximate sparsification.
  \item \S\ref{sec:ffn} walks dense \glsterm{relu}{ReLU} $\to$ \glsterm{gelu}{GELU} $\to$ \glsterm{swiglu}{SwiGLU} and
  ...
  \item \S\ref{sec:normalization-axis} walks \glsterm{layer-normalization}{LayerNorm} $\to$ \glsterm{rmsnorm}{RMSNorm}
    along the type axis and \glsterm{post-ln}{Post-LN} $\to$ \glsterm{pre-ln}{Pre-LN} $\to$ DeepNorm /
    Peri-LN along the placement axis, motivating frontier-2026's
    \glsterm{rmsnorm}{RMSNorm} + \glsterm{pre-ln}{Pre-LN} with $1/\sqrt{N}$ residual init.
  \item \S\ref{sec:state-space-and-hybrids} steps off the attention
    scaffold entirely: \glsterm{s4}{S4}/S5 $\to$ \glsterm{selective-ssm}{Mamba} $\to$ \glsterm{selective-ssm}{Mamba}-2 along the \glsterm{state-space-model}{SSM}
    lineage, and the \glsterm{state-space-model}{SSM}-attention hybrid family (Jamba, Samba,
  ...
  \item \S\ref{sec:multi-token-output} draws the distinction between
    ...
    \glsterm{multi-token-prediction}{multi-token prediction} (\glsterm{multi-token-prediction}{MTP}), with \glsterm{precision-pinning}{DeepSeek-V3} as the worked
    integration.
  \item \S\ref{sec:multimodal} catalogues the three-piece pattern --
    vision encoder, projector, \glsterm{llm}{LLM} body -- across the \glsterm{cross-attention}{cross-attention}
  ...
  \item \S\ref{sec:inference-adjuncts} folds in the architectural
    ...
    quantization regimes (INT8, INT4, \glsterm{fp8}{FP8}, \glsterm{nvfp4}{NVFP4}, BitNet), and
    structured-output enforcement (logit bias, Outlines FSM,
    \glsterm{context-independent-context-dependent-vocabulary-split}{XGrammar}).
\end{itemize}

\S\ref{sec:frontier-snapshot} cross-tabulates the frontier-2026
disclosed models -- DeepSeek V3, Llama 4, \glsterm{mistral}{Mistral} Large, Qwen3,
Gemini-class, \glsterm{claude}{Claude}-class, GPT-class -- on the ten axes, colour-coded
...
\texttt{[CONTRADICTION]} markers from the body into one place: \glsterm{multi-head-latent-attention}{MLA}
versus \glsterm{mha}{MHA} at scale, \glsterm{nsa}{NSA} versus FA3 head-to-head, \glsterm{state-space-model}{SSM}-hybrid quality
at frontier, the dense-MoE quality gap at matched active \glsterm{parameters}{parameters},
the long-context extrapolation method comparison, and \glsterm{multi-token-prediction}{MTP} at
