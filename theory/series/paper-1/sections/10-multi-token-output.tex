...
all assume the canonical autoregressive decoding loop: one forward pass
...
prediction} (MTP) \citep{gloeckle2024-mtp} is an architectural and
...
training loss; MTP is \emph{architectural} and changes both the loss
landscape and what the deployable system can do. They compose --- a
DeepSeek-V3-style MTP-trained model \citep[\S 2.3.1]{deepseek-v3} is
its own draft model in a Leviathan-style verification loop --- but
they are not interchangeable.

\subsection{Speculative decoding: the rejection rule}
\label{sec:mt-leviathan}

Let $M_p$ be a target LLM with conditional distribution $p(x_t \mid
x_{<t})$ over a vocabulary of size $V$, and $M_q$ a smaller draft
...
\end{equation}
A key analytic result is \citet[Thm.~3.5, Cor.~3.6]{leviathan2023}:
$\alpha = \E[\min(p, q)] = 1 - \E[D_{LK}(p, q)]$, where $D_{LK}$ is
total-variation distance. The speedup curve is therefore a function of
how close $q$ is to $p$ in TV. Reported $\alpha$ on T5-XXL with a
T5-Small drafter is $\sim 0.75$, yielding $2$--$3\times$ wallclock
...
\emph{Tree attention} is the mechanism that makes this a single pass:
...
\begin{contradiction}
Medusa with rejection sampling (\cref{eq:spec-reject} applied to
heads-as-drafter) is mathematically distribution-preserving; Medusa
with typical acceptance is not. Vendor blogs and benchmark tables
...
RAG, RLHF rollouts) must verify the verification rule, not just the
draft mechanism.
\end{contradiction}
...
\emph{features} --- the second-to-top-layer hidden state $f$, the
...
feature sequences are smoother and more predictable, since the LM head
absorbs the entropy. The drafter is a single decoder layer (an FC
projection plus one transformer block) taking a fused input of past
...
uncertainty that pure feature autoregression cannot commit to.
...
not raw feature L2 distance. EAGLE inherits Leviathan-style rejection
...
\citep{eagle3-2025} (NeurIPS 2025) abandons feature prediction in
favor of direct token prediction with multi-layer feature fusion,
...
\texttt{kb/excerpts/gloeckle2024-mtp.md\#sec-2}.}
\begin{equation}
...
a model can be MTP-trained and served as if it were a vanilla
next-token model, with the auxiliary heads either discarded or reused
as drafters. Headline quality results: 13B-parameter MTP models solve
...
DeepSeek-V3 \citep[\S 2.3.1]{deepseek-v3} productionizes a chained
variant: instead of $n$ independent heads each conditioned on $z_t$
alone, a sequence of $k$ small Transformer blocks $M_i$ refine a
...
\citep[\S 2.3.1]{deepseek-v3}. DeepSeek-V3 ships $k = 1$ (one MTP
...
on the previous prediction --- the natural autoregressive structure
\cref{eq:spec-reject} exploits at inference.

\begin{intuition}
MTP is one model, many heads, one objective. The shared trunk produces
a single hidden state $z_t = f_s(x_{1:t})$; the heads $f_1, \ldots,
...
(Medusa-2) or with a feature-level drafter trained on the target's
features (EAGLE). Distribution-preserving under inherited rejection
sampling. Cost: backbone fine-tune (Medusa-2) or drafter parameters
$\sim 1\text{--}3\%$ of target (EAGLE).

\item \textbf{MTP} \citep{gloeckle2024-mtp,deepseek-v3}: pretraining-
...
$\sim 1$ Transformer block of parameters (DeepSeek-V3).
\end{itemize}

The key invariant: every mechanism above commutes with
\cref{eq:spec-reject}, so they can be \emph{composed}. An MTP-trained
...
MTP is increasingly the convergent choice at frontier 2026: it
...
non-DeepSeek frontier scale, and how MTP interacts with the long
chain-of-thought generation patterns of reasoning models like
DeepSeek-R1, are open --- and the subject of the contradiction
\citet{gloeckle2024-mtp} surface vs.\ \citet{deepseek-v3} surface
about \emph{whether MTP is mainly a speedup or also a quality lift},
...
target distribution is over a single discrete vocabulary $\{1, \ldots,
...
three-piece pattern there --- vision encoder, projector, LLM body ---
turns out to be largely orthogonal to the choices in this section: a
multimodal LLM with MTP heads is a thing one can build (and DeepSeek-V3
