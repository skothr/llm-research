...
The discrete-token interface of a Transformer LLM is implemented by three
parameter blocks that bracket the residual stream: a tokenizer that maps
text to integer IDs in $\{0, \dots, |V|-1\}$, an input embedding
$E_{\text{in}} \in \R^{|V| \times \mathsf{D}}$ that lifts those IDs into
the residual stream of shape $\BSH$, and an LM head
$E_{\text{out}} \in \R^{\mathsf{D} \times |V|}$ that projects the final
residual stream back to logits of shape $\bsv$. The thesis of this
section is that the field has converged on a small lineage of choices
along this triple: subword BPE in either the SentencePiece or
byte-level (tiktoken) family, with a clear scale-dependent split on
weight tying — tied at $\lesssim 1$B parameters, untied at $\gtrsim 8$B —
...
interact: vocabulary size sets the embedding-table cost, which sets the
...
initialize the symbol vocabulary with the character (or byte) alphabet,
...
The final vocabulary size is the size of the initial alphabet plus the
...
list is replayed deterministically over new text. Crucially, BPE is
...
\citet{kudo2018-sentencepiece} packages BPE (and a unigram-LM
...
First, \emph{lossless tokenization}: SentencePiece treats raw input,
...
fallback}: out-of-vocabulary characters decompose into the 256 UTF-8
...
behind LLaMA-1/2/3 \citep{touvron2023, meta-llama3}, Mistral, Qwen,
Gemma, and OLMo 2 \citep{olmo2}.

\citet{radford2019-gpt2} addresses the orthogonal question of what
\emph{base alphabet} BPE should run on. A character-level base requires
...
\begin{quote}
\itshape We observed BPE including many versions of common words like
...
limited vocabulary slots and model capacity. To avoid this, we prevent
BPE from merging across character categories for any byte sequence. We
...
The category-respecting byte-level BPE of \citet{radford2019-gpt2} is
the direct ancestor of OpenAI's tiktoken (used by GPT-3, GPT-4, GPT-4o
at $|V|=200\text{K}$, and Phi-4 at $|V|=100\text{K}$
\citep[\S 2.1]{phi4}) and of DeepSeek-V3's $|V|=128\text{K}$ byte-level
BPE \citep[\S 2.1]{deepseek-v3}.
...
\label{eq:embed}
\end{equation}
producing the initial residual stream $X^0$ of shape $\BSH$. After $L$
blocks the LM head produces logits,
\begin{equation}
\mathrm{logits}_i = X^L_i \, E_{\text{out}} \in \R^{|V|}, \qquad
E_{\text{out}} \in \R^{\mathsf{D} \times |V|},
\label{eq:unembed}
\end{equation}
of shape $\bsv$ \citep[\S 3.4]{vaswani2017}. In modern decoder-only
LLMs $X^L$ is the output of the final RMSNorm rather than the raw
...
The original encoder-decoder Transformer scales the embedding by
$\sqrt{\mathsf{D}}$ before adding the positional signal
\citep[\S 3.4]{vaswani2017}, an artifact of the Xavier-initialization
variance regime; modern decoder-only LLMs typically omit the scaling
and rely on the first RMSNorm to fix the norm of $X^0$.

\subsection{Weight tying}
\label{sec:tok-tying}

\citet{press2017-tied-embed} observed that the input embedding $U$ and
the output projection $W$ both index the same vocabulary and proposed
constraining them:
\begin{equation}
...
$|V| \cdot \mathsf{D}$ parameters) and the empirical result on RNN-era
language models was a perplexity \emph{reduction}: PTB validation drops
from $78.4$ to $75.8$ on the medium LSTM, with similar gains on the
large LSTM and on Wikitext-2 \citep[\S 4]{press2017-tied-embed}. On NMT
encoder-decoder models with shared source/target vocabulary, three-way
tying reduced parameters by 28--52\% with no BLEU degradation
...
8B the I/O embeddings are $\sim$12\% of model parameters; at 70B,
...
softmax cross-entropy that scales with $|V|$, while the input-side
...
to be a meaningful share of total parameters.
\end{intuition}

\begin{contradiction}
\citet[\S 4--5]{press2017-tied-embed} reports tying \emph{improves}
perplexity on $\sim$10M-parameter LSTM language models. Frontier
2024--2026 open-weight LLMs at $\geq 8$B parameters
...
small share of total parameters — is currently an
...
Vocabulary size is the lever that connects tokenizer choice to
...
SentencePiece BPE \citep[\S 2.3]{touvron2023}; LLaMA-3 at
$|V|=128{,}256$ on expanded SentencePiece-BPE
\citep[\S 3.1]{meta-llama3}; DeepSeek-V3 at $|V|=128{,}000$ on
byte-level BPE \citep[\S 2.1]{deepseek-v3}; Phi-4 at $|V|=100{,}000$
...
parameters per embedding matrix at 8B scale, and untied means double
...
where 32K SentencePiece-BPE produced fertilities of $2$--$4$
...
single ID versus a sequence. If "12345" is one BPE merge but "1234" is
...
model is trained alongside the main LLM and used to score next-byte
entropy,
\begin{equation}
...
not depend on $x_{\geq i}$ — which BPE provably is not, because BPE
...
bytes to patch representations, a heavy Latent Transformer that runs
once per patch, and a lightweight Local Decoder that reads patch
outputs back to byte logits \citep[\S 3]{blt2024}. The Latent
Transformer's runtime scales with the number of patches $m$, not the
...
\begin{forumsignal}
As of 2026-Q2, no production frontier-LLM has shipped with a
...
follows: sinusoidal (\citealp[\S 3.5]{vaswani2017}), learned, T5
relative bias, ALiBi, RoPE, YaRN, NoPE. The next section
(\cref{sec:positional}) walks that lineage, and explains why RoPE plus
context-extension methods became the frontier-2026 default — a
choice that, like tokenization, is one axis of the small set of
choices the modern Transformer actually offers.
