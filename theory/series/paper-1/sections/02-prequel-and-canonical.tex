...
The modern \glsterm{transformer}{Transformer} is a small set of choices made on top of a
...
attention-implementation, \glsterm{ffn}{FFN}, normalization, positional encoding,
multi-token output, multimodality) is a localized perturbation of the
six-block \glsterm{encoder-decoder}{encoder-decoder} of \citet{vaswani2017}.} Establishing what
...
section. We start with what the \glsterm{transformer}{Transformer} replaced, why it
...
the recurrent \glsterm{encoder-decoder}{encoder-decoder} with attention. Three pathologies were
known well enough to be the abstract of \citet{vaswani2017} itself
(``dispensing with recurrence and convolutions
entirely'')%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#abstract}.}
...
$\partial \mathbf{h}_t / \partial \mathbf{h}_{t-1}$; the \glsterm{lstm}{LSTM} cell
...
of \glsterm{glu}{GLU}-gated 1D convolutions, gaining within-example parallelism
...
the \glsterm{transformer}{Transformer} at the moment of submission and forms the headline
comparison in \citet[Table~2]{vaswani2017}, where the \glsterm{transformer}{Transformer}
...
The \glsterm{transformer}{Transformer} of \citet{vaswani2017} is a stack of $L$ identical
...
\begin{tabular}{lll}
\toprule
\textbf{Symbol} & \textbf{Meaning} & \textbf{Vaswani-base \glsterm{value}{value}} \\
\midrule
...
$d_{\text{ff}}$     & \glsterm{ffn}{FFN} inner dimension                         & $2048$ ($= 4\mathsf{D}$) \\
$\mathsf{V}$        & tokenizer \glsterm{vocabulary}{vocabulary} size $|V|$             & $\sim$37k (\glsterm{bpe}{BPE}, EN--DE)~\citep[\S 5.1]{vaswani2017} \\
...
sublayers, a multi-head \glsterm{self-attention}{self-attention} block and a position-wise
feed-forward block, with a \glsterm{residual-connection}{residual connection} and layer
normalization wrapping each sublayer%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-1}.}.
The decoder also has $L = 6$ layers but inserts a third sublayer --
\glsterm{encoder-decoder}{encoder-decoder} \glsterm{cross-attention}{cross-attention} -- between the \glsterm{self-attention}{self-attention} and
the \glsterm{ffn}{FFN}, and masks the \glsterm{self-attention}{self-attention} so that position $i$ can attend
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-1}, the same anchor as immediately above.}:
\begin{equation}
  \mathbf{y} \;=\; \mathrm{LayerNorm}\bigl(\mathbf{x} + F(\mathbf{x})\bigr)
  \tag{Vaswani--\S 3.1}
  \label{eq:post-ln-original}
\end{equation}
\citep[\S 3.1]{vaswani2017}. This is the \emph{\glsterm{post-ln}{Post-LN}} placement
(norm \emph{after} the residual add), and it is the placement
\citet{radford2019-gpt2} will discard in GPT-2 in favor of \glsterm{pre-ln}{Pre-LN}
...
\glsterm{scaled-dot-product-attention}{scaled dot-product attention} on per-head matrices
...
$\mathsf{S} \times \mathsf{S}$ score matrix; row $i$ of the post-\glsterm{softmax}{softmax}
result is a probability distribution over keys, and the multiplication
by $V$ on the right reads out a convex combination of \glsterm{value}{value} rows --
\emph{content-based retrieval} from a set of \glsterm{key}{key}-\glsterm{value}{value} pairs. The
$\sqrt{d_k}$ scaling keeps the pre-\glsterm{softmax}{softmax} logit variance independent
...
unscaled dot products grow with $d_k$ ``pushing the \glsterm{softmax}{softmax} function
...
contents; the \glsterm{query}{query} is what we are asking. The \glsterm{softmax}{softmax} over
...
\citep[\S 3.2.2]{vaswani2017}. ``\glsterm{multi-head-attention}{Multi-head attention} allows the
...
3.2.2]{vaswani2017}\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-2-2-motivation}.}.
...
the post-\glsterm{softmax}{softmax}-times-$V$ output returns to $\BHSdh$ and the
...
\glsterm{multi-head-attention}{multi-head attention} in three places~\citep[\S
3.2.3]{vaswani2017}\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-2-3}.}:
encoder \glsterm{self-attention}{self-attention} (queries, keys, and values all from the
previous encoder layer), decoder \emph{masked} \glsterm{self-attention}{self-attention} (same
sources, but with positions $> i$ masked to $-\infty$ inside the
\glsterm{softmax}{softmax} to preserve autoregression), and \glsterm{encoder-decoder}{encoder-decoder}
\glsterm{cross-attention}{cross-attention} (queries from the previous decoder layer; keys and
values from the encoder output). The \glsterm{decoder-only}{decoder-only} LLMs of $\S 3$
onward keep only the second of these three -- masked \glsterm{self-attention}{self-attention}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-3}.}:
\begin{equation}
...
-- the \glsterm{ffn}{FFN} does not mix across positions, only across channels at
each position. The same $(W_1, b_1, W_2, b_2)$ is shared across all
$\mathsf{S}$ positions but \emph{not} across the $L$ layers; each
layer learns its own pair. The activation is \glsterm{relu}{ReLU}
...
attention sublayer's KV-sharing pattern, $\S 7$ replaces the \glsterm{ffn}{FFN}'s
\glsterm{relu}{ReLU}-2-matrix form (with \glsterm{swiglu}{SwiGLU} and then MoE), and the strict
...
license. The lineage that follows -- learned absolute, \glsterm{t5}{T5} relative
bias, \glsterm{alibi}{ALiBi}, \glsterm{rope}{RoPE} -- is the subject of \S\ref{sec:positional-encoding};
\glsterm{rope}{RoPE} in particular makes a different bet on \emph{multiplicative}
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/vaswani2017.md#sec-3-4}.}.
...
final \glsterm{residual-stream}{residual stream} to \glsterm{vocabulary}{vocabulary} \glsterm{logits}{logits}; a \glsterm{softmax}{softmax} produces
...
\glsterm{parameters}{parameters} -- a substantial fraction of the 2017 base model's $\sim 65$M
\glsterm{parameters}{parameters} when $\mathsf{V} \approx 37\text{k}$ and $\mathsf{D} = 512$.
...
tying, but LLaMA-3 and \glsterm{precision-pinning}{DeepSeek-V3} do not.
...
takes \glsterm{residual-stream}{residual stream} $X^\ell \in \R^{\mathsf{B} \times \mathsf{S} \times \mathsf{D}}$
to $X^{\ell+1}$ via
\begin{align}
...
takes the \glsterm{tokenization}{tokenization}, the input embedding, and the unembedding,
...
Eq.~\eqref{eq:vaswani-pe} and walks learned $\to$ \glsterm{t5}{T5} relative bias
$\to$ \glsterm{alibi}{ALiBi} $\to$ \glsterm{rope}{RoPE} $\to$ \glsterm{nope}{NoPE}.
...
projections (\glsterm{mha}{MHA} $\to$ \glsterm{mqa}{MQA} $\to$ \glsterm{gqa}{GQA} $\to$ \glsterm{multi-head-latent-attention}{MLA}).
...
(FA1 $\to$ FA2 $\to$ FA3 $\to$ \glsterm{nsa}{NSA}).
\S\ref{sec:feedforward-axis} replaces the \glsterm{relu}{ReLU}-2-matrix
Eq.~\eqref{eq:vaswani-ffn} with \glsterm{swiglu}{SwiGLU} and then with MoE.
\S\ref{sec:normalization-axis} replaces \glsterm{layer-normalization}{LayerNorm} in
Eq.~\eqref{eq:post-ln-original} with \glsterm{rmsnorm}{RMSNorm} and moves it to
\glsterm{pre-ln}{Pre-LN}.
...
position/channel factorization, the \glsterm{softmax}{softmax}-over-keys, the head
partition, the $\mathsf{B}\!\times\!\mathsf{S}\!\times\!\mathsf{D}$
\glsterm{residual-stream}{residual stream} as the workspace -- survives across all eight axes
