...
along the KV-sharing axis (\glsterm{mha}{MHA} $\to$ \glsterm{mqa}{MQA} $\to$ \glsterm{gqa}{GQA} $\to$ \glsterm{mla}{MLA}). This
...
\citealp{shah2024}) $\to$ \glsterm{nsa}{Native Sparse Attention} (block-sparse with
...
(vLLM, \glsterm{compressed-fsm}{SGLang}, TensorRT-\glsterm{llm}{LLM}).
...
\glsterm{mqa}{MQA} / \glsterm{gqa}{GQA}, and an \glsterm{mla}{MLA}-specific kernel exists), but \glsterm{nsa}{NSA} is designed for
...
swap \glsterm{rope}{RoPE} for \glsterm{yarn}{YaRN}, \glsterm{mha}{MHA} for \glsterm{mla}{MLA}, and FA2 for FA3 + \glsterm{nsa}{NSA} without any of
