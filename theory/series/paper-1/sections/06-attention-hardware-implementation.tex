...
\label{sec:hardware-attention}

\S\ref{sec:attention-mechanism-axis} fixed the attention \emph{formula}
along the KV-sharing axis (\glsterm{mha}{MHA} $\to$ \glsterm{mqa}{MQA} $\to$ \glsterm{gqa}{GQA} $\to$ \glsterm{multi-head-latent-attention}{MLA}). This
section fixes the formula --- \glsterm{scaled-dot-product-attention}{scaled dot-product attention} as defined
in Eq.~(1) of \citet{vaswani2017} --- and varies the
\emph{implementation} on the GPU memory hierarchy. The thesis: between
2022 and 2025 the field walked \glsterm{flashattention}{FlashAttention}~1 (block \glsterm{tiling}{tiling} +
recomputation; \citealp{dao2022}) $\to$ \glsterm{flashattention}{FlashAttention}~2 (warp-level
tile scheduling; \citealp{dao2023}) $\to$ \glsterm{flashattention}{FlashAttention}~3
(asynchronous + low-precision tensor-core scheduling on Hopper;
\citealp{shah2024}) $\to$ \glsterm{native-sparse-attention}{Native Sparse Attention} (block-sparse with
...
entirely from reducing \glsterm{hbm}{HBM} traffic and matching the tensor-core
schedule. \glsterm{nsa}{NSA} changes \emph{which keys participate} (the per-branch
...
The \glsterm{scaled-dot-product-attention}{scaled dot-product attention} block of \S\ref{sec:attention-mechanism-axis}
costs $\Theta(B \cdot H \cdot S^2 \cdot d_h)$ FLOPs and, in the naive
implementation that materializes
$\boldsymbol{S}, \boldsymbol{P} \in \R^{S \times S}$ in \glsterm{hbm}{HBM},
$\Theta(N d + N^2)$ \glsterm{hbm}{HBM} accesses per head (with $N \equiv S$ for
\glsterm{self-attention}{self-attention})~\citep[\S 3.2 Thm.\ 2]{dao2022}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dao2022.md#sec-3-2}.} The roofline diagnostic
...
\glsterm{hbm}{HBM} bandwidth: increasing FLOPs is free until the ridge is reached.
...
\glsterm{softmax}{softmax} architectures~\citep[\S 1]{yuan2025}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/yuan2025.md#sec-1}.}
...
FLOPs to spare. It is the \glsterm{hbm}{HBM} round-trip on the $S \times S$ score
...
SRAM, and stream a numerically stable \emph{\glsterm{online-softmax}{online softmax}} across
$\boldsymbol{K}, \boldsymbol{V}$ tiles so that the full $S \times S$
matrix is never written to \glsterm{hbm}{HBM}.\footnote{KB excerpt:
\nolinkurl{kb/excerpts/dao2022.md#sec-3-1}.} The tile sizes are chosen
...
bytes~\citep[Algorithm~1]{dao2022}. On A100 with $M \approx 192$~KiB and
half-precision, this gives $B_c \approx B_r \approx 192$ at $d_h = 64$,
$\approx 96$ at $d_h = 128$.

The \glsterm{online-softmax}{online softmax} decomposition is the algebraic move that makes
streaming work. For each \glsterm{query}{query} tile $\boldsymbol{Q}_i$, maintain
running statistics $m_i \in \R^{B_r}$ (running row-wise max) and
$\ell_i \in \R^{B_r}$ (running row-wise \glsterm{softmax}{softmax} denominator), updated
...
$\lceil S / B_c \rceil$ \glsterm{key}{key}/\glsterm{value}{value} tiles have been streamed,
$\boldsymbol{O}_i$ is the exact \glsterm{softmax}{softmax}-attention output for the \glsterm{query}{query}
...
$\boldsymbol{S}, \boldsymbol{P}$ in \glsterm{hbm}{HBM} during the forward pass and
reading them back on the backward pass, \glsterm{flashattention}{FlashAttention} stores only
...
\nolinkurl{kb/excerpts/dao2022.md#sec-3-1-recompute}.} This is selective
...
recomputation, but \glsterm{hbm}{HBM} bytes drop from $\Theta(N^2)$ to $\Theta(N
d_h)$, and the kernel moves up the roofline.

The IO complexity result is~\citep[Thm.~2]{dao2022}: standard
attention requires $\Theta(N d_h + N^2)$ \glsterm{hbm}{HBM} accesses, \glsterm{flashattention}{FlashAttention}
requires $\Theta(N^2 d_h^2 / M)$. For typical $d_h \in [64, 128]$ and
$M \sim 100$~KiB, $d_h^2 / M \sim 1/10$ to $1/6$, and Proposition~3
...
\glsterm{transformer}{Transformer} to achieve better-than-chance on Path-X at $S =
...
\glsterm{softmax}{softmax}-reduction synchronization is needed.

\paragraph{Reduce non-matmul FLOPs.} Tensor cores deliver $16\times$
the throughput of CUDA cores on matmul; non-matmul ops (\glsterm{softmax}{softmax}
...
\glsterm{softmax}{softmax}-mask combine. Both moves reduce CUDA-core FLOPs per output
element.
...
updates are independent across \glsterm{query}{query} rows. This recovers full SM
...
$\leftrightarrow$ shared memory transfers), and \glsterm{fp8}{FP8} tensor-core matmul
...
$\boldsymbol{K}_j, \boldsymbol{V}_j$ tile and run \glsterm{softmax}{softmax}. Producer and
consumer execute concurrently; the latency of the next \glsterm{hbm}{HBM} load is
...
\glsterm{softmax}{softmax}-weighted $\boldsymbol{P}_{ij} \boldsymbol{V}_j$ to update
$\boldsymbol{O}_i$) are issued asynchronously, and the \glsterm{softmax}{softmax}
...
constraint that \glsterm{softmax}{softmax} pipelines serially with WGMMA is relaxed.

\paragraph{FP8 path with block-quantization.} Hopper's \glsterm{fp8}{FP8} tensor cores
double FP16 throughput. Naive \glsterm{fp8}{FP8} attention loses accuracy because
\glsterm{softmax}{softmax} exponentiation amplifies the \glsterm{fp8}{FP8} quantization error on the
score matrix. FA3 quantizes $\boldsymbol{Q}, \boldsymbol{K},
\boldsymbol{V}$ in \emph{per-block} scales --- each $B_c \times d_h$
\glsterm{key}{key} tile gets its own scaling factor stored alongside the tile --- and
...
$\sim\!1.5\times$ from \glsterm{fp8}{FP8} on top, with end-to-end quality preserved
...
(vLLM, \glsterm{compressed-fsm-2}{SGLang}, TensorRT-\glsterm{llm}{LLM}).

\subsection{Native Sparse Attention (NSA)}
\label{sec:nsa}

The first three implementations preserve the attention formula
exactly. \glsterm{nsa}{NSA} \citep{yuan2025} departs: at long context, it replaces
the full-sequence keys/values $\boldsymbol{K}_{:t}, \boldsymbol{V}_{:t}$
with a sparse triple of branch-specific \glsterm{key}{key}/\glsterm{value}{value}
sets,\footnote{KB excerpt: \nolinkurl{kb/excerpts/yuan2025.md#sec-3-2}.}
\begin{equation}
...
length-$l$ block of keys to one compressed \glsterm{key}{key}, with stride $d < l$:
\begin{equation}
  \tilde K_t^{\mathrm{cmp}} \;=\;
...
selection-importance scores over \glsterm{gqa}{GQA}-group heads (Eq.~10) so that all
heads in a \glsterm{gqa}{GQA} group share the same selected blocks. Block size
$l' = 64$, top-$n = 16$ blocks: 1024 selected keys per \glsterm{query}{query}.
...
gather pattern would defeat the contiguous-tile loading of \glsterm{flashattention}{FlashAttention}.
\glsterm{nsa}{NSA} inverts the loop: where FA2/3 load contiguous \emph{\glsterm{query}{query} blocks}
into SRAM, \glsterm{nsa}{NSA} loads all \glsterm{gqa}{GQA}-group heads at \emph{one \glsterm{query}{query} position}
into SRAM and pairs them with that position's selected sparse KV
blocks~\citep[\S 3.4]{yuan2025}. The \glsterm{gqa}{GQA}-group-shared selection
...
FA2-on-\glsterm{gqa}{GQA} full attention, with \glsterm{nsa}{NSA} matching or exceeding full-attention
...
Two further long-context strategies sit alongside the FA1--3 / \glsterm{nsa}{NSA}
lineage and compose with it rather than replacing it.

\paragraph{Sliding-window attention.} \glsterm{mistral}{Mistral} 7B
\citep{jiang2023} uses a fixed window of $W = 4096$ keys per \glsterm{query}{query}, so
...
stream --- 32-layer \glsterm{mistral}{Mistral} 7B reaches a $\sim$131K-token effective
receptive field with a per-layer 4K window. \glsterm{sliding-window-attention}{Sliding-window attention}
...
outer loop. Each device runs FA-style on-chip \glsterm{tiling}{tiling} locally, so ring
...
optimizes the \emph{kernel} on each shard, \glsterm{nsa}{NSA} can replace the
...
\begin{contradiction}
\glsterm{nsa}{NSA}-vs-FA3 head-to-head benchmarks are single-source as of 2026-Q2.
\citet{yuan2025} report $11.6\times$ decode at 64K, but the comparison
baseline is FA2-on-\glsterm{gqa}{GQA}, not FA3 with the H100 asynchrony +
warp-specialization advantages. No published third-party reproduction
on Hopper has compared \glsterm{nsa}{NSA}'s three-branch sparse attention to FA3 on
...
Treat ``\glsterm{nsa}{NSA} $\gg$ dense \glsterm{flashattention}{FlashAttention} at long context'' as a
...
empirical asymmetry: FA1/2/3 work with any KV-sharing variant (\glsterm{mha}{MHA} /
\glsterm{mqa}{MQA} / \glsterm{gqa}{GQA}, and an \glsterm{multi-head-latent-attention}{MLA}-specific kernel exists), but \glsterm{nsa}{NSA} is designed for
\glsterm{gqa}{GQA}/\glsterm{mqa}{MQA} and would lose its group-centric kernel
optimization~\citep[\S 3.4]{yuan2025} on \glsterm{mha}{MHA}. The four implementations
form a progression in which three properties co-vary: (i) \glsterm{hbm}{HBM} traffic,
...
under \glsterm{nsa}{NSA}'s $N_t \ll N$ block selection; (ii) tensor-core utilization,
which rises from $\sim\!25\%$ (naive FA1 path on A100) to $\sim\!75\%$
(FA3 on H100); and (iii) precision, which descends from FP16 to \glsterm{fp8}{FP8}
...
\glsterm{nsa}{NSA} branch decomposition (Eq.~\eqref{eq:nsa-three-branch}), every
...
swap \glsterm{rope}{RoPE} for \glsterm{yarn}{YaRN}, \glsterm{mha}{MHA} for \glsterm{multi-head-latent-attention}{MLA}, and FA2 for FA3 + \glsterm{nsa}{NSA} without any of
those choices interfering with the others, because the formula at
which they agree --- \glsterm{scaled-dot-product-attention}{scaled dot-product attention} --- is the fixed point.

The next section (\S\ref{sec:ffn-axis}) leaves attention and walks the
feed-forward axis: \glsterm{relu}{ReLU} $\to$ \glsterm{gelu}{GELU} $\to$ \glsterm{swiglu}{SwiGLU} dense, then dense
$\to$ Switch $\to$ Mixtral $\to$ DeepSeekMoE fine-grained-with-shared
experts. The \glsterm{ffn}{FFN} axis is where the second half of the per-layer
