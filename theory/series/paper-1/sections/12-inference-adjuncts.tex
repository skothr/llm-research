...
through constrained decoding --- are the load-bearing reason a 2026
...
The autoregressive decode loop reads, at every step, the cached K/V
projection of every prior token at every layer. For an OPT-13B request
of 2048 tokens the KV cache is $\approx 1.6$~GB, with the per-token
...
sequence's KV cache into fixed-size \emph{KV blocks} of $B$ tokens
...
\begin{analogy}
PagedAttention is virtual memory for the KV cache. The block table is
...
The architectural pressure runs the other way too: PagedAttention's
block-decomposed kernel is what enables the cross-request prefix sharing
of RadixAttention \citep[\S 3]{zheng2024-sglang}\footnote{KB excerpt:
\texttt{kb/excerpts/zheng2024-sglang.md\#sec-3}.}, the
training-time KV compression of MQA / GQA / MLA in
...
LLM quantization is a family of decisions indexed by \emph{what} is
quantized (weights, activations, KV cache), \emph{when} (post-training
...
Past $\sim 6.7$B parameters, LLM activations develop systematic
...
\end{equation}
to LLM scale by accelerating the Optimal Brain Quantization baseline
...
perplexity, where round-to-nearest at the same precision diverges
($110$ and $571$ respectively
\citep[\S 1, Fig.~1]{frantar2022}).\footnote{KB excerpt:
\texttt{kb/excerpts/frantar2022.md\#sec-1-figure-1}.} GPTQ is the
canonical accurate one-shot weight-only PTQ method at the 100B+ scale
and is the reference against which AWQ and the GGUF Q$k$\_$\!$M family
are still benchmarked.

\paragraph{FP8 (training).}
Training in FP8 with tile-based scales (128$\times$128 weight blocks,
1$\times$128 activation vectors) was first validated at trillion-token
scale by DeepSeek-V3 \citep[\S 2.5]{deepseek-v3}, with H100/H200/Blackwell
hardware FP8 paths. We treat FP8 in
\cref{sec:training-mixed-precision-paper-2}; the relevance to this
section is that an FP8-trained checkpoint can be served \emph{without}
...
shared exponent. NVFP4 is NVIDIA's Blackwell-generation 4-bit
...
peer-reviewed primary source for NVFP4 LLM quality at frontier scale
exists in the KB as of \texttt{2026-05}; the citation here points to
the closest analogue (FP8 production training) and flags the gap.}
...
architecture is otherwise LLaMA-alike (RMSNorm, SwiGLU, RoPE, no
biases) so it integrates into the open-source serving stack
unchanged \citep[\S 2]{ma2024-bitnet}. At 3B parameters,
BitNet b1.58 matches FP16 LLaMA perplexity ($9.91$ vs.\ $10.04$) at
...
unchanged, the inputs are reshaped by \cref{eq:smoothquant-eq}. GPTQ
...
multiplications with sign-flip-plus-select inside the BitLinear layer
...
\begin{contradiction}
NVFP4's production status at frontier scale is contested. NVIDIA's
Blackwell-launch materials and vendor blogs report NVFP4 as
production-ready for inference of large models with quality matching
FP8 / FP16 baselines; third-party reproduction as of 2026-Q2 is sparse,
with most published benchmarks coming from NVIDIA-collaborator labs.
By contrast, FP8 production training has independent confirmation
through DeepSeek-V3's open release \citep[\S 2.5]{deepseek-v3} and
multiple downstream replications. The KB reflects this asymmetry: FP8
is a tier-A primary claim, NVFP4 is a tier-B forum-signal claim
...
conforms to a target language $\mathcal{G}$ (regex, JSON schema, GBNF,
...
distribution. Formally, if $\ell_t \in \R^V$ are the model logits at
...
\emph{transitions in an FSM whose alphabet is the model's vocabulary}.
The standard FSM operates over bytes; the contribution is a
precomputed \emph{index} mapping each FSM state $s$ to the set of
vocabulary tokens whose byte sequence keeps the FSM in a valid path
...
where the parser state and a stack-summary jointly key the index
...
exploits a property of typical schemas: most vocabulary tokens'
...
excerpt: \texttt{kb/excerpts/xgrammar2024.md\#sec-3}.} XGrammar
...
$100$K-vocab models \citep[\S 7]{xgrammar2024}. By 2026 XGrammar is
the default constrained-decoding backend in vLLM, SGLang, and
TensorRT-LLM \citep[FORUM-SIGNAL]{xgrammar2024}.

\begin{analogy}
Constrained decoding is the \emph{product} of the FSM (which says
which tokens are allowed) with the language model (which says which
tokens are likely): the masked distribution in
\cref{eq:constrained-decoding} is the model's softmax times the
...
LLM. We turn next (\cref{sec:frontier-snapshot}) to a model-by-model
table of the architectural choices the leading 2026 systems share, and
where they diverge. PagedAttention has become universal; GQA-or-MLA is
near-universal; ternary pretraining at 70B+ remains open; NVFP4
