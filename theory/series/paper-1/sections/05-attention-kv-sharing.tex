...
named points on it (\glsterm{mha}{MHA}, \glsterm{mqa}{MQA}, \glsterm{gqa}{GQA}, \glsterm{mla}{MLA}), and modern frontier-2026
...
the count of \glsterm{query}{query} heads $H$, and -- in the \glsterm{mla}{MLA} case -- by a
...
\glsterm{mha}{Multi-head attention} runs Eq.~\eqref{eq:vaswani-attn} $H$ times in
...
$\{1, \ldots, H\}$. \glsterm{mla}{MLA} instead \emph{compresses} the joint $K, V$
...
$t$. \glsterm{mla}{MLA} defines down-projection $W^{DKV} \in \R^{d_c \times D}$ and
...
\begin{intuition}
\glsterm{mqa}{MQA}, \glsterm{gqa}{GQA}, and \glsterm{mla}{MLA} are three answers to the same question: ``how do
...
per group. \glsterm{mla}{MLA} does not pick at all -- it stores a single
...
$\mathrm{cache} = 2 G d_h b$, \glsterm{mla}{MLA} fixes
...
\textbf{\glsterm{mla}{MLA}} \citep{deepseek-v2} & $(d_c + d_h^R)\,b \approx 4.5\,d_h\,b$
...
puts \glsterm{mla}{MLA} at \glsterm{gqa}{GQA}-2.25 cache footprint with \glsterm{mha}{MHA}-or-better quality --
...
makes \glsterm{mla}{MLA} the unambiguous choice. The contradiction below records
why ``if reproducible'' has not yet collapsed.

\begin{contradiction}
The \glsterm{mla}{MLA}-vs-\glsterm{mha}{MHA} quality result at frontier scale is single-source as
of 2026-Q2. The original ablation appears in
\citet[\S 2.1.4]{deepseek-v2} on a 16B-active-parameter
DeepSeek-V2 prototype; \citet{deepseek-v3} extends \glsterm{mla}{MLA} to a 671B
total / 37B active model with strong downstream benchmarks but does
not include a head-to-head \glsterm{mla}{MLA}-vs-\glsterm{mha}{MHA} at matched compute at that
scale. No third-party laboratory has published an \glsterm{mla}{MLA} reproduction
...
pre-training loss curves -- is not yet decomposed. Treat \glsterm{mla}{MLA}'s
...
\begin{itemize}
  ...
  \item \textbf{DeepSeek-V2, \glsterm{precision-pinning}{DeepSeek-V3}, DeepSeek-R1}: \glsterm{mla}{MLA} throughout
    \citep{deepseek-v2, deepseek-v3, deepseek-r1}. The Chinese
    open-weight ecosystem -- including labs that train on the
    DeepSeek architecture as a baseline -- has converged on \glsterm{mla}{MLA}
...
training stack, \glsterm{mla}{MLA} is the 2026 default}. The rest of the
...
Why hasn't \glsterm{mla}{MLA} crossed over? Three plausible reasons, each
under-evidenced and worth recording. \textbf{(a)~Training-recipe
risk}: \glsterm{mla}{MLA} changes both $K$/$V$ representations and the \glsterm{rope}{RoPE}
...
existing checkpoint. \textbf{(b)~Inference-stack support}: \glsterm{mla}{MLA}'s
...
attention kernel. As of 2026-Q2, \glsterm{flashattention}{FlashAttention}, vLLM, and \glsterm{compressed-fsm}{SGLang}
have first-class \glsterm{gqa}{GQA} support; \glsterm{mla}{MLA} support exists but is more
recently added and less battle-tested. \textbf{(c)~The contradiction
above}: if \glsterm{mla}{MLA}'s quality story were uncontested, the recipe risk
...
\glsterm{mla}{MLA}$\}$ and any of $\{$standard, \glsterm{flashattention}{FlashAttention}-1/2/3, \glsterm{nsa}{NSA}$\}$ as
...
interact -- in particular, how \glsterm{mla}{MLA}'s decoupled-\glsterm{rope}{RoPE} wrinkle
(Eqs.~\eqref{eq:mla-qr}--\eqref{eq:mla-k-cat}) interacts with the
\glsterm{flashattention}{FlashAttention} block-\glsterm{tiling}{tiling} pattern, and why \glsterm{mla}{MLA} serving stacks
...
contend with in Paper~4. The \glsterm{mla}{MLA} latent $\mathbf{c}_t^{KV}$ is not
a per-head representation; it is a joint compression. Probes,
lenses, and SAEs trained on \glsterm{mla}{MLA}-architecture activations are reading
