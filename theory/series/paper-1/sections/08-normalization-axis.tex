...
The Transformer's normalization layer evolved along two orthogonal axes
between 2017 and 2026: the \emph{type} of normalization (LayerNorm vs
RMSNorm) and the \emph{placement} of the norm relative to the residual
addition (Post-LN vs Pre-LN, plus stabilizer variants such as DeepNorm).
The frontier-2026 consensus across LLaMA, Mistral, DeepSeek, and Qwen is
\textbf{RMSNorm + Pre-LN with $1/\sqrt{N}$ residual scaling at
initialization}; the parameters of this consensus are the smallest
...
LayerNorm normalizes across the feature axis of a single example, so
...
\begin{equation}
  ...
  \label{eq:ba-eq3}
\end{equation}
and apply a per-feature gain $\mathbf{g}$ and bias $\mathbf{b}$ after
...
(ii) LayerNorm is invariant under re-scaling of the entire weight
...
In modern decoder-only notation, $H$ is the model dimension $\mathsf{D}$
and $\mathbf{a}^l$ is a single token's residual stream slice at depth
...
\citet{zhang2019} hypothesize that LayerNorm's success rests on its
\emph{re-scaling} invariance, not its \emph{re-centering} invariance,
and propose Root Mean Square Layer Normalization%
...
\begin{equation}
  ...
  \label{eq:rmsnorm}
\end{equation}
Comparing Eq.~\eqref{eq:rmsnorm} to Eq.~\eqref{eq:ba-eq3}: RMSNorm drops
the mean $\mu^l$ entirely (and with it the bias $\mathbf{b}$), keeping
only the scale parameter $\mathbf{g}$. Two reductions over the feature
...
placement~\citep[\S 4]{zhang2019}; on Transformer-base WMT'14 EN--DE
the swap costs $0.05$ BLEU and saves 9.4\% of training
time~\citep[\S 4]{zhang2019}. The shift-invariance that LayerNorm
afforded turns out not to be load-bearing in
practice~\citep[\S 3.2]{zhang2019}, and every frontier-2026 decoder-only
LLM (LLaMA-1/2/3, Mistral, Qwen, DeepSeek-V2/V3, Gemma, OLMo, Phi-4)
ships RMSNorm rather than
LayerNorm~\citep{touvron2023,jiang2023,meta-llama3,deepseek-v3,qwen3,olmo2,gemma3}.
...
sublayer (attention or FFN) and $\mathbf{x}$ for its input, the
...
\begin{align}
  ...
  \label{eq:pre-ln}
\end{align}
\citet{vaswani2017} used Post-LN \eqref{eq:post-ln} in the original
Transformer; the norm sits \emph{on} the residual path, so the residual
...
self-attention block. A modified initialization which accounts for the
...
\end{quote}
Two architectural decisions are bundled in that paragraph: the Pre-LN
...
for the fact that an unnormalized residual stream's variance grows with
depth. Both are still in effect in frontier-2026 models, with $N$
typically the count of attention~+~FFN sublayers (i.e., $2L$ for an
$L$-block decoder-only stack).
...
\begin{theorem}[Post-LN gradient scaling at init,
\citealp{xiong2020}, Thm.~1]
\label{thm:postln}
For the Post-LN Transformer with standard Xavier initialization, the
expected Frobenius gradient norm at the final FFN sublayer is
\[
...
\end{theorem}
\begin{theorem}[Pre-LN gradient scaling at init,
\citealp{xiong2020}, Thm.~2]
\label{thm:preln}
For the Pre-LN Transformer with the same initialization, the expected
...
\end{theorem}
The practical consequence~\citep[\S 4.1, \S 5]{xiong2020}: Post-LN
training requires the linear-warmup schedule
$\mathrm{lr}(t) = (t/T_{\mathrm{warmup}})\,\mathrm{lr}_{\max}$ to
compensate for the output-layer gradient blow-up, while Pre-LN trains
stably \emph{without warmup} at the same maximum learning rate. Their
WMT'14 and BERT-pretraining experiments confirm the prediction:
Post-LN with $\mathrm{lr}_{\max} = 10^{-3}$ and no warmup diverges, but
the same setting with $T_{\mathrm{warmup}} = 4000$ trains stably; the
Pre-LN counterpart trains stably at $\mathrm{lr}_{\max} = 10^{-3}$ with
no warmup at all~\citep[\S 5]{xiong2020}. Theorem~\ref{thm:preln} is the
load-bearing reason every 2020+ decoder-only LLM is Pre-LN.

\subsection{DeepNorm and beyond}
\label{sec:norm-deepnorm}

A natural follow-up question is whether one could keep the Post-LN
...
of depth $L$ such that the residual stream's variance and the
...
decoder-only, encoder-decoder)} DeepNorm thus generalizes the Pre-LN /
$1/\sqrt{N}$ trick to a regime where Pre-LN's residual-stream-norm
...
models but has not displaced Pre-LN at frontier scale; modern frontier
LLMs run at $L = 32\text{--}126$ layers~\citep{deepseek-v3,meta-llama3}
where Pre-LN is comfortable and the DeepNorm machinery is not yet
necessary.

\begin{intuition}
LayerNorm is ``make every token's residual stream have the same energy
along the feature axis, then re-scale per-channel.'' Energy of
$\mathbf{a}$ along the feature axis is $\sum_i a_i^2$, so RMSNorm
...
Composing the four moves: take LayerNorm
(Eq.~\eqref{eq:ba-eq3}--\eqref{eq:ba-eq3-affine}), drop the mean and
the bias to get RMSNorm (Eq.~\eqref{eq:rmsnorm}); place the norm
...
warmup-free trainability. The total cost on top of the canonical 2017
Transformer is one bias parameter removed, one mean-subtraction
...
LLaMA-/Mistral-/DeepSeek-/Qwen-class model has shipped some variant
...
replacements for, the RMSNorm + Pre-LN + $1/\sqrt{N}$ baseline
established between 2016 and 2020.

The next section (\S\ref{sec:state-space-and-hybrids}) leaves the
attention-and-FFN-with-residuals architecture entirely and asks what
happens when the residual stream is replaced by a state-space
recurrence -- a setting in which the normalization choices of this
section largely transfer (state-space-model-based LLMs use RMSNorm and
analogues of Pre-LN), but the gradient-scaling argument of
