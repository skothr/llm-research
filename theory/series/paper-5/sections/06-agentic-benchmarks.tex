...
The MMLU lineage (\cref{sec:knowledge-benchmarks}) measures what a model
\emph{knows}: a single text response, scored against a fixed answer key
...
dialogue-and-policy realism by replacing the user with another LLM,
...
end-state verifier $V(s_T, g) \in \{0, 1\}$ that accepts or rejects
...
``Claude 2 \ldots\ resolves only 1.96\% of the issues''
\citep[\S{}sec-headline]{jimenez2024-swebench}. By mid-2025 the
SWE-Bench Verified leaderboard sits above $70\%$ for frontier reasoning
...
SWE-Bench Verified (OpenAI, August 2024) filters the pool to $500$
...
GUI grounding an unavoidable sub-capability of every score.
...
(Anthropic's October 2024 ``computer use'' Claude release, and
follow-on agentic VLMs) push the AI number higher, but the failure
mode the abstract names --- GUI grounding, not high-level planning
--- has not gone away.
...
LLM-as-judge. The verifier is mechanical and reproducible, which
...
remains far from saturation. Why expert humans hit $\sim 92\%$ and
...
sidestepping LLM-as-judge contamination on the agent side. The
...
it is to score reliably}. SWE-Bench Verified is the most ecologically
...
the user side of the conversation is itself an LLM, which makes the
...
The end-state verifier $V(s_T, g)$ is silent on a class of failure
...
$1.96\% \to 70\%$ SWE-Bench Verified arc and the $15\% \to 50\%+$
GAIA arc are the most informative single capability measurements
the field has produced since MMLU saturated, and the realism gap is
...
$\$$-per-task axis), and METR's task-horizon metric%
\footnote{KB note: \texttt{kb/notes/evaluation/agentic-benchmarks.md\#5-frontier}.}
...
not yet trust them to predict deployment value.
\end{contradiction}
...
--- the jailbreak-resistance and dangerous-capability evaluation
