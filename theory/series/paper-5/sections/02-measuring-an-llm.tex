...
static benchmark vs adversarial \glsterm{probe}{probe} vs deployment audit), tied
together by a small psychometric \glsterm{vocabulary}{vocabulary} (validity, reliability,
construct, contamination, ecological validity), and that the closest
the field has to a theory of measurement is \glsterm{helm}{HELM}'s
scenario-by-metric matrix \citep{liang2022-helm} together with the
2PL form from item response theory.} The \glsterm{vocabulary}{vocabulary} fixed here is
...
\texttt{\glsterm{lm-evaluation-harness}{lm-evaluation-harness}} \citep{lm-eval-harness}, the
de-facto reference implementation for the academic-benchmark
column of \cref{sec:knowledge-benchmarks}\footnote{KB excerpt:
\nolinkurl{kb/excerpts/lm-eval-harness.md#sec-framing}.} ---
write a reported benchmark score as a sample from a distribution%
\footnote{KB note: \nolinkurl{kb/notes/evaluation/eval-methodology.md#1-formal-framing}.}
\begin{equation}
...
presented many ways: \glsterm{loglikelihood-scoring}{loglikelihood scoring} on
...
$1$--$4$ percentage points on \glsterm{mmlu}{MMLU}; combined effects can exceed $10$.
...
\glsterm{mmlu}{MMLU} to approximately $2\%$ on \glsterm{mmlu-pro}{MMLU-Pro}
...
\citet{mmlu-redux-2024} find $6.49\%$ of \glsterm{mmlu}{MMLU} questions contain
...
true score on gold-labeled \glsterm{mmlu}{MMLU}; the Virology subset contains errors
...
The psychometric \glsterm{vocabulary}{vocabulary} the field borrows --- and most often
...
\begin{description}
  ...
  \item[Validity.] Does the score reflect the construct it claims to
    ...
    on a verifier-broken benchmark (e.g., \glsterm{mmlu}{MMLU} Virology) is reliable
  ...
  \item[Construct.] What latent property is the benchmark trying to
    expose? The construct of \glsterm{mmlu}{MMLU} is broad-but-shallow factual
    recall; of \glsterm{frontiermath}{FrontierMath}, research-grade mathematical problem
    solving; of \glsterm{harmbench}{HarmBench}, refusal-rate under attack; of Apollo's
    \glsterm{scheming}{scheming} suite, in-context capability for strategic deception.
...
The four are not independent. A reliability fix (\glsterm{mmlu-pro}{MMLU-Pro}'s
$24$-prompt protocol) does not buy validity if the construct is wrong
(saturated retrieval). A validity fix (\glsterm{frontiermath}{FrontierMath}'s unpublished
...
side: they ask whether the model has a capacity at all. \glsterm{sycophancy}{Sycophancy}
...
environment. The split is empirically diagnostic: in \glsterm{mmlu-pro}{MMLU-Pro},
\glsterm{chain-of-thought}{chain-of-thought} scoring meaningfully outperforms direct-answer
scoring on the new harder items, where on the original \glsterm{mmlu}{MMLU} it did
not \citep[\S 6.2]{wang2024-mmlu-pro}. The \glsterm{chain-of-thought}{CoT}-vs-direct gap
...
the score is \glsterm{asr}{attack success rate} on a search-augmented set, not
...
generally requires a different instrument. \glsterm{mmlu}{MMLU} is
$(\text{capability}, \text{knowledge}, \text{static})$; \glsterm{harmbench}{HarmBench} is
$(\text{alignment-adjacent}, \text{agency-adjacent},
\text{adversarial})$; the (yet-unbuilt) deployment audit for
in-the-wild \glsterm{scheming}{scheming} would be
...
\glsterm{helm}{HELM} \citep{liang2022-helm} is the closest the field has to a
...
single number%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/liang2022-helm.md#sec-1-1-design}.}.
\glsterm{helm}{HELM} operationalizes this as a scenario-by-metric matrix: $7$ metrics
...
$(\rho_s, a^\star_s, V_s)$, and the \glsterm{helm}{HELM} evaluation of model
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/liang2022-helm.md#sec-4-metrics}
for the seven metric definitions;
\nolinkurl{kb/excerpts/liang2022-helm.md#sec-1-1-design} for the
$98/112 = 87.5\%$ populated-cell count.}.

The headline coverage finding is the methodological contribution:
``Before \glsterm{helm}{HELM}, models on average were evaluated on just $17.9\%$ of
the core \glsterm{helm}{HELM} scenarios''; \glsterm{helm}{HELM} evaluates $30$ prominent language
models on all $42$ scenarios under a single harness
\citep[\S 1.1]{liang2022-helm}. The reproducibility move
is to release all raw model prompts and completions, not just
aggregate scores \citep[\S 1.1]{liang2022-helm}.
...
\begin{contradiction}
The \glsterm{helm}{HELM} proposal is that $\hat{S}$ is a vector
...
publish a single \glsterm{mmlu}{MMLU} number, a single GPQA number, a single
\glsterm{harmbench}{HarmBench} \glsterm{asr}{ASR}. Per-axis breakdowns appear in tech reports
intermittently and rarely with the seven-metric coverage \glsterm{helm}{HELM}
...
``\glsterm{asr}{ASR}'' number across \glsterm{harmbench}{HarmBench}, \glsterm{jailbreakbench}{JailbreakBench}, AdvBench is
exactly the kind of scalarization \glsterm{helm}{HELM} warns against).
\end{contradiction}

\subsection{Item response theory: a calibration tool}
\label{sec:measuring-irt}

Where \glsterm{helm}{HELM} addresses the metric-bundle question, item response theory
...
The use of \eqref{eq:irt-2pl} in \glsterm{llm}{LLM} evaluation is calibration, not
benchmark construction. Given a benchmark $\mathcal{B}$ with item
\glsterm{parameters}{parameters} $\{(b_i, a_i)\}$ estimated from a panel of models, one
...
$a$. Frontier models on saturated \glsterm{mmlu}{MMLU} items have $\theta_X$ well
above the items' $b_i$, so the response is near $1$ and provides no
new information --- this is the formal version of the \glsterm{saturation}{saturation}
...
\glsterm{mmlu-pro}{MMLU-Pro} \citep{wang2024-mmlu-pro} or \glsterm{hle}{HLE} \citep{phan2025-hle} is
...
(\glsterm{mmlu-pro}{MMLU-Pro}: $4 \to 10$-way MCQ plus reasoning-focused items; \glsterm{hle}{HLE}:
expert-authored items with verifier discipline inherited from
GPQA and \glsterm{frontiermath}{FrontierMath}) but the abstract goal is the same: rebuild
...
\glsterm{scalable-oversight}{scalable oversight} asks whether a bounded-resource judge can align a
model whose capability already exceeds the judge's. Both are
downstream of the \glsterm{vocabulary}{vocabulary} fixed here without being expressible
solely within it. We turn first to the knowledge layer
(\cref{sec:knowledge-benchmarks}) and the contamination boundary
(\cref{sec:contamination}) where the \glsterm{vocabulary}{vocabulary} is most directly
exercised.
