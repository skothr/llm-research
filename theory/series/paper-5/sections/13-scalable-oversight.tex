...
deployment-distribution problems on \emph{current} models: scheming
under elicited goals, alignment-faking under training distribution
shift, watermark survival under removal pressure. The threat in this
...
debate between two AI agents, judged by a weaker human, decides
$\mathbf{PSPACE}$ under polynomial-time judging --- but that the 2024
LLM-debate empirical literature gives \emph{partial} support in
verifiable-claim domains, \emph{no clear} support in open-ended ones,
and that as of 2026-Q2 no candidate protocol --- debate, iterated
...
\footnote{KB excerpt: \texttt{kb/excerpts/irving2018-debate.md\#sec-1}.}
...
alignment tasks has a natural limit: a debate between competing agents
...
debate's job is to drive the dispute to a level the judge \emph{can}
...
(judge a single witness), and the next level up is what debate is
designed to reach.

\subsection{The debate game}
\label{sec:oversight-debate-game}

The debate-game protocol is defined verbatim%
\footnote{KB excerpt: \texttt{kb/excerpts/irving2018-debate.md\#sec-2}.}
...
\item The judge sees the debate $(q, a, s)$ and decides which agent
wins.
...
\begin{quote}\itshape
\textbf{Claim.} In the debate game, it is harder to lie than to refute
...
\footnote{KB excerpt: \texttt{kb/excerpts/irving2018-debate.md\#sec-2-1}.}
...
\footnote{KB excerpt: \texttt{kb/excerpts/irving2018-debate.md\#sec-2-2}.}
...
for $n$ rounds of debate. \ldots\ If the number of rounds $n$ is
...
choose a polynomial time judge so that optimal play in the debate game
...
$\mathbf{PSPACE}$ value of the underlying alternating-quantifier
formula.
...
\begin{analogy}
The debate-game theorem is the alignment-protocol cousin of an
...
arbitrary computation. Debate adds one structural ingredient ---
...
the verifier of a true claim, debate's two provers play competitively
...
sees the same number of pixels in both conditions; the debate
...
The 2024 LLM-debate literature is the first round of empirical work
...
generalize the Irving 2018 setup to LLM debaters and an
\emph{intentionally weaker} LLM judge, comparing three conditions ---
debate (two debaters, opposing positions), consultancy (single
debater), and direct QA (judge alone)%
\footnote{KB note: \texttt{kb/notes/alignment/oversight-and-scalable-alignment.md\#2-4-debate-with-weak-llm-judges-kenton-et-al-2024}.}
\deepencite{kenton2024-debate-llm-judges §abstract,
citation-pending}. The headline findings are mixed: debate
\emph{outperforms} consultancy across all task domains tested; debate
versus direct QA is task-dependent --- debate wins on
...
The Brown-Cohen et al.\ 2024 \emph{doubly-efficient debate} result
...
optimizing for \emph{persuasiveness} in debate \emph{does} drive
truth-finding under specific conditions, hitting $76$--$88\%$
accuracy in the reported regime%
\footnote{KB note: \texttt{kb/notes/alignment/oversight-and-scalable-alignment.md\#2-5-doubly-efficient-debate-brown-cohen-et-al-2024}.}.
...
externally-graded answer --- the debate condition lifts judge
...
Debate is one candidate scalable-oversight protocol among several,
...
\textbf{Iterated distillation/amplification} (IDA, Christiano 2018)
...
\footnote{KB note: \texttt{kb/notes/alignment/oversight-and-scalable-alignment.md\#3-iterated-amplification--ida-christiano-2018}.}.
...
into RLAIF and into debate itself. \textbf{Recursive reward modeling}
...
\textbf{Constitutional AI} \citep{bai2022-cai} decomposes the
\emph{value spec} into a constitution of natural-language principles,
then uses the model itself to critique-and-revise outputs against the
constitution; the supervision signal is concentrated in the
constitution, downstream labels are AI-generated. CAI is the only
candidate in this list with production deployment (Anthropic Claude
family), but its theoretical leverage decays as model capability
exceeds the constitution-author's ability to write a comprehensive
spec --- the constitution caps the supervision precision%
...
weak supervisor, measure the Performance Gap Recovered (PGR)
\[
...
With an auxiliary confidence loss, the GPT-2 $\to$ GPT-4-family
...
\emph{intermediate} reasoning step. The PRM is itself a learned
judge, trained on human or AI process annotations, and its operating
point is the same as the debate-judge's: a polynomial-cost grader
selecting among candidate continuations from a stronger reasoner.
Where debate is an inference-time protocol and W2S is a training-time
...
\S 7 (\textsc{speculation}).} is debate or W2S as the
...
\begin{contradiction}
\textbf{Debate-as-alignment-protocol viability.} The Irving 2018
...
optimal play recovers the alternating-quantifier value, and the
truthful strategy wins. The 2024 LLM-debate empirical work is
\emph{partial} support: \citet{kenton2024-debate-llm-judges} report
debate beats direct QA on information-asymmetry QA where the dispute
...
demonstration of debate-induced honesty on a domain where the judge
...
\emph{sub-linearly} in the capability being supervised. None of debate,
IDA, RRM, W2S, or constitutional approaches has yet been demonstrated
...
scheming results of \cref{sec:scheming} demonstrate is non-zero in
2024-frontier models.

\begin{intuition}
The relationship between debate's $\mathbf{PSPACE}$ theorem and a
...
debate-trained-on-LLMs implementation is the gap between
...
of them by behaving aligned to the weak supervisor / debate judge /
constitution-grader. This is the
...
Debate is one protocol among several, and the field as of 2026-Q2 has
...
LLM-debate empirics --- joins the alignment-evaluation contradictions
of \cref{sec:scheming,sec:alignment-faking} and the watermark-robustness
...
debate-induced honesty on a domain where the judge is strictly weaker
