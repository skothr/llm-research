...
\glsterm{harmbench}{HarmBench}, 100 in \glsterm{jailbreakbench}{JailbreakBench}), $\mathcal{A}$ is the attack family
...
attack budget). The headline metric is the \glsterm{asr}{attack success rate}
...
benchmarks evaluate against. \glsterm{harmbench}{HarmBench}'s $510 \times 18$ behavior-
attack matrix and \glsterm{jailbreakbench}{JailbreakBench}'s leaderboard are operational
...
\begin{quote}\itshape
we introduce \glsterm{harmbench}{HarmBench}, a standardized evaluation framework for
automated \glsterm{red-teaming}{red teaming}\ldots Using \glsterm{harmbench}{HarmBench}, we conduct a
...
\glsterm{harmbench}{HarmBench} enables codevelopment of attacks and defenses.
\end{quote}

The framework's structural commitments
\citep[\S 4]{harmbench2024}: \emph{510 carefully
...
\glsterm{harmbench}{HarmBench} fixes $\mathcal{B}$, $\mathcal{A}$, and $V$ once, and
...
``demonstrating how \glsterm{harmbench}{HarmBench} enables codevelopment of attacks and
defenses''
\citep[\S{}abstract]{harmbench2024}. A new attack reports its
\glsterm{asr}{ASR} on the \glsterm{harmbench}{HarmBench}-510-behavior set under the
...
\glsterm{harmbench}{HarmBench} frame in a way it was not in the 2023 literature, where
...
benchmark addresses: \glsterm{harmbench}{HarmBench}'s adversarial-prompt corpus is a
...
address these challenges, we introduce \glsterm{jailbreakbench}{JailbreakBench}, an
...
The complementarity with \glsterm{harmbench}{HarmBench} is structural. \glsterm{harmbench}{HarmBench} has
broader behavior coverage (510 vs.\ 100) and an attack-method
bake-off (18 attacks $\times$ 33 targets) inside one paper.
\glsterm{jailbreakbench}{JailbreakBench} has deeper artifact reproducibility and a public
leaderboard that admits new entries continuously%
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#2-1-adversarial-robustness-benchmarks}.}.
\glsterm{harmbench}{HarmBench} fixes the suite at publication time and runs a one-time
bake-off; \glsterm{jailbreakbench}{JailbreakBench} fixes the suite at publication time and
...
the other's. A score on \glsterm{harmbench}{HarmBench}-510 under \glsterm{harmbench-judge}{HarmBench-Judge} is not
recoverable from a score on \glsterm{jailbreakbench}{JailbreakBench}-100 under
\glsterm{jailbreakbench}{JailbreakBench}'s GPT-4-class judge, even for the same model.
...
$\mathrm{ASR} < 5\%$ on \glsterm{jailbreakbench}{JailbreakBench}'s single-turn behaviors may
...
The 2026 multi-turn picture: no equivalent of \glsterm{harmbench}{HarmBench} or
\glsterm{jailbreakbench}{JailbreakBench} yet exists for multi-turn attacks. Standardizing
...
\emph{Judge mis-specification.} \glsterm{jailbreakbench}{JailbreakBench} uses an \glsterm{llm-as-judge}{LLM-as-judge}
with published evaluation prompts; \glsterm{harmbench}{HarmBench} uses the fine-tuned
...
\emph{Behavior-set drift.} \glsterm{harmbench}{HarmBench}'s 510 behaviors span four
functional categories with explicit copyright and multimodal
sub-suites; \glsterm{jailbreakbench}{JailbreakBench}'s 100 behaviors are aligned with OpenAI's
...
\glsterm{jailbreakbench}{JailbreakBench}'s leaderboard discipline --- fix the judge, the
...
not measurement-theoretic but coordination-theoretic. \glsterm{harmbench}{HarmBench}'s
co-development substrate and \glsterm{jailbreakbench}{JailbreakBench}'s leaderboard
discipline are partial standardizations whose boundary is the
boundary of each project's adoption. A field-wide pin --- a
\glsterm{harmbench}{HarmBench}-or-equivalent standardized \glsterm{asr}{ASR} with cross-benchmark
