...
attack-success-rate (\glsterm{asr}{ASR}) number that is comparable across papers.
A $5\%$ \glsterm{asr}{ASR} on benchmark $X$ and a $30\%$ \glsterm{asr}{ASR} on benchmark $Y$ for
...
\glsterm{asr}{ASR} rank can be read as evidence of model robustness, the
\emph{across-paper} \glsterm{asr}{ASR}-comparability problem has to be acknowledged
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#1-formal-framing}.}
is the tuple
\[
\mathcal{S} = (\mathcal{B}, \mathcal{A}, V, \pi),
\]
where $\mathcal{B}$ is the set of harmful behaviors to test (510 in
\glsterm{harmbench-2}{HarmBench}, 100 in \glsterm{jailbreakbench-2}{JailbreakBench}), $\mathcal{A}$ is the attack family
...
classifier, \glsterm{llm-as-judge}{LLM-as-judge}, or human), and $\pi$ is the protocol
context (system prompt, decoding \glsterm{parameters}{parameters}, in-place defenses,
attack budget). The headline metric is the \glsterm{attack-success-rate}{attack success rate}
...
harmful%
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#1-formal-framing}, Eq.\,(1).}.
...
canonical benchmarks of this section. ``\glsterm{asr}{ASR}'' as a scalar is
...
\begin{intuition}
\glsterm{asr}{ASR} conflates three quantities under a single ratio: the model's
refusal robustness, the attack's strength, and the judge's
harshness. Two papers reporting ``GPT-4 \glsterm{asr}{ASR} $= 30\%$'' and ``GPT-4
\glsterm{asr}{ASR} $= 60\%$'' may both be correct under their respective
...
the \glsterm{asr}{ASR} equation is well defined for any fixed $\mathcal{S}$;
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/wei2023-jailbroken.md#abstract}.}
...
``every prompt'' result against GPT-4 and \glsterm{claude}{Claude} v1.3 establishes
...
\emph{\glsterm{safety-capability-parity}{safety-capability parity}} prescription that subsequent
benchmarks evaluate against. \glsterm{harmbench-2}{HarmBench}'s $510 \times 18$ behavior-
attack matrix and \glsterm{jailbreakbench-2}{JailbreakBench}'s leaderboard are operational
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#2-1-adversarial-robustness-benchmarks}.}.
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/harmbench2024.md#abstract}.}
\citep[\S{}abstract]{harmbench2024}:

\begin{quote}\itshape
we introduce \glsterm{harmbench-2}{HarmBench}, a standardized evaluation framework for
automated \glsterm{red-teaming}{red teaming}\ldots Using \glsterm{harmbench-2}{HarmBench}, we conduct a
large-scale comparison of 18 \glsterm{red-teaming}{red teaming} methods and 33 target
...
\glsterm{llm}{LLM} robustness across a wide range of attacks, demonstrating how
\glsterm{harmbench-2}{HarmBench} enables codevelopment of attacks and defenses.
\end{quote}
...
the same suite; and a fine-tuned \emph{\glsterm{harmbench-judge}{HarmBench-Judge}} classifier
...
\glsterm{harmbench-2}{HarmBench} fixes $\mathcal{B}$, $\mathcal{A}$, and $V$ once, and
...
``demonstrating how \glsterm{harmbench-2}{HarmBench} enables codevelopment of attacks and
defenses''
\citep[\S{}abstract]{harmbench2024}. A new attack reports its
\glsterm{asr}{ASR} on the \glsterm{harmbench-2}{HarmBench}-510-behavior set under the
\glsterm{harmbench-judge}{HarmBench-Judge} against a fixed list of defenses; a new defense
reports its \glsterm{asr}{ASR}-reduction on the same suite against a fixed list
of attacks. Cross-paper comparison becomes possible inside the
\glsterm{harmbench-2}{HarmBench} frame in a way it was not in the 2023 literature, where
...
benchmark addresses: \glsterm{harmbench-2}{HarmBench}'s adversarial-prompt corpus is a
research artifact, not a public reproducibility infrastructure
indexed against a leaderboard. The \glsterm{harmbench-judge}{HarmBench-Judge} classifier and
...
defense%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/chao2024-jailbreakbench.md#abstract}.}
\citep[\S{}abstract]{chao2024-jailbreakbench}:
...
address these challenges, we introduce \glsterm{jailbreakbench-2}{JailbreakBench}, an
...
The complementarity with \glsterm{harmbench-2}{HarmBench} is structural. \glsterm{harmbench-2}{HarmBench} has
broader behavior coverage (510 vs.\ 100) and an attack-method
bake-off (18 attacks $\times$ 33 targets) inside one paper.
\glsterm{jailbreakbench-2}{JailbreakBench} has deeper artifact reproducibility and a public
leaderboard that admits new entries continuously%
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#2-1-adversarial-robustness-benchmarks}.}.
\glsterm{harmbench-2}{HarmBench} fixes the suite at publication time and runs a one-time
bake-off; \glsterm{jailbreakbench-2}{JailbreakBench} fixes the suite at publication time and
...
the other's. A score on \glsterm{harmbench-2}{HarmBench}-510 under \glsterm{harmbench-judge}{HarmBench-Judge} is not
recoverable from a score on \glsterm{jailbreakbench-2}{JailbreakBench}-100 under
\glsterm{jailbreakbench-2}{JailbreakBench}'s GPT-4-class judge, even for the same model.
...
the harmful target%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/russinovich2024-crescendo.md#abstract}.}
...
\begin{quote}\itshape
\glsterm{crescendomation}{Crescendomation} surpasses other state-of-the-art jailbreaking
techniques on the AdvBench subset dataset, achieving 29-61\%
higher performance on GPT-4 and 49-71\% on Gemini-Pro.
\end{quote}

The 29--61\% \glsterm{asr}{ASR} \emph{uplift} on GPT-4 and 49--71\% on Gemini-Pro
over single-turn baselines ---
measured on the AdvBench subset \deepencite{zou2023-advbench,
citation-pending} under \glsterm{crescendomation}{Crescendomation}, the automated
implementation of the manual escalation strategy --- is not a new
\glsterm{asr}{ASR} scalar; it is a delta against an existing \glsterm{asr}{ASR} scalar. Under
the formal frame of \cref{sec:safety-formal}, \glsterm{crescendomation}{Crescendomation}
...
$\mathrm{ASR} < 5\%$ on \glsterm{jailbreakbench-2}{JailbreakBench}'s single-turn behaviors may
...
29--61\% \glsterm{asr}{ASR} uplift on GPT-4 (49--71\% on Gemini-Pro) is the
quantitative measurement of how much prior-self-reference shifts the
refusal probability under the same $(\mathcal{B}, V, \pi)$, with
$\mathcal{A}$ as the only term that varies.
\end{analogy}

The 2026 multi-turn picture: no equivalent of \glsterm{harmbench-2}{HarmBench} or
\glsterm{jailbreakbench-2}{JailbreakBench} yet exists for multi-turn attacks. Standardizing
the regime requires three commitments the field has not yet made
--- a fixed turn budget, an attack-judge interaction protocol
that is not gameable by the attacker \glsterm{llm}{LLM}, and a defense-reset
specification between turns%
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#5-frontier}.}.
Until those land, multi-turn \glsterm{asr}{ASR} is reported with paper-specific
...
modes break \glsterm{asr}{ASR} comparability across papers%
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#1-formal-framing}.}.

\emph{Judge mis-specification.} \glsterm{jailbreakbench-2}{JailbreakBench} uses an \glsterm{llm-as-judge}{LLM-as-judge}
with published evaluation prompts; \glsterm{harmbench-2}{HarmBench} uses the fine-tuned
\glsterm{harmbench-judge}{HarmBench-Judge} classifier; many 2023 papers use ad-hoc rule-based
...
\emph{Behavior-set drift.} \glsterm{harmbench-2}{HarmBench}'s 510 behaviors span four
functional categories with explicit copyright and multimodal
sub-suites; \glsterm{jailbreakbench-2}{JailbreakBench}'s 100 behaviors are aligned with OpenAI's
...
that the benchmark $\pi$ does not model. Benchmark \glsterm{asr}{ASR} systematically
\emph{overestimates} attack success against deployed systems
relative to standalone-API \glsterm{asr}{ASR}, but the size of the gap is itself
...
\glsterm{jailbreakbench-2}{JailbreakBench}'s leaderboard discipline --- fix the judge, the
...
field-wide: model cards continue to publish \glsterm{asr}{ASR} numbers without
all four substrate terms, and cross-benchmark comparisons in
informal venues continue to treat \glsterm{asr}{ASR} as a model property.

\begin{contradiction}
\textbf{\glsterm{asr}{ASR} is reported as a model attribute; it is not one.}
A single number ``GPT-4 \glsterm{asr}{ASR} $= 30\%$'' carries no model-level
...
\citet{russinovich2024-crescendo} document a 29--61\% uplift on
GPT-4 and 49--71\% on Gemini-Pro when
the attack family $\mathcal{A}$ alone shifts from single-turn to
multi-turn, holding everything else approximately fixed --- which
demonstrates exactly how much of the headline \glsterm{asr}{ASR} is determined by
substrate rather than model. As of 2026-Q2 there is no field-wide
$(\mathcal{B}, V, \pi)$ that the next benchmark can pin against.
The field publishes \glsterm{asr}{ASR} scalars at a fictional level of precision
...
the right \glsterm{vocabulary}{vocabulary} for the comparability problem; the issue is
not measurement-theoretic but coordination-theoretic. \glsterm{harmbench-2}{HarmBench}'s
co-development substrate and \glsterm{jailbreakbench-2}{JailbreakBench}'s leaderboard
discipline are partial standardizations whose boundary is the
boundary of each project's adoption. A field-wide pin --- a
\glsterm{harmbench-2}{HarmBench}-or-equivalent standardized \glsterm{asr}{ASR} with cross-benchmark
\glsterm{calibration-set}{calibration set} --- would close the contradiction; nothing about
