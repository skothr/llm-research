...
\begin{enumerate}
  \item \emph{Knowledge measurement and contamination} --- can the
    ...
    \emph{measurement-instrument saturation against an answer-key
    error floor}. The MMLU lineage \citep{hendrycks2021-mmlu} runs
  ...
  \item \emph{Reasoning-benchmark validity} --- does multi-step
    ...
    saturation, research-style stall}. AIME and MATH-500 saturate
    while FrontierMath \citep{glazer2024-frontiermath} does not, and
  ...
  \item \emph{Adversarial robustness} --- under a search-based
    ...
    \emph{ASR non-comparability}. The four canonical
    attack-success-rate (ASR) instruments
    \citep{harmbench2024} use different target behaviors,
    attacker LLMs, judges, and success criteria; the same model's
    $5\%$ ASR on one and $30\%$ ASR on another is not a
  ...
  \item \emph{Alignment-threat detection} --- does the model behave
    ...
    Three sub-layers compose here: sycophancy
    \citep{sharma2023-sycophancy} as a measured RLHF artifact
  ...
  \item \emph{Provenance and scalable oversight} --- once outputs
    ...
    part of scalable oversight is solved (debate decides
...
\begin{itemize}
  \item \emph{Necessary.} Drop any one layer and a real
    ...
    as capability. Without reasoning-benchmark validity, RLVR-trained
  ...
  \item \emph{Insufficient.} No single layer answers the
    deployment-relevant question on its own. A high MMLU score
    that survives MMLU-Redux still says nothing about agentic task
    completion or scheming under elicitation. A low HarmBench ASR
    says nothing about whether the model has dangerous capability
    in the first place. A clean Apollo-scheming evaluation says
...
\paragraph{What it is.} A monograph on the 2024--2026 state of LLM
...
empirical claim --- benchmark score, ASR, contamination percentage,
sycophancy rate, scheming-suite hit-rate, alignment-faking rate,
watermark detection threshold --- cites a primary source whose
excerpt the KB has tracked down to a section anchor. Every formal
construction --- the IRT 2PL form (\cref{sec:measuring-framing}),
HELM's scenario-by-metric matrix (\cref{sec:measuring-framing}),
ASR's set-cardinality definition (\cref{sec:adversarial-robustness}),
the green-list partition function (\cref{sec:watermarking}), the
debate game's $\mathbf{PSPACE}$ construction
...
\citep{greenblatt2024-alignment-faking}, the scheming suite
\citep{meinke2024-apollo-scheming}, FrontierMath
...
Paper~4's probing and circuit-tracing methods are a complementary
attack surface for
...
(MMLU and MMLU-Redux for layer 1; the contamination survey
detection methods for the contamination sub-layer; AIME, MATH-500,
GPQA, FrontierMath, HLE for layer 2; SWE-Bench, OSWorld, GAIA,
$\tau$-Bench for layer 3; HarmBench, JailbreakBench, Crescendo,
Wei~2023 for layer 4; the four sycophancy datasets, Apollo's six
scheming evaluations, Greenblatt's free-tier/paid-tier setup for
layer 5; Kirchenbauer's green-list construction, debate, IDA,
...
\Cref{sec:measuring-framing} fixes the eval-methodology vocabulary
...
benchmark vs adversarial probe vs deployment audit); HELM's
