...
\begin{itemize}
  \item \emph{Necessary.} Drop any one layer and a real
    ...
    as capability. Without reasoning-benchmark validity, \glsterm{rlvr}{RLVR}-trained
  ...
  \item \emph{Insufficient.} No single layer answers the
    ...
    that survives \glsterm{mmlu-redux}{MMLU-Redux} still says nothing about agentic task
    completion or \glsterm{scheming}{scheming} under elicitation. A low \glsterm{harmbench}{HarmBench} \glsterm{asr}{ASR}
...
(\glsterm{mmlu}{MMLU} and \glsterm{mmlu-redux}{MMLU-Redux} for layer 1; the contamination survey
...
$\tau$-Bench for layer 3; \glsterm{harmbench}{HarmBench}, \glsterm{jailbreakbench}{JailbreakBench}, Crescendo,
...
benchmark vs adversarial \glsterm{probe}{probe} vs deployment audit); \glsterm{helm}{HELM}'s
