...
construction --- a hash-keyed green/red partition of the \glsterm{vocabulary}{vocabulary} at
...
\glsterm{synthid-text}{SynthID-Text}, OpenAI's deferred deployment) carry that construction
forward. What the field does not yet have is a \glsterm{watermark}{watermark} robust to a
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/kirchenbauer2023-watermark.md#sec-algorithm}.}
\citep[\S{}abstract]{kirchenbauer2023-watermark}:

\begin{quote}\itshape
The \glsterm{watermark}{watermark} works by selecting a randomized set of ``green'' tokens
...
the \glsterm{vocabulary}{vocabulary}, $\gamma \in (0, 1)$ a partition fraction, and
...
\glsterm{softmax}{softmax}: $\tilde{\ell}_v = \ell_v + \delta \cdot \mathbb{1}[v \in G_t]$.
\item Sample $s^{(t)}$ from the modified distribution.
\end{enumerate}

The default settings reported in the paper are $\gamma = 0.5$ and
$\delta = 2.0$%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/kirchenbauer2023-watermark.md#sec-algorithm}.
$\gamma = 0.5$ is the primary experimental configuration (Table~1, Table~2,
Figure~6 unattacked baseline); $\gamma = 0.25$ appears only in parameter sweeps.}.
...
\glsterm{softmax}{softmax}, so the scheme is a \emph{soft} promotion: low-entropy positions
...
the green list is determined by the secret \glsterm{key}{key} plus the candidate text,
not by any property of the model --- detection needs the keyed hash
function and the candidate string, nothing more
\citep[\S{}abstract]{kirchenbauer2023-watermark}:

\begin{quote}\itshape
The \glsterm{watermark}{watermark} can be embedded with negligible impact on text quality,
and can be detected using an efficient open-source algorithm without
access to the language model API or \glsterm{parameters}{parameters}.
\end{quote}

This third property --- \emph{detector-only-needs-the-\glsterm{key}{key}} --- is what
...
one-sided z-score%
\footnote{KB excerpt: \nolinkurl{kb/excerpts/kirchenbauer2023-watermark.md#sec-zscore}.}
\citep[\S 3.1]{kirchenbauer2023-watermark}:
\[
z \;=\; \frac{|s|_G - \gamma T}{\sqrt{T \gamma (1 - \gamma)}}.
\]
A large $z$ rejects the null with a one-sided $p$-\glsterm{value}{value} computed
directly from the normal CDF. At $\gamma = 0.5$, a span of $T = 200$
tokens with $|s|_G = 150$ greens (against a null mean of $100$) yields
$z = (150 - 100)/\sqrt{200 \cdot 0.5 \cdot 0.5} \approx 7.07$,
corresponding to a one-sided $p < 10^{-12}$ --- comfortably below any
deployment-relevant false-positive budget. The paper exposes the
$p$-\glsterm{value}{value} as the deployment knob: a publisher willing to tolerate one
...
detection at a given \glsterm{watermark}{watermark} strength.
...
\footnote{KB excerpt: \nolinkurl{kb/excerpts/kirchenbauer2023-watermark.md#abstract};
the precise constants in the bound are pending direct PDF anchor
verification.}
\citep[\S{}abstract]{kirchenbauer2023-watermark}:

\begin{quote}\itshape
We propose a statistical test for detecting the \glsterm{watermark}{watermark} with
interpretable p-values, and derive an information-theoretic framework
for analyzing the sensitivity of the \glsterm{watermark}{watermark}.
\end{quote}
...
\nolinkurl{kb/notes/alignment/watermarking-and-provenance.md} \S 1; the
...
\begin{intuition}
The \glsterm{watermark}{watermark} is a stochastic side-channel that the model itself
supplies. At each step, the model's \glsterm{softmax}{softmax} expresses how much
...
mark --- there is no flexibility for the \glsterm{watermark}{watermark} to spend, and the
nudge does nothing. At a high-entropy position --- a stylistic
adverb, an open-ended sentence continuation --- the \glsterm{watermark}{watermark} is
...
quantitative statement of this picture: the \glsterm{watermark}{watermark} accumulates
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/watermarking-and-provenance.md#1}
\S~1 (\textsc{intuition}).}.
...
it makes low-entropy positions impossible to \glsterm{watermark}{watermark} because no
green-list-friendly alternative exists, and it is trivially detectable
by frequency analysis even without the \glsterm{key}{key}. The soft scheme avoids
all three by accepting a quantitative tradeoff: larger $\delta$ gives
a stronger \glsterm{watermark}{watermark} per token but introduces a measurable bias in
...
public conversation. Google DeepMind's \textbf{\glsterm{synthid-text}{SynthID-Text}} (Dathathri
et al., \emph{Nature} 2024) replaces the logit-bias step with a
\emph{tournament} over candidate tokens%
\footnote{KB note: \nolinkurl{kb/notes/alignment/watermarking-and-provenance.md#3-synthid-text}.}
...
a hash-derived \glsterm{watermark}{watermark} function $g$ on each, and advances the higher
$g$-\glsterm{value}{value} candidate; after $\log_2 N$ rounds, a single token wins. The
detection statistic averages $g$-values across the generated tokens and
flags spans whose average is above the no-\glsterm{watermark}{watermark} baseline. Two modes
...
§distortion-modes, citation-pending}. \glsterm{synthid-text}{SynthID-Text} is reported as
deployed in the Gemini App, the first production-scale text-\glsterm{watermark}{watermark}
deployment by a frontier lab; detection code accompanied the
\emph{Nature} publication%
\footnote{KB note: \nolinkurl{kb/notes/alignment/watermarking-and-provenance.md#3-3-production-deployment}.}.

OpenAI publicly described a \glsterm{watermark}{watermark} prototype in 2023--2024 but
...
Kirchenbauer-lineage \glsterm{watermark}{watermark} at scale, one large vendor with a
...
reports that even after strong human paraphrasing, the \glsterm{watermark}{watermark}
...
argues that any \glsterm{watermark}{watermark} detectable from \emph{short} spans must be
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/watermarking-and-provenance.md#2-2-the-fundamental-limit}
\S~2.2.}.
...
whichever internal \glsterm{feature}{feature} the \glsterm{watermark}{watermark} relies on. For a green/red
construction, the \glsterm{watermark}{watermark} is not a model \glsterm{feature}{feature} --- it is a
...
the \glsterm{watermark}{watermark} unless the fine-tuned deployer also runs the
...
omits the sampler breaks the \glsterm{watermark}{watermark} trivially).
...
of \cref{sec:watermarking-detection}: at $\gamma = 0.5$, a $50$/$50$
mix of watermarked and unwatermarked text has expected green rate
$0.5 \cdot E_{\mathrm{wm}} + 0.5 \cdot \gamma$, where $E_{\mathrm{wm}}$
is the \glsterm{watermark}{watermark}'s natural green rate. The detector applied to the
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/watermarking-and-provenance.md#5}
\S~5 (frontier).}.
...
green-list \glsterm{watermark}{watermark} survives moderate fine-tuning at the cost of a
slower detection accumulation rate, and the Google \glsterm{synthid-text}{SynthID-Text}
deployment account
...
enterprise content policy (vendor framing, where the \glsterm{watermark}{watermark} holds)
or a determined adversary with a comparable-quality model and a
modest fine-tune budget (academic framing, where the \glsterm{watermark}{watermark}
...
$x$-axis and detector-AUC is the $y$-axis with multiple \glsterm{watermark}{watermark}
...
locally can simply disable the \glsterm{watermark}{watermark} sampler, so coverage of the
deployed \glsterm{watermark}{watermark} fleet caps at the closed-source share of generated
...
\glsterm{debate}{debate} / scalable-oversight protocols, watermarking is an intervention
