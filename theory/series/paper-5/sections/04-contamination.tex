...
The MMLU lineage of \cref{sec:knowledge-benchmarks} closes on the
...
reproduce the answer key. The boundary between those two states is
fuzzy by construction --- the same exam-bank items that make MMLU a
...
\begin{quote}\itshape
To mitigate the risk of potential data contamination, LLM benchmarking
...
$n$-gram-overlap decontamination filters target. \citet{hendrycks2021-mmlu}'s
MMLU was constructed from publicly-available exam banks (AP exams,
...
scaffolds that browse the live web, and \textsc{llm}-as-judge
...
contamination is repaired upstream of training, by decontamination
...
or the ratio of the model's perplexity on the held-out test split
to its perplexity on training-distribution-matched text. Memorized
items have characteristic flat-low-perplexity signatures.
...
profile: degraded by post-training (RLHF / instruction-tuning) which
...
from one. The field's open-source decontamination pipelines run a
...
\begin{quote}\itshape
Figure 1(a) depicts the performance trajectory of DeepSeek-R1-Zero on
...
(further to $86.7\%$ under self-consistency) is the load-bearing
RL-on-verifiable-rewards demonstration of Paper 3 \deepencite{paper-3 §reasoning-RL}.
...
contradiction explicitly \deepencite{paper-3 §AIME-contamination}.
...
The two positions are not formally inconsistent --- decontamination
...
adoption --- the Open LLM Leaderboard moved its primary headline to
...
documented decontamination story; a third party auditing the same
...
dynamic-benchmark proposals (MathArena, LiveCodeBench, GAIA's gated
