...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#23-dangerous-capability-evaluations}.}.
...
autonomy axis. \glsterm{task-horizon-metric}{METR}'s autonomy evaluation suite ships $77$ tasks
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#metr-autonomy-task-suite}.}
...
\emph{\glsterm{task-horizon-metric}{task-horizon metric}}: the human-equivalent time horizon over
...
binary task accuracy does not, and grounds \glsterm{responsible-scaling-policy}{RSP}-style triggers in a
...
\citet{cybench2024}, which builds an end-to-end \glsterm{capture-the-flag}{CTF} agent benchmark
...
\emph{\glsterm{responsible-scaling-policy}{Responsible Scaling Policy}} \citep{anthropic-rsp-2024}
defines an \glsterm{asl}{AI Safety Level} (\glsterm{asl}{ASL}) tier framework. \glsterm{asl}{ASL}-1 covers
no-autonomy / low-risk systems; \glsterm{asl}{ASL}-2 is the current frontier
baseline (\glsterm{claude}{Claude} 3 / \glsterm{claude}{Claude} 4 family) with basic containment and a
list of capability triggers that must \emph{not} be reached; \glsterm{asl}{ASL}-3
...
procedures, and external evaluation; \glsterm{asl}{ASL}-4 is reserved for
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#anthropic-responsible-scaling-policy-rsp}.}.
...
external-replication bar. Anthropic's \glsterm{responsible-scaling-policy}{Responsible Scaling Policy}
\citep{anthropic-rsp-2024} defines the \glsterm{asl}{ASL} tier framework cited
...
\footnote{KB note: \nolinkurl{kb/notes/alignment/safety-evaluation.md#anthropic-responsible-scaling-policy-rsp}; cross-comparison sourced from the AI Index 2025.}.
External evaluators --- UK AISI, US AISI, \glsterm{task-horizon-metric}{METR}, Apollo Research ---
...
The structural contribution of the \glsterm{responsible-scaling-policy}{RSP} frame is to commit a vendor,
...
\glsterm{responsible-scaling-policy}{RSP} frame raises the consequences of getting them wrong.
...
surface of jailbreak benchmarks. \glsterm{harmbench-2}{HarmBench} \citep{harmbench2024}
ships $510$ behaviors, $18$ attack methods, and $33$ target LLMs as
a public package. The bio and CBRN evaluations cited in vendor \glsterm{responsible-scaling-policy}{RSP}
disclosures
...
model has \emph{not} crossed an \glsterm{asl}{ASL}-3 bio-uplift trigger does not
...
multiple-choice \glsterm{mmlu}{MMLU} item costs essentially zero to grade once the
gold \glsterm{key}{key} is built (\cref{sec:knowledge-mmlu}); a SWE-Bench instance
...
non-augmented attacker could produce; a cyber-uplift \glsterm{capture-the-flag}{CTF} task may
...
and informally negotiated. \glsterm{task-horizon-metric}{METR} autonomy results
...
The third structural distinction is the tension between the \glsterm{responsible-scaling-policy}{RSP}
...
The first is \emph{threshold \glsterm{sandbagging}{sandbagging}}: incentives push toward
...
\glsterm{sycophancy}{sycophancy}-in-rater-data Goodhart pattern of
...
is not, above a given \glsterm{responsible-scaling-policy}{RSP}-defined trigger. The vendor side argues
that this regime is the best feasible given dual-use constraints
and that external-evaluator access (UK AISI, US AISI, \glsterm{task-horizon-metric}{METR}, Apollo)
...
with its operator's intent. \glsterm{sycophancy}{Sycophancy} is a Goodhart artifact of
preference-RL that does not require capability for deception
(\cref{sec:sycophancy}). \glsterm{scheming}{Scheming} is capability for in-context
strategic deception under an instructed goal
(\cref{sec:scheming}). Alignment-faking is strategy under training
distribution shift, and reuses the legible-\glsterm{chain-of-thought}{CoT} signal as part of its
