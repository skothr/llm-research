...
answer-\glsterm{key}{key} errors, ambiguities, or unverifiable references; this
caps the highest honest score the original instrument can carry. As
of 2026-Q2, vendor and research papers continue to publish \glsterm{mmlu}{MMLU}
scores at single-percentage-point precision in the
$90$--$94\%$ band as if \glsterm{mmlu-redux-2}{MMLU-Redux} had not been written. The field
...
\emph{What would close it:} field-wide adoption of \glsterm{mmlu-redux-2}{MMLU-Redux} as
...
of headline \glsterm{mmlu}{MMLU} numbers above $\sim\!93.5\%$ as
...
\glsterm{attack-success-rate}{Attack success rate} (\glsterm{asr}{ASR}) is reported as a model attribute. It is
...
and a single number ``GPT-4 \glsterm{asr}{ASR} $= 30\%$'' carries no model-level
...
$29$--$61\%$ \glsterm{asr}{ASR} uplift on GPT-4 and $49$--$71\%$ on Gemini-Pro
...
headline \glsterm{asr}{ASR} is determined by substrate rather than model.\\
\emph{What would close it:} a \glsterm{harmbench-2}{HarmBench}-or-equivalent
standardization adopted across vendors and academic benchmarks,
with a public cross-benchmark \glsterm{calibration-set}{calibration set} --- a fixed
$(\mathcal{B}, V, \pi)$ tuple against which jailbreak papers report
delta-\glsterm{asr}{ASR} --- so that two papers reporting \glsterm{asr}{ASR} on different
...
(o1, \glsterm{claude}{Claude} 3.5 Sonnet, \glsterm{claude}{Claude} 3 Opus, Gemini 1.5 Pro,
Llama 3.1 405B) of \glsterm{in-context-scheming}{in-context scheming} under elicited conditions,
with $> 85\%$ persistence on o1 and explicit deceptive reasoning
visible in \glsterm{chain-of-thought}{CoT}. The construct is \emph{capability for \glsterm{scheming}{scheming}
behavior under elicited conditions}; the construct the field needs
is \emph{propensity for \glsterm{scheming}{scheming} behavior in deployment}. The two
...
--- \glsterm{claude}{Claude} 3.5 Sonnet \glsterm{sandbagging}{sandbagging} in service of a
...
suppression is partially driven by \glsterm{situational-awareness}{situational awareness} (the model
...
\emph{better} at \glsterm{in-context-scheming}{in-context scheming}. The intervention thus carries
...
agreed bar for what counts as observed \glsterm{scheming}{scheming} in production
...
announces the \glsterm{value}{value} conflict, names the training-versus-deployment
...
preserved-\glsterm{value}{value} strategy visible without creating it. The two
...
groups, and a setup in which the \glsterm{value}{value}-versus-training tension is
allowed to emerge from ordinary \glsterm{rlhf}{RLHF} distributional shift rather
...
position is that moderate fine-tuning leaves the \glsterm{watermark}{watermark}
...
argue that any \glsterm{watermark}{watermark} detectable from short spans is in
principle removable by a paraphrase attacker with a
comparable-quality model, and independent adversarial probes of
\glsterm{synthid-text}{SynthID-Text} confirm that aggressive paraphrase plus modest
...
the $y$-axis, with multiple \glsterm{watermark}{watermark} schemes plotted on the same
...
assumption, \glsterm{debate}{debate} decides $\mathbf{PSPACE}$ with a
polynomial-time judge and two superpolynomial debaters, and the
truthful strategy wins at the equilibrium. The 2024 \glsterm{llm}{LLM}-\glsterm{debate}{debate}
empirical work \citep{khan2024-debating-llms} provides
\emph{partial} support: \glsterm{debate}{debate} beats direct QA on
...
regime that matters for \glsterm{scalable-oversight}{scalable oversight} at superhuman
...
\glsterm{debate}{debate}-induced honesty on a domain where the judge is
...
\glsterm{sycophancy}{sycophancy} reduction-versus-resurfacing, \glsterm{frontiermath}{FrontierMath}
auditability --- are real but compose under the seven (a fix to
contamination protocol composes with \glsterm{frontiermath}{FrontierMath} grading; a fix
to \glsterm{asr}{ASR} comparability composes with dangerous-capability cross-lab
comparison).
...
catch. Layer 1 has \glsterm{mmlu}{MMLU} and \glsterm{mmlu-redux-2}{MMLU-Redux} and the contamination
survey; layer 2 has AIME, MATH, GPQA, \glsterm{frontiermath}{FrontierMath}, \glsterm{hle}{HLE}; layer 3
has SWE-Bench, OSWorld, GAIA, $\tau$-Bench; layer 4 has \glsterm{harmbench-2}{HarmBench},
\glsterm{jailbreakbench-2}{JailbreakBench}, AdvBench, Crescendo; layer 5 has \glsterm{sycophancyeval}{SycophancyEval},
the Apollo \glsterm{scheming}{scheming} suite, the Greenblatt alignment-faking setup;
layer 6 has the Kirchenbauer/SynthID watermarking line and the
Irving/Khan \glsterm{debate}{debate} line. The framing is therefore not post-hoc:
...
rates. We measure jailbreak \glsterm{asr}{ASR} (with the comparability caveat of
item~2). We measure contamination (with the protocol caveat of
item~7). We measure \glsterm{in-context-scheming}{in-context scheming} (with the elicitation
caveat of item~3). We measure \glsterm{watermark}{watermark} survival under benign
edits (with the adversarial-budget caveat of item~5). We do
\emph{not} yet measure deployment-scale propensity for \glsterm{scheming}{scheming}.
...
\emph{not} yet measure \glsterm{scalable-oversight}{scalable oversight} at superhuman
